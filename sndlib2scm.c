...
#if (!USE_SND)
static int TO_C_INT_OR_ELSE(SCM obj,0)
{
...
static SCM g_audio_outputs(SCM speakers, SCM headphones, SCM line_out)
{
  ...
  mus_audio_sun_outputs(TO_C_INT_OR_ELSE(speakers,0),TO_C_INT_OR_ELSE(headphones,0),TO_C_INT_OR_ELSE(line_out,0));
#endif
  return(SCM_BOOL_F);
}
...
static SCM g_make_sound_data(SCM chans, SCM frames)
{
  ...
  return(make_sound_data(TO_C_INT_OR_ELSE(chans,0),TO_C_INT_OR_ELSE(frames,0)));
}
...
static SCM g_close_sound_output(SCM fd, SCM bytes)
{
  ...
  return(TO_SCM_INT(mus_sound_close_output(TO_C_INT_OR_ELSE(fd,0),TO_C_INT_OR_ELSE(bytes,0))));
}
...
static SCM g_read_sound(SCM fd, SCM beg, SCM end, SCM chans, SCM sv)
{
  ...
  return(TO_SCM_INT(mus_sound_read(TO_C_INT_OR_ELSE(fd,0),TO_C_INT_OR_ELSE(beg,0),TO_C_INT_OR_ELSE(end,0),TO_C_INT_OR_ELSE(chans,0),get_sound_data(sv))));
}
...
static SCM g_write_sound(SCM fd, SCM beg, SCM end, SCM chans, SCM sv)
{
  ...
  return(TO_SCM_INT(mus_sound_write(TO_C_INT_OR_ELSE(fd,0),TO_C_INT_OR_ELSE(beg,0),TO_C_INT_OR_ELSE(end,0),TO_C_INT_OR_ELSE(chans,0),get_sound_data(sv))));
}
...
static SCM g_seek_sound(SCM fd, SCM offset, SCM origin)
{
  ...
  return(TO_SCM_INT(mus_sound_seek(TO_C_INT_OR_ELSE(fd,0),TO_C_INT_OR_ELSE(offset,0),TO_C_INT_OR_ELSE(origin,0))));
}
...
static SCM g_seek_sound_frame(SCM fd, SCM offset)
{
  ...
  return(TO_SCM_INT(mus_sound_seek_frame(TO_C_INT_OR_ELSE(fd,0),TO_C_INT_OR_ELSE(offset,0))));
}
...
static SCM g_open_audio_output(SCM dev, SCM srate, SCM chans, SCM format, SCM size)
{
  ...
  return(TO_SCM_INT(mus_audio_open_output(TO_C_INT_OR_ELSE(dev,0),TO_C_INT_OR_ELSE(srate,0),TO_C_INT_OR_ELSE(chans,0),TO_C_INT_OR_ELSE(format,0),TO_C_INT_OR_ELSE(size,0))));
}
...
static SCM g_open_audio_input(SCM dev, SCM srate, SCM chans, SCM format, SCM size)
{
  ...
  return(TO_SCM_INT(mus_audio_open_input(TO_C_INT_OR_ELSE(dev,0),TO_C_INT_OR_ELSE(srate,0),TO_C_INT_OR_ELSE(chans,0),TO_C_INT_OR_ELSE(format,0),TO_C_INT_OR_ELSE(size,0))));
}
...
static SCM g_read_audio_state(SCM dev, SCM field, SCM chan, SCM vals)
{
  ...
  val = mus_audio_mixer_read(TO_C_INT_OR_ELSE(dev,0),TO_C_INT_OR_ELSE(field,0),TO_C_INT_OR_ELSE(chan,0),fvals);
  vdata = SCM_VELTS(vals);
  for (i=0;i<len;i++) vdata[i] = TO_SCM_DOUBLE(fvals[i]);
  FREE(fvals);
...
static SCM g_write_audio_state(SCM dev, SCM field, SCM chan, SCM vals)
{
  ...
  res = mus_audio_mixer_write(TO_C_INT_OR_ELSE(dev,0),TO_C_INT_OR_ELSE(field,0),TO_C_INT_OR_ELSE(chan,0),fvals);
  FREE(fvals);
  return(TO_SCM_INT(res));
}
...
static SCM g_mus_set_data_clipped(SCM fd, SCM clipped)
{
  ...
  return(TO_SCM_INT(mus_file_set_data_clipped(TO_C_INT_OR_ELSE(fd,0),(SCM_FALSEP(clipped)) ? 0 : 1)));
}
...
static SCM g_mus_set_prescaler(SCM fd, SCM val)
{
  ...
  return(TO_SCM_DOUBLE(mus_file_set_prescaler(TO_C_INT_OR_ELSE(fd,0),TO_C_DOUBLE(val))));
}
