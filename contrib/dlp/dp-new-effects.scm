...
(define (cp-am-effect)
  "amplitude modulation"
  (map-chan-over-target-with-sync (lambda (ignored) (am-effect am-effect-amount)) am-effect-target "am" #f))
...
(define (cp-rm)
  (map-chan-over-target-with-sync (lambda (ignored) (rm-effect rm-frequency (list 0 0 1 (hz->radians rm-radians)))) rm-target "ring-modulation" #f))
            ...
            (let ((initial-jc-reverb-decay 2.0)
              ...
              (set! jc-reverb-dialog
                    (make-effect-dialog jc-reverb-label
                                        (lambda (w context info) 
					  (map-chan-over-target-with-sync jc-reverb-1 jc-reverb-target "jc-reverb" (and (not jc-reverb-truncate) jc-reverb-decay)))
                    ...
                    (make-effect-dialog place-sound-label
                                        ...
                                        (lambda (w context info)
                                          (help-dialog "Place sound"
                                                       "Mixes a mono soundfile into a pre-existing stereo sound field.\n\ Note that the stereo soundfile must be loaded before this effect will work properly.\n\ The pan position is controlled by the value slider and envelope editor."))
                                        (lambda (w c i)
                                          (set! mono-snd initial-mono-snd)
                                          (XtSetValues (list-ref sliders 0)
                                                        (list XmNvalue (inexact->exact (* mono-snd 1))))
                                          (set! stereo-snd initial-stereo-snd)
                                          (XtSetValues (list-ref sliders 1)
                                                        (list XmNvalue (inexact->exact (* stereo-snd 1))))
                                          (set! pan-pos initial-pan-pos)
                                          (XtSetValues (list-ref sliders 2)
                                                        (list XmNvalue (inexact->exact (* pan-pos 1)))))))
