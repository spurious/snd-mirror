...
def method_missing(method, *args)
  message "Method '#{method}(#{args.inspect})' not available in class '#{self.class}'."
end
...
def fm_bell(start = 0.0, dur = 1.0, freq = 220.0, amp = 0.3, *args)
  func_name = "\n" + get_func_name() + "()"
  usage = "fm_bell([start=0.0[, dur=1.0[, freq=220.0[, amp=0.3[, *args]]]]])
...
# It works with fm_play in difference to fm_play, which works with
# with_sound.

# fm_bell_snd(start, dur, freq, amp
#         [, amp_env=[0, 0, .1, 1, 10, .6, 25, .3, 50, .15, 90, .1, 100, 0]
#         [, index_env=[0, 1, 2, 1.1, 25, .75, 75, .5, 100, .2]
#         [, index=1.0]]])
#
...
def fm_bell_snd(start, dur, freq, amp, 
		amp_env = [0, 0, .1, 1, 10, .6, 25, .3, 50, .15, 90, .1, 100, 0], 
		index_env = [0, 1, 2, 1.1, 25, .75, 75, .5, 100, .2], 
		index = 1.0)
