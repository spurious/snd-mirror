...
Move the sliders to set the comb chord parameters."))
		     ...
		     (lambda (w data)
		       ...
		       ;;; (gtk_adjustment_value_changed (GTK_ADJUSTMENT (sliders 4)))
		       )
	      ...
	      (set! sliders
		    (add-sliders adsat-dialog
				 (list (list "adaptive saturation size" 0 initial-adsat-size 10
					     (lambda (w data)
					       (set! adsat-size (gtk_adjustment_get_value (GTK_ADJUSTMENT (sliders 0)))))
					     1))))
