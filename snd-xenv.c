...
static void display_env(env *e, char *name, GC cur_gc, int x0, int y0, int width, int height, bool dots, bool printing)
{
...
static XEN g_set_enved_selected_env(XEN name)
{
  ...
  if (selected_env)
    {
      if (enved_dialog) XmListSelectPos(screnvlst, find_env(cname) + 1, false);
    }
