...
(define group-id (make-procedure-with-setter (lambda (a) (a 1)) (lambda (a b) (list-set! a 1 b))))
(define group-size (make-procedure-with-setter (lambda (a) (a 2)) (lambda (a b) (list-set! a 2 b))))
(define group-vertices (make-procedure-with-setter (lambda (a) (a 3)) (lambda (a b) (list-set! a 3 b))))
(define group-speakers (make-procedure-with-setter (lambda (a) (a 4)) (lambda (a b) (list-set! a 4 b))))
(define group-matrix (make-procedure-with-setter (lambda (a) (a 5)) (lambda (a b) (list-set! a 5 b))))
...
(define speaker-config-number (make-procedure-with-setter (lambda (a) (a 1)) (lambda (a b) (list-set! a 1 b))))
(define speaker-config-dimension (make-procedure-with-setter (lambda (a) (a 2)) (lambda (a b) (list-set! a 2 b))))
(define speaker-config-coords (make-procedure-with-setter (lambda (a) (a 3)) (lambda (a b) (list-set! a 3 b))))
(define speaker-config-groups (make-procedure-with-setter (lambda (a) (a 4)) (lambda (a b) (list-set! a 4 b))))
(define speaker-config-delays (make-procedure-with-setter (lambda (a) (a 5)) (lambda (a b) (list-set! a 5 b))))
(define speaker-config-map (make-procedure-with-setter (lambda (a) (a 6)) (lambda (a b) (list-set! a 6 b))))
    ...
    (make-speaker-config :number (length speakers)
			 ...
			 :omap (let ((v (make-vector (length speakers))))
				 (do ((chan 0 (+ 1 chan)))
				   ...
				   (vector-set! v chan (or (and (not (null? channel-map)) (channel-map chan))
							   chan)))
				 v))))
...
(define path-rx (make-procedure-with-setter (lambda (p) (p 1)) (lambda (p val) (list-set! p 1 val))))
(define path-ry (make-procedure-with-setter (lambda (p) (p 2)) (lambda (p val) (list-set! p 2 val))))
(define path-rz (make-procedure-with-setter (lambda (p) (p 3)) (lambda (p val) (list-set! p 3 val))))
(define path-rv (make-procedure-with-setter (lambda (p) (p 4)) (lambda (p val) (list-set! p 4 val))))
(define path-rt (make-procedure-with-setter (lambda (p) (p 5)) (lambda (p val) (list-set! p 5 val))))
(define path-tx (make-procedure-with-setter (lambda (p) (p 6)) (lambda (p val) (list-set! p 6 val))))
(define path-ty (make-procedure-with-setter (lambda (p) (p 7)) (lambda (p val) (list-set! p 7 val))))
(define path-tz (make-procedure-with-setter (lambda (p) (p 8)) (lambda (p val) (list-set! p 8 val))))
(define path-tt (make-procedure-with-setter (lambda (p) (p 9)) (lambda (p val) (list-set! p 9 val))))
...
(define bezier-path      (make-procedure-with-setter (lambda (p) (p 10)) (lambda (p val) (list-set! p 10 val))))
(define bezier-3d        (make-procedure-with-setter (lambda (p) (p 11)) (lambda (p val) (list-set! p 11 val))))
(define bezier-polar     (make-procedure-with-setter (lambda (p) (p 12)) (lambda (p val) (list-set! p 12 val))))
(define bezier-x         (make-procedure-with-setter (lambda (p) (p 13)) (lambda (p val) (list-set! p 13 val))))
(define bezier-y         (make-procedure-with-setter (lambda (p) (p 14)) (lambda (p val) (list-set! p 14 val))))
(define bezier-z         (make-procedure-with-setter (lambda (p) (p 15)) (lambda (p val) (list-set! p 15 val))))
(define bezier-v         (make-procedure-with-setter (lambda (p) (p 16)) (lambda (p val) (list-set! p 16 val))))
(define bezier-bx        (make-procedure-with-setter (lambda (p) (p 17)) (lambda (p val) (list-set! p 17 val))))
(define bezier-by        (make-procedure-with-setter (lambda (p) (p 18)) (lambda (p val) (list-set! p 18 val))))
(define bezier-bz        (make-procedure-with-setter (lambda (p) (p 19)) (lambda (p val) (list-set! p 19 val))))
(define bezier-error     (make-procedure-with-setter (lambda (p) (p 20)) (lambda (p val) (list-set! p 20 val))))
(define bezier-curvature (make-procedure-with-setter (lambda (p) (p 21)) (lambda (p val) (list-set! p 21 val))))
...
(define initial-direction (make-procedure-with-setter (lambda (p) (p 22)) (lambda (p val) (list-set! p 22 val))))
(define final-direction   (make-procedure-with-setter (lambda (p) (p 23)) (lambda (p val) (list-set! p 23 val))))
...
(define (make-a-even)
  ...
  (define (g m)
	...
	(begin
	 ...
	 (do ((i 2 (+ i 1)))
	     ((= i path-maxcoeff))
	   (vector-set! path-gtab i (- (* -4 (path-gtab (- i 1)))
				       (path-gtab (- i 2)))))))
    (path-gtab m))
  ...
  (do ((m 1 (+ 1 m)))
    ...
    (do ((k 1 (+ 1 k)))
	((> k m))
      (vector-set! (path-ak-even (- m 1)) (- k 1) (exact->inexact (/ (- (g (- m k))) (g m)))))))
...
(define (make-a-odd)
  ...
  (define (f m)
	...
	(begin
	 ...
	 (do ((i 2 (+ i 1)))
	     ((= i path-maxcoeff))
	   (vector-set! path-ftab i (- (* -4 (path-ftab (- i 1)))
				       (path-ftab (- i 2)))))))
    (path-ftab m))
  ...
  (do ((m 1 (+ 1 m)))
    ...
    (do ((k 1 (+ 1 k)))
	((> k m))
      (vector-set! (path-ak-odd (- m 1)) (- k 1) (exact->inexact (/ (- (f (- m k))) (f m)))))))
  ...
  (cond ((not (eq? (car path) 'open-bezier-path))
	 (let* ((n (- (length (bezier-x path )) 1))
	   ...
	   (if (bezier-curvature path)
	       (do ((i 0 (+ 1 i)))
		   ((= i n))
		 (vector-set! (d 0) i (* ((d 0) i) (bezier-curvature path)))
		 (vector-set! (d 1) i (* ((d 1) i) (bezier-curvature path)))
		 (vector-set! (d 2) i (* ((d 2) i) (bezier-curvature path)))))
	   (list (- n 1) p d)))
	(else
	 (let* ((n (- (length (bezier-x path)) 1))
		(m (- n 1))
	   ...
	   (if (initial-direction path)
	       (begin
		 (vector-set! (d 0) 0 (car (initial-direction path)))
		 (vector-set! (d 1) 0 (cadr (initial-direction path)))
		 (vector-set! (d 2) 0 (third (initial-direction path))))
	       (begin
		 (vector-set! (d 0) 0 0.0)
		 (vector-set! (d 1) 0 0.0)
		 (vector-set! (d 2) 0 0.0)))
	   
	   ;; forced final direction
	   (if (final-direction path)
	       (begin
		 (vector-set! (d 0) n (car (final-direction path)))
		 (vector-set! (d 1) n (cadr (final-direction path)))
		 (vector-set! (d 2) n (caddr (final-direction path))))
	       (begin
		 (vector-set! (d 0) n 0.0)
		 (vector-set! (d 1) n 0.0)
		 (vector-set! (d 2) n 0.0)))
...
(define literal-points (make-procedure-with-setter (lambda (p) (p 10)) (lambda (p val) (list-set! p 10 val))))
(define literal-3d     (make-procedure-with-setter (lambda (p) (p 11)) (lambda (p val) (list-set! p 11 val))))
(define literal-polar  (make-procedure-with-setter (lambda (p) (p 12)) (lambda (p val) (list-set! p 12 val))))
...
(define spiral-start-angle (make-procedure-with-setter (lambda (p) (p 13)) (lambda (p val) (list-set! p 13 val))))
(define spiral-total-angle (make-procedure-with-setter (lambda (p) (p 14)) (lambda (p val) (list-set! p 14 val))))
(define spiral-step-angle  (make-procedure-with-setter (lambda (p) (p 15)) (lambda (p val) (list-set! p 15 val))))
(define spiral-turns       (make-procedure-with-setter (lambda (p) (p 16)) (lambda (p val) (list-set! p 16 val))))
(define spiral-distance    (make-procedure-with-setter (lambda (p) (p 17)) (lambda (p val) (list-set! p 17 val))))
(define spiral-height      (make-procedure-with-setter (lambda (p) (p 18)) (lambda (p val) (list-set! p 18 val))))
(define spiral-velocity    (make-procedure-with-setter (lambda (p) (p 19)) (lambda (p val) (list-set! p 19 val))))
...
(define (bezier-render path)
  ...
  (let ((xrx '()) (xry '()) (xrz '()) (xrv '()))
	...
	(do ((j 0 (+ 1 j)))
	    ((= j 3))
	  (vector-set! (cr 0) j (+ (* u1 ((c 0) j)) (* u ((c 0) (+ j 1)))))
	  (vector-set! (cr 1) j (+ (* u1 ((c 1) j)) (* u ((c 1) (+ j 1)))))
	  (vector-set! (cr 2) j (+ (* u1 ((c 2) j)) (* u ((c 2) (+ j 1))))))
	(do ((i 1 (- i 1)))
	    ((< i 0))
	  (do ((j 0 (+ 1 j)))
	      ((> j i))
	    
	    (vector-set! (cr 0) j (+ (* u1 ((cr 0) j)) (* u ((cr 0) (+ j 1)))))
	    (vector-set! (cr 1) j (+ (* u1 ((cr 1) j)) (* u ((cr 1) (+ j 1)))))
	    (vector-set! (cr 2) j (+ (* u1 ((cr 2) j)) (* u ((cr 2) (+ j 1)))))
	    ))
	(list ((cr 0) 0)
	      ((cr 1) 0)
	      ((cr 2) 0))))
...
(define* (transform-path path
  ...
  (define (rotation-matrix x y z angle)
	...
	(do ((col 0 (+ 1 col)))
	    ((= col 3))
	  (vector-set! (AA row) col 0.0)
	  (do ((mid 0 (+ 1 mid)))
	      ((= mid 3))
	    (vector-set! (AA row) col
			 (+ ((AA row) col)
			    (* ((A row) mid) 
			       ((A mid) col)))))))
...
(define* (make-dlocsig start-time
  ...
  (let* (;; speaker configuration for current number of channels
	...
	(do ((i 0 (+ 1 i)))
	    ((= i len))
	  (vector-set! channel-gains i (cons time (channel-gains i)))
	  (vector-set! channel-gains i (cons 0.0 (channel-gains i)))))
      (let ((len rev-channels))
	(do ((i 0 (+ 1 i)))
	    ((= i len))
	  (vector-set! channel-rev-gains i (cons time (channel-rev-gains i)))
	  (vector-set! channel-rev-gains i (cons 0.0 (channel-rev-gains i))))))
	...
	(let ((len (speaker-config-number speakers)))
	  (do ((i 0 (+ 1 i)))
		...
		(begin
		  (vector-set! channel-gains i (cons time (channel-gains i)))
		  (vector-set! channel-gains i (cons (outputs i) (channel-gains i)))))))
	...
	(if (> rev-channels 1)
		...
		((= i rev-channels))
		  ...
		  (begin
		    (vector-set! channel-rev-gains i (cons time (channel-rev-gains i)))
		    (vector-set! channel-rev-gains i (cons (rev-outputs i) (channel-rev-gains i)))))))
	...
	(if (= rev-channels 1)
	    (begin
		  ...
		  (begin
		    (vector-set! channel-rev-gains 0 (cons time (channel-rev-gains 0)))
		    (vector-set! channel-rev-gains 0 (cons ratt (channel-rev-gains 0)))))))))
	...
	;; W: 0.707
	(vector-set! channel-gains w-offset (cons time (channel-gains w-offset)))
	(vector-set! channel-gains w-offset (cons attW (channel-gains w-offset)))
	;; X: (* (cos A) (cos E))
	(vector-set! channel-gains x-offset (cons time (channel-gains x-offset)))
	(vector-set! channel-gains x-offset (cons (* (if (zero? dist) 0 (/ y dist)) att) (channel-gains x-offset)))
	;; Y: (* (sin A) (cos E))
	(vector-set! channel-gains y-offset (cons time (channel-gains y-offset)))
	(vector-set! channel-gains y-offset (cons (* (if (zero? dist) 0 (/ (- x) dist)) att) (channel-gains y-offset)))
	(if (>= ambisonics-v-order 1)
	    (begin
	      ;; Z: (sin E)
	      (vector-set! channel-gains z-offset (cons time (channel-gains z-offset)))
	      (vector-set! channel-gains z-offset (cons (* (if (zero? dist) 0 (/ z dist)) att) (channel-gains z-offset)))))
	(if (>= ambisonics-v-order 2)
	    (begin
	      ;; R
	      (vector-set! channel-gains r-offset (cons time (channel-gains r-offset)))
	      (vector-set! channel-gains r-offset (cons (* (if (zero? dist) 0 (- (* 1.5 z z (if (zero? dist) 1 (/ 1 (* dist dist)))) 0.5)) att)
							   (channel-gains r-offset)))
	      ;; S
	      (vector-set! channel-gains s-offset (cons time (channel-gains s-offset)))
	      (vector-set! channel-gains s-offset (cons (* (if (zero? dist) 0 2) z (- x) (if (zero? dist) 1 (/ 1 (* dist dist))) att)
							(channel-gains s-offset)))
	      ;; T
	      (vector-set! channel-gains t-offset (cons time (channel-gains t-offset)))
	      (vector-set! channel-gains t-offset (cons (* (if (zero? dist) 0 2) z y (if (zero? dist) 1 (/ 1 (* dist dist))) att)
							(channel-gains t-offset)))))
	(if (>= ambisonics-h-order 2)
	    (begin
	      ;; U
	      (vector-set! channel-gains u-offset (cons time (channel-gains u-offset)))
	      (vector-set! channel-gains u-offset (cons (* (if (zero? dist) 0 1) (- (* x x (if (zero? dist) 1 (/ 1 (* dist dist))))
										    (* y y (if (zero? dist) 1 (/ 1 (* dist dist))))) att)
							(channel-gains u-offset)))
	      ;; V
	      (vector-set! channel-gains v-offset (cons time (channel-gains v-offset)))
	      (vector-set! channel-gains v-offset (cons (* (if (zero? dist) 0 2) (- x) y (if (zero? dist) 1 (/ 1 (* dist dist))) att)
							(channel-gains v-offset)))))
	...
	(if (> rev-channels 1)
	    (let ((ho-ratt dlocsig-ambisonics-ho-rev-scaler))
	      ...
	      ;; W: 0.707
	      (vector-set! channel-rev-gains w-offset (cons time (channel-rev-gains w-offset)))
	      (vector-set! channel-rev-gains w-offset (cons rattW (channel-rev-gains w-offset)))
	      ;; X: (* (cos A)(cos E))
	      (vector-set! channel-rev-gains x-offset (cons time (channel-rev-gains x-offset)))
	      (vector-set! channel-rev-gains x-offset (cons (* (if (zero? dist) 0 1) y (if (zero? dist) 1 (/ dist)) ratt)(channel-rev-gains x-offset)))
	      ;; Y: (* (sin A)(cos E))
	      (vector-set! channel-rev-gains y-offset (cons time (channel-rev-gains y-offset)))
	      (vector-set! channel-rev-gains y-offset (cons (* (if (zero? dist) 0 1) (- x) (if (zero? dist) 1 (/ dist)) ratt)
							    (channel-rev-gains y-offset)))
	      (if (>= ambisonics-v-order 1)
		  (begin
		    ;; Z: (sin E)
		    (vector-set! channel-rev-gains z-offset (cons time (channel-rev-gains z-offset)))
		    (vector-set! channel-rev-gains z-offset (cons (* (if (zero? dist) 0 1) z (if (zero? dist) 1 (/ dist)) ratt)
								  (channel-rev-gains z-offset)))))
	      (if (>= ambisonics-v-order 2)
		  (begin
		    ;; R
		    (vector-set! channel-rev-gains r-offset (cons time (channel-rev-gains r-offset)))
		    ...
		    ;; S
		    (vector-set! channel-rev-gains s-offset (cons time (channel-rev-gains s-offset)))
		    ...
		    ;; T
		    (vector-set! channel-rev-gains t-offset (cons time (channel-rev-gains t-offset)))
		  ...
		  (begin
		    ;; U
		    (vector-set! channel-rev-gains u-offset (cons time (channel-rev-gains u-offset)))
		    ...
		    ;; V
		    (vector-set! channel-rev-gains v-offset (cons time (channel-rev-gains v-offset)))
	...
	(let ((len (speaker-config-number speakers))
	  ...
	  (do ((i 0 (+ 1 i)))
	    ...
	    (let* ((s (spkrs i))
	      ...
	      (vector-set! channel-gains i (cons time (channel-gains i)))
	      (vector-set! channel-gains i (cons signal (channel-gains i))))))
	...
	(if (= rev-channels 1)
		...
		(vector-set! channel-rev-gains i (cons time (channel-rev-gains i)))
		(vector-set! channel-rev-gains i (cons signal (channel-rev-gains i))))))))
    ...
    (list 
	 ...
	 (do ((i 0 (+ 1 i)))
	     ((= i out-channels))
	   (vector-set! v i (make-env (reverse (channel-gains i))
	 ...
	 v)
	   ...
	   (let ((v (make-vector rev-channels)))
	     (do ((i 0 (+ 1 i)))
		 ((= i rev-channels))
	       (vector-set! v i (make-env (reverse (channel-rev-gains i))
