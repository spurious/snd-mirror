...
(define (type-it type)
  (if (and (not (string=? type "void"))
	   ...
	   (not (string=? type "gdouble"))
	   (not (string=? type "double"))
	   (not (string=? type "gfloat"))
	   (not (string=? type "char"))
	   (not (string=? type "gchar"))
	   (not (string=? type "char*"))
	   (not (string=? type "gchar*"))

	   (not (string=? type "guint"))
	   (not (string=? type "guint16"))
	   (not (string=? type "gint"))
	   (not (string=? type "gshort"))
      ...
      (hey "XM_TYPE(~A, ~A)~%" (no-stars type) type)))
...
(define (helpify args)
  (let ((len (string-length args))
    ...
    (if (< spaces 3)
	args
	(let ((newstr (make-string (+ len (inexact->exact (/ (1- spaces) 2)))))
	      (typed #f)
	      (j 0))
	  (do ((i 0 (1+ i)))
	      ((= i len))
		...
		(if typed
		    (begin
		      ...
		      (set! typed #f))
		    (set! typed #t)))
	    (string-set! newstr j (string-ref args i))
	    (set! j (1+ j)))
	  newstr))))
...
(hey "  XEN_LIST_2(C_STRING_TO_XEN_SYMBOL(Name), \\~%")
(hey "             C_TO_XEN_ULONG((unsigned long)Value))~%~%")
...
(hey "/* untagged types (direct conversion) */~%")
(hey "#define C_TO_XEN_int(Arg) C_TO_XEN_INT(Arg)~%")
(hey "#define C_TO_XEN_gint(Arg) C_TO_XEN_INT(Arg)~%")
(hey "#define C_TO_XEN_glong(Arg) C_TO_XEN_INT(Arg)~%")
(hey "#define C_TO_XEN_gboolean(Arg) C_TO_XEN_BOOLEAN(Arg)~%")
(hey "#define C_TO_XEN_gchar_(Arg) C_TO_XEN_STRING(Arg)~%")
(hey "#define C_TO_XEN_char_(Arg) C_TO_XEN_STRING(Arg)~%")
(hey "#define C_TO_XEN_gdouble(Arg) C_TO_XEN_DOUBLE(Arg)~%")
(hey "#define C_TO_XEN_double(Arg) C_TO_XEN_DOUBLE(Arg)~%")
(hey "#define C_TO_XEN_gfloat(Arg) C_TO_XEN_DOUBLE(Arg)~%")
...
(hey "#define C_TO_XEN_guint(Arg) C_TO_XEN_ULONG(Arg)~%")
(hey "#define C_TO_XEN_guint16(Arg) C_TO_XEN_INT(Arg)~%")
(hey "#define C_TO_XEN_gint(Arg) C_TO_XEN_INT(Arg)~%")
(hey "#define C_TO_XEN_gshort(Arg) C_TO_XEN_INT(Arg)~%")
(hey "#define C_TO_XEN_gint16(Arg) C_TO_XEN_INT(Arg)~%")
(hey "#define C_TO_XEN_guint8(Arg) C_TO_XEN_INT(Arg)~%")
(hey "#define C_TO_XEN_gint8(Arg) C_TO_XEN_INT(Arg)~%~%")

(hey "#define XEN_int_P(Arg) XEN_INTEGER_P(Arg)~%")
(hey "#define XEN_gint_P(Arg) XEN_INTEGER_P(Arg)~%")
(hey "#define XEN_glong_P(Arg) XEN_INTEGER_P(Arg)~%")
(hey "#define XEN_gboolean_P(Arg) XEN_BOOLEAN_P(Arg)~%")
(hey "#define XEN_char__P(Arg) XEN_STRING_P(Arg)~%")
(hey "#define XEN_gchar__P(Arg) XEN_STRING_P(Arg)~%")
(hey "#define XEN_gdouble_P(Arg) XEN_NUMBER_P(Arg)~%")
(hey "#define XEN_double_P(Arg) XEN_NUMBER_P(Arg)~%")
(hey "#define XEN_gfloat_P(Arg) XEN_NUMBER_P(Arg)~%")
...
(hey "#define XEN_gint_P(Arg) XEN_INTEGER_P(Arg)~%")
(hey "#define XEN_gshort_P(Arg) XEN_INTEGER_P(Arg)~%")
(hey "#define XEN_gint16_P(Arg) XEN_INTEGER_P(Arg)~%")
(hey "#define XEN_guint8_P(Arg) XEN_INTEGER_P(Arg)~%")
(hey "#define XEN_gint8_P(Arg) XEN_INTEGER_P(Arg)~%~%")

(hey "#define XEN_TO_C_int(Arg) XEN_TO_C_INT(Arg)~%")
(hey "#define XEN_TO_C_gint(Arg) XEN_TO_C_INT(Arg)~%")
(hey "#define XEN_TO_C_glong(Arg) XEN_TO_C_INT(Arg)~%")
(hey "#define XEN_TO_C_gboolean(Arg) XEN_TO_C_BOOLEAN(Arg)~%")
(hey "#define XEN_TO_C_char_(Arg) XEN_TO_C_STRING(Arg)~%")
(hey "#define XEN_TO_C_gchar_(Arg) XEN_TO_C_STRING(Arg)~%")
(hey "#define XEN_TO_C_gdouble(Arg) XEN_TO_C_DOUBLE(Arg)~%")
(hey "#define XEN_TO_C_double(Arg) XEN_TO_C_DOUBLE(Arg)~%")
(hey "#define XEN_TO_C_gfloat(Arg) XEN_TO_C_DOUBLE(Arg)~%")
...
(hey "#define XEN_TO_C_gint(Arg) XEN_TO_C_INT(Arg)~%")
(hey "#define XEN_TO_C_gshort(Arg) XEN_TO_C_INT(Arg)~%")
(hey "#define XEN_TO_C_gint16(Arg) XEN_TO_C_INT(Arg)~%")
(hey "#define XEN_TO_C_guint8(Arg) XEN_TO_C_INT(Arg)~%")
(hey "#define XEN_TO_C_gint8(Arg) XEN_TO_C_INT(Arg)~%~%")
 ...
 (lambda (data)
   (let* ((name (car data))
	 ...
	 (if (>= (length args) max-args)
		...
		args))))
     (hey ")~%{~%")
     (hey "  #define H_~A \"~A ~A(~A)\"~%" name return-type name (helpify argstr))
     (if (> refargs 0)
	 (for-each
	  (lambda (arg)
	    (if (= (length arg) 3)
		 ...
		 (for-each
		  (lambda (arg)
		    (let ((argname (cadr arg))
		      ...
		      (set! previous-arg #t)
		      (if (= (length arg) 3)
			  (hey "&~A" (deref-name arg))
			  (hey "XEN_TO_C_~A(~A)" (no-stars argtype) argname))))
		  args)))
	   (if (not (eq? lambda-type 'fnc))
	       (if (not (string=? return-type "void")) 
		   (hey ")"))
	 ...
	 (begin ; 'lambda
	   ...
	   (if (not (string=? return-type "void"))
	       (hey "       return(C_TO_XEN_~A(~A(" (no-stars return-type) name)
	       (hey "       ~A(" name))
	   (let ((previous-arg #f))
	     (for-each
	      (lambda (arg)
...
(hey "#if HAVE_GUILE~%")
(hey "      XEN_EVAL_C_STRING(\"(define xm-version \\\"12-Feb-02\\\")\");~%")
(hey "#endif~%")
