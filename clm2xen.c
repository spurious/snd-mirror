...
#define GEN_RF(Type, Func1, Func2)				\
  ...
  {									\
	...
	if (!s7_is_null(sc, s7_cdddr(expr))) return(NULL);		\
	    ...
	    s7_pointer slot;						\
	    slot = s7_slot(sc, a2);					\
            if (slot != xen_undefined) {s7_xf_store(sc, (s7_pointer)slot); return(Type ## _rf_s);} \
	    return(NULL);						\
	  }								\
        if (s7_is_pair(a2))						\
...
static s7_rf_t is_delay_rf_3(s7_scheme *sc, s7_pointer expr)
{
  mus_any *g;
  int len;
  len = s7_list_length(sc, expr);
  if (len < 4) return(is_delay_rf(sc, expr));
  if (len > 5) return(NULL);
  g = cadr_gen(sc, expr);
...
static s7_double ssb_am_rf_sss(s7_scheme *sc, s7_pointer **p)
{
  ...
  s1 = (**p); (*p)++;
  s2 = (**p); (*p)++;
  return(mus_ssb_am(g, s7_number_to_real(sc, s7_slot_value(s1)), s7_number_to_real(sc, s7_slot_value(s2))));
}

static s7_rf_t is_ssb_am_rf_3(s7_scheme *sc, s7_pointer expr)
{
  mus_any *g;
  int len;
  len = s7_list_length(sc, expr);
  if (len < 4) return(is_ssb_am_rf(sc, expr));
  if (len > 5) return(NULL);
  g = cadr_gen(sc, expr);
  ...
  if ((g) && (mus_is_formant_bank(g)))
      ...
      if (s7_is_symbol(a1))
	{
	  ...
	  if (s7_is_float_vector(s7_slot_value(slot)))
	    {
	      s7_xf_store(sc, (s7_pointer)s7_float_vector_elements(s7_slot_value(slot)));
...
static s7_rf_t is_set_formant_frequency_rf(s7_scheme *sc, s7_pointer expr)
{
  mus_any *g;					
  if (!s7_is_null(sc, s7_cdddr(expr))) return(NULL);
  g = cadr_gen(sc, expr);						
...
static s7_double in_any_rf_srs(s7_scheme *sc, s7_pointer **p)
{
  ...
  chan = (**p); (*p)++;
  stream = (mus_any *)(**p); (*p)++;
  return(mus_in_any(s7_integer(s7_slot_value(ind_slot)), s7_integer(chan), stream));
}

static s7_rf_t is_in_any_rf(s7_scheme *sc, s7_pointer expr)
{
...
static s7_double pink_noise_rf(s7_scheme *sc, s7_pointer **p)
{
  s7_pointer s1;
  s1 = s7_slot_value(**p); (*p)++;
...
static s7_rf_t is_pink_noise_rf(s7_scheme *sc, s7_pointer expr)
{
  if (s7_is_symbol(s7_cadr(expr)))
    {
      s7_pointer slot;
...
static void init_choosers(s7_scheme *sc)
{
  ...
  s7_rf_set_function(s7_name_to_value(sc, S_outa), is_outa_rf);
  ...
  s7_rf_set_function(s7_name_to_value(sc, "granulate"), is_granulate_rf);
  s7_rf_set_function(s7_name_to_value(sc, S_ncos), is_ncos_rf);
  s7_rf_set_function(s7_name_to_value(sc, S_nrxycos), is_nrxycos_rf);
  s7_rf_set_function(s7_name_to_value(sc, "env"), is_env_rf_v);
  s7_rf_set_function(s7_name_to_value(sc, "readin"), is_readin_rf);
  s7_rf_set_function(s7_name_to_value(sc, "one-pole"), is_one_pole_rf);
  s7_rf_set_function(s7_name_to_value(sc, "moving-average"), is_moving_average_rf);
  s7_rf_set_function(s7_name_to_value(sc, "moving-max"), is_moving_max_rf);
  s7_rf_set_function(s7_name_to_value(sc, "fir-filter"), is_fir_filter_rf);
  s7_rf_set_function(s7_name_to_value(sc, "triangle-wave"), is_triangle_wave_rf);
  s7_rf_set_function(s7_name_to_value(sc, "pulse-train"), is_pulse_train_rf);
  s7_rf_set_function(s7_name_to_value(sc, "rand-interp"), is_rand_interp_rf);
  s7_rf_set_function(s7_name_to_value(sc, "formant"), is_formant_rf_3);
  s7_rf_set_function(s7_name_to_value(sc, "one-pole-all-pass"), is_one_pole_all_pass_rf);
  s7_rf_set_function(s7_name_to_value(sc, "delay"), is_delay_rf_3);
  s7_rf_set_function(s7_name_to_value(sc, "formant-bank"), is_formant_bank_rf);
  s7_rf_set_function(s7_name_to_value(sc, "oscil-bank"), is_oscil_bank_rf);
  s7_rf_set_function(s7_name_to_value(sc, "rand"), is_rand_rf);
  s7_rf_set_function(s7_name_to_value(sc, "filter"), is_filter_rf);
  s7_rf_set_function(s7_name_to_value(sc, S_table_lookup), is_table_lookup_rf);
  s7_rf_set_function(s7_name_to_value(sc, S_src), is_src_rf);
  s7_rf_set_function(s7_name_to_value(sc, "sawtooth-wave"), is_sawtooth_wave_rf);
  s7_rf_set_function(s7_name_to_value(sc, S_inb), is_inb_rf);
  ...
  s7_rf_set_function(s7_name_to_value(sc, "rxyk!cos"), is_rxykcos_rf);
  s7_rf_set_function(s7_name_to_value(sc, "tap"), is_tap_rf);
  s7_rf_set_function(s7_name_to_value(sc, "comb"), is_comb_rf_3);
  s7_rf_set_function(s7_name_to_value(sc, "comb-bank"), is_comb_bank_rf);
  s7_rf_set_function(s7_name_to_value(sc, "notch"), is_notch_rf_3);
  s7_rf_set_function(s7_name_to_value(sc, "two-zero"), is_two_zero_rf);
  s7_rf_set_function(s7_name_to_value(sc, "one-zero"), is_one_zero_rf);
  s7_rf_set_function(s7_name_to_value(sc, "two-pole"), is_two_pole_rf);
  s7_rf_set_function(s7_name_to_value(sc, "moving-norm"), is_moving_norm_rf);
  s7_rf_set_function(s7_name_to_value(sc, "iir-filter"), is_iir_filter_rf);
  s7_rf_set_function(s7_name_to_value(sc, "square-wave"), is_square_wave_rf);
  s7_rf_set_function(s7_name_to_value(sc, "firmant"), is_firmant_rf);
  s7_rf_set_function(s7_name_to_value(sc, "all-pass"), is_all_pass_rf_3);
  s7_rf_set_function(s7_name_to_value(sc, "all-pass-bank"), is_all_pass_bank_rf);
  s7_rf_set_function(s7_name_to_value(sc, "polyshape"), is_polyshape_rf);
  s7_rf_set_function(s7_name_to_value(sc, "pulsed-env"), is_pulsed_env_rf);
  s7_rf_set_function(s7_name_to_value(sc, S_ssb_am), is_ssb_am_rf_3);
  s7_rf_set_function(s7_name_to_value(sc, "asymmetric-fm"), is_asymmetric_fm_rf);
  s7_rf_set_function(s7_name_to_value(sc, "filtered-comb"), is_filtered_comb_rf);
  s7_rf_set_function(s7_name_to_value(sc, "filtered-comb-bank"), is_filtered_comb_bank_rf);
  s7_rf_set_function(s7_name_to_value(sc, "move-sound"), is_move_sound_rf);
  s7_rf_set_function(s7_name_to_value(sc, S_locsig), is_locsig_rf);
  s7_rf_set_function(s7_name_to_value(sc, S_out_bank), is_out_bank_rf);
  s7_rf_set_function(s7_name_to_value(sc, "phase-vocoder"), is_phase_vocoder_rf);
  s7_rf_set_function(s7_name_to_value(sc, "convolve"), is_convolve_rf);
  s7_rf_set_function(s7_name_to_value(sc, "sample->file"), is_sample_to_file_rf);
  s7_rf_set_function(s7_name_to_value(sc, "mus-srate"), is_srate_rf);
  s7_rf_set_function(s7_name_to_value(sc, "contrast-enhancement"), is_contrast_enhancement_rf);
  s7_rf_set_function(s7_name_to_value(sc, "mus-set-formant-frequency"), is_set_formant_frequency_rf);
  s7_rf_set_function(s7_name_to_value(sc, "odd-weight"), is_odd_weight_rf);
  s7_rf_set_function(s7_name_to_value(sc, "even-weight"), is_even_weight_rf);
  s7_rf_set_function(s7_name_to_value(sc, "odd-multiple"), is_odd_multiple_rf);
  s7_rf_set_function(s7_name_to_value(sc, "even-multiple"), is_even_multiple_rf);
  s7_rf_set_function(s7_name_to_value(sc, "hz->radians"), is_hz_to_radians_rf);
  s7_rf_set_function(s7_name_to_value(sc, "radians->hz"), is_radians_to_hz_rf);
  s7_rf_set_function(s7_name_to_value(sc, "sin"), is_sin_rf);
  s7_rf_set_function(s7_name_to_value(sc, "cos"), is_cos_rf);
  s7_rf_set_function(s7_name_to_value(sc, "mus-random"), is_random_rf);
  s7_rf_set_function(s7_name_to_value(sc, "amplitude-modulate"), is_am_rf);
  s7_rf_set_function(s7_name_to_value(sc, "array-interp"), is_array_interp_rf);
}
#endif
