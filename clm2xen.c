...
#if HAVE_SCHEME  
		      out_any_2(pos, mus_delay_unmodulated_noz(d1, x), 0, "out-bank");
#else
		      out_any_2(CLM_OUTPUT, pos, mus_delay_unmodulated_noz(d1, x), 0, "out-bank");
#endif
		    }
		}
	      else
...
#if HAVE_SCHEME  
		      for (i = 0; i < size; i++)
			out_any_2(pos, mus_delay_unmodulated_noz(ds[i], x), i, "out-bank");
#else
		      for (i = 0; i < size; i++)
