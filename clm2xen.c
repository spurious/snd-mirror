...
{
  #define H_radians_hz "(" S_radians_hz " rads) converts radians per sample to frequency in Hz: rads * srate / (2 * pi)"
...
{
  #define H_hz_radians "(" S_hz_radians " hz) converts frequency in Hz to radians per sample: hz * 2 * pi / srate"
...
static XEN g_ramp(XEN obj)
{
  ...
  return(C_TO_XEN_INT(mus_ramp(XEN_TO_MUS_ANY(obj))));
}
  ...
  if (vals > 0)
      ...
      if (XEN_PROCEDURE_P(keys[0]))
	{
	  if (XEN_REQUIRED_ARGS(keys[0]) == 1)
	    in_obj = keys[0];
	  else XEN_BAD_ARITY_ERROR(S_make_phase_vocoder, 1, keys[0], "input function wants 1 arg");
	}
      ...
      if (XEN_PROCEDURE_P(keys[5]))
	{
	  if (XEN_REQUIRED_ARGS(keys[5]) == 2)
	    analyze_obj = keys[5];
	  else XEN_BAD_ARITY_ERROR(S_make_phase_vocoder, 6, keys[5], "analyze function wants 2 args: pv infunc");
	}
      if (XEN_PROCEDURE_P(keys[6]))
	{
	  if (XEN_REQUIRED_ARGS(keys[6]) == 1)
	    edit_obj = keys[6];
	  else XEN_BAD_ARITY_ERROR(S_make_phase_vocoder, 7, keys[6], "edit function wants 1 arg");
	}
      if (XEN_PROCEDURE_P(keys[7]))
	{
	  if (XEN_REQUIRED_ARGS(keys[7]) == 1)
	    synthesize_obj = keys[7];
	  else XEN_BAD_ARITY_ERROR(S_make_phase_vocoder, 8, keys[7], "synthesize function wants 1 arg");
	}
    }
  gn = (mus_xen *)CALLOC(1, sizeof(mus_xen));
  old_error_handler = mus_error_set_handler(local_mus_error);
...
XEN_NARGIFY_1(g_x1_w, g_x1)
XEN_NARGIFY_2(g_set_x1_w, g_set_x1)
XEN_NARGIFY_1(g_x2_w, g_x2)
XEN_NARGIFY_2(g_set_x2_w, g_set_x2)
XEN_NARGIFY_1(g_y1_w, g_y1)
XEN_NARGIFY_2(g_set_y1_w, g_set_y1)
XEN_NARGIFY_1(g_y2_w, g_y2)
XEN_NARGIFY_2(g_set_y2_w, g_set_y2)
...
#define g_x1_w g_x1
#define g_set_x1_w g_set_x1
#define g_x2_w g_x2
#define g_set_x2_w g_set_x2
#define g_y1_w g_y1
#define g_set_y1_w g_set_y1
#define g_y2_w g_y2
#define g_set_y2_w g_set_y2
