   ...
   (lambda (return)
	 ...
	 (let ((biggest 0.0)
	   ...
	   (do ((j 0 (1+ j)))
		 ...
		 (begin
		   (do ((k 0 (1+ k)))
			 ...
			 (let ((val (abs (mixer-ref matrix j k))))
			   (if (> val biggest)
			       (begin
				 (set! col k)
				 (set! row j)
	   ...
	   (if (not (= row col))
		 ...
		 (do ((k 0 (1+ k)))
		     ((= k n))
		   (set! temp (mixer-ref matrix row k))
		   (mixer-set! matrix row k (mixer-ref matrix col k))
		   (mixer-set! matrix col k temp))))
	   ...
	   (let ((inverse-pivot (/ 1.0 (mixer-ref matrix col col))))
	     ...
	     (do ((k 0 (1+ k)))
		 ((= k n))
	       (mixer-set! matrix col k (* inverse-pivot (mixer-ref matrix col k))))
	     (if b (frame-set! b col (* inverse-pivot (frame-ref b col)))))
	   (do ((k 0 (1+ k)))
	       ((= k n))
	     (if (not (= k col))
		 (let ((scl (mixer-ref matrix k col)))
		   (mixer-set! matrix k col 0.0)
		   (do ((m 0 (1+ m)))
		       ((= m n))
		     (mixer-set! matrix k m (- (mixer-ref matrix k m) (* scl (mixer-ref matrix col m)))))
	 ...
	 (if (not (= (vector-ref rows i) (vector-ref cols i)))
	     (do ((k 0 (1+ k)))
		 ((= k n))
	       (let ((temp (mixer-ref matrix k (vector-ref rows i))))
		 (mixer-set! matrix k (vector-ref rows i) (mixer-ref matrix k (vector-ref cols i)))
		 (mixer-set! matrix k (vector-ref cols i) temp)))))
       (list matrix b)))))
