...
(definstrument (fm-voice beg dur pitch amp vowel-1 sex-1 ampfun1 ampfun2 ampfun3 indxfun skewfun vibfun ranfun
  ...
  (fillfnc)
  (let* ((c 261.62)
	 (vowel (inexact->exact (floor vowel-1)))
	 (sex (inexact->exact (floor sex-1)))
	 (ampref (expt amp .8))
	 (deg (- deg 45))
	 ...
	 (fmntfreq1 (inexact->exact (round form1)))
	 (fmntfreq2 (inexact->exact (round form2)))
	 (fmntfreq3 (inexact->exact (round form3)))
	 (fm2 3)
	 (mscale 1)
	 (mconst 0)
    ...
    (let* ((gens1 (make-oscil :frequency 0))
	   (gens2 (make-oscil :frequency 0 :initial-phase (/ pi 2.0)))
	   (gens2ampenv (make-env indxfun  :duration dur
				  :scaler (* scdev1 dev1)
				  :offset (* scdev1 dev0)))
	   (gens3 (make-oscil :frequency 0 :initial-phase (/ pi 2.0)))
	   (gens3ampenv (make-env indxfun  :duration dur
				  :scaler (* scdev2 dev1)
				  :offset (* scdev2 dev0)))
	   (gens4 (make-oscil :frequency 0 :initial-phase (/ pi 2.0)))
	   (gens4ampenv (make-env indxfun  :duration dur
				  :scaler (* scdev3 dev1)
				  :offset (* scdev3 dev0)))
	   (gens5 (make-oscil :frequency 0))
	   (gens5ampenv (make-env ampfun1 :duration dur
				  :scaler (* amp caramp1sc .75)))
	   (gens6 (make-oscil :frequency 0))
	   (gens6ampenv (make-env ampfun2 :duration dur
				  :scaler (* amp caramp2sc .75)))
	   (gens7 (make-oscil :frequency 0))
