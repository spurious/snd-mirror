 ...
 *
 * (with-sound () (fm-violin 0 20 440 .1))                          1068     642     561     528    479    3692   7.7     3248  6.8   [546 in v.c]
 * (with-sound (:channels 2) (fm-violin 0 20 440 .1 :degree 45))    1228     764     687     570    505    3722   7.4     3325  6.6
 * (with-sound (:reverb jc-reverb) (fm-violin 0 20 440 .1))         2577    1455    1335    1153    948    8747   9.2     8031  8.5
 * (with-sound (:reverb nrev) (fm-violin 0 20 440 .1))              2983    1812    1685    1503   1203   10443   8.7     9645  8.0
 * (with-sound () (p 0 3))                                         91020*   3011    2828    2817   1965   43241  22.0    41214 21.0
 * (with-sound () (expandn 0 10 "oboe.snd" 1 :expand 4))            1228     526     464     456    301    3526  11.7     3205 10.6
 * (calling-all-animals)                                           16359   11684   10306    9841  10529   55207   5.2    51451  4.9
 * (with-sound () (pins 0 3 "oboe.snd" 1.0 :max-peaks 8))           1207     783     660     700    544    5955  10.9     5484 10.1
 * (load "popi.scm")                                               11042    6391    5923    4756   4154   34163   8.2    32893  7.9
 *
 * (with-sound ()                                                   1015     641     562     674    363    7089  19.5     6828 18.8
 *   (singer 0 .1 
 *     (list (list .4 ehh.shp test.glt 523.0 .8 0.0 .01) 
 *           (list .6 oo.shp test.glt 523.0 .7 .1 .01))))
 *
 * (with-sound (:channels 2)                                         206     139      93     107     72     240   3.4      242  3.4
 *   (let ((file "oboe.snd")) 
 *     (grani 0 1 .5 "oboe.snd" 
 *       :grain-envelope '(0 0 0.2 0.2 0.5 1 0.8 0.2 1 0))))
 *
 * (with-sound ()                                                   7120    5069    4064    3996   3560   21380   6.0    18482  5.2
 *   (do ((i 0 (+ i 1))) 
 *       ((= i 10000)) 
 *     (fm-violin (* i .001) .01 440 .001)))
 *
 * (with-sound (:channels 2)                                         283     220     158     167    101     286   2.8      250  2.5
 *   (fullmix "pistol.snd" 0 2 0 #f .5)  
 *   (fullmix "oboe.snd" 1 2 0 (list (list .1 (make-env '(0 0 1 1) :duration 2 :scaler .5)))))
 *
 *                                      1st case in clm-ins.scm:    12201   1138    1043     968    707    3025   4.3     2849  4.0
 */
