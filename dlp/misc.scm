...
(if (not (defined? 'install-searcher)) (load-from-path "snd-motif.scm"))
(if (not (defined? 'explode-sf2)) (load-from-path "examp.scm"))
(if (not (defined? 'channel-property)) (load-from-path "extensions.scm"))
(if (not (defined? 'down-oct)) (load-from-path "dsp.scm"))
(if (not (defined? 'make-current-window-display)) (load-from-path "draw.scm"))
(if (not (defined? 'envelope-interp)) (load-from-path "env.scm"))
(if (not (defined? 'channel-envelope)) (load-from-path "enved.scm"))
(if (not (defined? 'reset-all-hooks)) (load-from-path "hooks.scm"))
(if (not (defined? 'describe-mark)) (load-from-path "marks.scm"))
(if (not (defined? 'mix-sound)) (load-from-path "mix.scm"))
(if (not (defined? 'moog-filter)) (load-from-path "moog.scm"))
(if (not (defined? 'play-often)) (load-from-path "play.scm"))
(if (not (defined? 'selection-popup-menu)) (load-from-path "popup.scm"))
(if (not (defined? 'rubber-sound)) (load-from-path "rubber.scm"))
(if (not (defined? 'zipper)) (load-from-path "zip.scm"))
(if (not (defined? 'make-effects-dialog)) (load-from-path "new-effects.scm"))
(if (not (defined? 'forward-mix)) (load-from-path "snd7.scm"))
(if (not (defined? 'special-menu)) (load-from-path "special-menu.scm"))
(if (not (defined? 'smoke)) (load-from-path "new-backgrounds.scm"))
(if (not (defined? 'marks-menu)) (load-from-path "marks-menu.scm"))
(if (not (defined? 'mix-menu))(load-from-path "mix-menu.scm"))
(if (not (defined? 'add-useful-icons)) (load-from-path "new-buttons.scm"))
(if (not (defined? 'panic-menu)) (load-from-path "panic.scm"))
(if (not (defined? 'install-ladspa-menues)) (load-from-path "ladspa.scm"))
(if (not (defined? 'ladspa-help-texts)) (load-from-path "ladspa-help.scm"))
(if (not (defined? 'fft-menu)) (load-from-path "fft-menu.scm"))
