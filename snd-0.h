...
typedef enum {SOUND_SAVE_AS, SELECTION_SAVE_AS, REGION_SAVE_AS, MIX_SAVE_AS, TRACK_SAVE_AS} save_dialog_t;
...
typedef enum {FILE_VIEWER, REGION_VIEWER} file_viewer_t;
typedef enum {COLOR_DIALOG, ORIENTATION_DIALOG, ENVED_DIALOG, UNUSED_DIALOG, YES_OR_NO_DIALOG, TRANSFORM_DIALOG,
	      ...
	      PRINT_DIALOG, RECORDER_DIALOG, REGION_DIALOG, POST_IT_DIALOG, TRACK_DIALOG, SELECTION_SAVE_AS_DIALOG,
              FILE_INSERT_DIALOG} snd_dialog_t;
#define NUM_DIALOGS 24
...
typedef enum {NOT_FILING, INPUT_FILING, SELECTION_FILING, CHANNEL_FILING, TEMP_FILING, CHANGE_FILING, 
	      INSERT_FILING, MACRO_FILING, DOIT_SELECTION_FILING, DOIT_CHANNEL_FILING} sp_filing_t;

typedef enum {IO_NO_ERROR, IO_SAVE_HOOK_CANCELLATION, IO_BAD_CHANNEL, IO_BAD_EDIT_POSITION, IO_READ_PROTECTED,
	      IO_NO_FILE, IO_BAD_LOCATION, IO_CANT_REOPEN_FILE, IO_TOO_MANY_OPEN_FILES, IO_UNKNOWN_SNDLIB_ERROR,
	      IO_NO_MEMORY, IO_CANT_OPEN_FILE, IO_NO_FILENAME, IO_BAD_DATA_FORMAT, IO_BAD_HEADER_TYPE,
	      IO_SNDLIB_UNINITIALIZED, IO_NOT_A_SOUND_FILE, IO_FILE_CLOSED, IO_WRITE_ERROR, IO_READ_ERROR,
	      IO_INTERRUPTED, IO_CANT_CLOSE_FILE, IO_BAD_HEADER, IO_DISK_FULL, IO_WRITE_PROTECTED,
	      IO_WRITE_CANCELLATION, IO_CANT_MOVE_FILE, IO_CANT_REMOVE_FILE, IO_CANT_READ_SELECTION_FILE,
	      IO_CANT_OPEN_TEMP_FILE, IO_NEED_WRITE_CONFIRMATION, IO_NO_CHANGES
} io_error_t;
...
typedef enum {SORT_BY_NAME, SORT_BY_DATE, SORT_BY_SIZE, SORT_BY_ENTRY, SORT_BY_PROC} view_files_sort_t;
...
typedef enum {NO_REQUESTOR, FROM_UPDATE, FROM_VIEW_FILES, FROM_SAVE_AS_DIALOG, FROM_DRAG_AND_DROP, FROM_OPEN_DIALOG,
	      ...
	      FROM_OPEN_RAW_SOUND, FROM_VIEW_SOUND, FROM_NEW_SOUND, FROM_RAW_DATA_DIALOG, FROM_MIX_DIALOG,
              FROM_INSERT_DIALOG} open_requestor_t;

typedef enum {AFTER_GRAPH_HOOK, LISP_GRAPH_HOOK, BEFORE_TRANSFORM_HOOK, MIX_RELEASE_HOOK, SAVE_HOOK, MUS_ERROR_HOOK, 
	      MOUSE_ENTER_GRAPH_HOOK, MOUSE_LEAVE_GRAPH_HOOK, OPEN_RAW_SOUND_HOOK, SELECT_CHANNEL_HOOK, AFTER_OPEN_HOOK, 
	      CLOSE_HOOK, DROP_HOOK, UPDATE_HOOK, JUST_SOUNDS_HOOK, MARK_CLICK_HOOK, MARK_DRAG_HOOK, NAME_CLICK_HOOK, 
	      OPEN_HOOK, HELP_HOOK, BEFORE_SAVE_STATE_HOOK, OUTPUT_COMMENT_HOOK, PLAY_HOOK, SND_ERROR_HOOK, SND_WARNING_HOOK, 
	      START_HOOK, START_PLAYING_HOOK, STOP_PLAYING_HOOK, MOUSE_ENTER_LISTENER_HOOK, MOUSE_LEAVE_LISTENER_HOOK, 
	      WINDOW_PROPERTY_CHANGED_HOOK, SELECT_SOUND_HOOK, PRINT_HOOK, EXIT_HOOK, OUTPUT_NAME_HOOK, DURING_OPEN_HOOK, 
	      AFTER_TRANSFORM_HOOK, MOUSE_ENTER_LABEL_HOOK, MOUSE_LEAVE_LABEL_HOOK, INITIAL_GRAPH_HOOK, GRAPH_HOOK, 
	      KEY_PRESS_HOOK, MOUSE_DRAG_HOOK, MOUSE_PRESS_HOOK, ENVED_HOOK, READ_HOOK, MOUSE_CLICK_HOOK, NEW_WIDGET_HOOK, 
	      MARK_HOOK, VIEW_FILES_SELECT_HOOK, DAC_HOOK, STOP_DAC_HOOK, STOP_PLAYING_SELECTION_HOOK, AFTER_APPLY_HOOK, 
	      DRAW_MARK_HOOK, BAD_HEADER_HOOK, SAVE_STATE_HOOK, NEW_SOUND_HOOK, COLOR_HOOK, ORIENTATION_HOOK, 
	      LISTENER_CLICK_HOOK, MIX_CLICK_HOOK, AFTER_SAVE_STATE_HOOK, MOUSE_ENTER_TEXT_HOOK, MOUSE_LEAVE_TEXT_HOOK, 
	      OPTIMIZATION_HOOK, MIX_DRAG_HOOK, MARK_DRAG_TRIANGLE_HOOK, START_PLAYING_SELECTION_HOOK, RECORDER_FILE_HOOK, 
	      SELECTION_CHANGED_HOOK, AFTER_SAVE_AS_HOOK, BEFORE_SAVE_AS_HOOK, EDIT_HOOK, AFTER_EDIT_HOOK, UNDO_HOOK} eval_hook_t;
...
#define DEFAULT_VIEW_FILES_SORT 0
