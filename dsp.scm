	 ...
	 (map-channel
	  (lambda (y)
	    (let ((sum 0.0)
	      ...
	      (do ((i 0 (+ i 1)))
		  ((= i pairs))
		(set! gen (vector-ref ssbs i))
		(set! inval (bandpass (vector-ref bands i) y))
		(set! sum (+ sum (ssb-am gen inval))))
	 ...
	 (map-channel
	  (lambda (y)
	    (let ((sum 0.0)
	      ...
	      (do ((i 0 (+ i 1)))
		  ((= i pairs))
		(set! ssb (vector-ref ssbs i))
		(set! band (bandpass (vector-ref bands i) y))
		(set! e (env (vector-ref frenvs i)))
		(set! sum (+ sum (ssb-am ssb band e))))
	      (set! nmx (max nmx (abs sum)))
	      sum))
	  beg dur snd chn edpos)
