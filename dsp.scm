...
(define* (down-oct n snd chn)
  "(down-n n) moves a sound down by n-1 octaves"
...
(define (lpc-coeffs data n m)
  ...
  (let ((d (make-vector m 0.0 #t))
	(wk1 (make-vector n 0.0 #t))
	(wk2 (make-vector n 0.0 #t))
	(wkm (make-vector n 0.0 #t)))
    (copy data wk1)
    (do ((j 1 (+ j 1))
	 (k 0 (+ k 1)))
	((= j n))
...
(define* (lpc-predict data n coeffs m nf clipped)
  ...
  "(lpc-predict data n coeffs m nf clipped) takes the output of lpc-coeffs ('coeffs', a float-vector) and the length thereof ('m'), \
...
(define* (make-savitzky-golay-filter size (order 2)) ;assuming symmetric filter (left = right)
  (if (even? size) 
      (set! size (+ size 1)))
  (let ((n (/ (- size 1) 2))
	(a (make-vector (list (+ 1 order) (+ 1 order)) 0.0 #t)))
    (do ((i 0 (+ i 1)))
	((> i (* order 2)))
    ...
    (let ((b (matrix-solve a (let ((f (make-vector (+ order 1) 0.0 #t)))
			      (set! (f 0) 1.0) ; set others instead for derivative
			      f)))
	  (result (make-vector size 0.0 #t)))
      (do ((k (- n) (+ k 1))
	   (i 0 (+ i 1)))
	  ((> k n))
	(let ((sum (b 0))
