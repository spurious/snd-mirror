  ...
  if (mus_audio_api() == MUS_ALSA_API) 
      ...
      for (d = 0; d < alloc_devs; d++) 
	{
	  ...
	  dev_fd[d] = mus_audio_open_output(alsa_devices[out_dev[d]], 
					    ...
					    channels, 
					    snd_dacp->out_format, 
					    snd_dacp->framples * channels * mus_bytes_per_sample(snd_dacp->out_format));
	  unset_dac_print();
...
{
#if HAVE_SCHEME
  xen_player_tag = s7_new_type_x("<player>", print_xen_player, free_xen_player, s7_xen_player_equalp, NULL, NULL, NULL, s7_xen_player_length, NULL, NULL, NULL);
#else
#if HAVE_RUBY
