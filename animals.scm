	...
	(let ((reset-stop (min stop (+ i pulse-out))))
	  ...
	  (do ((k i (+ k 1)))
	    ...
	    (let ((val (* (env ampf)
	      ...
	      (outa k (+ (* val (env ampf1))
			 (* (env ampfr1) (formant frm1 val))
			 (* (env ampfr2) (formant frm2 val))
			 (* ampfr3 (formant frm3 val))
			 (* (env ampfr4) (formant frm4 val))))))
	...
	(let ((reset-stop (min stop (+ i pulse-samps))))
	  (do ((k i (+ k 1)))
	    ...
	    (let ((frq (env frqf))
	      ...
	      (outa k (* (env ampf)
			 (env pulsef)
			 (+ (* .5 (oscil gen1 (+ frq (* index (oscil fmd (/ frq 15))))))
...
(defanimal (river-frog beg amp)
  (let ((dur 1.85)
    ...
    (let ((start (seconds->samples beg))
	  ...
	  (pulsef (make-env '(0 .1 3 .1 3.1 1 4 1 6 .1 9 .1) :duration (/ 1.0 pulse-pitch)))
	...
	(let ((reset-stop (min stop (+ i pulse-samps))))
	  (do ((k i (+ k 1)))
	    ...
	    (let ((frq (+ (env frqf)
	      ...
	      (outa k (* (env ampf)
			 (env pulsef)
			 (+ (* (- 1.0 intrp) (polywave poly1 frq))
			    (* intrp (polywave poly2 frq)))))))
	  (mus-reset pulsef))))))

;;; pulsed-env here was basically the same speed as using the nested do loop
;;;   so pulsed-env doesn't buy us anything if there's no run-time modulation
...
(defanimal (mosquito beg dur freq amp)
  (let ((start (seconds->samples beg))
	...
	(let ((pitch (oscil carrier frq)))
	  (outa i (* (env ampf)
		     (+ (* 0.6 (oscil modulator1 (+ (* 2 frq) (* hfreq (+ 1.0 (rand-interp indf)) pitch))))
...
(defanimal (linnaeus-cicada beg dur amp)
  (let ((start (seconds->samples beg))
	...
	((= i stop))
      (let* ((frq (env frqf))
	     (md (+ frq (rand-interp rnd)))
	     (sw (sawtooth-wave saw)))
	(outa i (* (env ampf)
		   (+ .75 (* sw sw))
		   (+ (* .8 (oscil gen1 md))
		      (* .15 (oscil gen2 (* md 0.8593))) ;(/ 1100 1280))))
		      (* .07 (oscil gen3 (* md 1.1406))) ;(/ 1460 1280))))
		      (* .02 (oscil gen4 (* 2.0 md)))
		      (* .02 (oscil gen5 (* 3.0 md))))))))))

;; (with-sound () (linnaeus-cicada 0 2 .5))
...
(defanimal (striped-ground-cricket beg dur amp)
  (let ((start (seconds->samples beg))
	...
	(long-pulse-out (seconds->samples .15)) ; start with 10 pulses .015 each
	...
	(set! pulses (+ 8 (random 3))))
      (set! long-pulse-out (seconds->samples (* .015 pulses))))))
	...
	(do ((i start (+ i pulse-samps)))
	  ...
	  (let ((reset-stop (min stop (+ i pulse-out))))
		...
		((= k reset-stop))
	      (outa k (* (env ampf)
			 (env pulsef)
			 (oscil gen1 (+ (env frqf) (rand-interp rnd))))))
	    (mus-reset pulsef)
	    (mus-reset frqf)))))))
	  ...
	  (let* ((frq (+ (env frqf)
		 ...
		 (val (* (env ampf)
			 (+ (polywave histuff (* 2 frq))
			    ...
			    (* (env midf) (polywave midstuff (* 2 frq)))))))
...
(defanimal (bushtit beg amp)
  (let ((dur 0.368)
    ...
    (let ((stop (seconds->samples (+ beg dur)))
      ...
      (let ((next-pulse (+ start pulse-spacing))
...
(define (virginia-rail beg amp)
  ...
  (defanimal (virginia-rail-1 beg amp)
	...
	(do ((i start (+ i 1)))
	    ((= i stop))
	  (outa i (* (env ampf)
		     (+ (* .55 (polywave gen1 (rand-interp rnd)))
			(* .45 (polywave gen2)))))))))
