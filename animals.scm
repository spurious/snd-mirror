...
(defanimal (linnaeus-cicada beg dur amp)
  (let ((start (seconds->samples beg))
	...
	(gp (make-polywave 1280 '(1 .8 2 .02 3 .02)))
	...
	(let ((md (+ frq (rand-interp rnd))))
	  (outa i (* (env ampf)
		     (+ .75 (* sw sw))
		     (+ (polywave gp md)
			(* .15 (oscil gen2 (* 0.8593 md)))          ;(/ 1100 1280)
			(* .07 (oscil gen3 (* 1.1406 md)))))))))))  ;(/ 1460 1280)
  ...
  (let ((pitch 1690))
	...
	(let ((reset-stop (min stop (+ i on-samps))))
	  (do ((k i (+ k 1)))
	      ((= k reset-stop))
	    (outa k (* (env pulsef)
		       (polywave gp (+ (rand-interp rnd) (env frqf))))))
	  (mus-reset pulsef)
	  (mus-reset frqf))))))
