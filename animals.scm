...
(defanimal (southwestern-toad beg dur amp)
  (let ((pulse-dur 0.0135)
	(pulse-space 0.0236)
	(start (seconds->samples beg)))
    (let ((pulse-samps (seconds->samples pulse-space)))
      (let ((stop (seconds->samples (+ beg dur)))
...
(defanimal (striped-ground-cricket beg dur amp)
  (let ((start (seconds->samples beg))
	...
	((>= i stop))
      (let ((reset-stop (min stop (+ i long-pulse-samps))))
	(do ((k i (+ k 1)))
	    ((= k reset-stop))
...
(defanimal (carolina-wren beg amp)
  (let ((dur 1.84)
    ...
    (let ((start (seconds->samples beg))
	  ...
	  (pulse-samps (seconds->samples (/ 1.6 6.0))))
      (do ((i start (+ i pulse-samps)))
	  ((>= i stop))
	(let ((reset-stop (min stop (+ i pulse-samps))))
	  (do ((k i (+ k 1)))
...
(defanimal (bachmans-sparrow beg amp)
  ...
  (let ((call1-dur .576)
	...
	(outa i (* (env ampf1)
	  ...
	  ((>= i stop2))
	(let ((reset-stop (min stop2 (+ i pulse-samps))))
	  (do ((k i (+ k 1)))
	      ((= k reset-stop))
	    (outa k (* (env ampf2)
		       (env ampf)
  ...
  (let ((dur 0.4)
    ...
    (let ((stop (seconds->samples (+ beg dur)))
	  ...
	  (frqf2 (make-env '(0.000 0.13 .6 .13   0.7 0.15 .9 .15 1.000 0.1)
			   :duration pulse-dur :scaler (hz->radians 10500.0))))
      (let ((next-pulse (+ start pulse-samps))
