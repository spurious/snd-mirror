...
(definstrument (barn-owl beg amp)
  ...
  (let* ((start (seconds->samples beg))
	 ...
	 (gen2 (make-polyshape 0.0 :partials (list 1 .2  2 .01 3 .8  4 .3  5 .1 6 .01)))
...
(definstrument (long-eared-owl beg amp)
  (let* ((start (seconds->samples beg))
	 ...
	 (gen1 (make-polyshape 0.0 :partials (list 1 1 2 .23 3 .034 4 .008 5 .005 7 .003))))
    (run
     (lambda ()
       (do ((i start (1+ i)))
	   ((= i stop))
...
(define (summer-tanager beg1 amp1)
  ...
  (definstrument (summer-tanager-1 beg dur amp ampl frq frql rndl)
    (let* ((start (seconds->samples beg))
	   ...
	   (gen1 (make-polyshape 0.0 :partials (list 1 .98  2 .01  3 .006  4 .008  5 .004)))
...
(definstrument (whooping-crane beg amp)
  ...
  (let* ((start (seconds->samples beg))
	 ...
	 (gen1 (make-polyshape 0.0 :partials (normalize-partials (list 1 .3  2 1  3 -.8  4 .25  5 .45  6 .22  7 .13  8 .05  9 .02  10 .005  11 .005))))
	 (gen2 (make-polyshape 0.0 :partials (normalize-partials (list 1 .91  2 .17  3 -.77  4 .25  5 .2  6 .2  7 .18  8 .1  10 .01))))
	 (gen3 (make-polyshape 0.0 :partials (normalize-partials (list 1 -1  2 .11  3 .45  4 .44  5 .62  6 .27  7 .02  8 .005))))
...
(definstrument (sandhill-crane beg amp)
  ...
  (let* ((start (seconds->samples beg))
	 ...
	 (gen1 (make-polyshape 0.0 :partials (list 1 .6  2 -.1  3 .01  4 .05  5 .03  6 .01  7 .01  8 .005)))
	 (rnd (make-rand-interp 3000 (hz->radians 100))))
    (run
     (lambda ()
       (do ((i start (1+ i)))
...
(definstrument (gray-crowned-rosy-finch beg amp)
  ...
  (let* ((start (seconds->samples beg))
	 ...
	 (frqf2 (make-env '(0.000 0.328 0.086 0.397 0.158 0.441 0.219 0.446 0.232 0.420 0.258 0.430 0.281 0.387 
			 ...
			 :duration dur :scaler (hz->radians 13400.0)))
	 (gen1 (make-polyshape 0.0 :partials (list 1 .99  2 .005)))
	 (gen2 (make-oscil 0.0))
...
(definstrument (sage-sparrow beg amp)
  ...
  (let* ((start (seconds->samples (+ beg 0.897)))
	 ...
	 (gen1 (make-polyshape 0.0 :partials (list 1 .98  2 .01  3 .006  4 .004))))
    (run
     (lambda ()
       (do ((i start (1+ i)))
	   ((= i stop))
...
(definstrument (hairy-woodpecker beg amp)
  ...
  (let* ((start (seconds->samples beg))
	 ...
	 (frqf (make-env '(0.000 0.180 0.056 0.213 0.135 0.241 0.167 0.305 0.191 0.396 0.212 0.402 0.242 0.485 
			 ...
			 :duration dur :scaler (hz->radians 10000.0)))
	 (gen1 (make-polyshape 0.0 :partials (list 1 .96  2 .03  3 .01)))
	 (gen2 (make-polyshape 0.0 :partials (list 1 .2  2 .1  3 .1  4 .1  5 .1  6 .05  7 .01)))
...
(definstrument (yellow-bellied-flycatcher beg amp)
  ...
  (let* ((start (seconds->samples beg))
	 ...
	 (frqf (make-env '(0.000 0.341 0.025 0.368 0.045 0.434 0.059 0.482 0.073 0.482 0.086 0.465 0.101 0.434 
			 ...
			 :duration dur :scaler (hz->radians 10000.0)))
	 (gen1 (make-polyshape 0.0 :partials (list 1 .98  2 .02  3 .005))))
    (run
     (lambda ()
       (do ((i start (1+ i)))
	   ((= i stop))
...
(define (black-throated-blue-warbler beg1 amp1)
  ...
  (definstrument (black-throated-blue-warbler-1 beg dur amp ampf frq frqf ind)
    (let* ((start (seconds->samples beg))
	   ...
	   (gen1 (make-polyshape 0.0 :partials (list 1 .99  2 .008 3 .003)))
	   (speed 200)
...
(define* (calling-all-frogs :optional (beg 0.0))
  (plains-spadefoot        (+ beg 0) 0.25)
  (barking-tree-frog       (+ beg 1) 0.25)
  (western-toad            (+ beg 1.5) 2 0.25)
  (southwestern-toad       (+ beg 4) 2 0.25)
  (american-toad           (+ beg 6.5) 3 0.25)
  (texas-toad              (+ beg 10) 2.0 0.125)
  (bullfrog                (+ beg 12.5) 0.125)
  (ornate-chorus-frog      (+ beg 14) 2 0.1)
  (squirrel-tree-frog-1    (+ beg 16.5) 1 0.1)
  (pinewoods-tree-frog     (+ beg 18) 1.5 0.15)
  (great-plains-narrow-mouthed-toad (+ beg 20) 2 .25)
  (northern-leopard-frog-2 (+ beg 22.5) 0.5)
  (pacific-chorus-frog     (+ beg 24.5) .5)
  (oak-toad                (+ beg 25) 0.3)
  (southern-cricket-frog   (+ beg 25.5) 0.5)
  (northern-leopard-frog-1 (+ beg 26) 0.5)
  (green-tree-frog         (+ beg 30.5) 0.5)
  (knudsens-frog           (+ beg 31) 0.5)
  (spring-peeper           (+ beg 31.5) 0.5)
...
(define* (calling-all-insects :optional (beg 0.0))
  (mosquito                    (+ beg 0) 5 560 0.2)
  (mosquito                    (+ beg 1) 3 880 0.05)
  (broad-winged-tree-cricket   (+ beg 5.5) 2.0 0.2)
  (long-spurred-meadow-katydid (+ beg 8) 0.5)
  (southern-mole-cricket       (+ beg 18.5) 3 0.15)
  (handsome-trig               (+ beg 22) 2 0.5)
  (fast-calling-tree-cricket   (+ beg 24.5) 2 0.25)
  (dog-day-cicada              (+ beg 27) 2 0.1)
  (linnaeus-cicada             (+ beg 29.5) 2 0.125)
  (lyric-cicada                (+ beg 32) 2 0.125)
  (confused-ground-cricket     (+ beg 34.5) 2 0.3)
  (tinkling-ground-cricket     (+ beg 37) 2 0.3)
  (marsh-meadow-grasshopper    (+ beg 39.5) 0.3)
  (striped-ground-cricket      (+ beg 45) 2 0.25)
  (sphagnum-ground-cricket     (+ beg 47.5) 2 0.3)
  (southeastern-field-cricket  (+ beg 50) 2 0.13)
  (snowy-tree-cricket          (+ beg 52.5) 2.1 0.3)
  (slightly-musical-conehead   (+ beg 55) 2 0.4)
  (pine-tree-cricket           (+ beg 57.5) 2 0.125)
  (davis-tree-cricket          (+ beg 60) 2 0.125)
  (carolina-grasshopper        (+ beg 62.5) 1.5 1.0)
  (black-horned-tree-cricket   (+ beg 64.5) 2 0.125)
  (+ beg 67))


(define* (calling-all-birds :optional (beg 0.0))
  (ruffed-grouse                 (+ beg 0.0) 0.5)
  (eastern-wood-pewee-1          (+ beg 11.0) 0.25)
  (eastern-wood-pewee-2          (+ beg 12.5) 0.25)
  (field-sparrow                 (+ beg 14.0) 0.25)
  (fox-sparrow                   (+ beg 17.5) 3 0.25)
  (white-throated-sparrow        (+ beg 21.0) 0.25)
  (tufted-titmouse               (+ beg 25.0) 0.3)
  (savannah-sparrow              (+ beg 26.5) 0.5)
  (chipping-sparrow              (+ beg 30.0) 0.3)
  (henslows-sparrow              (+ beg 33.0) 0.5)
  (least-flycatcher              (+ beg 33.5) 0.5)
  (acadian-flycatcher            (+ beg 34.0) 0.25)
  (swainsons-thrush              (+ beg 35.0) 0.25)
  (carolina-wren                 (+ beg 37.5) 0.25)
  (bachmans-sparrow              (+ beg 40.0) 0.25)
  (grasshopper-sparrow           (+ beg 43.0) 0.25)
  (american-robin                (+ beg 45.0) 0.25)
  (common-loon-1                 (+ beg 47.5) 0.125)
  (common-loon-2                 (+ beg 50.5) 0.125)
  (hermit-thrush                 (+ beg 51.5) 0.25)
  (chuck-wills-widow             (+ beg 53.5) 0.25)
  (california-towhee             (+ beg 55.0) 0.25)
  (black-chinned-sparrow         (+ beg 56.5) 0.25 #t)
  (mourning-dove                 (+ beg 60.0) 0.125)
  (bobwhite                      (+ beg 64.5) 0.25)
  (warbling-vireo                (+ beg 66.5) 0.25)
  (great-horned-owl              (+ beg 69.0) 0.25)
  (western-tanager               (+ beg 72.0) 0.25)
  (pileated-woodpecker           (+ beg 74.5) 0.125)
  (whip-poor-will                (+ beg 77.5) 0.25)
  (varied-thrush                 (+ beg 79.0) 0.125)
  (nashville-warbler             (+ beg 80.5) 0.25)
  (plumbeous-vireo-1             (+ beg 83.0) 0.25)
  (american-crow                 (+ beg 84.0) 0.5)
  (least-bittern                 (+ beg 85.0) 0.5)
  (orange-crowned-warbler        (+ beg 87.0) 0.25)
  (loggerhead-shrike-1           (+ beg 89.0) 0.125)
  (loggerhead-shrike-2           (+ beg 90.0) 0.125)
  (california-quail              (+ beg 91.0) 0.25)
  (vermillion-flycatcher         (+ beg 92.0) 0.25)
  (cardinal                      (+ beg 93.0) 0.25)
  (black-phoebe                  (+ beg 97.0) 0.25)
  (yellow-warbler                (+ beg 98.0) 0.25)
  (barred-owl-1                  (+ beg 100.0) 0.25)
  (says-phoebe                   (+ beg 102.0) 0.25)
  (yellow-rumped-warbler         (+ beg 103.0) 0.25)
  (purple-finch                  (+ beg 105.0) 0.25)
  (northern-goshawk              (+ beg 108.0) 0.125)
  (common-gull                   (+ beg 109.0) 0.25)
  (ash-throated-flycatcher       (+ beg 110.0) 0.25)
  (white-headed-woodpecker       (+ beg 111.0) 0.25)
  (phainopepla                   (+ beg 111.5) 0.5)
  (golden-crowned-sparrow        (+ beg 112.5) 0.25)
  (house-finch                   (+ beg 115.0) 0.25)
  (ruby-crowned-kinglet          (+ beg 118.5) 0.25)
  (green-tailed-towhee           (+ beg 121.0) 0.25)
  (white-faced-ibis              (+ beg 123.5) 0.25)
  (lucys-warbler                 (+ beg 124.0) 0.25)
  (cassins-vireo                 (+ beg 126.0) 0.25)
  (plain-chacalaca               (+ beg 127.0) 0.5)
  (black-billed-cuckoo           (+ beg 128.0) 0.25)
  (eared-grebe                   (+ beg 129.0) 0.25)
  (brown-jay                     (+ beg 130.0) 0.5)
  (blue-grosbeak                 (+ beg 131.0) 0.25)
  (acorn-woodpecker              (+ beg 134.0) 0.5)
  (red-shouldered-hawk           (+ beg 136.0) 0.5)
  (lesser-nighthawk              (+ beg 137.0) 2 0.25)
  (olive-sided-flycatcher        (+ beg 139.5) 0.125)
  (common-yellowthroat           (+ beg 141.0) 0.25)
  (cassins-sparrow               (+ beg 143.5) 0.25)
  (stellers-jay                  (+ beg 146.0) 0.25)
  (black-rail                    (+ beg 147.5) 0.25)
  (pinyon-jay                    (+ beg 148.5) 0.25)
  (sora                          (+ beg 149.5) 0.25)
  (killdeer                      (+ beg 150.5) 0.25)
  (oak-titmouse                  (+ beg 152.0) 0.25)
  (macgillivrays-warbler         (+ beg 154.0) 0.25)
  (huttons-vireo                 (+ beg 156.0) 0.25)
  (western-meadowlark            (+ beg 157.0) 0.25)
  (northern-beardless-tyrannulet (+ beg 159.0) 0.25)
  (scotts-oriole                 (+ beg 161.5) 0.25)
  (wilsons-warbler               (+ beg 164.0) 0.25)
  (willow-flycatcher             (+ beg 166.5) 0.25)
  (black-necked-stilt            (+ beg 167.5) 0.25)
  (bushtit                       (+ beg 168.0) 0.25)
  (red-breasted-nuthatch         (+ beg 169.0) 0.25)
  (white-breasted-nuthatch       (+ beg 170.0) 0.25)
  (pygmy-nuthatch                (+ beg 171.0) 0.25)
  (flammulated-owl               (+ beg 171.5) 0.25)
  (song-sparrow                  (+ beg 172.0) 0.25)
  (burrowing-owl                 (+ beg 175.0) 0.25)
  (gray-vireo-1                  (+ beg 176.0) 0.25)
  (gray-vireo-2                  (+ beg 176.5) 0.25)
  (bald-eagle                    (+ beg 177.0) 0.25)
  (eastern-meadowlark            (+ beg 178.5) 0.25)
  (plumbeous-vireo-2             (+ beg 180.5) 0.25)
  (yellow-green-vireo            (+ beg 181.5) 0.25)
  (magnolia-warbler              (+ beg 182.0) 0.25)
  (eastern-bluebird              (+ beg 183.5) 0.25)
  (evening-grosbeak              (+ beg 184.5) 0.25)
  (greater-roadrunner            (+ beg 185.0) 0.5)
  (dark-eyed-junco               (+ beg 190.0) 0.25)
  (groove-billed-ani             (+ beg 192.0) 0.25)
  (common-pauraque               (+ beg 193.0) 0.25)
  (hammonds-flycatcher           (+ beg 194.0) 0.25)
  (barn-owl                      (+ beg 194.5) 0.25)
  (long-eared-owl                (+ beg 196.0) 0.25)
  (summer-tanager                (+ beg 197.0) 0.25)
  (whooping-crane                (+ beg 201.0) .5)
  (sandhill-crane                (+ beg 202.0) .5)
  (gray-crowned-rosy-finch       (+ beg 203.0) .25)
  (virginia-rail                 (+ beg 203.5) .25)
  (sage-sparrow                  (+ beg 206.0) .25)
