...
(define (sample-if beg dur freq amp)
  (let ((os (make-oscil freq))
	...
	(bool #t))
    (do ((i start (+ i 1))) ((= i end))
      (if (and (= i start) (not (= k -123))) (format () "init k: ~A~%" k))
      ...
      (set! k (cond ((= j 0) -4)
		    ((= j 12) -5)
		    (#t -6)))
      (if (not (= k -6)) (format () "cond expr: ~A ~A~%" j k))
      (set! k (let ((a 123))
		(if (> a 0)
		    20
		    32)))
      (if (not (= k 20)) (format () "let expr: ~A ~A~%" j k))
      (let ((a 123))
	(set! k a))
      (if (not (= k 123)) (format () "let: ~A ~A~%" j k))
      (set! k 123)
      (set! bool (= k 123))
      (if (not bool) (format () "bool expr: ~A~%" bool))
      (set! bool (if (= k 123) (> k 0) (< k 0)))
      (if (not bool) (format () "if bool expr: ~A~%" bool))
      (set! j 0)
      (set! k (do ((m 0 (+ 1 m)))
		  ((= m 3) j)
		(set! j (+ j 1))))
      (if (not (= k 3)) (format () "do expr: ~A~%" k))
					;	 (dotimes (m 2)
					;		  (set! k (- k 1)))
					;	 (if (not (= k 1)) (format () "dotimes: ~A~%" k))
      (outa i (* amp (oscil os))))))
