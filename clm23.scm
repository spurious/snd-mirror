...
(define (double a) a)	
(define (int-floor a) (inexact->exact (floor a)))
(define (make-empty-frame a) (make-frame a))
...
(define (simple-out beg dur freq amp)
  (let* ((os (make-oscil freq))
	 (start (int-floor (* beg (mus-srate))))
	 (end (+ start (int-floor (* dur (mus-srate))))))
    (run
     (lambda ()
...
(define (simple-osc beg dur freq amp)
  (let* ((start (int-floor (* beg (mus-srate))))
	 (end (+ start (int-floor (* dur (mus-srate)))))
	 (arr (make-vector 20)))
    (do ((i 0 (1+ i)))
	((= i 20))
...
(define (simple-osb beg dur freq amp)
  (let* ((start (int-floor (* beg (mus-srate))))
	 (end (+ start (int-floor (* dur (mus-srate)))))
	 (arr (make-vector 20))
	 (amps (make-double-array 20))
	 (fms (make-double-array 20)))
    (do ((i 0 (1+ i)))
	((= i 20))
      (vector-set! arr i (make-oscil (* (1+ i) 100)))
      (set! (vct-ref amps i) (double (* amp .05)))
      (set! (vct-ref fms i) (double 0.0)))
    (run
     (lambda ()
       (do ((i start (1+ i))) ((= i end))
	 (out-any i (oscil-bank amps arr fms 20) 0 *output*))))))
    ...
    (run
	 ...
	 (out-any i (* amp (phase-vocoder sr
					   ...
					   #'(lambda (closure)
						 ...
						 (set! k (1+ k)))
					       (sine-bank (phase-vocoder-amps sr) (phase-vocoder-phases sr))))) 
		  0 *output*))))))
	 ...
	 (let ((val (* (oscil os) amp)))
	   ...
	   (do ((j 0 (1+ j)))
		 ...
		 ((= k 2))
	       ...
	       (set! (mixer-ref mx1 j k) (+ j k))
	       (set! (mixer-ref mx2 j k) (* j k))))
	   (mixer* mx1 mx2 nmx)
...
(define (sample-ardcl beg dur freq amp)
  (let* ((start (int-floor (* beg (mus-srate))))
	 ...
	 (phases (make-double-array 3 :initial-element 0.0))
	 (freqs (make-double-array 3 :initial-element 0.0))
	 (ints (make-integer-array 3 :initial-element 32)))
    (do ((i 0 (1+ i)))
	((= i 3))
      (set! (vct-ref freqs i) (double (* freq (1+ i))))
      (set! (vct-ref amps i) (double (/ amp (+ i 2)))))
    (run
	 ...
	 (do ((i 0 (1+ i)))
	     ((= i (vct-length phases)))
	   (set! (vct-ref phases i) (+ (vct-ref phases i) (hz->radians (vct-ref freqs i)))))
	 (if (not (= (vct-ref ints 0) 32)) (clm-print "int array trouble"))
	 (set! (vct-ref ints 1) 3)
	 (if (not (= (vct-ref ints 1) 3)) (clm-print "set int array trouble"))
...
(define (sample-strs beg dur freq amp)
  (let* ((start (int-floor (* beg (mus-srate))))
    ...
    (run
	 ...
	 (if (not (string=? (mus-header-type-name (mus-sound-header-type "oboe.snd")) "Sun"))
