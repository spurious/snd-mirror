...
(define (simple-ave beg dur freq amp)
  ...
  (let ((start (seconds->samples beg))
    ...
    (do ((i start (+ i 1))) ((= i end))
      (outa i (* amp (moving-average buf (oscil os)))))))
...
(define (simple-sr2 beg dur amp speed file)
  ...
  (let ((rd (make-readin file)))
    (let ((start (seconds->samples beg))
	  ...
	  (src-func (lambda (dir) (if (= dir 1) (readin rd)))))
      (do ((i start (+ i 1))) 
	  ((= i end))
	(outa i (* amp (src sr 0.0 src-func)))))))
...
(define (sample-grn2 beg dur amp speed freq)
  ...
  (let ((start (seconds->samples beg))
    ...
    (let ((read-func (lambda (dir) (oscil os)))
	  (edit-func (lambda (g) 0)))
      (do ((i start (+ i 1))) ((= i end))
	(outa i (* amp (granulate sr read-func edit-func)))))))
...
(define* (fmdoc-vox beg dur freq amp (indexes '(.005 .01 .02)) (formant-amps '(.86 .13 .01)))
  (let ((start (seconds->samples beg))
	...
	(even-amp 0.0) (odd-amp 0.0) (frq1 0.0) (sum 0.0) (frq 0.0) (carg 0.0) (frm 0.0) (frm0 0.0) (frm-int 0)
	...
	(if (even? frm-int)
	    (begin
	      (set! even-freq (* frm-int frq1))
	      (set! odd-freq (+ even-freq frq1))
	      (set! odd-amp (- frm0 frm-int))
	      (set! even-amp (- 1.0 odd-amp)))
	    (begin
	      (set! odd-freq (* frm-int frq1))
	      (set! even-freq (+ odd-freq frq1))
	      (set! even-amp (- frm0 frm-int))
	      (set! odd-amp (- 1.0 even-amp))))
	(let ((val (* (vct-ref indices k) carg)))
	  (set! sum (+ sum (* (amps k) 
...
(define (sndclmdoc-looper start dur sound freq amp)
  (let ((beg (seconds->samples start))
	...
	(let* ((loop-start (car loop-data))
	       ...
	       (sound-section (vct-scale! (file->array sound 0 loop-start loop-length (make-vct loop-length)) amp))
...
(definstrument (sndclmdoc-fm-table file start dur amp read-speed modulator-freq index-in-samples)
  (let ((table-length (frames file)))
    (let ((beg (seconds->samples start))
	  (end (seconds->samples (+ start dur)))
	  (tab (make-table-lookup :frequency (/ read-speed (mus-sound-duration file)) 
				  :wave (vct-scale! 
					 (file->array file 0 0 table-length (make-vct table-length))
					 amp)))
...
(definstrument (move-formants start file amp radius move-env num-formants)
  (let ((frms (make-vector num-formants))
    ...
    (let ((end (+ beg dur))
	  ...
	  (amps (make-vct num-formants amp)))
	...
	(let ((x (readin rd))
	      (frq (env menv)))
	  (outa k (formant-bank amps frms x))
	  (do ((i 0 (+ i 1))
