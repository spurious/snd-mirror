...
# Last: Thu Jan 08 06:55:05 CET 2004
...
def make_selection(beg = 0, len = false, snd = false, chn = false)
  ...
  current_sound = (snd or selected_sound() or sounds.first)
  unless sound?(current_sound)
    raise(format("make_selection(%s, %s, %s, %s): no-such-sound",
...
def check_for_unsaved_edits(check = true)
  ...
  if check
    unless $close_hook.member?("unsaved-edits-at-close?")
      $close_hook.add_hook!("unsaved-edits-at-close?") do |s| unsaved_edits_at_close_p.call(s) end
    end
    unless $exit_hook.member?("unsaved-edits-at-exit?")
      $exit_hook.add_hook!("unsaved-edits-at-exit?") do | |
        if sounds.kind_of?(Array) and sounds.detect do |snd| unsaved_edits_at_close_p.call(snd) end
          true
        else
          false
        end
...
def remember_sound_state
  ...
  channel_funcs = [:time_graph?, :transform_graph?, :lisp_graph?, :x_bounds, :y_bounds,
                   ...
                   :time_graph_type, :fft_window, :transform_type,
                   :transform_normalization, :time_graph_style, :show_mix_waveforms,
                   :dot_size, :x_axis_style, :show_axes, :graphs_horizontal, :lisp_graph_style,
...
def mix_channel(fdata, beg = 0, dur = false, snd = false, chn = false, edpos = false)
  fname = if fdata.kind_of?(String)
            ...
            raise(format("mix_channel(fdata, beg, dur, snd, chn, edpos):
FDATA should be a file name or an array [fname, beg = 0, chn = 0] (%s)", fdata.inspect))
  ...
  raise(format("mix_channel(): no-such-sample (%d)", beg)) if beg < 0
  if len > 0
...
def insert_channel(fdata, beg = 0, dur = false, snd = false, chn = false, edpos = false)
  fname = if fdata.kind_of?(String)
            ...
            raise(format("insert_channel(fdata, beg, dur, snd, chn, edpos):
FDATA should be a file name or an array [fname, beg = 0, chn = 0] (%s)", fdata.inspect))
          end
  fbeg = if fdata.kind_of?(String) or fdata.length < 2
           0
  ...
  len = (dur or (mus_sound_frames(fname) - fbeg))
  raise(format("insert_channel(): no-such-sample (%d)", beg)) if beg < 0
  if len > 0
    reader = make_sample_reader(fbeg, fname, fchan)
    data = make_vct(len)
