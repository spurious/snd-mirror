  ...
  (if (provided? 'snd-motif)
	...
	(let* ((drawer (XtCreateManagedWidget name xmDrawingAreaWidgetClass parent args))
      ...
      (let* ((drawer (gtk_drawing_area_new))
	     ...
	     (editor (list (list x0 y0 x1 y0) ; needs to be in user-coordinates (graph size can change)
			   ...
			   (list gc #f)
			   name))
	     (g_object_cast (if with-gtk2 GTK_OBJECT G_OBJECT))
	     (dragging #f))
	...
	(g_signal_connect_closure_by_id (GPOINTER drawer)
					(g_signal_lookup (if with-gtk3 "draw" "expose_event")
							 (G_OBJECT_TYPE (g_object_cast drawer)))
					...
					#f)
	(g_signal_connect_closure_by_id (GPOINTER drawer)
					(g_signal_lookup "configure_event" (G_OBJECT_TYPE (g_object_cast drawer)))
					0 (g_cclosure_new (lambda (w e d)
							    (list-set! editor 2 (apply draw-axes drawer gc name axis-bounds))
							    (xe-redraw editor)
							    #f)
					...
					#f)
	(g_signal_connect_closure_by_id (GPOINTER drawer)
					(g_signal_lookup "button_press_event" (G_OBJECT_TYPE (g_object_cast drawer)))
					0 (g_cclosure_new (lambda (w e d) 
							    (let* ((ev (GDK_EVENT e))
					...
					#f)
	(g_signal_connect_closure_by_id (GPOINTER drawer)
					(g_signal_lookup "button_release_event" (G_OBJECT_TYPE (g_object_cast drawer)))
					0 (g_cclosure_new (lambda (w e d) 
							    (let* ((ev (GDK_EVENT e))
								   (coords (gdk_event_get_coords ev))
								   (x (cadr coords))
					...
					#f)
	(g_signal_connect_closure_by_id (GPOINTER drawer)
					(g_signal_lookup "motion_notify_event" (G_OBJECT_TYPE (g_object_cast drawer)))
					0 (g_cclosure_new (lambda (w e d) 
							    (if dragging
								(let* ((ev (GDK_EVENT e))
								       (coords (gdk_event_get_coords ev))
	...
	(g_signal_connect_closure_by_id (GPOINTER drawer)
					(g_signal_lookup "enter_notify_event" (G_OBJECT_TYPE (g_object_cast drawer)))
					0 (g_cclosure_new (lambda (w e d)
					...
					#f)
	(g_signal_connect_closure_by_id (GPOINTER drawer)
					(g_signal_lookup "leave_notify_event" (G_OBJECT_TYPE (g_object_cast drawer)))
					0 (g_cclosure_new (lambda (w e d)
