...
instrument: jc-reverb-fs ( keyword-args -- )
  doc" ( keyword-args -- )  The Chowning reverb.\n\
...
instrument: stereo-flute ( start dur freq flow keyword-args -- )
 doc" ( start dur freq flow keyword-args -- )\n\
\\ keywords and default values\n\
\\ :flow-envelope     '( 0 1 100 1 )\n\
\\ :decay             0.01\n\
\\ :noise             0.0356\n\
\\ :embouchure-size   0.5 (mouthpiece)\n\
\\ :fbk-scl1          0.5\n\
\\ :fbk-scl2          0.55\n\
\\ :out-scl           1.0\n\
\\ :a0                0.7\n\
...
0 1 440 0.55 :flow-envelope '( 0 0 1 1 2 1 3 0 ) ' stereo-flute with-sound"
  ...
  :embouchure-size 0.5            get-args { mp-size } \ mouthpiece
  ...
  :scaler   flow
  :start    beg
  :end      dur decay f- seconds->samples beg b+ make-env         { flowf }
  ...
  mp-size mus-srate freq f/ floor f* floor f>s 	 make-delay       { mp }
  mus-srate freq f/ floor f>s                  	 make-delay       { bore }
  a0 b1                                        	 make-one-pole    { rlf }
  0.0 0.0 0.0 0.0 { mp-sig delay-sig out-sig prev-out-sig }
  0.0 0.0 0.0 0.0 { cur-diff cur-flow dc-blocked  prev-dc-blocked }
  start dur #{ :degree 90.0 random } run-instrument
    bore out-sig  0.0 delay to delay-sig
    mp   cur-diff 0.0 delay to mp-sig
    p-vib 0.0     0.0 oscil       vib-amount f*
    ran-vib       0.0 rand-interp ran-amount f* f+
    flowf env f+ to cur-flow
    noise cur-flow f* breath 0.0 rand f*
    cur-flow f+
    fbk-scl1 delay-sig f* f+ to cur-diff
    rlf
    mp-sig mp-sig mp-sig f* mp-sig f* f-  ( current-exitation )
    fbk-scl2 delay-sig f* f+ one-pole to out-sig
    out-sig prev-out-sig f- 0.995 prev-dc-blocked f* f+ to dc-blocked
    out-sig to prev-out-sig
    dc-blocked to prev-dc-blocked
    dc-blocked out-scl f*
  end-run
;instrument
...
event: flute-test ( keyword-args -- )
  :beg 0 get-args { start }
  :dur 1 get-args { dur }
  start dur 440 0.55 :out-scl 0.1 :flow-envelope '( 0 0 1 1 2 1 3 0 ) stereo-flute
  dur step
;event
...
event: fm-bell-test ( keyword-args -- )
  :beg 0 get-args { start }
  :dur 1 get-args { dur }
  :beg start :dur dur fm-bell
  dur step
;event

\ FM-INSECT
\ clm/insect.ins
instrument: fm-insect ( start dur freq amp amp-env mod-freq mod-skew mod-freq-env mod-index mod-index-env fm-index fm-ratio -- )
  { start dur freq amp amp-env mod-freq mod-skew mod-freq-env mod-index mod-index-env fm-index fm-ratio }
  :frequency freq                                                     make-oscil { carrier }
...
\
\ clm/nrev.ins
instrument: nrev-fs ( keyword-args -- )
  doc" nrev ( keyword-args -- )  NREV (the most popular Samson box reverb).\n\
\\ keywords and default values:\n\
\\ :reverb-factor   1.09\n\
\\ :lp-coeff        0.7\n\
\\ :lp-out-coeff    0.85\n\
...
\ RESON
instrument: reson ( start dur pitch amp indxfun skewfun pcskew skewat skewdc vibfreq vibpc ranvibfreq ranvibpc data -- )
  { start dur pitch amp indxfun skewfun pcskew skewat skewdc vibfreq vibpc ranvibfreq ranvibpc data }
  ...
  start dur #{ :degree 90.0 random } run-instrument
    ...
    values each to rs
      rs ampfs @ env
      rs carriers @  vib rs c-rats f@ f* rs indfs @ env modsig f* f+  0.0 oscil f* f+
    end-each
  end-run
;instrument
...
instrument: cellon ( start dur pitch0 amp ampfun betafun beta0 beta1 betaat betadc ampat ampdc pitch1 glissfun glissat glissdc pvibfreq pvibpc pvibfun pvibat pvibdc rvibfreq rvibpc rvibfun -- )
  { start dur pitch0 amp ampfun betafun beta0 beta1 betaat betadc ampat ampdc pitch1 glissfun glissat glissdc pvibfreq pvibpc pvibfun pvibat pvibdc rvibfreq rvibpc rvibfun }
...
instrument: gran-synth ( start dur freq grain-dur interval amp -- )
  { start dur freq grain-dur interval amp }
...
instrument: lbj-piano ( start dur freq amp -- )
  ...
  :envelope '( 0 1 100 0 )
  :scaler amp ampfun1 -1 list-ref f*
  :duration env1dur
  :base 1.0 make-env { ampenv2 }
  parts length 2/ 0.0 make-array map!
    alist i  parts i 2* 1+ object-ref  vct-set! drop
    :frequency  parts i 2* object-ref  freq f* make-oscil
  end-map { oscils }
  start dur #{ :degree 90.0 random } run-instrument
    0.0 oscils each 0.0 0.0 oscil alist i vct-ref f* f+ end-each
