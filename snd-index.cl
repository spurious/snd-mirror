	  ...
	  (let* ((line (read-line sndf nil :EOF)))
		...
		(if pos
		    (let* ((epos (search ">" (subseq line pos)))
			  ...
			  (print (format nil "multiple definitions of ~A?" name))
			(if (find name snd-all-names :test #'string=)
			    (print (format nil "multiple definitions of ~A (ext snd)?" name))
			  (progn
	  ...
	  (let* ((line (read-line sndf nil :EOF)))
		...
		(if pos
		    (let* ((epos (search ">" (subseq line pos)))
			  ...
			  (print (format nil "multiple definitions of ~A?" name))
			(if (find name ext-all-names :test #'string=)
			    (print (format nil "multiple definitions of ~A (grf ext)?" name))
			  (if (find name snd-all-names :test #'string=)
			      (print (format nil "multiple definitions of ~A (grf snd)?" name))
			    (progn
