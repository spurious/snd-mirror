...
#endif
{
  ...
  ss->Vu_Font = copy_string(DEFAULT_VU_FONT);
  ...
  ss->Temp_Dir = copy_string(DEFAULT_TEMP_DIR);
  ss->Save_Dir = copy_string(DEFAULT_SAVE_DIR);
  ...
  ss->Audio_State_File = copy_string(AUDIO_STATE_FILE);
