...
(define* (sndwarp begtime dur file 
  ...
  (define (clmsw-envelope-or-number in)
    ...
    (let* ((beg (seconds->samples begtime))
           ...
           (windf (make-oscil))
           (wsizef (let ((wsize-env (clmsw-envelope-or-number wsize)))
                     (make-env wsize-env :duration dur)))
           (ampf (make-env amp-env :scaler amp :duration dur))
           (sratef (let ((srate-env (clmsw-envelope-or-number srate)))
                     (make-env srate-env :duration dur)))
           (timef (let* ((time-env (clmsw-envelope-or-number stretch))
                    ...
                    (make-env
                      (if (and time-ptr scale-time-ptr)
                          ...
                          time-env)
                      :duration dur)))
           (locf (let ((loc-env (clmsw-envelope-or-number loc)))
                   (make-env loc-env :duration dur)))
           (writestart 0)
