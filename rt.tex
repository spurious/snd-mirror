...
Note, this is how I work. A wizard/expert might find the procedure below incredible stupid.
...
\item{Split your window in two parts: M-x 2}

\item{Go to that the lower part window: M-x o}

\item{Load snd-ls: M-x load-scheme}
...
\item{Go to the upper part window: M-x o}
...
\item{Unlimited number of instances can be run simultaneously. \footnote{Well, actually just 16.}}
\item{Protection mechanism provided to avoid locking up the computer when using to much CPU.}
...
The \scheme|eval-c| macro is found in the file eval-c.scm. \scheme|eval-c| also does the compiling and linking
by calling gcc.
...
\item[\scheme|(define-rt ...)|] \hspace{\stretch{1}} Macro \newline
...
\item{\scheme|(play [start] [duration])|  \hspace{\stretch{1}} Method \newline
  ...
  Default value for \scheme|start| is the current time.
  If \scheme|end| is not specified, a stop command is not scheduled.}

\item{\scheme|(stop [end])|  \hspace{\stretch{1}} Method \newline
  Stops playing \scheme|end| seconds into the future from the current time. Default value for \scheme|end| is the current time.}
...
The function \scheme|rt-expand-macros| works the same as \scheme|macroexpand|, but for
...
\end{schemedisplay}
...because only numbers can be set.



\subsection{Getting sound in and out}
...
\item{The following CLM generators are not supported: in-any and out-any. However, their functionality probably need to be supported someway,
...
\subsection{Sound-arrays and Buses}

I think it would be nice if \scheme|(out (bus 0.2 0.5)))| transformed
into \scheme|(begin (out 0 0.2) (out 1 0.5))|.

But this needs some more thinking...



\subsection{Order of execution and sharing of sound data}
The RT extension is missing one important functionality; How to share sound data
between functions; How to let a function use the sound produced by another
function.

The supercollider engine solves this by providing a bus-system, and letting the
user specify the order the synths are executed.

That could be done by doing something like this:
...
(let ((bus (make-bus 2)))
  (<rt-play>  0 10
	      ...
	      (lambda ()
	        (out- bus 1 (oscil ob)))
	      (lambda () #:add-to-tail
	        (out (vct (* vol (in 0 bus))
                          (* vol (in 1 bus)))))))
\end{schemedisplay}

Another way would be to let RT-functions be able to call other RT-functions.
That could be done by doing something like this:
...
I have a feeling that the first version can be a lot faster, because it executes smaller parts many times, which avoids
cache-misses and function-calls. It uses some more memory as well though, but that shouldn't be very important.



So, perhaps the best would be a combination of the two possibilities would be a good idea? Or
just provide both? Or perhaps even something completely different. I don't know.
Its yet to be decided...

Heres a workaround for how to solve the above example by using 
jack-port 6 and 7 for internal routing:

\begin{schemedisplay}
(let ((oa (make-oscil #:frequency 500))
      (ob (make-oscil #:frequency 600))
      (vol 0.8))
  (<rt-play> 0 10
	     (lambda ()
	       (out (vct (* vol (in 6))
...
But for the above to work, you have to do some acrobatic jack-port routing as well.



\subsection{Implementation of a try-and-see-how-it-works bus-system}

20.5.2005 I added bus-support. The API is definitely not settled yet, so please tell me what you think, in
...
\begin{schemedisplay}
  (definstrument (a)
    (let ((bus (make-bus 2))
      ...
      (<rt-play> start end
		 (lambda ()
		   (write-bus bus (oscil oa))))
      (<rt-play> start end
		 (lambda ()
		   (write-bus bus #f (oscil ob))))
      (<rt-play> start end
		 (lambda ()
		   (out (vct-scale! (read-bus bus)
				    vol))))))
