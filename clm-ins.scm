  ...
  (let ((formants
    ...
    (let ((start (seconds->samples beg))
      ...
      (let ((evens (make-vector fs))
	    (odds (make-vector fs))
	    (amps (make-vector fs 0.0))
	    (indices (make-vector fs 0.0))
	...
	(do ((i start (+ i 1))) 
	  ...
	  (do ((k 0 (+ k 1))) 
	    ...
	    (let ((car-mult (* (vector-ref indices k) carrier)))
	      (set! sum (+ sum 
			   (* (amps k) 
  ...
  (let ((osc0 (make-oscil (* freq 0.995)))
	 ...
	 ((= i nd))
       (outa i (* (+ (* .007 (oscil ampmod)) .993)
		  (+ (* g0 (env ampenv1) (oscil osc0 (* .203 (oscil osc1))))
		     (* g2 (env ampenv2) (oscil osc2 (* .144 (oscil osc3))))))))))
	 ...
	 ((= i nd))
       (outa i (* (env ampenv)
		  ...
		  (+ (* g0 (oscil osc0 (* .307 (oscil osc1))))
		     (* (env resenv) g2 (oscil osc2 (* .117 (env indenv) (oscil osc3))))))))))
	...
	(let ((start (seconds->samples beg))
	  ...
	  (let ((lamp1 (* (envelope-interp k ampfun1) amp (- 1 (abs (- harm1 (/ lfmt1 pitch))))))
	    ...
	    (let ((tampfun (make-env (stretch-envelope ampfun 25 atpt 75 dcpt) :duration dur))
		  ...
		  (gp (make-polywave pitch (list harm1 lamp1 harm2 lamp2 harm3 lamp3 harm4 lamp4))))
	      (do ((i start (+ i 1)))
		  ((= i end))
...
(definstrument (anoi infile start dur (fftsize 128) (amp-scaler 1.0) rr)
  ...
  (let ((r (or rr (* 2.0 pi)))
    ...
    (let ((spectr (make-vector freq-inc 1.0))
	  ...
	  (diffs (make-vector freq-inc 0.0))
