  ...
  if ((window = find_window(dpy,DefaultRootWindow(dpy),NS_VERSION,compare_window)))
    {
      ...
      XChangeProperty(dpy,window,XInternAtom(dpy,NS_COMMAND,False),XA_STRING,8,PropModeReplace,(unsigned char *)command,strlen(command)+1);
      XFlush(dpy);
    }
  else
    {
...
static void change_property(snd_state *ss, char *winat, char *name, char *command)
{
  ...
  if ((window = find_window(dpy,DefaultRootWindow(dpy),winat,compare_window)))
    {
      XChangeProperty(dpy,window,XInternAtom(dpy,name,False),XA_STRING,8,PropModeReplace,(unsigned char *)command,strlen(command)+1);
      XFlush(dpy);
    }
}
