...
;    (define calls 0)
	...
	(if (pair? x)
      ...
      ;; x then (not x) can be simplified, perhaps here
					  ...
					  (let ((v (or (var-member arg1 env)
						...
						(let ((sig (arg-signature (car arg2) env)))
						  (let ((arg-type (and (pair? sig)
								       ...
								       (symbol? (cadr sig)))))
						    (if arg-type
							(format outport " in ~S, perhaps change ~S to ~S~%"
    ...
    (define (check-special-cases name head form env)
      ...
      ;;
      ;; simplify-boolean often prints twice
      ;;
      ;; 310/52
	 ...
	 (if (pair? (cdr form))
		 ...
		 (if (code-constant? (cadr form))
		     (lint-format "perhaps ~A -> ~A" name 
				  form (length (if (and (pair? (cadr form))
							(eq? (caadr form) 'quote))
						   (cadadr form)
						   (cadr form))))))))
		   ...
		   (if (or (< (length form) 3)
			 ...
			 (let ((vars (if (and named-let 
			   ...
			   (let* ((cur-env (append vars env))
			     ...
			     ;(format *stderr* "e: ~A, nvars: ~A~%" e nvars)
			     (if (pair? nvars)
				 (if (eq? (var-name (car nvars)) lambda-marker) ; TODO: add this to let* etc
				     (begin
