...
(define lint
  (let ()
					  ...
					  (let ((sig (and (symbol? (caar body))
					    ...
					    (when sig
					      ...
					      (let ((new-sig (list (car sig)))
						    (body-args (cdar body))
						    (body-arg-num (length (cdar body)))
						    (sig-len (length sig)))
						(if (infinite? sig-len) (set! sig-len 123123123))
						;(format *stderr* "args: ~A, sig: ~A, body: ~A~%" arglist sig body)
						(if (proper-list? arglist)
						    (begin
						      (for-each
						       (lambda (arg) ; new function's parameter
							 (let* ((m (memq arg body-args))
								(loc (and m (- body-arg-num (length m)))))
							   (if (and loc
								    (< loc sig-len))
							       (set! new-sig (cons (list-ref sig loc) new-sig))
							       (set! new-sig (cons #t new-sig)))))
						       arglist)
						      ;(format *stderr* "~A sig: ~A~%" name (reverse new-sig))
						      (reverse new-sig))
						    new-sig)))))))
	...
	(let ((data (or (var-member name env) (hash-table-ref globals name))))
	  ...
	  (when (var? data)
#|
	    (if (eq? (var-set data) 'reverse!)
		(format *stderr* "~A clobbered by reverse!?~%" (car data)))
|#
	    (set! (var-ref data) #t)))
	env)
	...
	(let ((data (or (var-member name env) (hash-table-ref globals name))))
	  ;(format *stderr* "set-set? ~A ~A -> ~A ~A~%" name env data (var? data))
	  (when (var? data)
		 ...
		 (else 
	      ...
	      ((logxor)
	       ...
	       (cond ((null? args) 0)
		     ...
		     ((just-integers? args) (apply logxor args))
		     ((and (= len 2) (equal? (car args) (cadr args))) 0)
		     (else `(logxor ,@args))))
	      ...
	      ((eq? (car selector) 'quote)
	       (cond ((or (symbol? (cadr selector))
			  (memq (cadr selector) '(#f #t #<unspecified> #<undefined> #<eof>)))
		                                '(eq? eq?))
		     ((null? (cadr selector))   '(eq? null?))
		     ((char? (cadr selector))   '(eqv? char=?))
		     ((number? (cadr selector)) '(eqv? number?))
		     (else                      '(equal? equal?))))
...
|#
	...
	;; TODO:
	;; closure sig from body -- test these!
	...
	;; pp unquasiquote would make suggestions more readable (not {list} et al), but ,@() makes it unreconstructible?
	;;
	;; define-macro that is define f g, and (define-macro (x . a) `(y ,@a)) [4351] (lambda a ({list} 'y ({apply_values} a)))
	;;
	;; eqv? should be equal? in (eqv? '() (list)) and  eqv? should be equal? in (eqv? '#f '#f) etc
	;;   eqv?: eqv? could be not: (eqv? #f #t) -> (not #t), also eqv?+constants
	;;
	;; typecase confuses (integer?) as case key -- should just skip macros?
	;;
	;; format ~0C -> ~*?
	;;   check ~01C etc ~N~NC?, ~@<not P> is an error
	;;   format arg counter gets confused by ~4,T etc
	;;
	;; member append -> or mem 2x [see below] -- need caller type arg to special-case so boolean? -> replace member with or
	;;   desired-type to lint-walk (#f=return value ignored, how to handle set!?)
	;;   check member+4th arg if eq? etc
	;;   see memx code for other stuff
	;;   but assoc is ok here in either case? 
	;;
	;; catch the missing paren cond case: can be omitted checks will see most, but better to be explicit
	;;   (cond (f x) (g y) (else z)) where (cond ((f x) (g y)) (else z)) was meant
	;;   so we'll see func when bool is expected?
	;;
	;; collapse cxr isn't correct if it's the target of set! -- (set! (cdr (cdr...))) -- this needs to be passed into lint-walk?
	...
	;;   (fill! (reverse! x) #f) 
	;;
	;; see list->string et al below
#|
	(if (and (member 'length form (lambda (a b) (and (pair? b) (eq? (car b) a))))
		 (member 'pair? form (lambda (a b) (and (pair? b) (eq? (car b) a)))))
	  ...
	  ((memq assq memv assv member assoc)
	   ...
	   (when (= (length form) 3)
	     (let ((selector (cadr form))
	       ...
	       ;; (if (and (pair? items) (memq (car items) '(cons append list))) (format *stderr* "~A~%" form))
	       ;; these happen esp list
	   ...
	   (when (and (= (length form) 3)
	     ...
	     (let ((arg (cadr form)))
	       (if (and (pair? arg)
			(eq? (car arg) '-)
			(= (length arg) 3))
		   (lint-format "perhaps ~A" name (lists->string form `(,head ,(cadr arg) ,(caddr arg)))))))
	   ...
	   (let ((inverses '((reverse . reverse) 
		 ...
		 (cond ((eq? (car arg) inv-op)                 ; (vector->list (list->vector x)) -> x
			...
			(lint-format "perhaps ~A" name (lists->string form `(make-vector ,@(cdr arg)))))
		       ((and (memq (car arg) '(reverse reverse!))
			     (pair? (cadr arg))                ; (list->string (reverse (string->list x))) -> (reverse x)
			     (eq? (caadr arg) inv-op))
			(lint-format "perhaps ~A" name (lists->string form `(reverse ,(cadadr arg)))))

		       ;; cd?r here: (list->vector (cdr (vector->list x)))
		       ;; (string->symbol (list->string (cdr (string->list (symbol->string name)))))
		       ;; (list->vector (sort! (vector->list seq) less?))
		       ;; (list->vector (list 'first (N_ "First Option") (N_ "Help for first option.")))
		       ;; (list->vector (copy (vector->list x)))
		       ;; (list->string (list string))
		       ;; (reverse (cdr (reverse (get-cards end-slot)))) ??
		       ;; (string->list " hundred")
...
#|
	   ;; maybe move this to lint-walk after the args walk at the end, but will set! process in the right order?
...
|#
		 ...
		 (let ((len1 (length new-args)))
		   ...
		   (case len1
			...
			(if (or (null? arg2)           
			    ...
			    (if (null? arg1)                            ; (append () x) -> x
				(lint-format "perhaps ~A" name (lists->string form arg2))
				(if (pair? arg1)            
				    (if (and (pair? arg2)               ; (append (list x y) '(z)) -> (list x y 'z)
					     (or (eq? (car arg1) 'list)
					...
					(if (and (eq? (car arg1) 'list) ; (append (list x) y) -> (cons x y)
						 (pair? (cdr arg1))
						 (null? (cddr arg1)))
	     ...
	     (let ((len (length form)))
		   ...
		   (if (and (> len 2) ; it might be (apply)...
		       ...
		       (let ((happy #f)
			     (f (cadr form)))
			 (when (and (> len 2)
				    (not (eq? f 'macroexpand))) ; handled specially
		 ...
		 (let ((func (cadr form))
		   ...
		   (when (= args 1)
			 ...
			 (let* ((seq-func (cadr seq))
			   ...
			   (if (symbol? func)
			       (if (symbol? seq-func)              ; (map f (map g h)) -> (map (lambda (x) (f (g x))) h) -- dubious...
				   (lint-format "perhaps ~A" name 
                                     (lists->string form `(,head (lambda (,arg-name) 
								   (,func (,seq-func ,arg-name))) 
								 ,(caddr seq))))
				   (if (simple-lambda? seq-func)   ; (map f (map (lambda (x) (g x)) h)) -> (map (lambda (x) (f (g x))) h)
				       (lint-format "perhaps ~A" name 
                                         (lists->string form `(,head (lambda (,arg-name)
								       (,func ,(tree-subst arg-name (caadr seq-func) (caddr seq-func))))
								     ,(caddr seq))))))
			       (if (less-simple-lambda? func)
				   (if (symbol? seq-func)          ; (map (lambda (x) (f x)) (map g h)) -> (map (lambda (x) (f (g x))) h)
				       (lint-format "perhaps ~A" name 
                                          (lists->string form `(,head (lambda (,arg-name)
									,@(tree-subst (list seq-func arg-name) (caadr func) (cddr func)))
								      ,(caddr seq))))
				       (if (simple-lambda? seq-func) ; (map (lambda (x) (f x)) (map (lambda (x) (g x)) h)) -> (map (lambda (x) (f (g x))) h)
					   (lint-format "perhaps ~A" name  
				             (lists->string form `(,head (lambda (,arg-name)
				...
				(cdr arg)))
				 ...
				 (cond ((null? (cdr arg)) ; #<unspecified>
					(if (not (any-checker? checker #<unspecified>))
					    (report-arg-trouble name form head arg-number checker arg 'unspecified? env)))
				       ((null? (cddr arg))
					(check-arg (cadr arg)))
				       (else
					(for-each
	  ...
	  (if (null? args)
		...
		(if data
		  ...
		  (let ((arg-data (if (symbol? args)                            ; this is getting arg names to add to the environment
		    ...
		    (lint-walk-function-body name head args arg-data val (append arg-data (if data (cons data env) env)))
	...
	(and (pair? c1) (= (length c1) 2)
	     (pair? c2) (= (length c2) 2)
	     (boolean? (cadr c1))
	     (boolean? (cadr c2))
	     (memq (car c2) '(#t else))
	     (if (equal? (cadr c1) (cadr c2))
		 (and (not (side-effect? (car c1) env))
		      (begin (lint-format "perhaps ~A" name (lists->string form (cadr c1))) #t))
		 (begin
		   (if (eq? (cadr c1) #t)
...
#|
	(if (and (pair? form)
		 (pair? (cdr form))
		 (member 'reverse! form (lambda (a b) (and (pair? b) (eq? (car b) a)))))
	    (format *stderr* "~A~%" form))
|#
	(if (symbol? form)
...
#|
				 ;; this happens a lot but it's hard to tell which should be combined -- maybe if differ not in func?
				 (if (and (pair? true)
					  (pair? false)
					  (not (eq? (car true) 'quote))
					  (> (length true) 1)
					  (differ-in-one true false))
				     (format *stderr* "~A~%~%" form))
|#
							   ...
							   ((or (not (pair? iff))
							    ...
							    (append (reverse clauses)
								    (if (and (pair? iff)
									     (= (length iff) 3)
									     (eq? (car iff) 'if))
									`((,(cadr iff) ,@(unbegin (caddr iff))) (else #<unspecified>))
									`((else ,@(unbegin iff))))))
				...
				(if (not (pair? clause))
					...
					(if (or (not (= len 2))
					    ...
					    (lint-format "perhaps ~A" name 
					      (lists->string form (if (not all-eqv)
								      ...
								      (cond->case eqv-select (reverse new-clauses) has-else)))))
					(set! all-eqv #f)))
			     ...
			     (when (and all-eqv
			       ...
			       (lint-format "perhaps use case instead of cond: ~A" name
					    (lists->string form (cond->case eqv-select (cdr form) has-else))))))
		       env))
		    ...
		    ((let)
			 ...
			 (let ((named-let (and (symbol? (cadr form)) (cadr form))))
			   ...
			   (unless named-let
				 ...
				 (catch #t
				   (lambda ()
				     (let ((val (eval (copy form :readable) (rootlet))))
				       (lint-format "perhaps ~A" name (lists->string form val))))
				   (lambda args
				     'error))))
		    ...
		    ((define-syntax let-syntax letrec-syntax define-module) ; all meaningless in s7
		     ...
		     ;; of non-standard # and \ forms.  The # forms can mostly be kludged up via #*readers (although
		     ;; MIT-scheme's use of #!optional is a bother), but I'm not going to start building in all 
		     ;; the crazy \ stuff.  
			   ...
			   (when (and (pair? head)
			     ...
			     (let ((len (length (cadr head))))
				   ...
				   (if (and (null? (cadr head))
					   ...
					   (lint-format "perhaps ~A" name (truncated-lists->string form (caddr head)))
					   (lint-format "perhaps ~A" name (truncated-lists->string form `(let () ,@(cddr head)))))
				       (if (identity? head)
...
|#
		      ((eof-object? form))
		    ;(format *stderr* ";        ~%~A~%" (truncated-list->string form))
		    (if (pair? form)
