...
|#
	...
	(let ()
	  (define (sp-string-copy caller head form env)
		...
		(lint-format "~A could be ~A" caller (truncated-list->string form) (cadr form))))
	  (lint-hash h 'string-copy sp-string-copy))
	...
	(let ()
	  ...
	  (lint-hash h 'string-ref sp-string-ref))
	...
	(let ()
	 ...
	 (lint-hash h 'close-output-port sp-flush-output-port)
	...
	(let ()
	  ...
	  (lint-hash h 'magnitude sp-magnitude))
	...
	(let ()
	  ...
	  (lint-hash h 'and-let* sp-and-let))
	...
	(when (< 5 leaves 128)
	  (call-with-exit
	   (lambda (quit)
		 ...
		 (when (not (and (pair? lint-function-body)
		 ...
		 (let* ((rnames (map (lambda (v) 
		   ...
		   (let ((reducibles ()))
		     ...
		     ;(format *stderr* "vars: ~A, reduc: ~A, rvar: ~A, form: ~A~%" outer-vars reducibles rvars reduced-form)
		     ...
		     (when (pair? reducibles)
		       ...
		       ;(format *stderr* "outer: ~A~%" outer-vars)
    ...
    (define (reduce-function-tree fvar env)
      (let ((definition (cond ((var-initial-value fvar) => cddr) (else #f)))
	    (name (var-name fvar)))
	;(format *stderr* "reduce function: ~A ~A~%" name definition)
