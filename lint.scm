...
(define lint
  (let ((no-side-effect-functions 
	...
	(function-types (hash-table (cons '* 2) ; if it can be an int, use 2, else 1
				    (cons '+ 2)
				    (cons '- 2)
				    (cons '/ 2)
				    (cons '< #t)
    ...
    (define (simplify-numerics form env)
      ...
      ;;   I first tried a table of rules, but the code is unreadable, so
		...
		(if (just-rationals? args)
			...
			(if (equal? (cadr args) 0)
			    ...
			    (if (and (pair? (car args))
				     (eq? (caar args) '-))
				`(- ,@(cdar args) ,(cadr args)) ; (- (- x y) z) -> (- x y z)
				`(- ,@args))))))
	       (else 
		(if (just-rationals? args)
		    (apply - args)
		    ...
		    (let ((arity (procedure-arity (symbol->value head)))
			  ...
			  (begin
			    ...
			    (case head
				 ...
				 ;(if (not arg-data) (format #t "~A arg hash: ~A~%" head arg-data))
...
;;; TODO: what about top-level expressions that have no side-effects
;;;         (+ 3 4) sitting somewhere
