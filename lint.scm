						   ...
						   (let ((expr 'unset))
							 ...
							 (if (not (caddr form))
							     ...
							     (if (and (eq? (cadr form) #t)
								      (pair? (caddr form))
								      (eq? (return-type (caaddr form)) 'boolean?))
								 (set! expr (caddr form))
								 (if (and (eq? (caddr form) #t)
									  (pair? (cadr form))
									  (eq? (return-type (caadr form)) 'boolean?))
								     (set! expr (cadr form))))))
					...
					((eqv?) 
					 (if (< (length form) 3)
					     ...
					     (let ((arg1 (cadr form))
						   ...
						   (if (or (eq-able? arg1)
							   (eq-able? arg2))
						       (lint-format "eqv? could be eq? in ~S" name form)))))
