...
(define *report-bad-variable-names* ())                   ; a list of bad variable names: '(data datum new item info temp tmp val vals value foo bar aux dummy O var)
			 ...
			 (lambda (return)
			   ...
			   (let ((sym #f)
			     ...
			     (let ((selector #f)              ; (or (and (eq?...)...)....) -> (case ....)
				   (keys ()))
			       (when (every? (lambda (p)      ;   else has to be just #f here
					       (and (pair? p)
						    (eq? (car p) 'and)
						    (pair? (cdr p))
						    (pair? (cadr p))
...
|#
    ...
    (define (structures-equal? l1 l2 matches e1 e2)
      ;; (format *stderr* "    equal ~A ~A ~A" l1 l2 matches)
      (if (pair? l1)
	  (and (pair? l2)
	       (if (eq? (car l1) 'quote)
		   (and (eq? (car l2) 'quote)
...
#|
				     (for-each (lambda (p)
						 (let ((par (if (pair? p) (car p) p)))
						   (if (and (or (hash-table-ref built-in-functions par)
								(hash-table-ref syntaces par))
							    (tree-car-member par (cddr form)))
						       (lint-format "~A's parameter ~A is called ~A: this can only cause confusion" caller
								    (car sym) par
								    (car (tree-call-member par (cddr form)))))))
					       (cdr sym)))
|#	       
				     )
			   ...
			   (call-with-exit
				...
				(let ((p (car fp))
				  ...
				  (if (and (pair? p)
					   ...
					   (memq (caar p) '(or and))
					   (eq? (caar p) (caar np))
					   (equal? (cadar p) (cadar np)))
				      (ok (format *stderr* "~A~%~%" (lint-pp form))))))))
|#			   
			   ;; ----------------
		   ...
		   (if (or (< (length form) 3)
			 ...
			 (let ((vars (if (and named-let 
			   ...
			   (when (and (not named-let)           ; (let ((x 0)...) (set! x 1)...) -> (let ((x 1)...)...)
				      (pair? body)              ; this almost never happens in let*, and a second set! is rare
				      (pair? (car body))
				      (pair? (cdar body))
				      (eq? (caar body) 'set!)
				      (pair? (cddar body))
...
;;;    iterate over tree-leaves dim, copy matchlist, (structures-equal? code (v leaves i) matches () ())
...
;;; to get symbol->value hotspot, run profile: (or (s->v> sym) (s->v sym *e*))? [any-checker]
;;; x used as number then (if x...)
;;; check 1st repeated in ifs (if (and A B) a (if (and A C) b)) -> (if A (if B a (if C b))), but no others?
;;; define-values could add its args to the current env
;;;
;;; 109 21295
