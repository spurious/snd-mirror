...
(define lint
  ...
  (let ((no-side-effect-functions 
	...
	(constant-functions (let ((ht (make-hash-table)))
			      (for-each
			       (lambda (op) 
				 (hash-table-set! ht op #t))
			       '(* + - / < <= = > >= 
    ...
    (define (any-null? x)
	  ...
	  (and (pair? x)
		 ...
		 ((list)
		  (null? (cdr x)))
		 ;; no hits: (make-list 0 ...) (string->list "") (vector->list #()) (reverse ()) (copy ()) (append ())
		 (else #f)))))
			...
			(let ((cdrf #f)
			  ...
			  (when (and (or (and (or (any-null? (cadr nf))
					...
					(and (len=1? (cadr cdrf))
					     ...
					     (equal? (caddr (caddr cdrf)) `(,name (cdr ,iter)))
					     (set! cdrf `(cons ,(tree-subst (list 'car iter) (car (caadr cdrf)) (cadr (caddr cdrf)))
								,(caddr (caddr cdrf))))))
    ...
    (define (improper-arglist->define* name ftype arglist initial-value env)
			      ...
			      (lint-format "perhaps ~A" name
					   (lists->string initial-value
							  (if (eq? (car initial-value) 'define)
							      `(define* (,(caadr initial-value) ,@new-arglist)
								 ,new-body)
							      `(lambda* ,new-arglist
								 ,new-body)))))))))))))))))
	...
	(recursion->iteration name ftype arglist initial-value env))
      (improper-arglist->define* name ftype arglist initial-value env)
		    ...
		    ((not)
			 ...
			 (let* ((arg (cadr form))
			   ...
			   (cond ((boolean? val) 
				 ...
				 ((and (len>1? val)                 ; (not (not ...)) -> ...  this is usually internally generated, 
			   ...
			   (when (= len 3)
				...
				(let ((nots 0)
				  ...
				  (if (= nots arglen)                               ; every arg is `(not ...)
				      (let ((nf (simplify-boolean (cons 'and (map cadr (cdr form))) () () env)))
					(return (simplify-boolean (list 'not nf) () () env)))
				      (if (and (> arglen 2)
						   ...
						   (and (> arglen 2)
							(> nots (/ arglen 2))
							(> revers 0))))
					  (let ((nf (simplify-boolean (cons 'and (map (lambda (p)
											(cond ((not (pair? p))
											       (list 'not p))
											      ((eq? (car p) 'not)
											       (cadr p))
				     ...
				     (when start
					   ...
					   (let ((trailer (if (not (len>1? fp))
								...
								((if (and (pair? nfp)
								 ...
								 nfp)))))
					     (return (if (eq? start (cdr form))
							 (cons 'or (cons (gather-or-eqf-elements eqfnc sym vals) trailer))
							 `(or ,@(copy (cdr form) (make-list (do ((g (cdr form) (cdr g)) 
												 (len 0 (+ len 1)))
												((eq? g start) 
												 len))))
			...
			(and (not (contradictory? (cdr form)))
				...
				(let ((nots 0)
				  ...
				  (for-each (lambda (a)
					    ...
					    (cdr form))
				  (if (= nots arglen)                               ; every arg is `(not ...)
				      (let ((nf (simplify-boolean (cons 'or (map cadr (cdr form))) () () env)))
					(return (simplify-boolean (list 'not nf) () () env)))
				      (if (and (> arglen 2)
					       (or (>= nots (/ (* 3 arglen) 4))      ; > 2/3 seems to get some ugly rewrites
						   (and (>= nots (/ (* 2 arglen) 3)) ; was > 1/2 here
							(> revers 0))))
					  (let ((nf (simplify-boolean (cons 'or (map (lambda (p)
										       (cond ((not (pair? p))
											      (list 'not p))
											     ((eq? (car p) 'not)
											      (cadr p))
				...
				(if (every? (lambda (a)
					      (and (pair? a)
						   (eq? (car a) 'zero?)))
					    (cdr form))
				    (return (cons '= (cons 0 (map cadr (cdr form))))))
	...
	(define numerics-table
	  (let ((h (make-hash-table)))
		...
		(case (length args)
	      ...
	      (for-each (lambda (f) (hash-table-set! h f dumb+)) '(fix:+ fx+ flo:+ fl+))
	      )
...
|#
	 ...
	 (define (sp-= caller head form env)
	   (let ((len (length form)))
		 ...
		 (if (or (and (memv arg1 '(0 1))
			 ...
			 (lint-format "perhaps ~A" caller (lists->string form `(integer? ,(cadr (if (pair? arg1) arg1 arg2))))))
			  ...
			  (when (pair? (cdr arg))
			    (case (car arg)
			      ...
			      ((numerator)              ; (negative? (numerator x)) -> (negative? x)
			       (lint-format "perhaps ~A" caller (lists->string form `(,head ,(cadr arg)))))
			      
			      ((string-length)          ; (zero? (string-length x)) -> (string=? x "")
			       (if (eq? head 'zero?)
				   (lint-format "perhaps ~A" caller (lists->string form `(string=? ,(cadr arg) "")))))
			      
			      ((vector-length)          ; (zero? (vector-length c)) -> (equal? c #())
			       (if (eq? head 'zero?)
				   (lint-format "perhaps ~A" caller (lists->string form `(equal? ,(cadr arg) #())))))
		...
		(let ((seq (cadr form)))
		  ...
		  (when (pair? seq)
		    ...
		    (when (eq? head 'list-ref)
		      (if (eq? (car seq) 'quote)
			  (if (and (pair? (cadr seq))          ; ignore dumb (list-ref () 0)
				   (proper-list? (cadr seq)))  ; (list-ref '(#t #f) (random 2)) -> (vector-ref #(#t #f) (random 2))
		 ...
		 (do ((nargs ())               ; look for (string...) (string...) in the arg list and combine
		   ...
		   (let ((arg (car p)))
		     (cond ((and (len=3? arg)            ; (string-append (apply string-append strs) str) -> (string-append (apply values strs) str)
			    ...
			    (set! nargs (cons `(apply values ,(caddr arg)) nargs)))
						  ...
						  (case (car args) 
						    ...
						    ((quote)            ; (apply eq? '(a b)) -> (eq? 'a 'b)
						     (and (= suggestion made-suggestion)
							  (pair? (car cdr-args))
							  (lint-format "perhaps ~A" caller (lists->string form (cons f (distribute-quote (car cdr-args)))))))
						    
						    ((cons)             ; (apply f (cons a b)) -> (apply f a b)
						     (lint-format "perhaps ~A" caller 
								  (lists->string form 
										 (if (and (pair? (cdr cdr-args))
											  (pair? (cadr cdr-args))
	...
	(let ()
	  (define (sp-wots caller head form env)
		...
		(when (and (len=2? (car body))
			   (memq (caar body) '(write display)))
		  (if (null? (cdr body))
		      (lint-format "perhaps ~A" caller
				   (lists->string form (cons 'object->string (cons (cadar body) (if (eq? (caar body) 'display) '(#f) ())))))
	...
	(hash-special 'sort
			...
			(if (and (= (length form) 3)
				...
				(lint-format "perhaps ~A" caller
					     (truncated-lists->string form
								      `(sort! (copy ,(cadr form)) ,(caddr form))))))))
		 ...
		 (when (and (zero? (var-set local-var))
		   ...
		   (let ((hist (var-history local-var)))
		     (when (and (pair? hist)
		       ...
		       (if (> (length hist) 2) ; an experiment -- if all refs are by list-ref (in effect) suggest a vector
			   (let ((init (var-initial-value local-var)))
			     (if (and (pair? init)
				      (or (memq (car init) '(list make-list string->list vector->list)) ; perhaps: also *->list make-list memx??
				      ...
				      (every? (lambda (p)
						(and (pair? p)
							 ...
							 (memq (car p) '(list-ref length reverse map for-each)))))
					      hist))
		   ...
		   (let ((init (var-initial-value local-var))) ; look for various open-output-string problems
			 ...
			 (when (and (eq? (car var) vname)
			   ...
			   (let ((hist (var-history local-var)))
				 ...
				 (when (and (pair? (cddr outer-form))
					    (pair? (cdddr outer-form)))
    ...
    (define (lint-walk-open-body caller head body env)
	  ...
	  (let ((prev-f #f)
		...
		(when (= f-len prev-len 3)
		  ...
		  (when (and (eq? f-func 'set!)          ; other such funcs like fill! don't seem to happen
			     (eq? (car prev-f) 'set!))
    ...
    (define (evert-function-locals form vars env)
      ...
      ;; currently called only in let-walker, but it might make sense in let*-walker and letrec-walker.

      (when (and (pair? form) ; this is (car lint-function-body)
	 ...
	 (lambda (local-var)
	   (let ((vname (var-name local-var))
	     ...
	     (when (and (zero? (var-set local-var))
			...
			(or (code-constant? vvalue)
			    (and (pair? vvalue)
				 (hash-table-ref constant-functions (car vvalue))
				 (not (var-member (car vvalue) env))
				 (every? code-constant? (cdr vvalue))))
			(every? (lambda (p)
				  (not (and (pair? p)
	       ...
	       (lint-format "~A can ~Abe moved to ~A's closure" lint-function-name
			   ...
			   "probably "
			   "")
		       (if (not (keyword? lint-function-name))
			   lint-function-name
	...
	((char-ci=?)
	 (cons (if (equal? eqv-select (cadr test))
	   ...
	   (let ((len (length form)))
		 ...
		 (if (< len 3)
		     ...
		     (let ((test (cadr form))
		       ...
		       (when (and (pair? true)
				  (pair? false)
				  (not (memq true-op (list 'quote {list} 'not)))
				       ...
				       (let ((nform (cond ((eq? true-op (caadr diff))  ; very common!
							  ...
							  ((or (memq true-op '(set! begin and or))
								 ...
								 (and (pair? lst)
							       ...
							       (not (and (pair? expr)
									 (or (side-effect? expr env)
									     (memq (car expr) '(#_{list} #_{apply_values} #_{append} unquote))))))
							   (tree-subst-eq (cons 'if (cons expr (cadr diff))) subst-loc true))
			   ...
			   (let ((false-test (and (pair? false-rest) (car false-rest))))
				 ...
				 (let ((test1 (simplify-boolean `(or ,expr ,false-test) () () env)))
				   (lint-format "~Aperhaps ~A" caller 
						...
						(lists->string form `(if ,test1 ,true))))))
				 ...
				 (lambda (v)
				   (let ((new-init (let ((init (var-initial-value v)))
							 ...
							 (if (and (pair? init)
							     ...
							     `(,(car init) ,(cadr init) ,(caddar body))
							     :none)))))
	 ...
	 (define (let*-walker caller form env)
	   (if (< (length form) 3)
		 ...
		 (let ((vars (if (or (not named-let)
				     ...
				     (not (or (null? (caddr form))
					      (and (proper-list? (caddr form))
						   (every? pair? (caddr form)))))) ; i.e. the var&value stuff is messed up
				 ()
				 (list (make-fvar :name named-let 
						  :ftype 'let*
						  :decl (dummy-func caller form (list 'define* (cons '_ (map car (caddr form))) #f))
			 ...
			 (cond ((pair? tree)
				(case (car tree)
				  ...
				  ((define define*
				     ...
				     #_{list} #_{apply_values} #_{append} quasiquote unquote
			   ...
			   (let ((v (var-member head env)))
				 ...
				 (if (and (memq (var-ftype v) '(define lambda define* lambda*))
				     ...
				     (let ((cv (var-member caller env)))
				       (set! (var-scope v) 
					     (cons (if (and (var? cv)
							    (memq (var-ftype cv) '(define lambda define* lambda*))) ; named-let does not define ftype (?? -- 'let)
						       caller
						       (cons caller env))
						   (var-scope v)))))
				 (begin
				    ...
				    (lint-format "perhaps ~A" caller
						 (lists->string form
								(un_{list} (if (pair? (cadadr form))
									       (cadadr form)
									       `(copy ,(cadadr form))))))
				    (if (or (symbol? (cadr form))                
					    (quoted-symbol? (cadr form)))
					(lint-format "perhaps ~A" caller      ; `(,x) -> (list x) 
						     (lists->string form `(list ,(cadr form)))))))
			       ((3)
				(when (and (pair? (caddr form))
					   (eq? (caaddr form) #_{apply_values}))   ; `(+ ,@(map f x)) -> (cons '+ (map f x))
				  (if (not (or (and (pair? (cadr form))
						    (tree-set-member '(#_{apply_values} #_{list} #_{append} unquote) (cadr form)))
					       (tree-set-member '(#_{list} #_{apply_values} #_{append} unquote) (cdaddr form))))
				      (lint-format "perhaps ~A" caller
			       ...
			       (else 
				(if (and (= (length form) 4)                        ; `(+ ,y ,@(map f x)) -> (cons '+ (cons y (map f x)))
					 (not (and (pair? (cadr form))
						   (tree-set-member '(#_{apply_values} #_{list} #_{append} unquote) (cadr form))))
					 (pair? (cadddr form))
					 (eq? (car (cadddr form)) #_{apply_values})
					 (not (tree-set-member '(#_{list} #_{apply_values} #_{append} unquote) (cdr (cadddr form)))))
				    (if (not (and (pair? (caddr form))
						  (tree-set-member '(#_{apply_values} #_{list} #_{append} unquote) (caddr form))))
					(lint-format "perhaps ~A" caller
						     (lists->string form
								    `(cons ,(cadr form) (cons ,(caddr form) ,(cadr (cadddr form))))))
					(if (and (pair? (caddr form))               ; `(f ,@x ,@y) -> (cons 'f (append x y))
						 (eq? (car (caddr form)) #_{apply_values})
						 (not (tree-set-member '(#_{list} #_{apply_values} #_{append} unquote) (cdr (caddr form)))))
					    (lint-format "perhaps ~A" caller
							 (lists->string form
									`(cons ,(cadr form) (append ,(cadr (caddr form)) ,(cadr (cadddr form))))))))
				    (when (every? (lambda (a)                       ; `(,@x ,@y etc) -> (append x y ...)
						    (and (pair? a)
...
;;; (define (g x) `(display ,(map f x))) -> (list 'display (map f x))
;;;   `(f ,@x...) -> (cons 'f (append x...)) and combine 3/4/else clauses [19328]
;;;   all of these need line numbers: port-line?
;;; vector->float|int-vector, list|vector->let?
;;;   str->bv? lst->ht if assoc
;;; make-list in recur or built-in?
;;; length (et al) repeated while arg is not changed for fewer reps than car?
;;; use t471+eval replaced by read (to skip read-error)
