...
(define lint
  ...
  (let ((no-side-effect-functions 
    ...
    (define (side-effect-with-vars? form env vars)
	  ...
	  (or (and (not (hash-table-ref no-side-effect-functions (car form)))
		...
		((let let* letrec letrec*)
		 ...
		 (letrec ((let-effect? (lambda (f e v) ; check bindings
					 (and (pair? f)
					      (or (not (pair? (car f)))
		   ...
		   (or (< (length form) 3)
			 ...
			 (if (and (pair? vars)
			     ...
			     (for-each (lambda (sym)
					 (when (and (pair? sym)
						    (pair? (cdr sym))
						    (tree-list-member vars (cdr sym)))
					   (set! vars (cons (car sym) vars))))
				       syms))
			 (or (let-effect? syms env vars)
				 ...
				 ((and (pair? arg)
				  ...
				  (case (car arg)
					...
					char-ci<? char-ci>? char-ci<=? char-ci>=? string-ci<? string-ci>? string-ci<=? string-ci>=?)
				     `(,(hash-table-ref notables (car arg)) ,@(cdr arg)))
    ...
    (define (check-special-cases caller head form env)
      ...
      ;; in all-ok? case removal, need smarter traversal here
      ;;
      ;; perhaps report an overall order to definitions in a block that maximizes locality
      ;; constant expr in do: add also to recursive funcs, map/for-each?
      ;;   can this also work for the let-var value forgotten business?
      ;; 
      ...
      ;;   these should be ok because we won't have any signature unless the whole package is loaded
      ;;   any-checker? sees the symbol, so we can check down to the gtk|snd|clm|cload?-constant level
      ;;   (gtk-enum? func argnum arg) -- hash of constants->enums, see if arg func argnum type (via another hash?)
      ;;   perhaps only load this data when needed  or at compile time (snd-xref.c)
      ;;   there are about 2000 of these
      ;;
		 ...
		 (let ((arg-type #f))
		   (if (and (not (memq (var-definer arg) '(parameter named-let named-let*)))
			 ...
			 (if (and (> (var-ref arg) 3)
				  (zero? (var-set arg))
				  (memq (var-ftype arg) '(define define* lambda lambda*))
    ...
    (define (proper-list* lst)
      ...
      (if (and (pair? lst)
	       (not (eq? (car lst) :allow-other-keys)))
	  (if (eq? (car lst) :rest)
	      (cdr lst)
			  ...
			  (if (symbol? (car rest1))
			      (and (symbol? (car rest2))
				   (let ((new-matches (cons (cons (car rest1) (car rest2)) matches)))
				     (and (structures-equal? (cdr rest1) (cdr rest2) new-matches)
					  new-matches)))
			      (and (equal? (length (car rest1)) (length (car rest2))) ; (car rest2) might be a symbol, dotted lists ok here
				   (structures-equal? (cdr rest1) (cdr rest2)
						      (append (map cons (proper-list (car rest1)) (proper-list (car rest2)))
	  ...
	  (if (eq? (car l1) 'quote)
	      ...
	      (and (pair? l2)
		   (cond ((not (and (pair? (car l1))
			 ...
			 (else (or (structures-equal? (car l1) (car l2) matches)
				   (and (eq? (hash-table-ref reversibles (caar l1)) (caar l2))
					(do ((a (cdar l1) (cdr a))
					     (b (reverse (cdar l2)) (cdr b)))
					    ((or (null? a)
						 (null? b)
						 (not (structures-equal? a b matches)))
					     (and (null? a)
						  (null? b))))))))
	...
	(if (null? args)
		...
		(let ((e (lint-walk-function-body caller head val cur-env)))
		  (let ((nvars (and (not (eq? e cur-env))
				    (env-difference caller e cur-env ()))))
		    (if (pair? nvars)
			(report-usage caller 'parameter head nvars cur-env))))
		cur-env))
		...
		(let ((arg-data (if (symbol? args)                            ; this is getting arg names to add to the environment
		  ...
		  (let ((cur-env (append arg-data (if data (cons data env) env))))
		    (let ((e (lint-walk-function-body caller head val cur-env)))
		      (let ((nvars (and (not (eq? e cur-env))
					(env-difference caller e cur-env ()))))
			(report-usage caller 'parameter head (append (or nvars ()) arg-data) cur-env))))
    ...
    (define (lint-walk caller form env)
	  ...
	  (if (pair? form)
		...
		(case head
		  ...
		  ((lambda lambda*)
		   ;; ---------------- lambda ----------------		  
		   ;; the lambda case includes stuff like call/cc, call-with-input-string etc
		   (let ((len (length form)))
		     (if (< len 3)
			 (begin
									   ...
									   (cond ((not (and (null? (cdr a))
										 ...
										 ((not (car a))
										  (if (eq? (car b) #t)
										      ...
										      (simplify-boolean `(and (not ,nexpr) ,(car b)) () () env)))

										 (else `(if ,nexpr ,(car a) ,(car b)))))))))
				       (let ((arg1 (cadr form))
					     (arg2 (caddr form)))
					 (if (and (pair? arg1)
						  (pair? arg2)
		  ...
		  ((do)
		   ...
		   (let ((vars ())
			 ...
			 (let ((step-vars (cadr form))
			   ...
			   (when (not (tree-list-member '(call/cc call-with-current-continuation lambda lambda* define define* 
							  define-macro define-macro* define-bacro define-bacro* define-constant define-expansion)
							(cdddr form)))
			     (let* ((vs (out-vars 'do (map var-name vars) (cdddr form)))
				 ...
				 (define (expr-walk tree)
				   (if (pair? tree)
				       (if (all-ok? tree)
					   (if (and (not (eq? (car tree) 'quote))
						    (not (member tree constant-exprs)))
					       (set! constant-exprs (cons tree constant-exprs)))
					   (begin
					     (if (pair? (car tree))
						 (expr-walk (car tree)))
					     (if (pair? (cdr tree))
						 ;; (not (memq (car tree) '(case do let let* letrec letrec*)))) ; and define etc
						 ;; with these in, we get occasional bogus calls -- need code walker here
						 (for-each expr-walk (cdr tree)))))))
				 
				 (expr-walk (cdddr form))
				 ...
				 (let ((vname (car first-var))
				   ...
				   (when (and step
				     ...
				     (when (and (memq (car body) '(vector-set! float-vector-set! int-vector-set! list-set! string-set! byte-vector-set!))
					   ...
					   (lint-format "perhaps ~A" caller 
							(lists->string form `(fill! ,(cadr body) ,(car (cdddr body)) ,start ,end)))
					   (lint-format "perhaps ~A" caller 
							(lists->string form `(copy ,(cadr setv) ,(cadr body) ,start ,end))))))))))))
		     env))
