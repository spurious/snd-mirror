...
(define lint
  ...
  (let ((no-side-effect-functions 
	...
	(do ((selector #f)              ; (or (and (eq?...)...)....) -> (case ....)
      ...
      ;; -------- reduce-or --------
      (define (reduce-or return form len env)
	(do ((new-form ())
	     (retry #f)
      ...
      ;; -------- reduce-and --------
      (define (reduce-and return form len env)
	(do ((new-form ())
	     (retry #f)
	     (exprs (cdr form) (cdr exprs)))
	    ((null? exprs) 
		...
		(if (< len 0)
		  ...
		  (case (car form)
			...
			(and (not (contradictory? (cdr form)))
				...
				(reduce-and return form len env)))))))))))))))))))
    ...
    (define (find-constant-exprs caller vars form body)
	  ...
	  (let ((refs (let ((vs (out-vars caller vars body)))
		...
		(if (let all-ok? ((tree tree))
		    ...
		    (if (not (or (memq (car tree) '(quote values))
		...
		(cond ((not (and (len=2? c)
		      ...
		      ((and (null? (cdar c))                         ; ((a) a)
		       ...
		       (cond ((assq 'symbol->value svs) => (lambda (sv-data)
			     ...
			     (else (set! svs (cons (cons 'symbol->value (list (caar c))) svs)))))
		      ...
		      ((and (null? (cdar c))                 ; ((a) (f a))
		       ...
		       (cond ((assq (caadr c) svs) => (lambda (func-data)
			     ...
			     (else (set! svs (cons (cons (caadr c) (list (caar c))) svs)))))
		      ...
		      (let* ((lr-name (caaadr form))
			     (lr-args (let search ((tree (caddr form)))
					(and (pair? tree)
						 ...
						 (or (search (car tree))
						     (search (cdr tree))))))))
			;; slightly inaccurate -- we should make sure no free var in letrec lambda (ref to outer)
			;;   is shadowed at the point of the lr-lambda call in the body lambda, but as far as
			;;   I can see, that never happens.
			(lint-format "perhaps ~A" caller
				     (lists->string form
...
;;; (and ... (string? (car c)) (string=? (car c) x) ...) number? et al, char?, string? -> equal?/eqv? -- currently only works as first with string constant, and-redundant?
...
;;; constant exprs in recursive func, maybe also in func arg to member/assoc [find-constant-exprs]
;;; optional checks for redef rootlet names
;;;
;;; 187 28825 768679
