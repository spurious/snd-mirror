  ...
  (let ((no-side-effect-functions 
	...
	(let ((body ((if (memq ftype '(let let*)) cdddr cddr) initial-value))
	  ...
	  (when (and (pair? body)
	    ...
	    ;; another map form: (let loop ((lst x) (res ())) (if (null? lst) (reverse res) (loop (cdr lst) (cons ... res))))
	    ;;   args can be reversed, if reversed, need new-form check on arg->res 
	    ;;   to spot-check -- look for reverse, 2 args one cdr, other cons
		...
		(when (and (pair? nf)
		  ...
		  (let ((sequence (case (car initial-value) 
		    ...
		    (define (make-new-form form iter)
		      `(lambda (_1_)
			 ,@(let rem ((tree form))
			...
			(when (and (or (and (any-null? (cadr nf))
			  ...
			  (let ((new-form (make-new-form (unbegin (copy (cadr cdrf))) iter)))
			    (unless (tree-memq iter new-form) ; did we catch every reference to the iter?
			      (set! for-each-case #t)
			      (if (eq? '_1_ (caddr new-form))
				  (lint-format "perhaps ~A" name (lists->string initial-value `(copy ,sequence)))
				  (begin
				    (if (and (pair? (cddr new-form))
					     (null? (cdddr new-form))
					     (pair? (caddr new-form))
					     (pair? (cdaddr new-form))
					     (null? (cddr (caddr new-form)))
			...
			(and (pair? lst)
		      ...
		      (when (and (case (car f)
				 ...
				 (or (equal? (car nf) (list 'pair? (caar iters)))
				     (equal? (car nf) `(not (null? ,(caar iters))))))
			(let ((new-form (make-new-form (case (car f) 
							 ((if)
							  (unbegin (copy (cadr nf) 
									 (make-list (- (length (cadr nf)) 1)))))
							 ((when)
							  (if (null? (cddr nf))
							      (list (copy (cadr nf)
									  (make-list (- (length (cadr nf)) 1))))
							      (copy (cdr nf) (make-list (- (length (cdr nf)) 1))))))
						       (caar iters))))
	    ...
	    (when (and (not for-each-case)
			 ...
			 (case (car p) 
			   ((if) #t)
			   ((cond)
			    (and (= (length p) 3) ; (cond ((a b)) (else c)) -> (if a b c)
				 (not (tree-memq '=> p))
				 (eq? 'else (caaddr p))
				 (set! body `((if ,(caadr p) 
						  ...
						  (begin ,@(cdaddr p)))))))
			   ((when)
			    (set! body `((if ,(cadr p) (begin ,@(cddr p)) (begin)))))
			   ((unless)
			    (set! body `((if (not ,(cadr p)) (begin ,@(cddr p)) (begin)))))
			   ((case)
			    (and (= (length p) 4)
				 (not (tree-memq '=> p))
				 (eq? 'else (car (cadddr p)))
				 (set! body `((if (memv ,(cadr p) ',(car (caddr p))) (begin ,@(cdr (caddr p))) (begin ,@(cdr (cadddr p))))))))
			   (else #f))))

	      (let ((f (cdar body)))
		(let ((test (car f))
		      (true (cadr f))
		      (false (if (pair? (cddr f)) (caddr f) (list 'begin)))) ; assume end is ,@(unbegin...)
		  (when false
		    (let ((end-test test)
			  (result true)
			  (do-body false))
	...
	(hash-special 'nth
			...
			(if (not (var-member head env))
				...
				(if (integer? (cadr form))
				    (lint-format "perhaps ~A" caller (lists->string form `(list-ref ,@(reverse (cdr form))))))))))
		 ...
		 (unless (memq vname '(:lambda :dilambda))
		   ...
		   (let ((init (var-initial-value local-var))) ; look for various open-output-string problems
		     (when (and (pair? init)
				...
				(pair? (cadr outer-form))
				(null? (cdadr outer-form)))
		   ...
		   (if (zero? (var-ref local-var))
			 ...
			 (when (and (not (memq (var-definer local-var) '(parameter named-let named-let*)))
			   ...
			   (let ((vtype (or arg-type                ; this can't be #f unless no sets so despite appearances there's no contention here
			     ...
			     (when (and (> (var-ref local-var) 2) ; was 3
					(zero? (var-set local-var))
			       ...
			       (let ((pars (map list (proper-list (var-arglist local-var))))
				     (recursive (tree-memq vname (cddr (var-initial-value local-var)))))
				 ...
				 (let ((init (cddr (var-initial-value local-var)))
				   ...
				   (for-each (lambda (p)
					       (if (and (pair? (cdr p))
							...
							(if recursive
							    (or (code-constant? (cadr p))      ; repeated recursive call arg is not interesting
								(and (memq (cadr p) arglist)
								     (not (shadowed? (cadr p) init))
								     (not (let search ((tree init))
									    (and (pair? tree)
										 (or (and (eq? (car tree) 'set!)
											  (pair? (cdr tree))
											  (eq? (cadr p) (cadr tree)))
...
;;; spot check for recur->map via 2 args, maybe recur->for-each via 2 args? check cond etc here also else+#t elsewhere
