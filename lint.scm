			     ...
			     (let ((selector #f))             ; (or (and (eq?...)...)....) -> (case ....)
			       (when (every? (lambda (p)
					       (and (pair? p)
						    ...
						    (let ((expr (cadr p)))
						      (case (car expr)
							((null? boolean? eof-object? zero?) 
							 (equal? selector (cadr expr)))
...
;;; also (or (and x y z) (and y z) (and x z)) -> if or something
;;; do we have (or (and x y) (and (not x) (not y))) -> (eq? (not x) (not y))?
