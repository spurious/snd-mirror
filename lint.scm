...
(define *report-repeated-long-forms* #f)                  ; report long statements that are repeated a lot
...
(define lint
  ...
  (let ((no-side-effect-functions 
    ...
    (define* (make-var name (type :none) value definer)
      ;(format *stderr* "make ~A: ~A ~A ~A~%" name value (->type value) definer)
      (cons name (inlet 'value value 
			'type (if (eq? type :none)
				  (->type value)
				  type)
			'definer definer 
			'set #f 
    ...
    (define* (make-fvar name ftype arglist decl value env)
      (cons name 
	    (inlet 'type 'closure
		   ...
		   'env env
		   'value value
		   'values (count-values (cddr value))
		   'decl decl
		   'arglist arglist
		   'ftype ftype
		   'set #f 
...
;    (define calls 0)
    ...
    (define (set-set? name new-val env)
      ...
      (let ((data (or (var-member name env) 
		      (hash-table-ref globals name))))
	(when (var? data)
	  (let ((ntype (->type new-val)))
	    ;(format *stderr* "~A~%" data)
	    (if (not (var-set data))
		(set! (var-set data) (list (var-type data))))
	    (if (and (symbol? ntype)
		     (not (memq ntype (var-set data))))
		(set! (var-set data) (cons ntype (var-set data))))
	    (set! (var-value data) new-val)
	    (set! (var-type data) ntype)
	    (set! (var-signature data) #f)
	    ;(format *stderr* "  (set! ~A ~A) -> ~A: ~A~%" name new-val ntype data)
				    ...
				    (or (and (= (length b) 2)
					     ...
					     (case (car a)
					       ((complex?)  (memq (car b) '(number? real? rational? integer? even? odd? 
									    positive? negative? zero? exact? inexact? infinite? nan?)))
					       ((number?)   (memq (car b) '(complex? real? rational? integer? even? odd? 
									    positive? negative? zero? exact? inexact? infinite? nan?)))
					       ((real?)     (memq (car b) '(rational? integer? even? odd? positive? negative? inexact? exact?)))
					       ((rational?) (memq (car b) '(integer? even? odd?)))
					       ((integer?)  (memq (car b) '(even? odd?)))
					       ((exact?)    (memq (car b) '(integer? rational? even? odd?)))
					       (else #f)))
					(and (pair? (cdr b))
					     (pair? (cddr b))
					     (member (cadr a) (cdr b))
	...
	(define (bad-arg-match a b)
	  ...
	  (define (number-op? x) (memq x '(= < > <= >= even? odd? positive? negative? zero? exact? inexact?)))
	...
	(and (symbol? type1)
	     (symbol? type2)
	     (memq type1 bools)
	     (or (memq type2 bools)       ; return #f if not (obviously) redundant, else return which of the two to keep
...
#|
				   (if (and (pair? arg1)
					    ...
					    (pair? (cdr arg1))
					    (pair? (cdr arg2))
					    (or (and (not (eq? (car arg1) 'not))
						     (eq? (return-type (car arg1) env) 'boolean?))
						(and (eq? (car arg1) 'not)
						     (pair? (cadr arg1))
						     (eq? (return-type (caadr arg1) env) 'boolean?)))
					    (or (and (not (eq? (car arg2) 'not))
					    ...
					    (or (eq? (car arg1) 'not)
						(eq? (car arg2) 'not))
					    (and (equal? (if (eq? (car arg1) 'not) (cadadr arg1) (cadr arg1))
							 (if (eq? (car arg2) 'not) (cadadr arg2) (cadr arg2)))))
				       (format *stderr* "~A~%" form))
				   ;; (or (not (list? term)) (null? term)) -- (not pair?)??
				   ;; (or (not (pair? expr)) (null? expr)...) -- null? is redundant
				   ;; (or (eof-object? o) (not (list? o)) (< (length o) 2)) -- drop eof
				   ;;   (or (string? exp) (not (pair? exp)))
				   ;; (or (not (number? tempo)) (<= tempo 0)) -- should this recommend real?

			           ;; (or (not (or x y)) (not z)) -> (not (or x y z))??
			           ;; (or (not p) (not (integer? p)) (not (> p 0))) currently just changes the last
|#
			    ;; the other side: (and (or A B) (or (not A) C)) -> (if A C (and B #t)), but it never happens
			    (when (and (= len 3)    ; kinda dumb, but common: (or (and A B) (and (not A) C)) -> (if A B C)
		...
		((and)
		 ...
		 ;;          (and (real? x) (not (rational? x))) -> float? etc
		 (case len
		   ((1) #t)
		    ...
		    (let ((arg1 (cadr form)))
		      (and (not (contradictory? (cdr form)))
			   (let* ((arg2 (caddr form))
			     ...
			     (if t1
				 (cond ((eq? t1 :equal) `(equal? ,(cadr arg1) ,((if (equal? (cadr arg1) (cadr arg2)) caddr cadr) arg2)))
				       ((eq? t1 :eqv)   `(eqv? ,(cadr arg1) ,((if (equal? (cadr arg1) (cadr arg2)) caddr cadr) arg2)))
				       ((eq? t1 :eq)    `(eq? ,(cadr arg1) ,((if (equal? (cadr arg1) (cadr arg2)) caddr cadr) arg2)))
				       ((eq? t1 :memv)
...
#|
				   ...
				   ;; (and (not (eof-object? c)) (char=? c #\-)) -> eqv?
				   ;; (and (list? form) (not (null? form))) -> pair?
				   ;; (and (pair? (car data)) (not (list? (car data)))) -- is this proper-list?
				   ;; (and (list? second) (not (null? second)) ...)
				   ;; (and (pair? obj) (not (null? obj)) ...) or (and (not (null? alot)) (pair? alot))
				   ;; (and (not (pair? gen)) (symbol? gen) (capitol-first? gen)) -- drop not pair
				   ;; (and (number? x) (not (= x 0))) -> memv
				   ;; (and (real? n) (not (exact? n))) -> float?
|#				   
				 (let ((new-form ()))
				   (if (and (= len 3)
					    (not (side-effect? arg1 env))
					   ...
					   (do ((exprs (cdr form) (cdr exprs)))
						...
						(or (null? new-form)      ; (and) -> #t
						    (let* ((nform (reverse new-form)) 
						      ...
						      (if (null? newer-form)
							  (car new-form)
							  (if (and (eq? (car new-form) #t) ; trailing #t is dumb if next-to-last is boolean func
								   (pair? (cdr new-form))
								   (pair? (cadr new-form))
    ...
    (define (check-boolean-affinity name form env)
	  ...
	  (let ((expr (simplify-boolean form () () env)))
		...
		(let ((rt (if (eq? (caadr form) 'quote)
		      ...
		      (head (car form)))
		  (if (subsumes? head rt)
		      (lint-format "~A is always #t" (truncated-list->string name) (truncated-list->string form))
		      (if (and (not (memq rt '(#t #f values)))
			       (not (any-compatible? head rt)))
    ...
    (define (check-special-cases name head form env)
      ...
      ;; how to trap funny numbers using dfsl exponents?
      ;;
      ;; does (let ((x x)) ... no set!/define etc ...) ever do any good?  if string? has been subverted, why go on?
      ;;    maybe look at these cases
      ;; see and/or/not for more
      ;; and-redundant could also accept funcs with correct return-type (as well as constants)
      ;; (not (not x)) should suggest (and x #t) and list? should mention proper-list?
      ;;
      ;; 266/45
	 ...
	 (if (= (length form) 4)
	     (let ((target (cadr form))
	       ...
	       (cond ((and (pair? val)                 ; (vector-set! x 0 (vector-ref x 0))
		     ...
		     ;; to do these right, we should save these statements in the var data,
		     ;;   then in report-usage, when we've seen every setting of the var, 
		     ;;   look for errors.  At this point, we're guessing as to order of
		     ;;   events at runtime.
		     ...
		     ((symbol? target)
		      (let ((v (or (var-member target env)
				   (hash-table-ref globals target))))
			(if (and (var? v)
				 (not (eq? (var-definer v) 'parameter))
				 (not (var-set v)))

			    (if (or (string? (var-value v))
				    (vector? (var-value v))
				    (and (pair? (var-value v))
					 (eq? (car (var-value v)) 'quote)
					 (pair? (cadr (var-value v)))))
				(lint-format "~A is a literal constant; this set! is a bad idea: ~A" name 
					     (cadr form) (truncated-list->string form))

				(if (and (not (memq (var-type v) '(boolean? symbol? null?)))
					 (not (compatible? (var-type v)
						      (cdr (assq (car form) '((vector-set!       . vector?)
									      (list-set!         . pair?)
									      (hash-table-set!   . hash-table?)
									      (string-set!       . string?)
									      (byte-vector-set!  . byte-vector?)
									      (let-set!          . let?)
									      (float-vector-set! . float-vector?)
									      (int-vector-set!   . int-vector?)))))))
				    (lint-format "~A: ~A is ~A, but setter is ~A" name (truncated-list->string form) 
					 target (var-type v)
					 (car form)))))))
		     ))))
	...
	((null eq eqv equal character?) ; (null (cdr...)) 
	 (if (and (not (var-member head env))
	...
	(let ((name? (string->symbol (string-append (symbol->string name) "?"))))
	  (hash-table-set! globals name? (make-fvar :name name?
						    :ftype 'define
						    :decl (dummy-func 'define `(define (,name? x) (let? x)) '(define (_ x) #f))
						    :value `(define (,name? x) (let? x))
						    :arglist (list 'x)
						    :env ())))))
    ...
    (define (lint-walk-body name head body env)
	  ...
	  (let ((prev-f #f)
		...
		(if (and *report-repeated-long-forms*
			 (> (tree-length12 f 0) 12))
		    (set! (form-counts f) (+ (or (form-counts f) 0) 1)))
			...
			(if (and (memq (car prev-f) '(set! define define* define-macro define-constant define-macro*
				 ...
				 (or (and (equal? (caddr prev-f) f)    ; (begin ... (set! x (...)) (...))
				     ...
				     (and (not (eq? (car prev-f) 'set!))
					  ...
					  (eq? f (caadr prev-f)))))
			    (lint-format "this could be omitted: ~A (~A returns its value)" name (truncated-list->string f) (car prev-f)))
	...
	(if (null? args)
		...
		(let ((arg-data (if (symbol? args)                            ; this is getting arg names to add to the environment
				    (list (make-var :name args :definer 'parameter :type #t))
				    (map
				     (lambda (arg)
				       (if (symbol? arg)
					   (if (memq arg '(:rest :allow-other-keys))
					       (values)                  ; omit :rest and :allow-other-keys
					       (make-var :name arg :definer 'parameter :type #t))
					   (if (or (not (pair? arg))
						   (not (= (length arg) 2))
						 ...
						 (make-var :name (car arg) :definer 'parameter :type #t)))))
				     (proper-list args)))))
			     ...
			     (begin
			       ...
			       (cond ((keyword? sym)               ; (define :x 1)
					...
					(and (var? v)
					     ...
					     (not (equal? (caddr form) (var-value v)))))
				      (lint-format "~A in ~A is already a constant, defined ~A~A" name sym
						   (truncated-list->string form)
						   (if (and (pair? (var-value v))
							    (positive? (pair-line-number (var-value v))))
						       (format #f "(line ~D): " (pair-line-number (var-value v)))
						       "")
						   (truncated-list->string (var-value v)))))
				   ...
				   (let ((e (lint-walk sym (car val) env)))
					 ...
					 (cons (make-var :name sym :value (car val) :definer head) env)))
				   (cons (make-var :name sym :value val :definer head) env)))
			     ...
			     (let ((new-clauses ())
				   ...
				   ((null? clauses)
				    (let ((len (length new-clauses)))
				      ...
				      (if (or (not (= len 2)) ; i.e. don't go to check-bool-cond
			   ...
			   (when (and (pair? varlist)        ; (let ((x (A))) (if x (f x) B)) -> (cond ((A) => f) (else B)
				 ...
				 (if (= (tree-count2 (car var) p 0) 2)
					 ...
					 ((or (null? bp)
						...
						(and (pair? b)
						     (eq? (car b) 'if)
						     (= (tree-count2 (car var) b 0) 2)
						     (eq? (car var) (cadr b))
						     (pair? (caddr b))
						     (pair? (cdaddr b))
						     (null? (cddr (caddr b)))
						     (eq? (car var) (cadr (caddr b))))))
					  (if (pair? bp)
					      (let ((else-clause (if (pair? (cdddar bp)) `((else ,@(cdddar bp))) ())))
						(lint-format "perhaps ~A" name
							     (lists->string form `(,@(copy p (make-list (+ i 1)))
										   (cond (,(cadr var) => ,(caaddr (car bp))) ,@else-clause)
										 ,@(cdr bp))))))))))
				 ...
				 (when (and (eq? (car p) 'cond) ; (let ((x (f y))) (cond (x (g x)) ...)) -> (cond ((f y) => g) ...)
					    ...
					    (or (and (pair? (cadadr p))
						     ...
						     (eq? (car var) (cadr (cadadr p))))
						(eq? (car var) (cadadr p)))
					    (or (null? (cddr p))
						(not (tree-member (car var) (cddr p)))))
				   (if (eq? (car var) (cadadr p))
				       (if (and (pair? (cddr p))
						(pair? (caddr p))
						(memq (caaddr p) '(else #t t)))
					   (lint-format "perhaps ~A" name 
							(lists->string form (if (null? (cddr (caddr p)))
										`(or ,(cadr var) ,(cadr (caddr p)))
										`(or ,(cadr var) (begin ,@(cdaddr p))))))
					   (lint-format "perhaps ~A" name (lists->string form `(or ,(cadr var) 
												   (cond ,@(cddr p))))))
				       (lint-format "perhaps ~A" name (lists->string form `(cond (,(cadr var) => ,(caadr (cadr p))) 
												 ,@(cddr p))))))
				 ...
				 (when (pair? (cddr p))
				   (when (and (pair? (cdddr p))
				     ...
				     (when (and (eq? (cadr p) (car var)) ; (let ((x (g y))) (if x #t #f)) -> (g y)
						(boolean? (caddr p))
						(boolean? (cadddr p))
						(not (eq? (caddr p) (cadddr p))))
				       (lint-format "perhaps ~A" name
						    (lists->string form (if (caddr p) (cadr var) `(not ,(cadr var))))))
				   ...
				   (let ((crf #f))
				     ...
				     (when (and (or (and (memq (car p) '(if and)) ; (let ((x (f y))) (and x (g x))) -> (cond ((f y) => g) (else #f))
						    ...
						    (and (pair? (cadr var))
							 (memq (caadr var) '(assoc assv assq member memv memq))
							 (pair? (cadr p))
							 (or (eq? (caadr p) 'pair?)
							     (and (eq? (caadr p) 'null?)
								  (lint-format "in ~A, ~A can't be null because ~A in ~A only returns #f or a pair" 
									       name p (car var) (caadr var) (truncated-list->string var))
								  #f))
							 (eq? (cadadr p) (car var))))
						(or (and (pair? (caddr p))
							 (pair? (cdaddr p))
							 (null? (cddr (caddr p))) ; one func arg
							 (or (eq? (car var) (cadr (caddr p)))
						    ...
						    (and (eq? (car p) 'if)
							 (eq? (caddr p) (car var))
							 (not (tree-member (car var) (cdddr p)))
							 (lint-format "perhaps ~A" name 
								      (lists->string form 
										     (if (null? (cdddr p))
											 (cadr var)
											 `(or ,(cadr var) ,(cadddr p)))))
							 #f))
						(pair? (caddr p))
						(or (eq? (car p) 'if)
						    (null? (cdddr p))))
					 ...
					 (unless (eq? else-clause :oops!)
					   (lint-format "perhaps ~A" name 
							(lists->string form `(cond (,(cadr var) => ,(or crf (caaddr p))) ,@else-clause))))))))
				 )))
		   ...
		   (if (< (length form) 3)
			 ...
			 (do ((bindings (cadr form) (cdr bindings)))
			   ...
			   (when (binding-ok? name head (car bindings) env #f)
			     (set! vars (cons (make-var :name (caar bindings) 
							:value (if (and (eq? (caar bindings) (cadar bindings))
									(or (eq? head 'letrec)
...
|#
	...
	(if *report-repeated-long-forms* (set! form-counts (make-hash-table)))
	...
	(let ((fp (if (input-port? file)
	  ...
	  (if (input-port? fp)
		...
		(do ((form (read fp) (read fp)))
		  ...
		  (if (and *report-repeated-long-forms*
			   (> (tree-length12 form 0) 12))
		      (set! (form-counts form) (+ (or (form-counts form) 0) 1)))
		...
		(if *report-repeated-long-forms*
		    (for-each
		     (lambda (f)
		       (if (> (cdr f) 3)
			   (format outport "~A occurs ~A times~%" (car f) (cdr f))))
		     form-counts))
