			...
			((if)
			 (and (= len 4)
			      (let ((a1 (signer (caddr endb) env))
				    (a2 (signer (cadddr endb) env)))
				(and (equal? a1 a2) a1))))
			   
			   ((let let* letrec letrec* unless when)
			    (and (> len 2)
				 (signer (list-ref endb (- len 1)) env)))
	  ...
	  (or (and (not (hash-table-ref no-side-effect-functions (car form))) ; includes quote, let, etc
		...
		((let let* letrec letrec*)
		 ...
		 (or (< (length form) 3)
			 ...
			 (set! syms (caddr form))
			 (let ((fv (make-var (cadr form) #f 'let)))
    ...
    (define (binding-ok? caller head binding env second-pass)
      ...
      (cond (second-pass
	    ...
	    ((and *report-shadowed-variables*      ; (let ((x 1)) (+ (let ((x 2)) (+ x 1)) x))
	     ...
	     #t)
	    ((and (len=1? (cadr binding))
		  (eq? (caadr binding) 'list)
		  (not (var-member 'list env)))
	     (lint-format "the binding ~A could be ~A; there is only one nil" caller
			  binding (cons (car binding) (cons () (cddr binding))))
	     #t)))
		   ...
		   (case len1
			...
			(cond ((or (any-null? arg2)           
			      ...
			      ((and (eq? (car arg1) #_{list})
			       ...
			       (sp-append caller 'append (list 'append (un_{list} arg1) arg2) env))
				   ...
				   (if (memq true-op '(let let*))
				       ...
				       (let ((nform (cond ((eq? true-op (caadr diff))  ; very common!
							  ...
							  ((and (not (caadr diff))
							   ...
							   (tree-subst-eq (simplify-boolean (list 'not expr) () () env)
									  subst-loc true))
			 ...
			 (do ((iff form (cadddr iff))
		       ...
		       (cond ((side-effect? test env))
			     ...
			     ((or (equal? test true)               ; (if x x y) -> (or x y)
			      ...
			      (lint-format "perhaps ~A" caller 
					   (lists->string form 
							  (simplify-boolean (list 'or expr (if (eq? false 'no-false) #<unspecified> false))
									    () () env))))
			     ...
			     ((or (equal? test false)              ; (if x y x) -> (and x y)
				  (equal? expr false))
			      (lint-format "perhaps ~A" caller 
					   (lists->string form (simplify-boolean (list 'and expr true) () () env))))
...
#|
					       ...
					       (and (memq (caadr true) '(and or not))
						    (any? (lambda (p)
								...
								(and (pair? test)
								     (member p (cdr test)))))
							  (cdadr true)))))))
			     (format *stderr* "true: ~A~%~%" (lint-pp form)))
					       ...
					       (and (memq (caadr false) '(and or not))
						    (any? (lambda (p)
								...
								(and (pair? test)
								     (member p (cdr test)))))
							  (cdadr false)))))))
			     (format *stderr* "false: ~A~%~%" (lint-pp form)))
			 ...
			 (if (and (symbol? test)
			     ...
			     (format *stderr* "symbol: ~A~%~%" (lint-pp form))))
|#		       )))
		...
		(let ((sel-type #t)
		  ...
		  (if (every? (lambda (c)               ; (case x ((a) a) ((b) b)) -> (symbol->value x)
				(and (len=2? c)
			      ...
			      (cddr form))
		      (lint-format "perhaps (ignoring the unmatched case) ~A" caller (lists->string form (list 'symbol->value (cadr form)))))
		     ...
		     (lambda (clause)
			   ...
			   (let ((keys (car clause))
				 ...
				 (if (and (pair? keys)
				     ...
				     ;; (case x ((0 1) (abs x)))
				     ;; else clause is possible here, so don't move (pair? keys) outward
				     (lint-format "perhaps use => here: ~A" caller 
			 ...
			 (let ((p (car body))
			   ...
			   (when (and (null? (cddr p))         ; (let ((x (+ y 1))) (abs x)) -> (abs (+ y 1))
			     ...
			     (let ((v (var-member (car p) env)))
			       (if (or (and (var? v)
				       ...
				       (hash-table-ref built-in-functions (car p)))
				   (lint-format "perhaps ~A" caller (lists->string form (list (car p) vvalue)))
				   (if (not (or (any-macro? vname env)
				       ...
				       (lint-format "perhaps, assuming ~A is not a macro, ~A" caller (car p)
						    (lists->string form (list (car p) vvalue)))))))
			   ...
			   (when (pair? (cddr p))
				 ...
				 (when (and (eq? (cadr p) vname) ; (let ((x (g y))) (if x #t #f)) -> (g y)
				   ...
				   (lint-format "perhaps ~A" caller
						(lists->string form (if if-true vvalue (list 'not vvalue)))))
			     ...
			     (lint-format "perhaps split this let*: ~A" caller
					  (lists->string form
							 `(,(if (pair? (cdr outer-vars)) 'let* 'let)
							   ,(map (lambda (v)
								   (list (var-name v) (var-initial-value v)))
								 (reverse outer-vars))
							   (let ,(map (lambda (v)
									(list (var-name v) (var-initial-value v)))
								      (reverse inner-vars))
...
;;;   most common: (if X (if (car X)...)) but this is assoc related sometimes [see 13807 above]
...
;;; cond else [or internal] as if/cond + simple result split as in 5559? [case + if -> split?]
;;; this result->split->relint process should work anywhere
...
;;;   then see if ordering can simplify, or some sequence can be omitted (no side-effects, not use anywhere else)
;;;   non-intersecting seqs: possible dead code
;;; (inlet 'pi 1)
;;; other nils: (append) "" #() as singletons? (list) in set!, define, etc
;;; nonce-arg in report-usage?
;;;
;;; 174 28539 728909
