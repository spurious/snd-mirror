	   ...
	   (if (= (length form) 4)
		 ...
		 (if (symbol? func)
		     (let ((sig (procedure-signature (symbol->value func))))
			   ...
			   (lint-format "~A is a questionable member function" name func)))
		     (if (and (pair? func)
			      (= (length func) 3)
			      (eq? (car func) 'lambda)
			      (pair? (cadr func))
	   ...
	   (let ((len (length form)))
		 ...
		 (if (< len 3)
		     ...
		     (let ((test (cadr form))
			   ...
			   (if (and (never-true test)
				    (not (eq? true #f))) ; complain about (if #f #f) later
			   ...
			   (let ((expr (simplify-boolean test () () env)))
				 ...
				 (if (not expr)
					 ...
					 (if true                             ; (if #f x) as a kludgey #<unspecified>
					     (lint-format "possible simplification:~A" name (lists->string form #<unspecified>)))
...
|#	  
	  ...
	  ((vector-ref list-ref hash-table-ref let-ref)
	   (when (not (= line-number last-simplify-boolean-line-number))
	     (if (= (length form) 3)
		 (let ((seq (cadr form)))
		   (if (and (pair? seq)
			 ...
			 (if (and (pair? seq1)
				  (eq? (car seq1) head)
				  (= (length seq1) 3))
			     (lint-format "possible simplification:~A" name (lists->string form `(,(cadr seq1) ,(caddr seq1) ,(caddr seq) ,(caddr form))))
			     (lint-format "possible simplification:~A" name (lists->string form `(,seq1 ,(caddr seq) ,(caddr form)))))))))
	     (set! last-simplify-boolean-line-number line-number)))
			 ...
			 (lambda (done)
			   (for-each
			    (lambda (d)
			      ...
			      (set! dctr (+ dctr 1))
			      (case (car d)
				((display write-string) ; TODO: write-string has start/end args (use substring?), perhaps write-byte?
				 (if (string? (cadr d))
			    ...
			    dpy-f))))
		      (set! dpy-start #f))
		    (when (not dpy-case) (set! dpy-start #f)))
...
#|
				...
				; lots that reject cases: (cond ((eof-object? o)) ((not seen-at?)) ((not (list? o)))...)
|#
				...
				(if (not (pair? clause))
					...
					(if (and (pair? (cdr clause))
					  ...
					  (set! result :unequal)))))
			      (cdr form))
			     (if (and has-else (pair? result)) ; pair? result -> all result clauses are the same
				 (if (null? (cdr result))
