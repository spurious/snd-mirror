...
(define (make-hidden-controls-dialog)
  ...
  (if (not hidden-controls-dialog)
	...
	(g_signal_connect_closure_by_id (GPOINTER hidden-controls-dialog)
					...
					0 (g_cclosure_new (lambda (w ev data) (gtk_widget_hide hidden-controls-dialog) #t) #f #f) #f)
...
(define (make-variables-dialog)
  (let ((dismiss-button (gtk_button_new_with_label "Dismiss")))
    ...
    (g_signal_connect_closure_by_id (GPOINTER variables-dialog)
				    (g_signal_lookup "delete_event" (G_OBJECT_TYPE (GTK_OBJECT variables-dialog)))
				    0 (g_cclosure_new (lambda (w ev data) (gtk_widget_hide variables-dialog) #t) #f #f) #f)

    (gtk_box_pack_start (GTK_BOX (.action_area (GTK_DIALOG variables-dialog))) dismiss-button #t #t 20)
    (g_signal_connect_closure_by_id (GPOINTER dismiss-button)
				    (g_signal_lookup "clicked" (G_OBJECT_TYPE (GTK_OBJECT dismiss-button)))
				    0 (g_cclosure_new (lambda (w data) (gtk_widget_hide variables-dialog)) #f #f) #f)
    (gtk_widget_show dismiss-button)
  ...
  (let ((page-info (assoc page-name variables-pages)))
	...
	((meter)
	 ...
	 (let* ((height 70)
	   ...
	   (make-level-meter pane width height '() #f)))
	((graph)
...
(define (make-font-selector-dialog)
  (if (not font-selector-dialog)
	...
	(g_signal_connect_closure_by_id (GPOINTER font-selector-dialog)
					...
					0 (g_cclosure_new (lambda (w ev data) (gtk_widget_hide font-selector-dialog) #t) #f #f) #f)
...
(define (make-color-selector-dialog)
  (if (not color-selector-dialog)
	...
	(g_signal_connect_closure_by_id (GPOINTER color-selector-dialog)
					...
					0 (g_cclosure_new (lambda (w ev data) (gtk_widget_hide color-selector-dialog) #t) #f #f) #f)
