...
(if (or full-test (= snd-test 5))
	...
	(if (> (abs (- samp 1000)) 1) (snd-print (format #f ";left-sample: ~A?" samp))))
      (let ((eds (edits)))
	(if (or (not (= (vector-ref eds 0) 0)) (not (= (vector-ref eds 1) 0)))
	    (snd-print (format #f ";edits: ~A?" eds)))
	(if (not (= (edit-position) (vector-ref eds 0)))
...
(if (or full-test (= snd-test 8))
	...
	(undo) (w)
	(let ((eds (edits)))
	  (if (or (not (= (vector-ref eds 0) 1)) (not (= (vector-ref eds 1) 1)))
	...
	(play-and-wait 0)
	(let ((eds (edits)))
	  (if (or (not (= (vector-ref eds 0) 6)) (not (= (vector-ref eds 1) 0)))
...
(if (or full-test (= snd-test 9))
	...
	(let ((eds (edits (list mix-id))))
	  ...
	  (if (not (= (edit-position (list mix-id)) (vector-ref eds 0)))
...
(if (or full-test (= snd-test 14))
	...
	(let* ((len (length open-files))
	  ...
	  (let ((choose-fd (lambda () (w) (list-ref open-files (my-random open-ctr)))))
		...
		(if (number? id)
		    (let* ((cl (mark-sample id curfd))
			   (new-marks (marks curfd)))
			...
			(if (not (= id new-id)) (snd-print (format #f ";find-mark (by sample): ~A ~A?" id new-id))))
		      (set-mark-name id "hiho" curfd)
		      (let ((new-id (find-mark "hiho" curfd)))
			(if (not (= id new-id)) (snd-print (format #f ";find-mark (by name): ~A ~A?" id new-id))))
		      (if (not (string=? (mark-name id curfd) "hiho")) (snd-print (format #f ";mark name: ~A?" (mark-name id curfd))))
		      (set-mark-sample id (max 0 (- curloc 100)) curfd)
		      (set! cl (mark-sample id curfd))
		      (if (not (= cl (max 0 (- curloc 100)))) (snd-print (format #f ";set mark ~A /= ~A?" cl curloc)))
		      (delete-mark id curfd)))
