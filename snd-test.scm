...
(if (and (not (provided? 'snd-nogui))
    ...
    (begin
     ...
     (if (not (string=? (snd-help enved-base) "(enved-base) -> envelope editor exponential base value (1.0)"))
	 (snd-display (format #f ";snd-help enved-base: ~A?" (snd-help enved-base))))
     (if (not (string=? (snd-help x-axis-style) "(x-axis-style (snd #t) (chn #t)) -> labelling of time domain x axis (x-in-seconds)"))
	 (snd-display (format #f ";snd-help x-axis-style: ~A?" (snd-help x-axis-style))))
     (if (not (string=? (snd-help vu-font) "(vu-font) -> name of font used to make VU meter labels (courier)"))
     ...
     (let ((str1 (snd-help 'hamming-window))
       ...
       (if (or (not (string=? str1 str2))
	       (not (string=? str1 "A raised cosine")))
...
(if (or full-test (= snd-test 22) (and keep-going (<= snd-test 22)))
    ...
    (do ((clmtest 0 (1+ clmtest))) ((= clmtest tests)) (if (> tests 1) (snd-display (format #f ";test ~D " clmtest)))
	  ...
	  (let ((snd-return-key #xFF0D)
		...
		(key-event (widget-window name-button) (char->integer #\g) 4) (force-event)
	    ...
	    (enved-dialog)
	    (force-event)
	    ;; TODO: env list is 25 -- select-item -> enved-selected-env? 
	    ;; TODO: try the various apply button combinations
