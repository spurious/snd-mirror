...
(if (or full-test (= snd-test 0) (and keep-going (<= snd-test 0)))
	...
	'selection-context selection-context 2
	  ...
	  (snd-display (format #f ";ask-before-overwrite set def: ~A" (ask-before-overwrite))))
      (set! (audio-output-device) (audio-output-device))
      (if (not (equal? (audio-output-device)  0)) 
	  (snd-display (format #f ";audio-output-device set def: ~A" (audio-output-device))))
      (set! (audio-state-file) (audio-state-file))
      (if (not (equal? (audio-state-file)  ".snd-mixer" )) 
	  (snd-display (format #f ";audio-state-file set def: ~A" (audio-state-file))))
      (set! (auto-resize) (auto-resize))
      (if (not (equal? (auto-resize)  #t )) 
	  (snd-display (format #f ";auto-resize set def: ~A" (auto-resize))))
      (set! (auto-update) (auto-update))
      (if (not (equal? (auto-update)  #f)) 
	  (snd-display (format #f ";auto-update set def: ~A" (auto-update))))
      (set! (channel-style) (channel-style))
      (if (not (equal? (channel-style)  0 )) 
	  (snd-display (format #f ";channel-style set def: ~A" (channel-style))))
      (set! (color-cutoff) (color-cutoff))
      (if (fneq (color-cutoff)  0.003 )
	  (snd-display (format #f ";color-cutoff set def: ~A" (color-cutoff))))
      (set! (color-inverted) (color-inverted))
      (if (not (equal? (color-inverted)  #t)) 
	  (snd-display (format #f ";color-inverted set def: ~A" (color-inverted))))
...
(if (or full-test (= snd-test 4) (and keep-going (<= snd-test 4)))
    ...
    (let ((chns (mus-sound-chans "oboe.snd"))
      ...
      (if (and m1 (= clmtest 0)) (snd-display (format #f ";oboe: mus-sound-max-amp-exists before max-amp: ~A" m1)))
	...
	(if (not (equal? vc1 vc2)) (snd-display (format #f ";samples->sound-data->vct: ~A ~A" vc1 vc2)))
	(if (not (equal? sd1 sd2)) (snd-display (format #f ";sound-data->vct->sound-data: ~A ~A" sd1 sd2)))
	(scale-by 2.0 ind 0)
	...
	(if (not (equal? vc1 vc2)) (snd-display (format #f ";edpos samples->sound-data->vct: ~A ~A" vc1 vc2)))
	(if (not (equal? sd1 sd2)) (snd-display (format #f ";edpos sound-data->vct->sound-data: ~A ~A" sd1 sd2)))
	(set! sd1 (samples->sound-data 12000 10 ind 1))
	(set! vc1 (sound-data->vct sd1))
	(set! vc2 (samples->vct 12000 10 ind 1))
	(set! sd2 (vct->sound-data vc2))
	(if (not (equal? vc1 vc2)) (snd-display (format #f ";1 samples->sound-data->vct: ~A ~A" vc1 vc2)))
	(if (not (equal? sd1 sd2)) (snd-display (format #f ";1 sound-data->vct->sound-data: ~A ~A" sd1 sd2)))
	(scale-by 2.0 ind 1)
	(set! sd1 (samples->sound-data 12000 10 ind 1))
	(set! vc1 (sound-data->vct sd1))
	(set! vc2 (samples->vct 12000 10 ind 1))
	(set! sd2 (vct->sound-data vc2))
	(if (not (equal? vc1 vc2)) (snd-display (format #f ";1 scaled samples->sound-data->vct: ~A ~A" vc1 vc2)))
	(if (not (equal? sd1 sd2)) (snd-display (format #f ";1 scaled sound-data->vct->sound-data: ~A ~A" sd1 sd2)))
	(close-sound ind))
...
(define (test-edpos test-func func-name change-thunk ind1)
  (let ((fr1 (test-func ind1 0))
    ...
    (if (not (and (= fr1 fr2)
		  (= fr1 fr3)
		  (= fr1 fr4)))
	(snd-display (format #f ";initial ~A: ~A ~A ~A ~A?" func-name fr1 fr2 fr3 fr4)))
    (change-thunk)
    (let ((fr5 (test-func ind1 0))
      ...
      (if (not (and (= fr5 fr6)
		    (= fr5 fr7)
		    (= fr5 fr8)))
	  (snd-display (format #f ";~A (edpos 1): ~A ~A ~A ~A?" func-name fr5 fr6 fr7 fr8)))
      (set! fr5 (test-func ind1 0 0))
      (set! fr6 (test-func ind1 0 (lambda (snd chn) 0)))
      (if (not (and (= fr1 fr5)
		    (= fr1 fr6)))
	  (snd-display (format #f ";~A (edpos -1): ~A ~A ~A?" func-name fr1 fr5 fr6)))
      (undo 1 ind1 0)
      (set! fr5 (test-func ind1 0 1))
      (set! fr6 (test-func ind1 0 (lambda (snd chn) (+ (edit-position snd chn) 1))))
      (if (not (and (= fr8 fr5)
		    (= fr8 fr6)))
	  (snd-display (format #f ";~A (edpos +1): ~A ~A ~A?" func-name fr8 fr5 fr6)))))
  (revert-sound ind1))
...
(define (test-edpos-1 test-func func-name ind1)
  (let ((v0 (samples->vct 12000 10 ind1 0)))
    ...
    (let ((v1 (samples->vct 12000 10 ind1 0)))
      (if (vequal v0 v1)
	  (snd-display (format #f ";~A (0) no change! ~A ~A" func-name v0 v1)))
      (test-func ind1 0)
      (let ((v2 (samples->vct 12000 10 ind1 0)))
	(if (not (vequal v1 v2))
	    (snd-display (format #f ";~A (1) ~A ~A" func-name v1 v2)))
	(test-func ind1 (lambda (snd chn) 0))
	(set! v2 (samples->vct 12000 10 ind1 0))
	(if (not (vequal v1 v2))
	    (snd-display (format #f ";~A (2) ~A ~A" func-name v1 v2)))
	(let ((edp (edit-position ind1 0)))
	  ...
	  (if (not (vequal v1 v2))
	      (snd-display (format #f ";~A (3) ~A ~A" func-name v1 v2))))))
    (revert-sound ind1)))
...
(define (test-orig func0 func1 func-name ind1)
  (let ((v0 (samples->vct 12000 10 ind1 0)))
    ...
    (let ((v1 (samples->vct 12000 10 ind1 0)))
      (if (vfequal v0 v1)
	  (snd-display (format #f ";~A (orig: 0) no change! ~A ~A" func-name v0 v1)))
      (func1 ind1)
      (let ((v2 (samples->vct 12000 10 ind1 0)))
	(if (not (vfequal v0 v2))
	    (snd-display (format #f ";~A (orig: 1) ~A ~A" func-name v0 v2)))))
    (revert-sound ind1)))
...
(if (or full-test (= snd-test 5) (and keep-going (<= snd-test 5)))
	...
	(if (not (= (mus-sound-header-type "fmv5.snd") mus-next))
	    (snd-display (format #f ";save-selection type: ~A?" (mus-header-type-name (mus-sound-header-type "fmv5.snd")))))
	(if (not (= (mus-sound-data-format "fmv5.snd") mus-bint))
	    (snd-display (format #f ";save-selection format: ~A?" (mus-data-format-name (mus-sound-data-format "fmv5.snd")))))
	(if (not (= (mus-sound-srate "fmv5.snd") 22050))
	    (snd-display (format #f ";save-selection srate: ~A?" (mus-sound-srate "fmv5.snd"))))
	(vct-fill! v0 0.0)
	(vct-set! v0 100 .5)
	(vct-set! v0 2 -.5)
	(vct->samples 0 128 v0 index 0) 
	...
	(if (or (fneq (sample 27) 0.5) (fneq (sample 125) -.5))
	    (snd-display (format #f ";save reverse-selection: ~A?" v0)))
	(if (not (= (mus-sound-header-type "fmv4.snd") mus-riff))
	    (snd-display (format #f ";save-selection type 1: ~A?" (mus-header-type-name (mus-sound-header-type "fmv4.snd")))))
	(if (not (= (mus-sound-data-format "fmv4.snd") mus-lfloat))
	    (snd-display (format #f ";save-selection format 1: ~A?" (mus-data-format-name (mus-sound-data-format "fmv4.snd")))))
	(if (not (= (mus-sound-srate "fmv4.snd") 44100))
	    (snd-display (format #f ";save-selection srate 1: ~A?" (mus-sound-srate "fmv4.snd"))))
	(if (not (string=? (mus-sound-comment "fmv4.snd") "this is a comment"))
	    (snd-display (format #f ";save-selection comment: ~A?" (mus-sound-comment "fmv4.snd"))))
	...
	(let ((newamps (maxamp obind #t)))
	  ...
	  (if (not (provided? 'snd-nogui))
		...
		(if (not (= losamp (left-sample obind 0)))
		    (snd-display (format #f ";axis-info[0 losamp]: ~A ~A?" losamp (left-sample obind 0))))
		(if (not (= hisamp (right-sample obind 0)))
		    (snd-display (format #f ";axis-info[1 hisamp]: ~A ~A?" hisamp (right-sample obind 0))))
		(if (fneq (list-ref axinfo 6) 0.0)
		    (snd-display (format #f ";axis-info[6 xmin]: ~A?" (list-ref axinfo 6))))
		...
		(if (fneq (list-ref (axis-info obind 0) 9) 3.0)
		    (snd-display (format #f ";axis-info[9 ymax 3.0]: ~A?" (list-ref (axis-info obind 0) 9))))
	...
	(if (not (= (mus-sound-frames "fmv.snd") (frames ind1 0 0)))
	    (snd-display (format #f ";save-sound-as (edpos): ~A ~A?" (mus-sound-frames "fmv.snd") (frames ind1 0 0))))
	(if (not (= (mus-sound-frames "fmv1.snd") (frames ind1 0 1)))
	    (snd-display (format #f ";save-sound-as (edpos 1): ~A ~A?" (mus-sound-frames "fmv.snd") (frames ind1 0 1))))
	...
	(let ((ind2 (open-sound "fmv.snd"))
	  ...
	  (if (not (vequal (samples->vct 12000 10 ind1 0 #f 0) (samples->vct 12000 10 ind2 0)))
	      (snd-display (format #f ";save-sound-as (edpos 3): ~A ~A?" (samples->vct 12000 10 ind1 0 #f 0) (samples->vct 12000 10 ind2 0))))
	  (if (not (vequal (samples->vct 12000 10 ind1 0 #f 1) (samples->vct 12000 10 ind3 0)))
	      (snd-display (format #f ";save-sound-as (edpos 4): ~A ~A?" (samples->vct 12000 10 ind1 0 #f 1) (samples->vct 12000 10 ind3 0))))
	  (if (vequal (samples->vct 12000 10 ind2 0) (samples->vct 12000 10 ind3 0))
	      (snd-display (format #f ";save-sound-as (edpos 5): ~A ~A?" (samples->vct 12000 10 ind2 0) (samples->vct 12000 10 ind3 0))))
	  (close-sound ind2)
	  (close-sound ind3))
	(delete-file "fmv.snd")
...
(if (or full-test (= snd-test 8) (and keep-going (<= snd-test 8)))
	...
	(if (not (equal? (frame->frame mx4 fr4) (make-frame 4 1 1 1 1))) (snd-display (format #f ";frame->frame 4 4: ~A?"         (frame->frame mx4 fr4)))))
	 ...
	 (let ((m1 (make-mixer chans)))
	   (if (or (not (= (mus-channels m1) chans))
	       ...
	       (snd-display (format #f ";mixer ~A chans but: ~A ~A" chans (mus-channels m1) (mus-length m1))))
	   (do ((i 0 (1+ i)))
	       ((= i chans))
	     (do ((j 0 (1+ j)))
		 ((= j chans))
	   ...
	   (do ((i 0 (1+ i)))
		 ...
		 ((= j chans))
		   ...
		   (snd-display (format #f ";mixer[~A ~A] = ~A (~A)?" i j (mixer-ref m1 i j) (+ (* i .01) (* j .1)))))))
	   (let ((mempty (make-mixer chans))
		 ...
		 (do ((j 0 (1+ j)))
		     ((= j chans))
		   (if (fneq (mixer-ref mzero i j) 0.0) (snd-display (format #f ";mzero ~A ~A = ~A?" i j (mixer-ref mzero i j))))
		   (if (fneq (mixer-ref m1 i j) (mixer-ref msame i j)) (snd-display (format #f ";msame ~A ~A?" (mixer-ref msame i j) (mixer-ref m1 i j))))
		   (if (and (fneq (mixer-ref mone i j) 0.0)
			    (not (= i (1- chans)))
			    (not (= j (1- chans))))
		       (snd-display (format #f ";mone ~A ~A = ~A?" i j (mixer-ref mone i j))))))))))
       (list 1 2 4 8))
	...
	(if (or (fneq (frame-ref fr0 3) .333) (fneq (frame-ref fr0 0) .167)) (snd-display (format #f ";300 locsig quad output: ~A" fr0))))
	 ...
	 (let ((m1 (make-locsig :channels chans)))
	   (if (or (not (= (mus-channels m1) chans))
		   (not (= (mus-length m1) chans)))
	       (snd-display (format #f ";locsig ~A chans but: ~A ~A" chans (mus-channels m1) (mus-length m1))))
	   (do ((i 0 (1+ i)))
	       ((= i chans))
	     (locsig-set! m1 i (* i .1)))
	   (do ((i 0 (1+ i)))
		 ...
		 (snd-display (format #f ";locsig[~A] = ~A (~A)?" i (locsig-ref m1 i) (* i .1)))))))
       (list 1 2 4 8))
...
(if (or full-test (= snd-test 9) (and keep-going (<= snd-test 9)))
	...
	(let ((nid (forward-mix)))
	  (if (or (not (mix? nid))
	      ...
	      (snd-display (format #f ";forward-mix ~A ~A ~A?" nid (cursor) (and (mix? nid) (mix-position nid)))))
	  (let ((nid1 (forward-mix 2)))
	    (if (or (not (mix? nid1))
		    (= nid nid1)
		    (not (= (cursor) (mix-position nid1))))
		(snd-display (format #f ";forward-mix(2) ~A ~A ~A ~A?" nid nid1 (cursor) (and (mix? nid1) (mix-position nid1)))))
	    (set! nid1 (backward-mix))
	    (if (or (not (mix? nid1))
		    (not (= (cursor) (mix-position nid1))))
		(snd-display (format #f ";backward-mix(2) ~A ~A ~A?" nid1 (cursor) (and (mix? nid1) (mix-position nid1)))))))
	(let ((nid (find-mix 100)))
	  (if (or (not (mix? nid))
		  (not (= (mix-position nid) 100)))
	      (snd-display (format #f ";find-mix(100): ~A ~A?" nid (and (mix? nid) (mix-position nid))))))
	(let ((nid (find-mix 200)))
	  (if (or (not (mix? nid))
		  (not (= (mix-position nid) 200)))
	      (snd-display (format #f ";find-mix(200): ~A ~A?" nid (and (mix? nid) (mix-position nid))))))
...
(if (or full-test (= snd-test 10) (and keep-going (<= snd-test 10)))
	...
	(let ((fd (open-sound "oboe.snd"))
	  ...
	  (let ((m2 (without-errors (add-mark 12345 fd 0))))
		...
		(if (not (eq? m5 m6 #f)) (snd-display (format #f ";find-not-a-mark: ~A ~A?" m5 m6)))
    ...
    (begin
	 ...
	 (let ((held (help-dialog "Test" "snd-test here")))
	   ...
	   (let ((wids (dialog-widgets)))
	     (if (not (= envd (list-ref wids 2))) (snd-display (format #f ";dialog-widgets 2: ~A ~A?" (list-ref wids 2) envd)))
	     (if (not (= cold (list-ref wids 0))) (snd-display (format #f ";dialog-widgets 0: ~A ~A?" (list-ref wids 0) cold)))
	     (if (not (= ord (list-ref wids 1))) (snd-display (format #f ";dialog-widgets 1: ~A ~A?" (list-ref wids 1) ord)))
	     (if (not (= trd (list-ref wids 5))) (snd-display (format #f ";dialog-widgets 5: ~A ~A?" (list-ref wids 5) trd)))
	     (if (not (= fild (list-ref wids 8))) (snd-display (format #f ";dialog-widgets 8: ~A ~A?" (list-ref wids 8) fild)))
	     (if (and (> regd 0) (not (= regd (list-ref wids 19)))) (snd-display (format #f ";dialog-widgets 19: ~A ~A?" (list-ref wids 19) regd)))
	     (if (and (number? ehd) (not (= ehd (list-ref wids 12)))) (snd-display (format #f ";dialog-widgets 12: ~A ~A?" (list-ref wids 12) ehd)))
	     (if (not (= held (list-ref wids 14))) (snd-display (format #f ";dialog-widgets 14: ~A ~A?" (list-ref wids 14) held)))))))
	 ...
	 (snd-display (format #f ";snd-help enved-base: ~A?" (snd-help enved-base))))
	 ...
	 (snd-display (format #f ";snd-help x-axis-style: ~A?" (snd-help x-axis-style))))
     (if (not (string-equal-ignoring-white-space (snd-help vu-font) "(vu-font) -> name of font used to make VU meter labels (courier)"))
	 (snd-display (format #f ";snd-help vu-font: ~A?" (snd-help vu-font))))
     (if (not (string-equal-ignoring-white-space (snd-help 'enved-base) "(enved-base) -> envelope editor exponential base value (1.0)"))
	 (snd-display (format #f ";snd-help 'enved-base: ~A?" (snd-help 'enved-base))))
     (if (not (string-equal-ignoring-white-space (snd-help 'x-axis-style) "(x-axis-style (snd #t) (chn #t)) -> labelling of time domain x axis (x-in-seconds)"))
	 (snd-display (format #f ";snd-help 'x-axis-style: ~A?" (snd-help 'x-axis-style))))
	  ...
	  (let ((choose-fd (lambda () (list-ref open-files (my-random open-ctr)))))
		...
		(if (and (number? id) (not (= id -1)))
			...
			(if (or (not (mark? new-id))
				(not (= id new-id)))
			    (snd-display (format #f ";find-mark (by sample): ~A ~A (~A for ~A ~A)?" 
						 id new-id curloc (mark-sample id) (mark-sample new-id)))))
		      (set! (mark-name id) "hiho")
		      (let ((new-id (find-mark "hiho" curfd)))
			(if (or (not (mark? new-id))
				(not (= id new-id)))
			    (snd-display (format #f ";find-mark (by name): ~A ~A?" id new-id))))
	  ...
	  (let ((snd-return-key #xFF0D)
		...
		(let ((pos (cursor-position)))
		  (if (> (abs (- (car pos) 100)) 1)
		      (snd-display (format #f ";pos ~A: ~A?" 100 (car pos)))))
		(click-event cwin 1 0 300 (inexact->exact (/ (cadr size) 2))) (force-event)
		(let ((pos (cursor-position)))
		  (if (> (abs (- (car pos) 300)) 1)
		      (snd-display (format #f ";pos ~A: ~A?" 300 (car pos)))))
		
		(expose-event cwin 20 20 200 200)
		(resize-event cwin 500 500)
		(focus-widget (car (channel-widgets)))
		(let ((pos (cursor)))
		  (key-event cwin (char->integer #\f) 4) (force-event)
		  (if (>= pos (cursor))
		      (snd-display (format #f ";C-f: ~A ~A?" pos (cursor)))))
		
		(key-event cwin (char->integer #\<) 5) (force-event)
		(if (not (= (cursor) 0))
		    (snd-display (format #f ";C-<: ~A?" (cursor))))
		(key-event cwin (char->integer #\>) 9) (force-event)
		(if (not (= (cursor) (- (frames) 1)))
		    (snd-display (format #f ";C->: ~A (~A)?" (cursor) (frames))))
		
		(drag-event cwin 1 0 100 50 400 50) (force-event)
		(if (not (selection?))
		    ...
		    (let* ((pos (selection-position))
		      ...
		      (if (or (> (abs (- x0 100)) 1)
			      (> (abs (- x1 400)) 1))
			  (snd-display (format #f ";selectpos: ~A ~A ~A ~A " pos end x0 x1)))
		  
		      (key-event cwin (char->integer #\x) 4) (force-event)
		      (key-event cwin (char->integer #\v) 0) (force-event)
		      (if (or (> (abs (- pos (left-sample))) 2)
			      (> (abs (- end (right-sample))) 2))
			  (snd-display (format #f ";C-x v selectpos: ~A ~A ~A ~A " pos end (left-sample) (right-sample))))
		      ))
		(set! (cursor) 6000)
		(let ((ls (left-sample))
		      (rs (right-sample))
		      (len (- (right-sample) (left-sample))))
		  (key-event cwin snd-right-key 0) (force-event)
		  (if (> (abs (- (left-sample) rs)) 1)
		      (snd-display (format #f ";-> not by window? ~A ~A" rs (left-sample))))
		  (if (> (abs (- len (- (right-sample) (left-sample)))) 1)
		      (snd-display (format #f ";-> size diff? ~A ~A" len (- (right-sample) (left-sample)))))
		  (key-event cwin snd-left-key 0) (force-event)
		  (if (> (abs (- (left-sample) ls)) 1)
		      (snd-display (format #f ";<- not by window? ~A ~A" ls (left-sample))))
		  (if (> (abs (- len (- (right-sample) (left-sample)))) 1)
		      (snd-display (format #f ";<- size diff? ~A ~A" len (- (right-sample) (left-sample)))))
		  (key-event cwin snd-up-key 0) (force-event)
		  (if (> (abs (- (* 2 len) (- (right-sample) (left-sample)))) 3)
		      (snd-display (format #f ";up size diff? ~A ~A" ( * 2 len) (- (right-sample) (left-sample)))))
		  (key-event cwin snd-down-key 0) (force-event)
		  (if (> (abs (- len (- (right-sample) (left-sample)))) 3)
		      (snd-display (format #f ";down size diff? ~A ~A" len (- (right-sample) (left-sample)))))
		  (key-event cwin snd-up-key 4) (force-event)
		  (if (> (abs (- (* 1.5 len) (- (right-sample) (left-sample)))) 2)
		      (snd-display (format #f ";C-up size diff? ~A ~A" ( * 1.5 len) (- (right-sample) (left-sample)))))
		  (key-event cwin snd-down-key 4) (force-event)
		  (if (> (abs (- len (- (right-sample) (left-sample)))) 2)
		      (snd-display (format #f ";C-down size diff? ~A ~A" len (- (right-sample) (left-sample)))))
		  ...
		  (key-event cwin (char->integer #\p) 0) (force-event)
		  (if (> (abs (- (cursor) (left-sample))) 1)
		      (snd-display (format #f ";cursor-on-left: ~A ~A" (cursor) (left-sample))))
		  (key-event cwin (char->integer #\r) 0) (force-event)
		  (if (> (abs (- (cursor) (right-sample))) 1)
		      (snd-display (format #f ";cursor-on-right: ~A ~A" (cursor) (right-sample))))
		  (key-event cwin (char->integer #\q) 0) (force-event)
		  (if (> (abs (- (cursor) (inexact->exact (* .5 (+ (left-sample) (right-sample)))))) 1)
		      (snd-display (format #f ";cursor-in middle: ~A ~A" (cursor) (inexact->exact (* .5 (+ (left-sample) (right-sample)))))))
		  (key-event cwin (char->integer #\p) 0) (force-event)
		  (if (> (abs (- (cursor) (left-sample))) 1)
		      (snd-display (format #f ";cursor-on-left (1): ~A ~A" (cursor) (left-sample))))
		    
		  (set! (graph-style) graph-dots)
		  (let ((ds (dot-size)))
		    (key-event cwin snd-kp-decimal-key 1) (force-event)
		    (if (not (= (dot-size) (1+ ds)))
			(snd-display (format #f ";decimal dot-size: ~A -> ~A?" ds (dot-size))))
		    (key-event cwin snd-kp-0-key 1) (force-event)
		    (if (not (= (dot-size) ds))
			(snd-display (format #f ";0 dot-size: ~A -> ~A?" ds (dot-size)))))
		  (set! (graph-style) graph-lines)

		  (set! (graph-transform?) #t)
		  (let ((ds (transform-size)))
		    (key-event cwin snd-kp-multiply-key 0) (force-event)
		    (if (not (= (transform-size) (* 2 ds)))
			(snd-display (format #f ";multiply transform-size: ~A -> ~A?" ds (transform-size))))
		    (key-event cwin snd-kp-divide-key 0) (force-event)
		    (if (not (= (transform-size) ds))
			(snd-display (format #f ";divide transform-size: ~A -> ~A?" ds (transform-size)))))
		  (set! (graph-transform?) #f)

		  (focus-widget (car (channel-widgets)))
		  (key-event cwin (char->integer #\a) 4) (force-event)
		  (if (not (= (cursor) (left-sample)))
		      (snd-display (format #f ";C-a: ~A?" (cursor) (left-sample))))
		  (key-event cwin (char->integer #\e) 4) (force-event)
		  (if (not (= (cursor) (right-sample)))
		      (snd-display (format #f ";C-e: ~A?" (cursor) (right-sample))))
		  (key-event cwin (char->integer #\b) 4) (force-event)
		  (if (not (= (cursor) (1- (right-sample))))
		      (snd-display (format #f ";C-b: ~A?" (cursor) (1- (right-sample)))))
		  (key-event cwin (char->integer #\f) 4) (force-event)
		  (if (not (= (cursor) (right-sample)))
		      (snd-display (format #f ";C-f: ~A?" (cursor) (right-sample))))
		  (if (not (selection?))
		      (snd-display "what happened to our selection?"))
		  (key-event cwin snd-space-key 0) (force-event)
		  (if (selection?)
		      (snd-display "C-space did not cancel selection?"))
		  (set! (cursor) 100)
		  (key-event cwin (char->integer #\d) 4) (force-event)
		  (if (not (equal? (edits) '(1 0)))
		      (snd-display (format #f ";C-d (edits) -> ~A?" (edits))))
		  (if (not (equal? (edit-fragment 1) (list "C-d" "delete" 100 1)))
		      (snd-display (format #f ";C-d (edit) -> ~A?" (edit-fragment 1))))
		  (key-event cwin (char->integer #\u) 4) (force-event)
		  (key-event cwin (char->integer #\0) 4) (force-event)
		  (key-event cwin (char->integer #\d) 4) (force-event)
		  (if (not (equal? (edits) '(1 0)))
		      (snd-display (format #f ";0 C-d (edits) -> ~A?" (edits))))
		  (key-event cwin (char->integer #\h) 4) (force-event)
		  (if (not (equal? (edits) '(2 0)))
		      (snd-display (format #f ";C-h (edits) -> ~A?" (edits))))
		  (if (not (equal? (edit-fragment 2) (list "C-h" "delete" 99 1)))
		      (snd-display (format #f ";C-h (edit) -> ~A?" (edit-fragment 2))))
		  (set! (cursor) 5000)
		  (let ((fr (frames)))
		    (key-event cwin (char->integer #\o) 4) (force-event)
		    (if (not (equal? (edits) '(3 0)))
			(snd-display (format #f ";C-o (edits) -> ~A?" (edits))))
		    (if (not (equal? (edit-fragment 3) (list "C-o" "insert" 5000 1)))
			(snd-display (format #f ";C-o (edit) -> ~A?" (edit-fragment 3))))
		    (if (fneq (sample (cursor)) 0.0)
			(snd-display (format #f ";C-o sample: ~A?" (sample (cursor)))))
		    (if (not (= (frames) (1+ fr)))
			(snd-display (format #f ";frames after C-o: ~A ~A?" (frames) fr)))
		    (key-event cwin (char->integer #\v) 4) (force-event)
		    (if (> (abs (- (cursor) (inexact->exact (* .5 (+ (left-sample) (right-sample)))))) 1)
			(snd-display (format #f ";C-v loc: ~A ~A?" (cursor) (inexact->exact (* .5 (+ (left-sample) (right-sample)))))))
		    (key-event cwin (char->integer #\z) 4) (force-event)
		    (if (not (equal? (edits) '(4 0)))
			(snd-display (format #f ";C-z (edits) -> ~A?" (edits))))
		    (if (not (string=? (car (edit-fragment 4)) "C-z"))
			(snd-display (format #f ";C-z (edit) -> ~A?" (edit-fragment 4))))
		    (if (fneq (sample (cursor)) 0.0)
			(snd-display (format #f ";C-z sample: ~A?" (sample (cursor)))))
		    (if (not (= (frames) (1+ fr)))
			(snd-display (format #f ";frames after C-z: ~A ~A?" (frames) fr))))
		  (if (> (length (marks ind 0)) 0)
		      (snd-display "there are marks here already!"))
		  (key-event cwin (char->integer #\m) 4) (force-event)
		  (if (null? (marks ind 0))
		      (snd-display "C-m but no mark!")
		      (let ((m (car (marks ind 0))))
			(if (not (= (mark-sample m) (cursor)))
			    (snd-display (format #f ";C-m mark sample: ~A ~A?" (mark-sample m) (cursor))))))
		  (key-event cwin (char->integer #\-) 4) (force-event)
		  (key-event cwin (char->integer #\m) 4) (force-event)
		  (if (mark? (find-mark (cursor)))
		      (snd-display (format #f ";C-- C-m mark: ~A?" (find-mark (cursor)))))
		  (key-event cwin (char->integer #\x) 4) (force-event)		  
		  (key-event cwin (char->integer #\o) 4) (force-event)
		  (if (not (show-controls))
		      (snd-display (format #f "C-x C-o: ~A?" (show-controls))))
		  (key-event cwin (char->integer #\x) 4) (force-event)		  
		  (key-event cwin (char->integer #\c) 4) (force-event)
		  (if (show-controls)
		      (snd-display (format #f "C-x C-c: ~A?" (show-controls))))
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\u) 4) (force-event)
		  (if (not (equal? (edits) '(3 1)))
		      (snd-display (format #f ";C-x C-u (edits) -> ~A?" (edits))))
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\r) 4) (force-event)
		  (if (not (equal? (edits) '(4 0)))
		      (snd-display (format #f ";C-x C-r (edits) -> ~A?" (edits))))
		  (key-event cwin (char->integer #\u) 4) (force-event)
		  (key-event cwin (char->integer #\1) 0) (force-event)
		  (key-event cwin (char->integer #\2) 0) (force-event)
		  (key-event cwin (char->integer #\3) 0) (force-event)
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\b) 4) (force-event)
		  (if (not (= (left-sample) 123))
		      (snd-display (format #f ";C-u 123 C-x: ~A?" (left-sample))))
		  (key-event cwin (char->integer #\u) 4) (force-event)
		  (key-event cwin (char->integer #\5) 0) (force-event)
		  (key-event cwin (char->integer #\0) 0) (force-event)
		  (key-event cwin (char->integer #\0) 0) (force-event)
		  (key-event cwin (char->integer #\0) 0) (force-event)
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\b) 4) (force-event)
		  (if (not (= (left-sample) 5000))
		      (snd-display (format #f ";C-u 5000 C-x: ~A?" (left-sample))))
		  (set! (cursor) 123)
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\b) 0) (force-event)
		  (if (not (= (left-sample) 123))
		      (snd-display (format #f ";C-x b(123): ~A?" (left-sample))))
		  (key-event cwin (char->integer #\u) 4) (force-event)
		  (key-event cwin (char->integer #\1) 0) (force-event)
		  (key-event cwin (char->integer #\0) 0) (force-event)
		  (key-event cwin (char->integer #\f) 4) (force-event)	
		  (if (not (= (cursor) 133))
		      (snd-display (format #f ";C-u 10 C-f -> ~A?" (cursor))))
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\/) 0) (force-event)
		  (widget-string minibuffer "away!")
		  (key-event (widget-window minibuffer) snd-return-key 0) (force-event)
		  (set! (cursor) 0)
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\j) 0) (force-event)
		  (widget-string minibuffer "away!")
		  (key-event (widget-window minibuffer) snd-return-key 0) (force-event)
		  (if (not (= (cursor) 133))
		      (snd-display (format #f ";named mark? ~A ~A" (cursor) (find-mark "away!"))))
		  (let ((m (find-mark "away!")))
		    (if (or (not (mark? m))
			    (not (= (mark-sample m) 133)))
			(snd-display (format #f ";are we away? ~A" (if (mark? m) (mark-sample m) "lost")))))
		  (set! (cursor) 130)
		  (key-event cwin (char->integer #\j) 4) (force-event)
		  (if (not (= (cursor) 133))
		      (snd-display (format #f ";goto mark? ~A" (cursor))))
		  (if (key-binding (char->integer #\n) 4)
		      (unbind-key (char->integer #\n) 4))
		  (key-event cwin (char->integer #\n) 4) (force-event)
		  (if (not (= (cursor) (+ 128 133)))
		      (snd-display (format #f ";ahead line? ~A" (cursor))))
		  (if (key-binding (char->integer #\p) 4)
		      (unbind-key (char->integer #\p) 4))
		  (key-event cwin (char->integer #\p) 4) (force-event)
		  (if (not (= (cursor) 133))
		      (snd-display (format #f ";back line? ~A" (cursor))))
		  (let ((fr (frames)))
		    (if (key-binding (char->integer #\k) 4)
			(unbind-key (char->integer #\k) 4))
		    (key-event cwin (char->integer #\k) 4) (force-event)
		    (if (not (equal? (edits) '(5 0)))
			(snd-display (format #f ";C-k (edits) -> ~A?" (edits))))
		    (if (not (equal? (edit-fragment 5) (list "C-k" "delete" 133 128)))
			(snd-display (format #f ";C-k (edit) -> ~A?" (edit-fragment 5))))
		    (if (not (= (frames) (- fr 128)))
			(snd-display (format #f ";frames after C-k: ~A ~A?" (frames) fr))))
		  (key-event cwin (char->integer #\s) 4) (force-event)
		  (widget-string minibuffer "(lambda (n) (< n -.1))")
		  (key-event (widget-window minibuffer) snd-return-key 0) (force-event)
		  (let ((cs (cursor)))
		    (if (not (< (sample cs) -.1))
			(snd-display (format #f ";C-s -.1 -> ~A at ~A?" (sample cs) cs)))
		    (key-event cwin (char->integer #\s) 4) (force-event)
		    (key-event cwin (char->integer #\s) 4) (force-event)
		    (if (not (< (sample (cursor)) -.1))
			(snd-display (format #f ";C-s -.1 (1) -> ~A at ~A?" (sample (cursor)) (cursor))))
		    (if (= cs (cursor))
			(snd-display (format #f ";C-s 2 -> ~A at ~A?" cs (cursor))))
		    (key-event cwin (char->integer #\b) 4) (force-event)
		    (key-event cwin (char->integer #\r) 4) (force-event)
		    ...
		    (set! cs (cursor))
		    (if (not (>= (sample cs) .05))
			(snd-display (format #f ";C-r >= .05 -> ~A at ~A?" (sample cs) cs)))
		    (key-event cwin (char->integer #\r) 4) (force-event)
		    (key-event cwin (char->integer #\r) 4) (force-event)
		    (if (not (>= (sample (cursor)) .05))
			(snd-display (format #f ";C-r >= .05 (1) -> ~A at ~A?" (sample (cursor)) (cursor))))
		    (if (= cs (cursor))
			(snd-display (format #f ";C-r 2 -> ~A at ~A?" cs (cursor))))
		    (key-event cwin (char->integer #\>) 9) (force-event)
		    (key-event cwin (char->integer #\r) 4) (force-event)
		    (key-event cwin (char->integer #\r) 4) (force-event)
		    (set! cs (cursor))
		    (if (not (>= (sample cs) .05))
			(snd-display (format #f ";C-r back >= .05 -> ~A at ~A?" (sample cs) cs)))
		    (key-event cwin (char->integer #\>) 9) (force-event)
		    (key-event cwin (char->integer #\b) 4) (force-event)
		    (key-event cwin (char->integer #\r) 4) (force-event)
		    (key-event cwin (char->integer #\r) 4) (force-event)
		    (set! cs (cursor))
		    (if (not (>= (sample cs) .05))
			(snd-display (format #f ";C-r back 2 >= .05 -> ~A at ~A?" (sample cs) cs))))
		  (key-event cwin (char->integer #\q) 4) (force-event)
		  (key-event cwin (char->integer #\i) 4) (force-event) ; just for completeness
		  (key-event cwin (char->integer #\u) 4) (force-event)
		  (key-event cwin (char->integer #\5) 0) (force-event)
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\u) 4) (force-event)
		  (if (not (equal? (edits) '(0 5)))
		      (snd-display (format #f ";C-x C-u -> ~A?" (edits))))
		  (key-event cwin (char->integer #\<) 5) (force-event)
		  (if (not (= (cursor) 0))
		      (snd-display (format #f ";C-< (1): ~A?" (cursor))))
		  (let ((s1000 (sample 1000))
			...
			(snd-display (format #f ";C-w: ~A ~A (~A ~A)" s1000 (sample 0) fr (frames))))
		    (if (not (= fr (+ (frames) 1000)))
			(snd-display (format #f ";C-w len: ~A ~A" fr (frames))))
		    (if (not (equal? (edits) '(1 0)))
			(snd-display (format #f ";C-w -> ~A?" (edits))))
		    (if (not (equal? (edit-fragment 1) (list "C-w" "delete" 0 1000)))
			(snd-display (format #f ";C-w (edit) -> ~A [~A]?" 
					     (edit-fragment 1)
					     (car (edit-fragment 1)))))
		    (key-event cwin (char->integer #\<) 5) (force-event)
		    (key-event cwin (char->integer #\y) 4) (force-event)
		    (if (fneq (sample 1000) s1000)
			(snd-display (format #f ";C-y: ~A ~A (~A ~A)" s1000 (sample 1000) fr (frames))))
		    (if (not (= fr (frames)))
			(snd-display (format #f ";C-y len: ~A ~A" fr (frames))))
		    (if (not (equal? (edits) '(2 0)))
			(snd-display (format #f ";C-y -> ~A?" (edits))))
		    (if (not (equal? (edit-fragment 2) (list "C-y" "insert" 0 1000)))
			(snd-display (format #f ";C-y (edit) -> ~A?" (edit-fragment 2)))))
		  (key-event cwin (char->integer #\<) 5) (force-event)
		  (key-event cwin (char->integer #\x) 8) (force-event)
		  (widget-string minibuffer "(do ((i 0 (1+ i))) ((= i 10)) (set! (sample i) 1.0))")
		  (key-event (widget-window minibuffer) snd-return-key 0) (force-event)
		  (if (not (equal? (edits) '(3 0)))
		      (snd-display (format #f ";M-x edits 10 -> 1.0: ~A?" (edits))))
		  (if (not (equal? (edit-fragment 3) (list "set-sample" "set" 9 1)))
		      (snd-display (format #f ":M-x fragment (wierd!): ~A" (edit-fragment 3))))
		  (key-event cwin snd-space-key 4) (force-event)
		  (do ((i 0 (1+ i)))
		      ((= i 9))
		    (key-event cwin (char->integer #\f) 4) (force-event))
		  (if (not (selection?))
		      (snd-display (format #f ";C-space for selection failed?")))
		  (if (not (= (selection-length) 10))
		      (snd-display (format #f ";C-space for selection len: ~A?" (selection-length))))
		  ...
		  (do ((i 0 (1+ i)))
			...
			(snd-display (format #f ";C-x a [~A]: ~A" i (sample i)))))
		  (if (not (equal? (edit-fragment) (list "C-x a" "set" 0 10)))
		      (snd-display (format #f ";C-x a edit: ~A?" (edit-fragment))))
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\() 1) (force-event)
		  (key-event cwin (char->integer #\f) 4) (force-event)
		  (key-event cwin (char->integer #\f) 4) (force-event)
		  (key-event cwin (char->integer #\f) 4) (force-event)
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  ...
		  (set! (cursor) 1000)
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\e) 0) (force-event)
		  (if (not (= (cursor) 1003))
		      (snd-display (format #f ";kbd macro: ~A?" (cursor))))
		  
		  (key-event cwin (char->integer #\u) 4) (force-event)
		  (key-event cwin (char->integer #\8) 0) (force-event)
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\e) 0) (force-event)
		  (if (not (= (cursor) (+ 1003 (* 8 3))))
		      (snd-display (format #f ";kbd macro (8): ~A?" (cursor))))
		  (key-event cwin (char->integer #\t) 4) (force-event)

		  (set! (cursor) 4000)
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\f) 0) (force-event)
		  (if (> (abs (- (cursor) (right-sample))) 1)
		      (snd-display (format #f ";C-x f -> ~A ~A" (cursor) (right-sample))))

		  (if (file-exists? "hiho.eps") (delete-file "hiho.eps"))
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\g) 4) (force-event)
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\d) 4) (force-event)
		  (widget-string minibuffer "hiho.eps")
		  (key-event (widget-window minibuffer) snd-return-key 0) (force-event)
		  (if (not (file-exists? "hiho.eps"))
		      (snd-display (format #f ";C-x C-d no output?"))
		  ...
		  (if (or (not (defined? 'a-test))
			  (not (= a-test 32)))
		      (snd-display (format #f ";C-x C-l failed?"))
		  ...
		  (if (fneq (/ (- (right-sample) (left-sample)) (srate)) 1.0)
		      (snd-display (format #f ";C-x C-p 1.0: ~A?" (/ (- (right-sample) (left-sample)) (srate)))))
		  (key-event cwin (char->integer #\<) 5) (force-event)
		  (key-event cwin (char->integer #\u) 4) (force-event)
		  (key-event cwin (char->integer #\1) 0) (force-event)
		  (key-event cwin (char->integer #\.) 0) (force-event)
		  ...
		  (if (fneq (/ (cursor) (srate)) 1.0)
		      (snd-display (format #f ";C-u 1.0 C-f: ~A?" (/ (cursor) (srate)))))

		  (let ((edhist (list-ref (channel-widgets) 7)))
		    (select-item edhist 1) (force-event)
		    (if (not (= (edit-position) 1))
			(snd-display (format #f ";click edit history: ~A ~A" (edit-position) (edits)))))
		  ...
		  (if (not (selection?))
			...
			(if (fneq (sample (1+ pos)) (* 5 samp))
			    (snd-display (format #f ";eval-over-selection: ~A ~A (~A)" samp (sample (1+ pos)) (edit-fragment))))))
		  (revert-sound ind)
		  (let ((fr (frames)))
		    (key-event cwin (char->integer #\x) 4) (force-event)
		    ...
		    (key-event (widget-window minibuffer) snd-return-key 0) (force-event)
		    (if (not (= (frames) (* 2 fr)))
			(snd-display (format #f ";C-x C-i oboe: ~A ~A" (* 2 fr) (frames))))
		    (if (fneq (sample 4000) (sample (+ (srate) 4000)))
			(snd-display (format #f ";inserted file: ~A ~A" (sample 4000) (sample (+ (srate) 4000)))))
		    (if (not (equal? (edit-fragment 1) (list "C-x C-i" "insert" (inexact->exact (srate)) fr)))
			(snd-display (format #f ";C-x C-i edit: ~A" (edit-fragment 1))))
		    (key-event cwin (char->integer #\x) 4) (force-event)
		    (key-event cwin (char->integer #\q) 4) (force-event)
		    (widget-string minibuffer "oboe.snd")
		    (key-event (widget-window minibuffer) snd-return-key 0) (force-event)
		    (if (not (= (frames) (* 2 fr)))
			(snd-display (format #f ";C-x C-q oboe: ~A ~A" (* 2 fr) (frames))))
		    (if (fneq (* 2 (sample 4000)) (sample (+ (srate) 4000)))
			(snd-display (format #f ";mixed file: ~A ~A" (* 2 (sample 4000)) (sample (+ (srate) 4000)))))
		    (if (not (equal? (edit-fragment 2) (list "C-x C-q" "set" (inexact->exact (srate)) fr)))
			(snd-display (format #f ";C-x C-q edit: ~A" (edit-fragment 2)))))
		  (set! (cursor) 0)
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\j) 4) (force-event)
		  (if (not (= (cursor) (srate)))
		      (snd-display (format #f ";C-x C-j: ~A?" (cursor))))
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin (char->integer #\u) 4) (force-event)
		  ...
		  (if (fneq (/ (cursor) (srate)) (/ (* .5 (+ (left-sample) (right-sample))) (srate)))
		      (snd-display (format #f ";C-l: ~A ~A?" (/ (cursor) (srate)) (/ (* .5 (+ (left-sample) (right-sample))) (srate)))))
		  )
		...
		(key-event (widget-window name-button) (char->integer #\f) 4) (force-event)
		(widget-string minibuffer "pistol.snd")
		(key-event (widget-window minibuffer) snd-return-key 0) (force-event)
		(if (not (= (length (sounds)) 2))
		    (snd-display (format #f ";C-x C-f -> ~A?" (sounds))))
		(backward-graph)
		(focus-widget name-button)
		(key-event (widget-window name-button) (char->integer #\x) 4) (force-event)
		(key-event (widget-window name-button) (char->integer #\k) 0) (force-event)
		(if (not (= (length (sounds)) 1))
		    (snd-display (format #f ";C-x k -> ~A?" (sounds))))
		(if (not (string=? (short-file-name (car (sounds))) "pistol.snd"))
		    (snd-display (format #f ";backgraph + kill: ~A?" (short-file-name (car (sounds))))))
		...
		(let ((curmax (maxamp)))
		  (focus-widget name-button)
		  (key-event (widget-window name-button) (char->integer #\x) 4) (force-event)
		  (key-event (widget-window name-button) (char->integer #\a) 4) (force-event)
		  (widget-string minibuffer "'(0 .5 1 .5)")
		  (key-event (widget-window minibuffer) snd-return-key 0) (force-event)
		  (if (not (equal? (edits) '(1 0)))
		      (snd-display (format #f ";C-x C-a (edits) -> ~A?" (edits))))
		  (if (fneq (maxamp) (* .5 curmax))
		      (snd-display (format #f ";C-x C-a (amp) -> ~A ~A?" (maxamp) (* .5 curmax))))
		  (let ((grf (car (channel-widgets))))
		    (focus-widget grf)
		    (key-event (widget-window grf) (char->integer #\_) 5) (force-event)
		    (if (not (equal? (edits) '(0 1)))
			(snd-display (format #f ";C-x _ (edits) -> ~A?" (edits))))
		    (if (fneq (maxamp) curmax)
			(snd-display (format #f ";C-x _ (amp) -> ~A ~A?" (maxamp) curmax)))))
		...
		(let ((tmp (temp-dir)))
		  ...
		  (if (or (not (string? (temp-dir)))
		      ...
		      (snd-display (format #f ";temp-dir via prompt: ~A?" (temp-dir))))
		  (set! (temp-dir) tmp)
		  (if (not (equal? (temp-dir) tmp))
		      (snd-display (format #f ";temp-dir reset: ~A ~A?" (temp-dir) tmp))))
		
		(set! (cursor) 4000)
		(key-event (widget-window name-button) (char->integer #\x) 4) (force-event)
		(key-event (widget-window name-button) (char->integer #\m) 4) (force-event)
		(widget-string minibuffer "hiho!")
		(key-event (widget-window minibuffer) snd-return-key 0) (force-event)
		(let ((m (find-mark 4000)))
		  (if (or (not (mark? m))
			  (not (string=? (mark-name m) "hiho!")))
		      (snd-display (format #f ";named mark prompt: ~A ~A" m (if (mark? m) (mark-name m) "no name")))))
		...
		(if (not (equal? fe '(0.0 1.0 1.0 1.0)))
		    (snd-display (format #f ";filter-env (initial): ~A?" fe)))
		...
		(force-event)
		(click-event (widget-window filter-grf) 1 0 100 100) (force-event)
		(if (not (= (length (filter-control-env ind)) 6))
		    (snd-display (format #f ";click filter-env: ~A?" (filter-control-env ind))))
		(click-event (widget-window filter-grf) 1 0 100 100) (force-event)
		(set! fe (filter-control-env ind))
		(if (not (equal? fe '(0.0 1.0 1.0 1.0)))
		    (snd-display (format #f ";filter-env (point deleted): ~A?" fe)))
		(close-sound ind)
		))
	    ...
	    ;; -------- edit find dialog
	      ...
	      (if (or (not (= (cursor) 4423))
		      (not (> (sample (cursor)) .1)))
		  (snd-display (format #f ";edit find <cr>: ~A ~A" (cursor) (sample (cursor)))))
	      (click-button next-button) (force-event)
	      (if (or (not (= (cursor) 4463))
		      (not (> (sample (cursor)) .1)))
		  (snd-display (format #f ";edit find (1): ~A ~A" (cursor) (sample (cursor)))))
	      (click-button previous-button) (force-event)
	      (if (or (not (= (cursor) 4423))
		      (not (> (sample (cursor)) .1)))
		  (snd-display (format #f ";edit find (2): ~A ~A" (cursor) (sample (cursor)))))
	      (do ((i 0 (1+ i)))
		  ((= i 10))
		(click-button next-button) (force-event)
		(if (not (> (sample (cursor)) .1))
		    (snd-display (format #f ";edit find (~A): ~A ~A" (+ i 3) (cursor) (sample (cursor))))))
	      (if (not (= (cursor) 4741))
		  (snd-display (format #f ";edit find end: ~A?" (cursor))))
		...
		(let ((e1 (enved-selected-env)))
		  (if (or (not e1)
			  (null? e1))
		      (snd-display (format #f ";select env failed? ~A" e1)))
		  (select-item env-list 0)  (force-event)
		  (if (equal? e1 (enved-selected-env))
		      (snd-display (format #f ";select env 0 is same as 1? ~A" e1))))

		(click-event ewin 1 0 (enved-x 0.5) (enved-y 1.0)) (force-event)
		(let ((active-env (enved-active-env)))
		  (if (not (ffeql active-env (list 0.0 0.0 0.5 1.0 1.0 0.0)))
		      (snd-display (format #f ";enved mid-click: ~A?" active-env))))
		(widget-string text-widget "new-env")
		(click-button save-button) (force-event)
		(if (not (defined? 'new-env))
		    (snd-display (format #f ";save new-env failed?")))
		(if (not (ffeql new-env (list 0.0 0.0 0.5 1.0 1.0 0.0)))
		    (snd-display (format #f ";saved new-env: ~A?" new-env)))
		(click-event ewin 1 0 (enved-x 0.5) (enved-y 1.0)) (force-event)
		(let ((active-env (enved-active-env)))
		  (if (not (ffeql active-env (list 0.0 0.0 1.0 0.0)))
		      (snd-display (format #f ";enved mid-click to delete: ~A?" active-env))))
		(do ((i 0 (1+ i)))
		    ((= i 50))
		  (click-event ewin 1 0 (enved-x (random 0.999)) (enved-y (random 1.0))) (force-event)
		  (click-event ewin 1 0 (enved-x (random 0.999)) (enved-y (random 1.0))) (force-event)
		  (let* ((e (enved-active-env))
			...
			(let* ((pos (+ 2 (random (- len 4))))
			  ...
			  (click-event ewin 1 0 (enved-x rx) (enved-y ry)) (force-event)
			  (if (>= (length (enved-active-env)) (length e))
			      (snd-display (format #f "; enved loop missed a hit ~A ~A ~A ~A~%" (enved-axis-info) axis e (enved-active-env))))))))
		...
		(let ((active-env (enved-active-env)))
		  (if (not (ffeql active-env (list 0.0 0.0 1.0 0.0)))
		      (snd-display (format #f ";enved revert: ~A?" active-env))))
		(do ((i 0 (1+ i)))
		    ((= i 50))
		  (click-event ewin 1 0 (enved-x (random 0.999)) (enved-y (random 1.0))) (force-event)
		  (if (> (random 1.0) .5) 
		  ...
		  (let* ((e (enved-active-env)))
			...
			(let* ((pos (+ 2 (random (- (length e) 4))))
			       (rx (list-ref e (if (odd? pos) (- pos 1) pos)))
			       (ry (list-ref e (if (odd? pos) pos (+ pos 1)))))
			  (click-event ewin 1 0 (enved-x rx) (enved-y ry))
			  (force-event)))))
		...
		(let* ((ind (open-sound "oboe.snd"))
		  ...
		  (if (not (= (enved-target) enved-spectrum))
		      (snd-display (format #f ";click flt button but target: ~A" (enved-target))))
		  (click-button src-button) (force-event)
		  (if (not (= (enved-target) enved-srate))
		      (snd-display (format #f ";click src button but target: ~A" (enved-target))))
		  ...
		  (if (not (equal? (edits ind) '(1 0)))
		      (snd-display (format #f ";apply amp: ~A?" (edits ind))))
		  (if (not (equal? (edit-fragment 1) (list "Enved: amp" "set" 0 50828)))
		      (snd-display (format #f ";apply amp fragment: ~A?" (edit-fragment 1))))
		  (click-event ewin 1 0 (enved-x 0.25) (enved-y 1.0)) (force-event)
		  (click-button undo-apply-button) (force-event)
		  (if (not (equal? (edits ind) '(1 0)))
		      (snd-display (format #f ";undo-apply amp: ~A?" (edits ind))))
		  (if (not (equal? (edit-fragment 1) (list "Enved: amp" "set" 0 50828)))
		      (snd-display (format #f ";undo-apply amp fragment: ~A?" (edit-fragment 1))))
		  (click-button flt-button) (force-event)
		  (click-button apply-button) (force-event)
		  (if (not (equal? (edits ind) '(2 0)))
		      (snd-display (format #f ";apply flt: ~A?" (edits ind))))
		  (if (not (equal? (edit-fragment 2) (list "Enved: flt" "set" 0 50868)))
		      (snd-display (format #f ";apply flt fragment: ~A?" (edit-fragment 2))))
		  (click-button reset-button) (force-event)
		  ...
		  (if (not (equal? (edits ind) '(3 0)))
		      (snd-display (format #f ";apply src: ~A?" (edits ind))))
		  (if (not (equal? (edit-fragment 3) (list "Enved: src" "set" 0 113510)))
		      (snd-display (format #f ";apply flt fragment: ~A?" (edit-fragment 3))))
		  (if (not (> (frames ind) (* 2 fr)))
		      (snd-display (format #f ";apply src length: ~A ~A?" fr (frames ind))))
		  (close-sound ind))
		...
		(click-button dismiss-button) (force-event)
            ...
            ;; -------- region dialog
            (region-dialog)
            (let* ((rwids (region-dialog-widgets))
		   (edit-button (list-ref rwids 2))
		   (ok-button (list-ref rwids 3))
		   (delete-button (list-ref rwids 4))
		   (grf (list-ref rwids 6))
		   (grf-w (list-ref rwids 7))
		   (grf-f (list-ref rwids 8))
		   (size (widget-size grf))
		   (regs (regions))
		   (len (length regs))
	      ...
	      (do ((i 0 (1+ i)))
		  ((= i len))
		(click-button (list-ref row i)) (force-event)
		(let ((id (list-ref regs i)))
		  (if (> (region-chans id) 1)
		      (begin
			(click-button grf-f) (force-event)
			(click-button grf-w) (force-event)))
		  (let ((samp10 (region-sample 10 id)))
		    (click-button edit-button)
		    (if (not (= (length (sounds)) 1))
			(snd-display (format #f ";sounds in region ~A ~A: ~A" id i (sounds))))
		    (set! (sample 10) (+ .25 (* i .01)))
		    (let ((cwin (widget-window (car (channel-widgets)))))
		      (key-event cwin (char->integer #\x) 4) (force-event)
		      (key-event cwin (char->integer #\s) 4) (force-event)
		      (if (fneq (region-sample 10 id) (+ .25 (* i .01)))
			  (snd-display (format #f ";edit region: ~A ~A" i (region-sample 10 id)))))
		    (close-sound (car (sounds))))
		(click-event (widget-window grf) 1 0 (inexact->exact (/ (car size) 2)) (inexact->exact (/ (cadr size) 2))) (force-event)))
	      (let ((pos (random len)))
		(click-button (list-ref row pos))
		(click-button delete-button)
		(let ((new-regs (regions)))
		  (if (not (= (length new-regs) (1- len)))
		      (snd-display (format #f ";click delete region: ~A ~A" regs new-regs)))))
