    ...
    (begin
	  ...
	  (let ((snd-return-key #xFF0D)
		...
		(let ((ls (left-sample))
		  ...
		  (key-event cwin (char->integer #\x) 4) (force-event) (key-event cwin snd-right-key 4) (force-event)
		  (if (> (abs (- (left-sample) (* 0.5 len) ls)) 1)
		      (snd-display ";cx-> not by window? ~A ~A" rs (left-sample)))
		  (if (> (abs (- len (- (right-sample) (left-sample)))) 1)
		      (snd-display ";cx-> size diff? ~A ~A" len (- (right-sample) (left-sample))))
		  (key-event cwin (char->integer #\x) 4) (force-event) (key-event cwin snd-left-key 4) (force-event)
		  (if (> (abs (- (left-sample) ls)) 1)
		      (snd-display ";cx<- not by window? ~A ~A" ls (left-sample)))
		  (if (> (abs (- len (- (right-sample) (left-sample)))) 1)
		      (snd-display ";cx<- size diff? ~A ~A" len (- (right-sample) (left-sample))))
		  ...
		  (key-event cwin snd-up-key 4) (force-event)
		  (if (> (abs (- (* 1.5 len) (- (right-sample) (left-sample)))) 2)
		      (snd-display ";Cx-up size diff? ~A ~A" ( * 1.5 len) (- (right-sample) (left-sample))))
		  (key-event cwin (char->integer #\x) 4) (force-event)
		  (key-event cwin snd-down-key 4) (force-event)
		  (if (> (abs (- len (- (right-sample) (left-sample)))) 2)
		      (snd-display ";Cx-down size diff? ~A ~A" len (- (right-sample) (left-sample))))
