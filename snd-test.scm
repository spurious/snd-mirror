...
(defmacro without-errors (func)
  `(catch #t 
	  (lambda ()
	    ,func)
	  (lambda args 
	    ;(snd-print (format #f ";warning: ~A" (car args)))
	    (car args))))
...
(if (or full-test (= snd-test 5))
	...
	(if (> (abs (- samp 3000)) 1) (snd-print (format #f ";right-sample: ~A?" samp))))
      ...
      (if (not (= (channel-sync) 123)) (snd-print (format #f ";channel-sync: ~A?" (channel-sync))))
...
(define (safe-make-region beg end snd)
  (let ((len (frames snd)))
    ;(snd-print (format #f "make ~D ~D " beg end))
    (if (> len 1)
	(if (< end len)
...
(if (or full-test (= snd-test 15))
	...
	(let* ((diff 0.0)
	  ...
	  (do ((i 0 (1+ i)))
	    ...
	    (let* ((ov (* scaler (next-sample old-reader)))
		   ...
		   (val (abs (- ov nv))))
	      (set! diff (+ diff val))))
	  (if (> diff 0.0) (snd-print (format #f ";diff (~D ~D): ~A" beg len diff)))
	  (set! diff 0.0)
	  (do ((i 0 (1+ i)))
	      ((= i 100))
	    (let* ((ov (next-sample old-reader))
		   ...
		   (val (abs (- ov nv))))
	      (set! diff (+ diff val))))
	  (if (> diff 0.0) (snd-print (format #f ";zdiff (~D ~D): ~A" beg len diff)))
	...
	(let* ((newmax 0.0)
	  ...
	  (if (fneq newmax maxval)
	      (snd-print (format #f ";scale-selection-to (~D ~D) ~A: ~A?" beg len maxval newmax)))
	  (free-sample-reader new-reader)))
	...
	(if (not (= (frames ind) (+ 100 len)))
	    (snd-print (format #f ";C-o len: ~A? " (frames))))
	(let ((reader (make-sample-reader 1200 ind)))
	  (do ((i 0 (1+ i)))
	      ((= i 100))
	    (let ((val (next-sample reader)))
	      (if (fneq val 0.0) (snd-print (format #f ";C-o[~D]: ~A?" i val)))))
	  (free-sample-reader reader))
	(revert-sound ind)
	(set! (cursor ind) 1200)
	(key (char->integer #\u) 4 ind)
	...
	(if (not (= (frames ind) len))
	    (snd-print (format #f ";C-z len: ~A? " (frames))))
	(let ((reader (make-sample-reader 1200 ind)))
	  (do ((i 0 (1+ i)))
	      ((= i 100))
	    (let ((val (next-sample reader)))
	      (if (fneq val 0.0) (snd-print (format #f ";C-z[~D]: ~A?" i val)))))
	  (free-sample-reader reader))
	(revert-sound ind)
	(set! (cursor ind) 1200)
	(key (char->integer #\u) 4 ind)
	...
	(if (not (= (frames ind) (+ (srate ind) len)))
	    (snd-print (format #f ";C-o 1.0 len: ~A? " (frames))))
	(let ((reader (make-sample-reader 1200 ind)))
	  (do ((i 0 (1+ i)))
	      ((= i (srate ind)))
	    (let ((val (next-sample reader)))
	      (if (fneq val 0.0) (snd-print (format #f ";C-o 1.0[~D]: ~A?" i val)))))
	  (free-sample-reader reader))
	(revert-sound ind)
	(set! (cursor ind) 1200)
	(key (char->integer #\u) 4 ind)
	...
	(key (char->integer #\z) 4 ind)
	(if (not (= (frames ind) len))
	    (snd-print (format #f ";C-z 1.0 len: ~A? " (frames))))
	(let ((reader (make-sample-reader 1200 ind)))
	  (do ((i 0 (1+ i)))
	      ((= i (srate ind)))
	    (let ((val (next-sample reader)))
	      (if (fneq val 0.0) (snd-print (format #f ";C-z 1.0[~D]: ~A?" i val)))))
	  (free-sample-reader reader))
	(close-sound ind))
	...
	(if (not (selection?)) (snd-print ";no selection from 1 samp region?"))
