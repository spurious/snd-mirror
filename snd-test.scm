				  ...
				  (let ((lst (mus-sound-loop-info file)))
					...
					(begin
					  (if (not (equal? (car lst) (list-ref testf 6))) 
					      (snd-print (format #f ";~A: loop start: ~A ~A" (car lst) (list-ref testf 6))))
					  (if (not (equal? (cadr lst) (list-ref testf 7))) 
					      (snd-print (format #f ";~A: loop end: ~A ~A" (cadr lst) (list-ref testf 7)))))
					(if (not (null? lst))
...
(if (or full-test (= snd-test 3))
    (begin
	  ...
	  (snd-print (format #f ";output-name-hook: ~A?" output-name-hook)))
      (if (or (not (hook? output-comment-hook)) (not (hook-empty? output-comment-hook)))
	  (snd-print (format #f ";output-comment-hook: ~A?" output-comment-hook)))
      (if (or (not (hook? mouse-drag-hook)) (not (hook-empty? mouse-drag-hook)))
	  (snd-print (format #f ";mouse-drag-hook: ~A?" mouse-drag-hook)))
      (if (or (not (hook? mouse-release-hook)) (not (hook-empty? mouse-release-hook)))
	  (snd-print (format #f ";mouse-release-hook: ~A?" mouse-release-hook)))
      (if (or (not (hook? mouse-press-hook)) (not (hook-empty? mouse-press-hook)))
	  (snd-print (format #f ";mouse-press-hook: ~A?" mouse-press-hook)))
      (if (or (not (hook? start-playing-hook)) (not (hook-empty? start-playing-hook)))
	  (snd-print (format #f ";start-playing-hook: ~A?" start-playing-hook)))
			...
			(let ((nowval (getfnc)))
			  (if (not (equal? newval nowval))
				  ...
				  (if (> (abs (- newval nowval)) .01)
				      (snd-print (format #f ";~A /= ~A (~A)~%" name newval nowval)))
				  (snd-print (format #f ";~A /= ~A (~A)~%" name newval nowval))))
			  (setfnc initval)
			  (test-vars (cdr lst))))))))
	(test-vars 
...
(define play-sound
  (lambda (file)
	...
	(if (= audio-fd -1)
	    (snd-print (format #f ";can't play ~A" file))
	    (begin
	      (do ((i 0 (+ i bufsize)))
		  ((>= i frames))
...
(if (or full-test (= snd-test 4))
    (let ((chns (mus-sound-chans "oboe.snd"))
      ...
      (if (not (= fr 50828)) (snd-print (format #f ";oboe: mus-sound-frames ~D?" fr)))
      (if (not (= smps 50828)) (snd-print (format #f ";oboe: mus-sound-samples ~D?" smps)))
      (if (not (= len (+ 28 (* 2 50828)))) (snd-print (format #f ";oboe: mus-sound-length ~D?" len)))
      (if (not (= size 2)) (snd-print (format #f ";oboe: mus-sound-datum-size ~D?" size)))
      (if (not (= bytes 2)) (snd-print (format #f ";oboe: sound-bytes ~D?" bytes)))
      (if (not (= sr 22050)) (snd-print (format #f ";oboe: mus-sound-srate ~D?" sr)))
      (if (not (string=? com "")) (snd-print (format #f ";oboe: mus-sound-comment ~A?" com)))
      (if (not (string=? (mus-audio-error-name 2) "srate not available")) (snd-print (format #f ";mus-audio-error: ~S?" (mus-audio-error-name 2))))
      (if (fneq (vector-ref ma 1) .14724) (snd-print (format #f ";oboe: mus-sound-max-amp ~F?" (vector-ref ma 1))))
      (if (not (= (vector-ref ma 0) 24971)) (snd-print (format #f ";oboe: mus-sound-max-amp at ~D?" (vector-ref ma 0))))
      (play-sound "oboe.snd")
      (let* ((ob (view-sound "oboe.snd"))
	     (samp (sample 1000 ob)))
	(save-sound-as "test.snd" ob mus-aifc)
	(let ((ab (open-sound "test.snd")))
	  (if (not (= (car (header-type ab)) mus-aifc)) 
	      (snd-print (format #f "save-as aifc -> ~A?" (cadr (header-type ab)))))
	  (if (not (= (mus-sound-header-type "test.snd") mus-aifc)) 
	      (snd-print (format #f "saved-as aifc -> ~A?" (mus-header-type-name (mus-sound-header-type "test.snd")))))
	  (if (fneq (sample 1000 ab) samp) (snd-print (format #f "aifc[1000] = ~A?" (sample 1000 ab))))
	  (close-sound ab))
	...
	(let ((ab (open-sound "test.snd")))
	  (if (not (= (car (header-type ab)) mus-raw)) 
	      (snd-print (format #f "save-as raw -> ~A?" (cadr (header-type ab)))))
	  (if (not (= (mus-sound-header-type "test.snd") mus-raw)) 
	      (snd-print (format #f "saved-as raw -> ~A?" (mus-header-type-name (mus-sound-header-type "test.snd")))))
	  (if (fneq (sample 1000 ab) samp) (snd-print (format #f "raw[1000] = ~A?" (sample 1000 ab))))
	  (close-sound ab))
	(save-sound-as "test.snd" ob mus-nist mus-bint)
	(let ((ab (open-sound "test.snd")))
	  (if (not (= (car (header-type ab)) mus-nist)) 
	      (snd-print (format #f "save-as nist -> ~A?" (cadr (header-type ab)))))
	  (if (not (= (mus-sound-header-type "test.snd") mus-nist)) 
	      (snd-print (format #f "saved-as nist -> ~A?" (mus-header-type-name (mus-sound-header-type "test.snd")))))
	  (if (not (= (car (data-format ab)) mus-bint)) 
	      (snd-print (format #f "save-as int -> ~A?" (cadr (data-format ab)))))
	  (if (not (= (mus-sound-data-format "test.snd") mus-bint)) 
	      (snd-print (format #f "saved-as int -> ~A?" (mus-data-format-name (mus-sound-data-format "test.snd")))))
	  (if (fneq (sample 1000 ab) samp) (snd-print (format #f "nist[1000] = ~A?" (sample 1000 ab))))
	  (close-sound ab))
	(save-sound-as "test.snd" ob mus-riff mus-lfloat)
	(let ((ab (open-sound "test.snd")))
	  (if (not (= (car (header-type ab)) mus-riff)) 
	      (snd-print (format #f "save-as riff -> ~A?" (cadr (header-type ab)))))
	  (if (not (= (mus-sound-header-type "test.snd") mus-riff)) 
	      (snd-print (format #f "saved-as riff -> ~A?" (mus-header-type-name (mus-sound-header-type "test.snd")))))
	  (if (not (= (car (data-format ab)) mus-lfloat)) 
	      (snd-print (format #f "save-as float -> ~A?" (cadr (data-format ab)))))
	  (if (not (= (mus-sound-data-format "test.snd") mus-lfloat)) 
	      (snd-print (format #f "saved-as float -> ~A?" (mus-data-format-name (mus-sound-data-format "test.snd")))))
	  (if (fneq (sample 1000 ab) samp) (snd-print (format #f "riff[1000] = ~A?" (sample 1000 ab))))
	  (close-sound ab))
	(save-sound-as "test.snd" ob mus-aiff mus-b24int)
	(let ((ab (open-sound "test.snd")))
	  (if (not (= (car (header-type ab)) mus-aiff)) 
	      (snd-print (format #f "save-as aiff -> ~A?" (cadr (header-type ab)))))
	  (if (not (= (mus-sound-header-type "test.snd") mus-aiff)) 
	      (snd-print (format #f "saved-as aiff -> ~A?" (mus-header-type-name (mus-sound-header-type "test.snd")))))
	  (if (not (= (car (data-format ab)) mus-b24int))
	      (snd-print (format #f "save-as 24-bit -> ~A?" (cadr (data-format ab)))))
	  (if (not (= (mus-sound-data-format "test.snd") mus-b24int))
	      (snd-print (format #f "saved-as 24-bit -> ~A?" (mus-data-format-name (mus-sound-data-format "test.snd")))))
	  (if (fneq (sample 1000 ab) samp) (snd-print (format #f "aiff[1000] = ~A?" (sample 1000 ab))))
	  (close-sound ab))
	(save-sound-as "test.snd" ob mus-ircam mus-mulaw)
	(let ((ab (open-sound "test.snd")))
	  (if (not (= (car (header-type ab)) mus-ircam)) 
	      (snd-print (format #f "save-as ircam -> ~A?" (cadr (header-type ab)))))
	  (if (not (= (mus-sound-header-type "test.snd") mus-ircam)) 
	      (snd-print (format #f "saved-as ircam -> ~A?" (mus-header-type-name (mus-sound-header-type "test.snd")))))
	  (if (not (= (car (data-format ab)) mus-mulaw))
	      (snd-print (format #f "save-as mulaw -> ~A?" (cadr (data-format ab)))))
	  (if (not (= (mus-sound-data-format "test.snd") mus-mulaw))
	      (snd-print (format #f "saved-as mulaw -> ~A?" (mus-data-format-name (mus-sound-data-format "test.snd")))))
	  (if (fneq (sample 1000 ab) samp) (snd-print (format #f "ircam[1000] = ~A?" (sample 1000 ab))))
	  (close-sound ab))
	(save-sound-as "test.snd" ob mus-next mus-alaw)
	(let ((ab (open-sound "test.snd")))
	  (if (not (= (car (header-type ab)) mus-next)) 
	      (snd-print (format #f "save-as next -> ~A?" (cadr (header-type ab)))))
	  (if (not (= (mus-sound-header-type "test.snd") mus-next)) 
	      (snd-print (format #f "saved-as next -> ~A?" (mus-header-type-name (mus-sound-header-type "test.snd")))))
	  (if (not (= (car (data-format ab)) mus-alaw)) 
	      (snd-print (format #f "save-as alaw -> ~A?" (cadr (data-format ab)))))
	  (if (not (= (mus-sound-data-format "test.snd") mus-alaw)) 
	      (snd-print (format #f "saved-as alaw -> ~A?" (mus-data-format-name (mus-sound-data-format "test.snd")))))
	  (if (fneq (sample 1000 ab) samp) (snd-print (format #f "next (alaw)[1000] = ~A?" (sample 1000 ab))))
	  (close-sound ab))
	...
	(let ((ab (open-sound "test.snd")))
	  ...
	  (if (not (= (mus-sound-header-type "test.snd") mus-next)) 
	      (snd-print (format #f "saved-as next -> ~A?" (mus-header-type-name (mus-sound-header-type "test.snd")))))
	  (if (not (= (car (data-format ab)) mus-bshort)) 
	      (snd-print (format #f "save-as short -> ~A?" (cadr (data-format ab)))))
	  (if (not (= (mus-sound-data-format "test.snd") mus-bshort)) 
	      (snd-print (format #f "saved-as short -> ~A?" (mus-data-format-name (mus-sound-data-format "test.snd")))))
	  (if (fneq (sample 1000 ab) samp) (snd-print (format #f "next (short)[1000] = ~A?" (sample 1000 ab))))
	  (close-sound ab))
	...
	(let ((ab (open-sound "test.snd")))
	  ...
	  (if (not (= (mus-sound-header-type "test.snd") mus-next)) 
	      (snd-print (format #f "saved-as next -> ~A?" (mus-header-type-name (mus-sound-header-type "test.snd")))))
	  (if (not (= (car (data-format ab)) mus-bfloat)) 
	      (snd-print (format #f "save-as float -> ~A?" (cadr (data-format ab)))))
	  (if (not (= (mus-sound-data-format "test.snd") mus-bfloat)) 
	      (snd-print (format #f "saved-as float -> ~A?" (mus-data-format-name (mus-sound-data-format "test.snd")))))
	  (if (fneq (sample 1000 ab) samp) (snd-print (format #f "next (float)[1000] = ~A?" (sample 1000 ab))))
	  (close-sound ab))
	(close-sound ob))
      (let* ((ob (open-sound "oboe.snd"))
	     (sd (samples->sound-data)))
	(if (not (= (sound-data-length sd) 50828)) (snd-print (format #f ";oboe->sd: len ~A?" (sound-data-length sd))))
	(if (fneq (sound-data-ref sd 0 1000) .0328369) (snd-print (format #f ";oboe->sd[1000]: ~A?" (sound-data-ref sd 0 1000))))
	(close-sound ob))
      (let* ((vals (make-vector 32))
	     (err (mus-audio-mixer-read mus-audio-microphone mus-audio-amp 0 vals)))
	(if (= err -1) 
	    (snd-print (format #f ";mus-audio-mixer-read: ~A?" (mus-audio-error-name (mus-audio-error))))
	    (begin
	      (mus-audio-save)
		...
		(if (= err -1) 
		    (snd-print (format #f ";mus-audio-mixer-write: ~A?" (mus-audio-error-name (mus-audio-error)))))
		(mus-audio-restore)
		(mus-audio-mixer-read mus-audio-microphone mus-audio-amp 0 vals)
		(if (fneq (vector-ref vals 0) old-val) (snd-print (format #f ";mus-audio-restore: ~A ~A?" old-val (vector-ref vals 0))))))))
	...
	(if fil
	    (let ((loops (and fil (soundfont-info))))
		  ...
		  (snd-print (format #f "soundfont-info: ~A?" loops)))
	      (close-sound fil))))
	...
	(let ((v0 (make-vct 100)))
	  (sound-data->vct sdata 0 v0) (w)
	  (if (fneq (vct-ref v0 10) .1) (snd-print (format #f ";sound-data->vct: ~A?" v0)))
	  (vct->sound-data v0 sdata 0) (w)
	  (if (fneq (sound-data-ref sdata 0 10) .1) (snd-print (format #f ";vct->sound-data: ~A?" (sound-data-ref sdata 0 10)))))
	(if (rs .5) (gc))
	...
	(if (rs .5) (gc))
	(if (fneq (sound-data-ref sdata 0 10) .1) (snd-print (format #f ";mus-sound-write: ~A?" (sound-data-ref sdata 0 10))))
	(mus-sound-seek-frame fd 20)
	(mus-sound-read fd 0 10 1 sdata)
	(if (fneq (sound-data-ref sdata 0 0) .2) (snd-print (format #f ";mus-sound-seek: ~A?" (sound-data-ref sdata 0 0))))
...
(if (or full-test (= snd-test 5))
    (let* ((index (open-sound "oboe.snd"))
      ...
      (set-ffting #t)
      (set-waving #t)
      (if (read-only index) (snd-print (format #f ";read-only open-sound: ~A?" (read-only index))))
      (set-read-only #t index)
      (if (not (read-only index)) (snd-print (format #f ";set-read-only: ~A?" (read-only index))))
      (bind-key (char->integer #\a) 0 (lambda () (set! a-ctr 3)))
      (key (char->integer #\a) 0) (w)
      (if (not (= a-ctr 3)) (snd-print (format #f ";bind-key: ~A?" a-ctr)))
	...
	(if (and psf (string? psf))
	    (begin
		  ...
		  (snd-print (format #f ";graph->ps: ~A?" psf))
		  (delete-file psf)))))
      (if (not (= a-ctr 0)) (snd-print (format #f ";unbind-key: ~A?" a-ctr)))
      (if (fneq xp 0.0) (snd-print (format #f ";x-position-slider: ~A?" xp)))
      (if (fneq yp 0.0) (snd-print (format #f ";y-position-slider: ~A?" yp)))
      (if (fneq xz 0.04338) (snd-print (format #f ";x-zoom-slider: ~A?" xz)))
      ...
      (if (not (= (srate index) 22050)) (snd-print (format #f ";oboe: srate ~D?" (srate index))))
      (if (not (= (data-location index) 28)) (snd-print (format #f ";oboe: location ~D?" (data-location index))))
      (if (not (= (car (data-format index)) 1)) (snd-print (format #f ";oboe: format ~A?" (data-format index))))
      (if (fneq (maxamp index) .14724) (snd-print (format #f ";oboe: maxamp ~F?" (maxamp index))))
      (if (not (string=? (comment index) "")) (snd-print (format #f ";oboe: comment ~A?" (comment index))))
      (if (not (= (string-length "asdf") 4)) (snd-print (format #f ";string-length: ~A?" (string-length "asdf"))))
      (if (not (string=? (short-file-name index) "oboe.snd")) (snd-print (format #f ";oboe short name: ~S?" (short-file-name index))))
      (let ((matches (count-matches (lambda (a) (and a (> a .125))))))
	(if (not (= matches 1313)) (snd-print (format #f ";count-matches: ~A?" matches))))
      (let ((spot (find (lambda (a) (and a (> a .13))))))
	(if (or (null? spot) (not (= (cadr spot) 8862))) (snd-print (format #f ";find: ~A?" spot))))
      (set-right-sample 3000) (w)
      (let ((samp (right-sample)))
	(if (> (abs (- samp 3000)) 1) (snd-print (format #f ";right-sample: ~A?" samp))))
      (set-left-sample 1000) (w)
      (let ((samp (left-sample)))
	(if (> (abs (- samp 1000)) 1) (snd-print (format #f ";left-sample: ~A?" samp))))
      (let ((eds (edits)))
	(if (or (not (= (car eds) 0)) (not (= (cadr eds) 0)))
	    (snd-print (format #f ";edits: ~A?" eds)))
	(if (not (= (edit-position) (car eds)))
	    (snd-print (format #f ";edit-position: ~A ~A?" (edit-position) eds))))
	...
	(free-sample-reader vr))
      (w)
      (revert-sound index)
      (insert-sample 100 .5 index) (w)
      (update-fft index) (w)
      (update-graph index) (w)
      (if (or (fneq (sample 100) .5)
	      (not (= (frames index) 50829)))
	  (snd-print (format #f ";insert-sample: ~A ~A?" (sample 100) (frames index))))
      (let ((v0 (make-vector 3))
	    (v1 (make-vct 3)))
	(if (rs .5) (gc))
	(vct-fill! v1 .75)
	(do ((i 0 (1+ i))) ((= i 3)) (vector-set! v0 i .25))
	(insert-samples 200 3 v0 index) (w)
	(insert-samples 300 3 v1 index) (w)
	(if (or (fneq (sample 201) .25)
		(fneq (sample 301) .75)
	      (not (= (frames index) 50835)))
	    (snd-print (format #f ";insert-samples: ~A ~A ~A?" (sample 201) (sample 301) (frames index)))))
	...
	(if (fneq (sample 101 nindex) (sample 101 index))
	    (snd-print (format #f ";save-sound-as: ~A ~A?" (sample 101 nindex) (sample 101 index))))
	(if (not (read-only nindex)) (snd-print (format #f ";read-only view-sound: ~A?" (read-only nindex))))
	(close-sound nindex))
      (revert-sound index)
      (set-sample 50 .5 index) (w)
      (if (fneq (sample 50) .5) (snd-print (format #f ";set-sample: ~A?" (sample 50))))
      (let ((v0 (make-vector 3)))
	(do ((i 0 (1+ i))) ((= i 3)) (vector-set! v0 i .25))
	(set-samples 60 3 v0 index) (w)
	(if (or (fneq (sample 60) .25) (fneq (sample 61) .25))
	    (snd-print (format #f ";set-samples: ~A ~A ~A?" (sample 60) (sample 61) (sample 62)))))
      (revert-sound index)
      (delete-sample 100 index) (w)
      (if (not (= (frames index) 50827)) (snd-print (format #f ";delete-sample: ~A?" (frames index))))
      (delete-samples 0 100 index) (w)
      (if (not (= (frames index) 50727)) (snd-print (format #f ";delete-samples: ~A?" (frames index))))
      (revert-sound index)
      (let ((maxa (maxamp index)))
	(scale-to .5 index) (w)
	(let ((newmaxa (maxamp index)))
	  (if (fneq newmaxa .5) (snd-print (format #f ";scale-to: ~A?" newmaxa)))
	  (undo 1 index) (w)
	  (scale-by 2.0 index) (w)
	  (set! newmaxa (maxamp index))
	  (if (fneq newmaxa (* 2.0 maxa)) (snd-print (format #f "scale-by: ~A?" newmaxa)))
	  (revert-sound index)
	  (select-all index) (w)
	  (if (not (= (length (regions)) 2)) (snd-print (format #f ";regions(2): ~A?" (regions))))
	  (scale-selection-to .5) (w)
	  (set! newmaxa (maxamp index))
	  (if (fneq newmaxa .5) (snd-print (format #f ";scale-selection-to: ~A?" newmaxa)))
	  (revert-sound index)
	  (select-all index) (w)
	  (scale-selection-by 2.0) (w)
	  (set! newmaxa (maxamp index))
	  (if (fneq newmaxa (* 2.0 maxa)) (snd-print (format #f "scale-selection-by: ~A?" newmaxa)))
	  (revert-sound index)
	  (select-all index) (w)
	  (let ((rread (make-region-sample-reader 0 0))
		...
		(snd-print (format #f ";region-sample: ~A ~A?" (region-sample 1 0) (vector-ref rvect 1))))
	    (do ((i 0 (1+ i)))
		((= i 100))
	      (let ((rval (next-sample rread))
		    (sval (next-sample sread)))
		(if (rs .5) (gc))
		(if (fneq rval sval) (snd-print (format #f ";sample-read: ~A ~A?" rval sval)))
		(if (fneq rval (vector-ref rvect i)) (snd-print (format #f ";region-samples: ~A ~A?" rval (vector-ref rvect i))))
		(if (fneq sval (vector-ref svect i)) (snd-print (format #f ";samples: ~A ~A?" sval (vector-ref svect i))))))
	    (free-sample-reader rread) (w)
	    (let ((val0 (next-sample sread)))
	      (if (sample-reader-at-end? sread) (snd-print "premature end?"))
	      (previous-sample sread)
	      (let ((val1 (previous-sample sread)))
		(if (fneq val0 val1) (snd-print (format #f ";previous-sample: ~A ~A?" val0 val1)))))
	...
	(if (not (= (frames) (+ len addlen))) (snd-print (format #f ";insert-sound len: ~A?" (frames))))
	(revert-sound)
	(make-region 0 99) (w)
	(insert-region 60 0 index) (w)
	(if (not (= (frames) (+ len 100))) (snd-print (format #f ";insert-region len: ~A?" (frames))))
	(if (fneq (sample 100) s40) (snd-print (format #f ";insert-region: ~A ~A?" (sample 100) s40))))
      (close-sound index)
	...
	(if (or (fneq (sample 127) .5) (fneq (sample 120) .4962) (fneq (sample 32) 0.07431) (fneq (sample 64) 0.25308))
	    (snd-print (format #f ";smooth-selection: ~A?" v0)))
	(revert-sound index)
	(vct-fill! v0 0.0)
	(vct-set! v0 10 .5)
	...
	(if (or (fneq (sample 20) .5) (fneq (sample 30) 0.0) (fneq (sample 17) -.1057) )
	    (snd-print (format #f ";src-selection: ~A?" v0)))
	(revert-sound index)
	(vct-fill! v0 0.0)
	(vct-set! v0 10 .5)
	(vct->samples 0 128 v0 index 0) (w)
	(select-all) (w)
	(filter-selection '(0 0 .1 1 1 0) 40) (w)
	(set! v0 (samples->vct 0 128 index 0 v0)) (w)
	(if (or (fneq (sample 29) .1945) (fneq (sample 39) -.0137) (fneq (sample 24) -0.01986))
	    (snd-print (format #f ";filter-selection: ~A?" v0)))
	(revert-sound index)
	(vct-fill! v0 1.0)
	(vct->samples 0 128 v0 index 0) (w)
	(select-all) (w)
	(env-selection '(0 0 1 1 2 0) 1.0) (w)
	(set! v0 (samples->vct 0 128 index 0 v0)) (w)
	...
	(revert-sound index)
	(file->array "fmv5.snd" 0 0 128 v0) (w)
	...
	(vct->samples 0 128 v0 index 0) (w)
	(select-all) (w)
	(convolve-selection-with "fmv5.snd" .5) (w)
	(set! v0 (samples->vct 0 128 index 0 v0))
	...
	(let ((newamp (maxamp obind)))
	  ...
	  (let ((newdur (frames obind)))
	    (set-speed 1.0 obind)
	    (if (not (< (- newdur (* 2.0 dur)) 256)) (snd-print (format #f "apply speed: ~A -> ~A?" dur newdur)))
	    ;; within 256 which is apply's buffer size (it always flushes full buffers) 
	    (set-contrasting #t obind)
	    (set-contrast 1.0 obind)
	    (call-apply obind)
	    (let ((secamp (maxamp obind))
		  (secdur (frames obind)))
	      (if (fneq secamp .989) (snd-print (format #f "apply contrast: ~A?" secamp)))
	      (if (not (= secdur newdur)) (snd-print (format #f "apply contrast length: ~A -> ~A?" newdur secdur)))
	      (undo 3 obind)
	      (set-reverbing #t obind)
	      (set-reverb-scale .2 obind)
	      (call-apply obind)
	      (let ((revamp (maxamp obind))
		    (revdur (frames obind)))
		(if (fneq revamp .213) (snd-print (format #f "apply reverb scale: ~A?" revamp)))
		(if (not (< (- revdur (+ 50828 (inexact->exact (* (reverb-decay) 22050)))) 256)) 
		    (snd-print (format #f "apply reverb length: ~A?" revdur)))
		(undo 1 obind)
		(set-expanding #t obind)
		(set-expand 1.5 obind)
		(call-apply obind)
		(let ((expamp (maxamp obind))
		  ...
		  (set-filter-order 40 obind)
		  (set-filter-env '(0 0 1 .5 1 0) obind)
		  (call-apply obind)
		  (let ((fltamp (maxamp obind))
			(fltdur (frames obind)))
		    (if (> (abs (- fltamp .01)) .005) (snd-print (format #f "apply filter scale: ~A?" fltamp)))
		    (if (> (- fltdur (+ 40 50828)) 256) (snd-print (format #f "apply filter length: ~A?" fltdur)))
		    (undo obind)
		    (close-sound obind))))))))))
...
(if (or full-test (= snd-test 6))
    (let ((v0 (make-vct 10))
      ...
      (vct-fill! v1 0.5)
      (vct-set! vl 1 .1)
      (if (not (feql (vct->list vl) (list 0.0 0.1 0.0))) (snd-print (format #f "vct->list: ~A?" (vct->list vl))))
      (do ((i 0 (1+ i)))
	  ((= i 10))
	(if (fneq (vct-ref v0 i) 1.0) (snd-print (format #f ";fill v0[~D] = ~F?" i (vct-ref v0 i))))
	(if (fneq (vct-ref v1 i) 0.5) (snd-print (format #f ";preset v1[~D] = ~F?" i (vct-ref v1 i)))))
      (vct-add! v0 v1)
      (do ((i 0 (1+ i)))
	  ((= i 10))
	(if (fneq (vct-ref v0 i) 1.5) (snd-print (format #f ";add v0[~D] = ~F?" i (vct-ref v0 i)))))
      (vct-subtract! v0 v1)
      (if (rs .5) (gc))
      (do ((i 0 (1+ i)))
	  ((= i 10))
	(if (fneq (vct-ref v0 i) 1.0) (snd-print (format #f ";subtract v0[~D] = ~F?" i (vct-ref v0 i)))))
      (let ((v2 (vct-copy v0)))
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (if (fneq (vct-ref v2 i) 1.0) (snd-print (format #f ";copy v0[~D] = ~F?" i (vct-ref v2 i)))))
	(vct-scale! v2 5.0)
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (if (fneq (vct-ref v2 i) 5.0) (snd-print (format #f ";scale v2[~D] = ~F?" i (vct-ref v2 i)))))
	(vct-offset! v0 -1.0)
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (if (fneq (vct-ref v0 i) 0.0) (snd-print (format #f ";offset v0[~D] = ~F?" i (vct-ref v0 i)))))
	(vct-multiply! v2 v1)
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (if (fneq (vct-ref v2 i) 2.5) (snd-print (format #f ";multiply v2[~D] = ~F?" i (vct-ref v2 i)))))
	(if (fneq (vct-peak v2) 2.5) (snd-print (format #f ";v2's peak is ~F?" (vct-peak v2))))
	(vct-set! v2 5 123.0)
	(if (fneq (vct-peak v2) 123.0) (snd-print (format #f ";v2's set peak is ~F?" (vct-peak v2))))
	(vct-do! v0 (lambda (i) (* i .5)))
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (if (fneq (vct-ref v0 i) (* i .5)) (snd-print (format #f ";do v0[~D] = ~F?" i (vct-ref v0 i)))))
	(vct-map! v0 (lambda () 1.0))
	(if (rs .5) (gc))
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (if (fneq (vct-ref v0 i) 1.0) (snd-print (format #f ";map v0[~D] = ~F?" i (vct-ref v0 i)))))
	(vcts-do! v0 v1 v2 (lambda (num i) (list i 0.0 (* i 2))))
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (if (fneq (vct-ref v0 i) i) (snd-print (format #f ";dos v0[~D] = ~F?" i (vct-ref v0 i))))
	  (if (fneq (vct-ref v1 i) 0.0) (snd-print (format #f ";dos v1[~D] = ~F?" i (vct-ref v1 i))))
	  (if (fneq (vct-ref v2 i) (* i 2)) (snd-print (format #f ";dos v2[~D] = ~F?" i (vct-ref v2 i)))))
	(vcts-map! v0 v1 v2 (lambda (num) (list 0.0 1.0 2.0)))
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (if (fneq (vct-ref v0 i) 0.0) (snd-print (format #f ";maps v0[~D] = ~F?" i (vct-ref v0 i))))
	  (if (fneq (vct-ref v1 i) 1.0) (snd-print (format #f ";maps v1[~D] = ~F?" i (vct-ref v1 i))))
	  (if (fneq (vct-ref v2 i) 2.0) (snd-print (format #f ";maps v2[~D] = ~F?" i (vct-ref v2 i)))))
...
(if (or full-test (= snd-test 8))
    (do ((clmtest 0 (1+ clmtest))) ((= clmtest tests))
      ...
      (if (fneq (mus-srate) 22050.0) (snd-print (format #f ";mus-srate: ~F?" (mus-srate))))
      (if (fneq (hz->radians 1.0) 2.84951704088598e-4) (snd-print (format #f ";hz->radians: ~F?" (hz->radians 1.0))))
      (if (fneq (radians->hz 2.84951704088598e-4) 1.0) (snd-print (format #f ";radians->hz: ~F?" (radians->hz 2.84951704088598e-4))))
      (if (fneq (radians->degrees 1.0) 57.2957801818848) (snd-print (format #f ";radians->degrees: ~F?" (radians->degrees 1.0))))
      (if (fneq (degrees->radians 57.2957801818848) 1.0) (snd-print (format #f ";degrees->radians: ~F?" (degrees->radians 57.2957801818848))))
      (if (fneq (linear->db .25) -12.0411996841431) (snd-print (format #f ";linear->db: ~F?" (linear->db .25))))
      (if (fneq (db->linear -12.0411996841431) .25) (snd-print (format #f ";db->linear: ~F?" (db->linear -12.0411996841431))))
      (if (fneq (hz->radians 1.0) (in-hz 1.0)) (snd-print (format #f ";in-hz: ~F? " (in-hz 1.0))))
      (if (fneq (ring-modulate .4 .5) .2) (snd-print (format #f ";ring-modulate: ~F?" (ring-modulate .4 .5))))
      (if (fneq (amplitude-modulate 1.0 .5 .4) .7) (snd-print (format #f ";amplitude-modulate: ~F?" (amplitude-modulate 1.0 .5 .4))))
      (if (fneq (contrast-enhancement 0.1 0.75) (sin (+ (* 0.1 (/ pi 2)) (* .75 (sin (* 0.1 2.0 pi))))))
	  (snd-print (format #f ";contrast-enhancement: ~F (0.562925306221587)" (contrast-enhancement 0.1 0.75))))
	...
	(if (not (fveql v0 '(-1.000 1.000 2.000) 0)) (snd-print (format #f "partials->polynomial(1): ~A?" v0)))
	(if (not (fveql v1 '(1.000 2.000 0.0) 0)) (snd-print (format #f "partials->polynomial(2): ~A?" v1)))
	(if (not (fveql v2 '(-1.000 3.000 2.000 -16.000 0.000 16.000) 0)) (snd-print (format #f "partials->polynomial(3): ~A?" v2)))
	...
	(let ((v0 (spectrum rdat idat (make-fft-window rectangular-window 16) 16 1)) ;rectangular here to avoid clobbering 0-th data point
	  ...
	  (do ((i 0 (1+ i)))
		...
		(snd-print (format #f "spectra not equal: ~A ~A" v0 v1))))))
      (if (rs .5) (gc))
	...
	(let ((v0 (convolution rdat idat 8))
	  ...
	  (if (or (fneq (vct-ref v0 0) 0.0) (fneq (vct-ref v0 1) 1.0)) (snd-print (format #f ";convolution: ~A?" v0)))
	  (if (or (fneq (vct-ref v1 0) 0.0) (fneq (vct-ref v1 1) 1.0)) (snd-print (format #f ";convolve-arrays: ~A?" v1)))
	  (do ((i 0 (1+ i)))
	      ((= i 8)) 
	    (if (fneq (vct-ref v0 i) (vct-ref v1 i))
		(snd-print (format #f "convolutions not equal: ~A ~A" v0 v1))))))
	...
	(if (fneq (vct-ref rdat 0) (vct-ref xdat 0)) (snd-print (format #f ";ffts: ~A ~A?" rdat xdat)))
	(fft rdat idat -1)
	(mus-fft xdat ydat 16 -1)
	(do ((i 0 (1+ i)))
	    ((= i 16))
	  (if (or (and (= i 3) (or (fneq (vct-ref rdat i) 16.0) (fneq (vct-ref xdat i) 16.0)))
	      ...
	      (snd-print (format #f ";fft real[~D]: ~A ~A?" i (vct-ref rdat i) (vct-ref xdat i))))
	  (if (or (fneq (vct-ref idat i) 0.0) (fneq (vct-ref ydat i) 0.0))
	      (snd-print (format #f ";fft imag[~D]: ~A ~A?" i (vct-ref idat i) (vct-ref ydat i))))))
	...
	(if (fneq (vct-ref v0 0) 0.5) (snd-print (format #f ";multiple-arrays: ~F?" (vct-ref v0 0))))
	(let ((sum (dot-product v0 v1)))
	  (if (fneq sum 2.5) (snd-print (format #f ";dot-product: ~F?" sum))))
	(clear-array v0)
	(if (fneq (vct-ref v0 3) 0.0) (snd-print (format #f ";clear-array: ~A?" v0)))
	(vct-fill! v0 1.0)
	(vct-fill! v1 0.5)
	(let ((v2 (rectangular->polar v0 v1)))
	  (if (fneq (vct-ref v2 0) 1.118) (snd-print (format #f ";rectangular->polar: ~A?" v2)))))

      (let ((rdat (make-vct 16)))
	(vct-set! rdat 3 1.0)
	(autocorrelate rdat)
	(if (fneq (vct-ref rdat 0) 2.0) (snd-print (format #f ";autocorrelate: ~A?" rdat)))
	(do ((i 1 (1+ i)))
	    ((= i 8))
	  (if (fneq (vct-ref rdat i) 0.0) (snd-print (format #f ";autocorrelate[~D]: ~A?" i (vct-ref rdat i))))))
      (if (rs .5) (gc))
	...
	(if (or (fneq (polynomial v0 0.0) 1.0)
	    ...
	    (snd-print (format #f ";polynomial: ~A ~A ~A?"
			       (polynomial v0 0.0)
			       (polynomial v0 1.0)
			       (polynomial v0 2.0)))))
	...
	(do ((i 0 (1+ i))) ((= i 10))
	  (vct-set! v0 i i))
	(if (fneq (array-interp v0 3.5) 3.5) (snd-print (format #f ";array-interp: ~F?" (array-interp v0 3.5)))))
	...
	(if (not (delay? gen)) (snd-print (format #f ";~A not delay?" gen)))
	(if (not (= (mus-length gen) 3)) (snd-print (format #f ";delay length: ~D?" (mus-length gen))))
	(if (or (fneq (vct-ref v0 1) 0.0) (fneq (vct-ref v0 4) 1.0) (fneq (vct-ref v0 8) 5.0))
	    (snd-print (format #f ";delay output: ~A" v0))))
	...
	(if (not (all-pass? gen)) (snd-print (format #f ";~A not all-pass?" gen)))
	(if (not (= (mus-length gen) 3)) (snd-print (format #f ";all-pass length: ~D?" (mus-length gen))))
	(if (not (= (mus-order gen) 3)) (snd-print (format #f ";all-pass order: ~D?" (mus-order gen))))
	(if (fneq (mus-feedback gen) .4) (snd-print (format #f ";all-pass feedback: ~F?" (mus-feedback gen))))
	(if (fneq (mus-feedforward gen) .6) (snd-print (format #f ";all-pass feedforward: ~F?" (mus-feedforward gen))))
	(if (or (fneq (vct-ref v0 1) 0.6) (fneq (vct-ref v0 4) 1.84) (fneq (vct-ref v0 8) 2.336))
	    (snd-print (format #f ";all-pass output: ~A" v0)))
	(mus-set-feedback gen 0.5) (if (fneq (mus-feedback gen) .5) (snd-print (format #f ";all-pass set-feedback: ~F?" (mus-feedback gen))))
	(mus-set-feedforward gen 0.5) (if (fneq (mus-feedforward gen) .5) (snd-print (format #f ";all-pass set-feedforward: ~F?" (mus-feedforward gen)))))
	...
	(if (not (comb? gen)) (snd-print (format #f ";~A not comb?" gen)))
	(if (not (= (mus-length gen) 3)) (snd-print (format #f ";comb length: ~D?" (mus-length gen))))
	(if (not (= (mus-order gen) 3)) (snd-print (format #f ";comb order: ~D?" (mus-order gen))))
	(if (fneq (mus-feedback gen) .4) (snd-print (format #f ";comb feedback: ~F?" (mus-feedback gen))))
	(if (or (fneq (vct-ref v0 1) 0.0) (fneq (vct-ref v0 4) 1.0) (fneq (vct-ref v0 8) 1.4))
	    (snd-print (format #f ";comb output: ~A" v0))))
	...
	(if (not (notch? gen)) (snd-print (format #f ";~A not notch?" gen)))
	(if (not (= (mus-length gen) 3)) (snd-print (format #f ";notch length: ~D?" (mus-length gen))))
	(if (not (= (mus-order gen) 3)) (snd-print (format #f ";notch order: ~D?" (mus-order gen))))
	(if (fneq (mus-feedforward gen) .4) (snd-print (format #f ";notch feedforward: ~F?" (mus-feedforward gen))))
	(if (or (fneq (vct-ref v0 1) 0.4) (fneq (vct-ref v0 4) 1.4) (fneq (vct-ref v0 8) 1.4))
	    (snd-print (format #f ";notch output: ~A" v0))))
	...
	(if (not (one-pole? gen)) (snd-print (format #f ";~A not one-pole?" gen)))
	(if (not (= (mus-order gen) 1)) (snd-print (format #f ";one-pole order: ~D?" (mus-order gen))))
	(if (fneq (mus-a0 gen) .4) (snd-print (format #f ";one-pole a0: ~F?" (mus-a0 gen))))
	(if (fneq (mus-b1 gen) .7) (snd-print (format #f ";one-pole b1: ~F?" (mus-b1 gen))))
	(if (or (fneq (vct-ref v0 1) 0.120) (fneq (vct-ref v0 4) 0.275) (fneq (vct-ref v0 8) 0.245))
	    (snd-print (format #f ";one-pole output: ~A" v0))))
	...
	(if (not (one-zero? gen)) (snd-print (format #f ";~A not one-zero?" gen)))
	(if (not (= (mus-order gen) 1)) (snd-print (format #f ";one-zero order: ~D?" (mus-order gen))))
	(if (fneq (mus-a0 gen) .4) (snd-print (format #f ";one-zero a0: ~F?" (mus-a0 gen))))
	(if (fneq (mus-a1 gen) .7) (snd-print (format #f ";one-zero a1: ~F?" (mus-a1 gen))))
	(if (fneq (vct-ref v0 1) 1.1) (snd-print (format #f ";one-zero output: ~A" v0))))
	...
	(if (not (two-zero? gen)) (snd-print (format #f ";~A not two-zero?" gen)))
	(if (not (= (mus-order gen) 2)) (snd-print (format #f ";two-zero order: ~D?" (mus-order gen))))
	(if (fneq (mus-a0 gen) .4) (snd-print (format #f ";two-zero a0: ~F?" (mus-a0 gen))))
	(if (fneq (mus-a1 gen) .7) (snd-print (format #f ";two-zero a1: ~F?" (mus-a1 gen))))
	(if (fneq (mus-a2 gen) .3) (snd-print (format #f ";two-zero a2: ~F?" (mus-a2 gen))))
	(if (or (fneq (vct-ref v0 1) 1.1) (fneq (vct-ref v0 8) 1.4)) (snd-print (format #f ";two-zero output: ~A" v0))))
	...
	(if (not (two-pole? gen)) (snd-print (format #f ";~A not two-pole?" gen)))
	(if (not (= (mus-order gen) 2)) (snd-print (format #f ";two-pole order: ~D?" (mus-order gen))))
	(if (fneq (mus-a0 gen) .4) (snd-print (format #f ";two-pole a0: ~F?" (mus-a0 gen))))
	(if (fneq (mus-b1 gen) .7) (snd-print (format #f ";two-pole b1: ~F?" (mus-b1 gen))))
	(if (fneq (mus-b2 gen) .3) (snd-print (format #f ";two-pole b2: ~F?" (mus-b2 gen))))
	(if (or (fneq (vct-ref v0 1) 0.12) (fneq (vct-ref v0 8) 0.201)) (snd-print (format #f ";two-pole output: ~A" v0))))
	...
	(if (not (oscil? gen)) (snd-print (format #f ";~A not oscil?" gen)))
	(if (fneq (mus-phase gen) 1.253787) (snd-print (format #f ";oscil phase: ~F?" (mus-phase gen))))
	(if (fneq (mus-frequency gen) 440.0) (snd-print (format #f ";oscil frequency: ~F?" (mus-frequency gen))))
	(if (not (= (mus-cosines gen) 1)) (snd-print (format #f ";oscil cosines: ~D?" (mus-cosines gen))))
	(if (or (fneq (vct-ref v0 1) 0.125) (fneq (vct-ref v0 8) 0.843)) (snd-print (format #f ";oscil output: ~A" v0)))
	(mus-set-phase gen 0.0)
	(if (fneq (mus-phase gen) 0.0) (snd-print (format #f ";oscil set-phase: ~F?" (mus-phase gen))))
	(mus-set-frequency gen 100.0)
	(if (fneq (mus-frequency gen) 100.0) (snd-print (format #f ";oscil set-frequency: ~F?" (mus-frequency gen))))
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (if (fneq (vct-ref v0 i) (vct-ref v1 i))
	      (snd-print (format #f "mus-apply oscil at ~D: ~A ~A?" i (vct-ref v0 i) (vct-ref v1 i))))))
	...
	(if (or (fneq (vct-ref v0 1) 0.125) (fneq (vct-ref v0 6) 0.830) (fneq (vct-ref v0 8) 0.987))
	    (snd-print (format #f ";oscil fm output: ~A" v0)))
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (if (fneq (vct-ref v0 i) (vct-ref v1 i))
	      (snd-print (format #f "mus-apply fm oscil at ~D: ~A ~A?" i (vct-ref v0 i) (vct-ref v1 i))))))
	...
	(if (or (fneq (vct-ref v0 1) 0.367) (fneq (vct-ref v0 6) 0.854) (fneq (vct-ref v0 8) 0.437))
	    (snd-print (format #f ";oscil pm output: ~A" v0))))
	...
	(if (or (fneq (vector-ref results 1) 0.12639)
	    ...
	    (snd-print (format #f "oscil-bank: ~A?" results))))

      (let ((gen (make-buffer 3)))
	(if (not (buffer-empty? gen)) (snd-print (format #f ";new buf not buffer-empty: ~A?" gen)))
	(sample->buffer gen 1.0)
	(sample->buffer gen 0.5)
	(sample->buffer gen 0.25)
	(if (not (buffer-full? gen)) (snd-print (format #f ";buffer-full: ~A?" gen)))
	(if (not (buffer? gen)) (snd-print (format #f ";~A not buffer?" gen)))
	(if (not (= (mus-length gen) 3)) (snd-print (format #f ";buffer length: ~D?" (mus-length gen))))
	(if (or (fneq (buffer->sample gen) 1.0) (fneq (buffer->sample gen) 0.5) (fneq (buffer->sample gen) 0.25))
	    (snd-print (format "buffer output?")))
	(if (not (buffer-empty? gen)) (snd-print (format #f ";emptied buf not buffer-empty: ~A?" gen)))
	(let ((fr0 (make-frame 2 .1 .2))
	      (fr1 (make-frame 2 0.0 0.0)))
	  (frame->buffer gen fr0)
	  (set! fr1 (buffer->frame gen fr1))
	  (if (not (equal? fr0 fr1)) (snd-print (format #f ";frame->buffer: ~A ~A?" fr0 fr1)))))
	...
	(if (not (sum-of-cosines? gen)) (snd-print (format #f ";~A not sum-of-cosines?" gen)))
	(if (fneq (mus-phase gen) 1.253787) (snd-print (format #f ";sum-of-cosines phase: ~F?" (mus-phase gen))))
	(if (fneq (mus-frequency gen) 440.0) (snd-print (format #f ";sum-of-cosines frequency: ~F?" (mus-frequency gen))))
	(if (fneq (mus-scaler gen) (/ 1.0 21.0)) (snd-print (format #f ";sum-of-cosines scaler: ~F?" (mus-scaler gen))))
	(if (not (= (mus-cosines gen) 10)) (snd-print (format #f ";sum-of-cosines cosines: ~D?" (mus-cosines gen))))
	(if (not (= (mus-length gen) 10)) (snd-print (format #f ";sum-of-cosines length: ~D?" (mus-length gen))))
	(if (or (fneq (vct-ref v0 1) 0.736) (fneq (vct-ref v0 8) -0.089)) (snd-print (format #f ";sum-of-cosines output: ~A" v0)))
	(mus-set-scaler gen .5) (if (fneq (mus-scaler gen) 0.5) (snd-print (format #f ";sum-of-cosines set-scaler: ~F?" (mus-scaler gen)))))
	...
	(if (not (sine-summation? gen)) (snd-print (format #f ";~A not sine-summation?" gen)))
	(if (fneq (mus-phase gen) 1.253787) (snd-print (format #f ";sine-summation phase: ~F?" (mus-phase gen))))
	(if (fneq (mus-frequency gen) 440.0) (snd-print (format #f ";sine-summation frequency: ~F?" (mus-frequency gen))))
	(if (or (fneq (vct-ref v0 1) 0.249) (fneq (vct-ref v0 8) 1.296)) (snd-print (format #f ";sine-summation output: ~A" v0))))
	...
	(if (not (asymmetric-fm? gen)) (snd-print (format #f ";~A not asymmetric-fm?" gen)))
	(if (fneq (mus-phase gen) 1.253787) (snd-print (format #f ";asymmetric-fm phase: ~F?" (mus-phase gen))))
	(if (fneq (mus-frequency gen) 440.0) (snd-print (format #f ";asymmetric-fm frequency: ~F?" (mus-frequency gen))))
	(if (or (fneq (vct-ref v0 1) 0.248) (fneq (vct-ref v0 8) .962)) (snd-print (format #f ";asymmetric-fm output: ~A" v0))))
	...
	(if (not (fir-filter? gen)) (snd-print (format #f ";~A not fir-filter?" gen)))
	(if (not (= (mus-length gen) 3)) (snd-print (format #f ";fir-filter length: ~D?" (mus-length gen))))
	(if (or (fneq (vct-ref v0 1) 0.25) (fneq (vct-ref v0 2) .125)) (snd-print (format #f ";fir-filter output: ~A" v0)))
	(let ((data (mus-xcoeffs gen)))
	  (if (fneq (vct-ref data 1) .25) (snd-print (format #f ";fir-filter xcoeffs: ~A?" data)))))
	...
	(let ((data (make-vct 100)))
	  ...
	  (if (or (fneq (vct-ref data 1) .2)
	      ...
	      (snd-print (format #f "filter xcoeffs: ~A?" data)))))
	...
	(if (not (iir-filter? gen)) (snd-print (format #f ";~A not iir-filter?" gen)))
	(if (not (= (mus-length gen) 3)) (snd-print (format #f ";iir-filter length: ~D?" (mus-length gen))))
	(if (or (fneq (vct-ref v0 1) -0.25) (fneq (vct-ref v0 2) -.062)) (snd-print (format #f ";iir-filter output: ~A" v0)))
	(let ((data (mus-ycoeffs gen)))
	  (if (fneq (vct-ref data 1) .25) (snd-print (format #f ";iir-filter ycoeffs: ~A?" data)))))
	...
	(if (not (filter? gen)) (snd-print (format #f ";~A not filter?" gen)))
	(if (not (= (mus-length gen) 3)) (snd-print (format #f ";filter length: ~D?" (mus-length gen))))
	(if (or (fneq (vct-ref v0 1) 0.125) (fneq (vct-ref v0 2) .031)) (snd-print (format #f ";filter output: ~A" v0))))
	...
	(if (not (sawtooth-wave? gen)) (snd-print (format #f ";~A not sawtooth-wave?" gen)))
	(if (fneq (mus-phase gen) 4.39538) (snd-print (format #f ";sawtooth-wave phase: ~F?" (mus-phase gen)))) ;starts at pi
	(if (fneq (mus-frequency gen) 440.0) (snd-print (format #f ";sawtooth-wave frequency: ~F?" (mus-frequency gen))))
	(if (fneq (mus-scaler gen) 1.0) (snd-print (format #f ";sawtooth-wave scaler: ~F?" (mus-scaler gen))))
	(if (or (fneq (vct-ref v0 1) 0.04) (fneq (vct-ref v0 8) .319)) (snd-print (format #f ";sawtooth-wave output: ~A" v0))))
	...
	(if (not (square-wave? gen)) (snd-print (format #f ";~A not square-wave?" gen)))
	(if (fneq (mus-phase gen) 1.253787) (snd-print (format #f ";square-wave phase: ~F?" (mus-phase gen))))
	(if (fneq (mus-frequency gen) 440.0) (snd-print (format #f ";square-wave frequency: ~F?" (mus-frequency gen))))
	(if (fneq (mus-scaler gen) 1.0) (snd-print (format #f ";square-wave scaler: ~F?" (mus-scaler gen))))
	(if (or (fneq (vct-ref v0 1) 1.0) (fneq (vct-ref v0 8) 1.0)) (snd-print (format #f ";square-wave output: ~A" v0))))
	...
	(if (not (triangle-wave? gen)) (snd-print (format #f ";~A not triangle-wave?" gen)))
	(if (fneq (mus-phase gen) 1.253787) (snd-print (format #f ";triangle-wave phase: ~F?" (mus-phase gen))))
	(if (fneq (mus-frequency gen) 440.0) (snd-print (format #f ";triangle-wave frequency: ~F?" (mus-frequency gen))))
	(if (fneq (mus-scaler gen) 1.0) (snd-print (format #f ";triangle-wave scaler: ~F?" (mus-scaler gen))))
	(if (or (fneq (vct-ref v0 1) 0.080) (fneq (vct-ref v0 8) 0.639)) (snd-print (format #f ";triangle-wave output: ~A" v0))))
	...
	(if (not (pulse-train? gen)) (snd-print (format #f ";~A not pulse-train?" gen)))
	(if (fneq (mus-phase gen) 1.253787) (snd-print (format #f ";pulse-train phase: ~F?" (mus-phase gen))))
	(if (fneq (mus-frequency gen) 440.0) (snd-print (format #f ";pulse-train frequency: ~F?" (mus-frequency gen))))
	(if (fneq (mus-scaler gen) 1.0) (snd-print (format #f ";pulse-train scaler: ~F?" (mus-scaler gen))))
	(if (or (fneq (vct-ref v0 0) 1.0) (fneq (vct-ref v0 8) 0.0)) (snd-print (format #f ";pulse-train output: ~A" v0))))
	...
	(if (not (two-pole? gen)) (snd-print (format #f ";~A not ppolar?" gen)))
	(if (not (= (mus-order gen) 2)) (snd-print (format #f ";ppolar order: ~D?" (mus-order gen))))
	(if (fneq (mus-a0 gen) 1.0) (snd-print (format #f ";ppolar a0: ~F?" (mus-a0 gen))))
	(if (fneq (mus-b1 gen) -.188) (snd-print (format #f ";ppolar b1: ~F?" (mus-b1 gen))))
	(if (fneq (mus-b2 gen) .01) (snd-print (format #f ";ppolar b2: ~F?" (mus-b2 gen))))
	(if (or (fneq (vct-ref v0 0) 1.0) (fneq (vct-ref v0 1) .188)) (snd-print (format #f ";ppolar output: ~A" v0))))
	...
	(if (not (two-zero? gen)) (snd-print (format #f ";~A not zpolar?" gen)))
	(if (not (= (mus-order gen) 2)) (snd-print (format #f ";zpolar order: ~D?" (mus-order gen))))
	(if (fneq (mus-a0 gen) 1.0) (snd-print (format #f ";zpolar a0: ~F?" (mus-a0 gen))))
	(if (fneq (mus-a1 gen) -.188) (snd-print (format #f ";zpolar a1: ~F?" (mus-a1 gen))))
	(if (fneq (mus-a2 gen) .01) (snd-print (format #f ";zpolar a2: ~F?" (mus-a2 gen))))
	(if (or (fneq (vct-ref v0 0) 1.0) (fneq (vct-ref v0 1) -.188)) (snd-print (format #f ";zpolar output: ~A" v0))))
	...
	(if (not (formant? gen)) (snd-print (format #f ";~A not formant?" gen)))
	(if (not (= (mus-order gen) 2)) (snd-print (format #f ";formant order: ~D?" (mus-order gen))))
	(if (fneq (mus-a0 gen) 0.06371) (snd-print (format #f ";formant a0: ~F?" (mus-a0 gen))))
	(if (fneq (mus-a1 gen) 1.0) (snd-print (format #f ";formant a1: ~F?" (mus-a1 gen))))
	(if (fneq (mus-a2 gen) -0.9) (snd-print (format #f ";formant a2: ~F?" (mus-a2 gen))))
	(if (fneq (mus-b1 gen) -1.6957893) (snd-print (format #f ";formant b1: ~F?" (mus-b1 gen))))
	(if (fneq (mus-b2 gen) 0.81) (snd-print (format #f ";formant b2: ~F?" (mus-b2 gen))))
	(if (fneq (mus-formant-radius gen) .9) (snd-print (format #f ";formant radius: ~F?" (mus-formant-radius gen))))
	(if (fneq (mus-frequency gen) 1200.0) (snd-print (format #f ";formant frequency: ~F?" (mus-frequency gen))))
	(if (or (fneq (vct-ref v0 0) .064) (fneq (vct-ref v0 1) .108)) (snd-print (format #f ";formant output: ~A" v0)))
	(mus-set-a0 gen .5) (if (fneq (mus-a0 gen) 0.5) (snd-print (format #f ";formant set-a0: ~F?" (mus-a0 gen))))
	(mus-set-a1 gen .5) (if (fneq (mus-a1 gen) 0.5) (snd-print (format #f ";formant set-a1: ~F?" (mus-a1 gen))))
	(mus-set-a2 gen .5) (if (fneq (mus-a2 gen) 0.5) (snd-print (format #f ";formant set-a2: ~F?" (mus-a2 gen))))
	(mus-set-b1 gen .5) (if (fneq (mus-b1 gen) 0.5) (snd-print (format #f ";formant set-b1: ~F?" (mus-b1 gen))))
	...
	(mus-set-formant-radius gen .01) (if (fneq (mus-formant-radius gen) 0.01) (snd-print (format #f ";formant set-radius: ~F?" (mus-formant-radius gen)))))
      ...
      (let ((gen (make-mixer 2 .5 .25 .125 1.0))
	    ...
	    (fr1 (make-frame 2 0.0 0.0)))
	(if (not (frame? fr0)) (snd-print "~A not a frame?" fr0))
	(if (not (mixer? gen)) (snd-print "~A not a mixer?" gen))
	(if (equal? fr0 fr1) (snd-print (format #f ";frame=? ~A ~A?" fr0 fr1)))
	(if (not (= (mus-channels fr0) 2)) (snd-print (format #f ";frame channels: ~D?" (mus-channels fr0))))
	(if (not (= (mus-length fr1) 2)) (snd-print (format #f ";frame length: ~D?" (mus-length fr0))))
	(if (not (= (mus-channels gen) 2)) (snd-print (format #f ";mixer channels: ~D?" (mus-channels gen))))
	(frame->frame gen fr0 fr1)
	(if (or (fneq (frame-ref fr0 0) 1.0)
		(fneq (frame-ref fr1 1) 1.25)
		(fneq (mixer-ref gen 0 0) .5))
	    (snd-print (format #f ";fr0: ~A" fr0)))
	(frame-set! fr1 0 1.0)
	(frame-set! fr1 1 1.0)
	(let ((fr3 (frame+ fr0 fr1))
	  ...
	  (if (or (fneq (frame-ref fr3 0) 2.0)
	      ...
	      (snd-print (format #f ";fr+*: ~A ~A" fr3 fr4))))
	(let* ((mx1 (make-mixer 2 1.0 0.0 0.0 1.0))
	       (mx2 (mixer* gen mx1))
	       (val (frame->sample mx1 fr1)))
	  (if (fneq val 1.0) (snd-print (format #f ";frame->sample: ~A?" val)))
	  (if (not (equal? (frame->list fr1) (list 1.0 1.0))) (snd-print (format #f ";frame->list: ~A?" (frame->list fr1))))
	  (if (or (fneq (mixer-ref mx2 0 1) .25) (fneq (mixer-ref mx2 1 0) .125)) (snd-print (format #f ";mixer*: ~A?" mx2)))
	  (if (not (equal? mx2 gen)) (snd-print (format #f ";mixer=? ~A ~A?" gen mx2)))
	  (if (equal? mx2 mx1) (snd-print (format #f ";mixer/=? ~A ~A?" mx1 mx2)))
	  (mixer-set! mx2 0 0 2.0)
	  (if (fneq (mixer-ref mx2 0 0) 2.0) (snd-print (format #f ";mixer-set: ~A?" mx2)))
	  (set! fr0 (sample->frame mx2 1.0))
	  (if (or (fneq (frame-ref fr0 0) 2.0) (fneq (frame-ref fr0 1) .25)) (snd-print (format #f ";sample->frame: ~A?" fr0)))
	  (let ((frout (make-frame 2)))
	    (sample->frame mx2 1.0 frout)
	    (if (not (equal? frout fr0)) (snd-print (format #f "sample->frame via frout: ~A ~A?" frout fr0))))))

      (let ((gen (make-fft-window hamming-window 16)))
	(if (or (fneq (vct-ref gen 0) 0.080) (fneq (vct-ref gen 8) 1.0))
	    (snd-print (format #f ";hamming window: ~A" gen))))
      
      (let ((v0 (make-vct 10))
	    (gen (make-env '(0 0 1 1 2 0) :scaler 0.5 :end 9)))
	(if (not (env? gen)) (snd-print (format #f ";~A not env?" gen)))
	(if (fneq (mus-scaler gen) 0.5) (snd-print (format #f ";env scaler ~F?" (mus-scaler gen))))
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (if (rs .1) (gc))
	  (vct-set! v0 i (env gen)))
	(if (or (fneq (vct-ref v0 0) 0.0) (fneq (vct-ref v0 1) .1) (fneq (vct-ref v0 6) .4))
	    (snd-print (format #f ";~A output: ~A" gen v0)))
	(if (fneq (env-interp 1.5 gen) 0.25) (snd-print (format #f ";env-interp ~A at 1.5: ~F?" gen (env-interp 1.5 gen))))
	...
	(do ((i 0 (1+ i)))
	  ...
	  (if (= i 3)
		  ...
		  (snd-print (format #f ";env location: ~A?" (mus-location gen)))))
	  (vct-set! v0 i (env gen)))
	(if (or (fneq (vct-ref v0 0) 2.0) (fneq (vct-ref v0 6) 1.5) (fneq (vct-ref v0 8) 1.5))
	    (snd-print (format #f ";~A output: ~A" gen v0)))
	(if (fneq (env-interp 1.5 gen) 1.5) (snd-print (format #f ";env-interp ~A at 1.5: ~F?" gen (env-interp 1.5 gen))))
	(mus-set-location gen 6)
	(if (not (= (mus-location gen) 6)) (snd-print (format #f "mus-set-location ~A: ~A (6)?" (mus-location gen))))
	(let ((val (env gen)))
	  (if (fneq val 1.5) (snd-print (format #f "mus-set-location 6 -> ~A (1.5)?" val))))
	(mus-set-location gen 0)
	(let ((val (env gen)))
	  (if (fneq val 2.0) (snd-print (format #f "mus-set-location 0 -> ~A (2.0)?" val))))
	(set! gen (make-env '(0 0 1 1 2 0) :end 9))
	(do ((i 0 (1+ i))) ((= i 4)) (env gen))
	(let ((val (env gen)))
	  (if (fneq val .8) (snd-print (format #f ";env(5): ~A?" val)))
	  (restart-env gen)
	  (do ((i 0 (1+ i))) ((= i 4)) (env gen))
	  (set! val (env gen))
	  (if (fneq val .8) (snd-print (format #f ";restart-env: ~A?" val)))
	  (mus-set-location gen 6)
	  (let ((val (env gen)))
	    (if (fneq val 0.8) (snd-print (format #f "mus-set-location 6 -> ~A (0.8)?" val))))))
      
      (let ((gen (make-table-lookup 440.0 :wave (partials->wave '(1 1 2 1))))
	    (gen1 (make-table-lookup 440.0 :wave (partials->wave '(1 1 2 1))))
	    (v0 (make-vct 10))
	    (v1 (make-vct 10)))
	(if (not (= (mus-length gen) 512)) (snd-print (format #f ";table-lookup length: ~A?" (mus-length gen))))
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (if (rs .1) (gc))
	  (vct-set! v0 i (table-lookup gen 0.0))
	  (vct-set! v1 i (mus-apply gen1 0.0)))
	(if (not (table-lookup? gen)) (snd-print "~A not table-lookup?" gen))
	(if (fneq (mus-phase gen) 1.253787) (snd-print (format #f ";table-lookup phase: ~F?" (mus-phase gen))))
	(if (fneq (mus-frequency gen) 440.0) (snd-print (format #f ";table-lookup frequency: ~F?" (mus-frequency gen))))
	(if (or (fneq (vct-ref v0 1) 0.373) (fneq (vct-ref v0 8) 1.75)) (snd-print (format #f ";table-lookup output: ~A" v0)))
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (if (fneq (vct-ref v0 i) (vct-ref v1 i))
	      (snd-print (format #f "mus-apply table-lookup at ~D: ~A ~A?" i (vct-ref v0 i) (vct-ref v1 i)))))
	(set! gen (make-table-lookup 440.0 :wave (phase-partials->wave (list 1 1 0 2 1 (* pi .5)))))
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (vct-set! v0 i (table-lookup gen 0.0)))
	(if (or (fneq (vct-ref v0 1) 1.094) (fneq (vct-ref v0 8) .421)) (snd-print (format #f ";table-lookup phase output: ~A" v0))))
      
      (let ((gen0 (make-waveshape 440.0 :wave (partials->waveshape '(1 1))))
	    (gen (make-waveshape 440.0 :partials '(1 1)))
	    (v0 (make-vct 10)))
	(if (not (= (mus-length gen) 512)) (snd-print (format #f ";waveshape length: ~A?" (mus-length gen))))
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (let ((val0 (waveshape gen0 1.0 0.0))
		(val (waveshape gen 1.0 0.0)))
	    (if (fneq val val0) (snd-print (format #f ";waveshape: ~A /= ~F?" val val0)))
	    (if (rs .1) (gc))
	    (vct-set! v0 i val)))
	(if (not (waveshape? gen)) (snd-print "~A not waveshape?" gen))
	(if (fneq (mus-phase gen) 1.253787) (snd-print (format #f ";waveshape phase: ~F?" (mus-phase gen))))
	(if (fneq (mus-frequency gen) 440.0) (snd-print (format #f ";waveshape frequency: ~F?" (mus-frequency gen))))
	(if (or (fneq (vct-ref v0 1) 0.125) (fneq (vct-ref v0 8) .843)) (snd-print (format #f ";waveshape output: ~A" v0))))
	...
	(if (not (= (vct-length (mus-data gen)) 20)) (snd-print (format #f ";wave-train data length: ~A?" (vct-length (mus-data gen)))))
	(if (not (= (mus-length gen) 20)) (snd-print (format #f ";wave-train length: ~A?" (mus-length gen))))
	(do ((i 0 (1+ i)))
	    ((= i 10))
	  (vct-set! v0 i (wave-train gen 0.0)))
	(if (not (wave-train? gen)) (snd-print "~A not wave-train?" gen))
	(if (fneq (mus-phase gen) 0.0) (snd-print (format #f ";wave-train phase: ~F?" (mus-phase gen))))
	(if (fneq (mus-frequency gen) 440.0) (snd-print (format #f ";wave-train frequency: ~F?" (mus-frequency gen))))
	(if (or (fneq (vct-ref v0 1) 0.5) (fneq (vct-ref v0 8) 4.0)) (snd-print (format #f ";wave-train output: ~A" v0))))
	...
	(if (not (readin? gen)) (snd-print (format #f "~A not readin?" gen)))
	(if (not (mus-input? gen)) (snd-print (format #f "~A not input?" gen)))
	(if (not (= (mus-channel gen) 0)) (snd-print (format #f "readin chan: ~A?" (mus-channel gen))))
	(if (or (fneq (vct-ref v0 1) -0.009) (fneq (vct-ref v0 7) .029)) (snd-print (format #f ";readin output: ~A" v0)))
	(mus-set-location gen 1000)
	(if (not (= (mus-location gen) 1000)) (snd-print (format #f ";mus-set-location: ~A?" (mus-location gen))))
	(let ((val (readin gen)))
	  (if (fneq val 0.033) (snd-print (format #f ";mus-set-location readin: ~A?" val)))))
	...
	(if (not (file->sample? gen)) (snd-print (format #f "~A not file->sample?" gen)))
	(if (or (fneq (vct-ref v0 1) -0.009) (fneq (vct-ref v0 7) .029)) (snd-print (format #f ";file->sample output: ~A" v0))))
	...
	(if (not (file->frame? gen)) (snd-print (format #f "~A not file->frame?" gen)))
	(if (or (fneq (vct-ref v0 1) -0.009) (fneq (vct-ref v0 7) .029)) (snd-print (format #f ";file->frame output: ~A" v0))))
	...
	(if (not (mus-output? gen)) (snd-print (format #f ";~A not output?" gen)))
	(if (not (sample->file? gen)) (snd-print (format #f ";~A not sample->file?" gen)))
	(do ((i 0 (1+ i)))
	    ((= i 100))
	...
	(if (not (= (mus-channels gen) 2)) (snd-print (format #f ";make-sample->file chans: ~A?" (mus-channels gen))))
	(if (not (mus-input? gen)) (snd-print (format #f "~A not input?" gen)))
	(if (or (fneq val0 .02) (fneq val1 .2)) (snd-print (format #f ";in-any: ~A ~A?" val0 val1)))
	(if (or (fneq val2 .03) (fneq val3 .3)) (snd-print (format #f ";inab: ~A ~A?" val2 val3)))
	(if (or (fneq val4 .04) (fneq val5 .4)) (snd-print (format #f ";sample->file: ~A ~A?" val4 val5)))
	(if (or (fneq val6 .065) (fneq val7 .65)) (snd-print (format #f ";outab: ~A ~A?" val6 val7)))
	(if (or (fneq val8 .075) (fneq val9 .75)) (snd-print (format #f ";out-any: ~A ~A?" val8 val9))))

      (let ((gen (make-frame->file "fmv1.snd" 2 mus-bshort mus-next)))
	(if (not (mus-output? gen)) (snd-print (format #f ";~A not output?" gen)))
	(if (not (frame->file? gen)) (snd-print (format #f ";~A not frame->file?" gen)))
	...
	(if (or (fneq (frame-ref val4 0) .04) (fneq (frame-ref val4 1) .4))
	    (snd-print (format #f ";frame->file output: ~A?" val4)))
	(file->frame gen 40 frout)
	(if (not (equal? frout val4))
	    (snd-print (format #f ";frame->file output via frame: ~A ~A?" frout val4))))
	...
	(if (not (= (mus-channels gen) 4)) (snd-print (format #f ";make-sample->file (4) chans: ~A?" (mus-channels gen))))
	(if (or (fneq val0 .04) (fneq val1 .06)) (snd-print (format #f ";in-any(4): ~A ~A?" val0 val1)))
	(if (or (fneq val2 .12) (fneq val3 .18)) (snd-print (format #f ";sample->file(4): ~A ~A?" val2 val3)))
	(if (or (fneq val4 .14) (fneq val5 .21)) (snd-print (format #f ";out-any(4): ~A ~A?" val4 val5))))
	...
	(let ((v1 (make-vct 1000)))
	  ...
	  (do ((i 0 (1+ i)))
	      ((= i 1000))
	    (if (fneq (vct-ref v0 i) (vct-ref v1 i)) (snd-print (format #f ";array->file->array: ~A ~A ~A?" i (vct-ref v0 i) (vct-ref v1 i)))))))
	...
	(if (not (rand? gen)) (snd-print "~A not rand?" gen))
	(if (fneq (mus-phase gen) 3.3624296) (snd-print (format #f ";rand phase: ~F?" (mus-phase gen))))
	(if (fneq (mus-frequency gen) 10000.0) (snd-print (format #f ";rand frequency: ~F?" (mus-frequency gen))))
	(if (= (vct-ref v0 1) (vct-ref v0 8)) (snd-print (format #f ";rand output: ~A" v0))))
	...
	(if (not (rand-interp? gen)) (snd-print "~A not rand-interp?" gen))
	(if (fneq (mus-phase gen) 5.114882) (snd-print (format #f ";rand-interp phase: ~F?" (mus-phase gen))))
	(if (fneq (mus-frequency gen) 4000.0) (snd-print (format #f ";rand-interp frequency: ~F?" (mus-frequency gen))))
	(if (= (vct-ref v0 1) (vct-ref v0 8)) (snd-print (format #f ";rand-interp output: ~A" v0))))
      
      (let* ((gen (make-locsig 30.0 :channels 2))
	     (fr0 (locsig gen 0 1.0)))
	(if (not (locsig? gen)) (snd-print (format #f "~A not locsig?" gen)))
	(if (or (fneq (frame-ref fr0 0) .667) (fneq (frame-ref fr0 1) .333)) (snd-print (format #f ";locsig output: ~A" fr0)))
	(if (or (fneq (locsig-ref gen 0) .667) (fneq (locsig-ref gen 1) .333))
	    (snd-print (format #f ";locsig ref: ~F ~F?" (locsig-ref gen 0) (locsig-ref gen 1))))
	(locsig-set! gen 0 .5)
	(if (rs .1) (gc))
	(set! fr0 (locsig gen 0 1.0))
	(if (fneq (frame-ref fr0 0) .5) (snd-print (format #f ";locsig-set: ~F?" (frame-ref fr0 0))))
	(set! gen (make-locsig 120.0 2.0 .1 :channels 4))
	(set! fr0 (locsig gen 0 1.0))
	(if (or (fneq (frame-ref fr0 1) .333) (fneq (frame-ref fr0 2) .167)) (snd-print (format #f ";locsig quad output: ~A" fr0))))
	...
	(do ((i 0 (1+ i)))
	    ((= i 100))
	  (locsig lc i 1.0))
	(if (fneq (locsig-reverb-ref lc 0) .1) (snd-print (format #f ";locsig reverb ref: ~A?" (locsig-reverb-ref lc 0))))
	(locsig-reverb-set! lc 0 .2)
	(if (fneq (locsig-reverb-ref lc 0) .2) (snd-print (format #f ";locsig reverb set: ~A?" (locsig-reverb-ref lc 0))))
	(mus-close gen)
	(mus-close rev)
	(let ((v0 (make-vct 100))
	      (v1 (make-vct 100))
	  ...
	  (if (fneq (vct-ref v2 0) .1) (snd-print (format #f ";locsig reverb: ~A?" v2)))
	  (if (fneq (* 2 (vct-ref v0 0)) (vct-ref v1 0)) (snd-print (format #f ";locsig direct: ~A?" (vct-ref v0 0) (vct-ref v1 0))))))
	...
	(if (not (src? gen)) (snd-print (format #f ";~A not scr?" gen)))
	(if (or (fneq (vct-ref v0 1) .001) (fneq (vct-ref v0 7) .021)) (snd-print (format #f ";src output: ~A" v0)))
	(if (fneq (mus-increment gen) 2.0) (snd-print (format #f ";src increment: ~F?" (mus-increment gen)))))
	...
	(if (= (vct-peak v0) 0.0) (snd-print (format #f ";granulate output peak: ~F?" (vct-peak v0))))
	(if (not (granulate? gen)) (snd-print (format #f ";~A not granulate?" gen)))
	(if (fneq (mus-increment gen) 2.0) (snd-print (format #f ";granulate increment: ~F?" (mus-increment gen))))
	(if (fneq (mus-scaler gen) 0.6) (snd-print (format #f ";granulate scaler: ~F?" (mus-scaler gen))))
	(if (fneq (mus-frequency gen) 0.05) (snd-print (format #f ";granulate frequency: ~F?" (mus-frequency gen))))
	(if (not (= (mus-ramp gen) 1323)) (snd-print (format #f ";granulate ramp: ~F?" (mus-ramp gen))))
	(if (not (= (mus-length gen) 3308)) (snd-print (format #f ";granulate length: ~A?" (mus-length gen))))
	(if (not (= (mus-hop gen) 1102)) (snd-print (format #f ";granulate hop: ~A?" (mus-hop gen))))
	(mus-set-hop gen 1000) (if (not (= (mus-hop gen) 1000)) (snd-print (format #f ";granulate set-hop: ~A?" (mus-hop gen))))
	(mus-set-ramp gen 1000) (if (not (= (mus-ramp gen) 1000)) (snd-print (format #f ";granulate set-ramp: ~A?" (mus-ramp gen))))
	(mus-set-length gen 3000) (if (not (= (mus-length gen) 3000)) (snd-print (format #f ";granulate set-length: ~A?" (mus-length gen))))
	(mus-set-increment gen 3.0)
	(if (> (abs (- (mus-increment gen) 3.0)) .01) (snd-print (format #f ";granulate set-increment: ~F?" (mus-increment gen)))))
	...
	(let ((gen (make-convolve :filter v0))
	      (n -1))
	  (if (not (convolve? gen)) (snd-print (format #f ";~A not convolve?" gen)))
	  (if (not (= (mus-length gen) 64)) (snd-print (format #f ";convolve fft len: ~D?" (mus-length gen))))
	  (do ((i 0 (1+ i)))
	      ((= i 128))
	    (if (rs .01) (gc))
	...
	(if (not (sound-data? data)) (snd-print (format #f ";~A not sound-data?" data)))
	(if (not (= (sound-data-chans data) 1)) (snd-print (format #f ";sound-data chans: ~A?" (sound-data-chans data))))
	(if (not (= (sound-data-length data) 2000)) (snd-print (format #f ";sound-data length: ~A?" (sound-data-length data))))
	(mus-sound-read fd 0 1999 chans data)
	(let ((val (sound-data-ref data 0 1497)))
	  (mus-sound-close-input fd)
	  (if (fneq val 0.02893066) (snd-print (format #f ";mus-sound-read: ~F?" val)))))

      (let ((nind (new-sound "fmv.snd" mus-aifc mus-bshort 22050 1 "this is a comment")))
	(fm-violin 0 1 440 .1) (w)
	(fofins 1 1 270 .2 .001 730 .6 1090 .3 2440 .1) (w)
	(scissor 2.0) (w)
	(play-and-wait 0 nind)
	(save-sound nind)
	(let ((oboe-index (or (find-sound "oboe.snd") (open-sound "oboe.snd"))))
	  (cnvtest oboe-index nind .1) (w)
	  (select-sound nind)
	  (select-channel 0)
	  (if (not (= (selected-sound) nind)) (snd-print (format #f ";selected-sound: ~A?" (selected-sound))))
	  (if (not (= (selected-channel) 0)) (snd-print (format #f ";selected-channel: ~A?" (selected-channel))))
	  (jc-reverb 1.0 #f .1 #f) (w)
	  (play-and-wait 0 nind)
	  (voiced->unvoiced 1.0 256 2.0 2.0) (w)
	  (map-chan (fltit))
	...
	(set-sample 0 1.0 nind) (w)
	(convolve-with "oboe.snd") (w)
	(if (fneq (sample 1000) -0.22299) (snd-print (format #f ";convolve-with: ~A?" (sample 1000))))
	(revert-sound nind)
	(set-sample 200 .0001) (w)
	(set-sample 100 1.0) (w)
	(smooth 0 100) (w)
	(if (or (fneq (sample 50) .5) (fneq (sample 30) 0.20608) (fneq (sample 90) 0.9755))
	    (snd-print (format #f ";smooth: ~A ~A ~A?" (sample 50) (sample 30) (sample 90))))
	(undo) (w)
	(set-sinc-width 40) (w)
	(set-sample 100 0.5) (w)
	(if (fneq (sample 100) 0.5) (snd-print (format #f ";set-sample 100: ~A?" (sample 100))))
	(src-sound .1) (w)
	(if (or (fneq (sample 1000) 0.5) (fneq (sample 1024) 0.0625) (fneq (sample 1010) 0.0))
	    (snd-print (format #f ";src-sound: ~A ~A ~A?" (sample 1000) (sample 1024) (sample 1010))))
	(revert-sound)
	(close-sound nind))
      (let ((nind (new-sound "fmv.snd" mus-riff mus-lshort 22050 1 "this is a comment")))
	(mix "pistol.snd") (w)
	(map-chan (expsrc 2.0)) (w)
	(play-and-wait 0 nind)
	(undo) (w)
	(let ((eds (edits)))
	  (if (or (not (= (car eds) 1)) (not (= (cadr eds) 1)))
	      (snd-print (format #f ";undo edits: ~A?" eds)))
	  (if (not (= (edit-position) (car eds)))
	      (snd-print (format #f ";undo edit-position: ~A ~A?" (edit-position) eds))))
	(expsnd '(0 1 2 .4)) (w)
	(map-chan (comb-chord .95 100 .3)) (w)
	(map-chan (formants .99 900 .02 1800 .01 2700)) (w)
	(map-chan (moving-formant .99 '(0 1200 1 2400))) (w)
	(scale-to .3) (w)
	(play-and-wait 0)
	(let ((eds (edits)))
	  (if (or (not (= (car eds) 6)) (not (= (cadr eds) 0)))
	      (snd-print (format #f ";edits(6): ~A?" eds)))
	  (if (not (= (edit-position) (car eds)))
	      (snd-print (format #f ";edit-position(6): ~A ~A?" (edit-position) eds))))
	(revert-sound nind)
	(mix "pistol.snd") (w)
	(map-chan (zecho .5 .75 6 10.0) 0 65000) (w)
	(map-chan (am 440)) (w)
	(reverse-sound nind) (w)
	(play-and-wait 0 nind)
	(revert-sound nind)
	(mix-sound "pistol.snd" 0) (w)
	(hello-dentist 40.0 .1) (w)
	(fp 1.0 .3 20) (w)
	(play-and-wait 0 nind)
	(revert-sound nind)
	...
	(do ((i 0 (1+ i))) ((= i 12)) (if (fneq (vct-ref v0 i) (+ 0.1 (* i .01))) (snd-print (format #f ";mus-mix(1->1): ~A?" v0))))
	(mus-mix "fmv.snd" "fmv2.snd" 3 9 0 (make-mixer 2 0.3 0.0 0.7 0.0))
	(file->array "fmv.snd" 0 0 12 v0)
	(if (or (fneq (vct-ref v0 0) .1) (fneq (vct-ref v0 3) .33) (fneq (vct-ref v0 9) .19)) (snd-print (format #f ";mus-mix(2->1): ~A?" v0)))
	(mus-mix "fmv.snd" "fmv3.snd")
	(file->array "fmv.snd" 0 0 12 v0)
	(if (or (fneq (vct-ref v0 0) .4) (fneq (vct-ref v0 3) .33)) (snd-print (format #f ";mus-mix(4->1): ~A?" v0)))
	(let ((e0 (make-env '(0 0 1 1) :end 9))
	  ...
	  (file->array "fmv.snd" 0 0 12 v0)
	  (if (or (fneq (vct-ref v0 0) .4) (fneq (vct-ref v0 3) .360) (fneq (vct-ref v0 9) .28)) (snd-print (format #f ";mus-mix(env): ~A?" v0))))
	(delete-file "fmv.snd")
	(do ((i 0 (1+ i))) ((= i 12)) (vct-set! v0 i (* i .01)))
	(array->file "fmv.snd" v0 12 22050 4)
	(mus-mix "fmv.snd" "fmv1.snd")
	(file->array "fmv.snd" 0 0 3 v0) ; chan 0 start 0 len 3
	(if (or (fneq (vct-ref v0 0) .1) (fneq (vct-ref v0 2) .18)) (snd-print (format #f ";mus-mix(1->4): ~A?" v0)))
	(mus-mix "fmv.snd" "fmv2.snd"  0 3 0 (make-mixer 2 0.3 0.0 0.7 0.0))
	(file->array "fmv.snd" 0 0 3 v0)
	(if (or (fneq (vct-ref v0 0) .3) (fneq (vct-ref v0 2) .38)) (snd-print (format #f ";mus-mix(2->4): ~A?" v0)))
...
(if (or full-test (= snd-test 9))
	  ...
	  (let ((pos (mix-position mix-id))
		...
		((= i 100))
	      (let ((mx (next-mix-sample mr))
		    (sx (sample (+ 100 i))))
		(if (fneq mx sx) (snd-print (format #f ";next-mix-sample: ~A ~A?" mx sx)))))
	    (free-mix-sample-reader mr)
	    (if (not (= pos 100)) (snd-print (format #f ";mix-position: ~A?" pos)))
	    (if (not (= len 41623)) (snd-print (format #f ";mix-length: ~A?" len)))
	    (if loc (snd-print (format #f ";mix-locked: ~A?" loc)))
	    (if (not (= anc 0)) (snd-print (format #f ";mix-anchor: ~A?" anc)))
	    (if (not (= trk 0)) (snd-print (format #f ";mix-track: ~A?" trk)))
	    ...
	    (play-mix mix-id) (w)
	    (set-mix-position mix-id 200) (w)
	    (set-mix-name mix-id "asdf") (w)
	    (set-mix-amp mix-id 0 0.5) (w)
	    (set-mix-speed mix-id 2.0) (w)
	    (set-mix-track mix-id 3) (w)
	    (set-mix-console-state mix-id 2) (w)
	    (set-mix-anchor mix-id 30) (w)
	    (set-mix-amp-env mix-id 0 '(0.0 0.0 1.0 1.0)) (w)
	    (set-mix-console-y mix-id 20) (w)
	    (let ((pos (mix-position mix-id))
		  (spd (mix-speed mix-id))
		  (trk (mix-track mix-id))
		  (amp (mix-amp mix-id 0))
	...
	(let ((mix-id (mix "oboe.snd" 100)))
	  ...
	  (if (not (= (srate (list mix-id)) 22050)) (snd-print (format #f ";mix oboe: srate ~D?" (srate (list mix-id)))))
	  (if (not (= (data-location (list mix-id)) 28)) (snd-print (format #f ";mix oboe: location ~D?" (data-location (list mix-id)))))
	  (if (not (= (car (data-format (list mix-id))) 1)) (snd-print (format #f ";mix oboe: format ~A?" (data-format (list mix-id)))))
	  (if (fneq (maxamp (list mix-id)) .14724) (snd-print (format #f ";mix oboe: maxamp ~F?" (maxamp (list mix-id)))))
	  (if (not (string=? (comment (list mix-id)) "")) (snd-print (format #f ";mix oboe: comment ~A?" (comment (list mix-id)))))
	  (if (not (string=? (short-file-name (list mix-id)) "oboe.snd")) (snd-print (format #f ";mix oboe short name: ~S?" (short-file-name (list mix-id)))))
	  (let ((matches (count-matches (lambda (a) (and a (> a .125))) 0 (list mix-id))))
	    (if (not (= matches 1313)) (snd-print (format #f ";mix count-matches: ~A?" matches))))
	  (let ((spot (find (lambda (a) (and a (> a .13))) 0 (list mix-id))))
	    (if (or (null? spot) (not (= (cadr spot) 8862))) (snd-print (format #f ";mix find: ~A?" spot))))
	  (let ((eds (edits (list mix-id))))
	    (if (or (not (= (car eds) 0)) (not (= (cadr eds) 0)))
		(snd-print (format #f ";mix edits: ~A?" eds)))
	    (if (not (= (edit-position (list mix-id)) (car eds)))
		(snd-print (format #f ";mix edit-position: ~A ~A?" (edit-position (list mix-id)) eds))))
	  (let ((samps1 (samples->vct 0 50828 (list mix-id) 0))
		(vr (make-sample-reader 0 (list mix-id) 0 1)))
	    (if (not (sample-reader? vr)) (snd-print (format #f ";(mix) ~A not sample-reader?" vr)))
	    (do ((i 0 (1+ i)))
		((= i 50828))
	      (if (not (= (next-sample vr) (vct-ref samps1 i)))
		  (snd-print (format #f ";(mix) readers disagree at ~D" i))))
	    (if (rs .5) (gc))
	    (free-sample-reader vr))
	  (insert-sample 100 .5 (list mix-id)) (w)
	  (if (or (fneq (sample 100 (list mix-id)) .5)
		  (not (= (frames (list mix-id)) 50829)))
	      (snd-print (format #f ";(mix) insert-sample: ~A ~A?" (sample 100 (list mix-id)) (frames (list mix-id)))))
	  (let ((v0 (make-vector 3))
		(v1 (make-vct 3)))
	    (if (rs .5) (gc))
	    (vct-fill! v1 .75)
	    (do ((i 0 (1+ i))) ((= i 3)) (vector-set! v0 i .25))
	    (insert-samples 200 3 v0 (list mix-id)) (w)
	    (insert-samples 300 3 v1 (list mix-id)) (w)
	    (if (or (fneq (sample 201 (list mix-id)) .25)
		    (fneq (sample 301 (list mix-id)) .75)
		    (not (= (frames (list mix-id)) 50835)))
		(snd-print (format #f ";(mix) insert-samples: ~A ~A ~A?" (sample 201 (list mix-id)) (sample 301 (list mix-id)) (frames (list mix-id))))))
	  (revert-sound (list mix-id))
	  (set-sample 50 .5 (list mix-id)) (w)
	  (if (fneq (sample 50 (list mix-id)) .5) (snd-print (format #f ";(mix) set-sample: ~A?" (sample 50 (list mix-id)))))
	  (let ((v0 (make-vector 3)))
	    (do ((i 0 (1+ i))) ((= i 3)) (vector-set! v0 i .25))
	    (set-samples 60 3 v0 (list mix-id)) (w)
	    (if (or (fneq (sample 60 (list mix-id)) .25) (fneq (sample 61 (list mix-id)) .25))
		(snd-print (format #f ";(mix) set-samples: ~A ~A ~A?" (sample 60 (list mix-id)) (sample 61 (list mix-id)) (sample 62 (list mix-id))))))
	  (revert-sound (list mix-id))
	  (delete-sample 100 (list mix-id)) (w)
	  (if (not (= (frames (list mix-id)) 50827)) (snd-print (format #f ";(mix) delete-sample: ~A?" (frames (list mix-id)))))
	  (delete-samples 0 100 (list mix-id)) (w)
	  (if (not (= (frames (list mix-id)) 50727)) (snd-print (format #f ";(mix) delete-samples: ~A?" (frames (list mix-id)))))
	  (revert-sound (list mix-id))
	  (let ((maxa (maxamp (list mix-id))))
	  (scale-to .5 (list mix-id)) (w)
	  (let ((newmaxa (maxamp (list mix-id))))
	    (if (fneq newmaxa .5) (snd-print (format #f ";(mix) scale-to: ~A?" newmaxa)))
	    (undo 1 (list mix-id)) (w)
	    (scale-by 2.0 (list mix-id)) (w)
	    (set! newmaxa (maxamp (list mix-id)))
	    (if (fneq newmaxa (* 2.0 maxa)) (snd-print (format #f ";(mix) scale-by: ~A?" newmaxa)))
	    (revert-sound (list mix-id)))))
	...
	(let ((mix-ids (make-vector 6)))
	  ...
	  (if (not (= (track-position (track 1)) 0)) (snd-print (format #f ";track-position: ~D?" (track-position (track 1)))))
	  (let ((curend (track-end (track 1))))
	    (set-track-position (track 1) 500)
	    (if (not (= (track-position (track 1)) 500)) (snd-print (format #f ";set-track-position: ~D?" (track-position (track 1)))))
	    (if (not (= (mix-position (vector-ref mix-ids 0)) 500)) (snd-print (format #f ";track-position ~D = ~D?" 0 (mix-position (vector-ref mix-ids 0)))))
	    (if (not (= (mix-position (vector-ref mix-ids 1)) 1000)) (snd-print (format #f ";track-position ~D = ~D?" 1 (mix-position (vector-ref mix-ids 1)))))
	    (if (not (= (mix-position (vector-ref mix-ids 4)) 4500)) (snd-print (format #f ";track-position ~D = ~D?" 4 (mix-position (vector-ref mix-ids 4)))))
	    (if (not (= (track-end (track 1)) (+ curend 500))) (snd-print (format #f ";track-end: ~D (~D)?" (track-end (track 1)) (+ curend 500)))))
	  (if (not (= (track-length (track 1)) (+ (frames (list (vector-ref mix-ids 0))) 4000)))
	      (snd-print (format #f ";track-length: ~D (~D)?" (track-length (track 1)) (+ 4000 50828))))
	  (set-track-amp (track 1) .5)
	  (if (fneq (mix-amp (vector-ref mix-ids 2) 0) .5) (snd-print (format #f ";set-track-amp ~F?" (mix-amp (vector-ref mix-ids 2) 0))))
	  (incf-track-amp (track 1) .25)
	  (if (fneq (mix-amp (vector-ref mix-ids 4) 0) .75) (snd-print (format #f ";incf-track-amp ~F?" (mix-amp (vector-ref mix-ids 4) 0))))
	  (transpose-track (track 1) 12)
	  (if (fneq (mix-speed (vector-ref mix-ids 2)) 2.0)
	      (snd-print (format #f ";transpose-track: ~F?" (mix-speed (vector-ref mix-ids 2)))))
	  (if (not (= (frames (list (vector-ref mix-ids 1))) 50828))
	      (snd-print (format #f ";transpose-track mixup: ~D (~D)?" (frames (list (vector-ref mix-ids 1))) 50828)))
	  (set-track-tempo (track 1) 2.0)
	  (if (not (= (track-length (track 1)) (/ (+ 4000 50828) 2)))
	      (snd-print (format #f ";track-tempo: ~D (~D)?" (track-length (track 1)) (/ (+ 4000 50828) 2))))
	  ...
	  (let ((t2 (track->vct (track 2)))
		...
		(snd-print (format #f ";track->vct: ~F, mix->vct: ~F (.0328369)?" (vct-ref t2 1000) (vct-ref t3 1000)))))
	...
	(let ((v1 (envelope-interp 1.0 '(0 0 2.0 1.0)))
	  ...
	  (if (fneq v1 0.5) (snd-print (format #f ";envelope-interp(1): ~F (0.5)?" v1)))
	  (if (fneq v2 1.0) (snd-print (format #f ";envelope-interp(2): ~F (1.0)?" v2)))
	  (if (fneq v3 1.0) (snd-print (format #f ";envelope-interp(3): ~F (1.0)?" v3)))
	  (if (fneq v4 0.5) (snd-print (format #f ";envelope-interp(4): ~F (0.5)?" v4))))
	(let ((v1 (multiply-envelopes '(0.0 0.0 2.0 0.5) '(0.0 0.0 1.0 2.0 2.0 1.0)))
	      (v2 (window-envelope 1.0 3.0 '(0.0 0.0 5.0 1.0))))
	  (if (not (feql v1 (list 0.0 0.0 0.5 0.5 1.0 0.5))) (snd-print (format #f ";multiply-envelopes: ~A?" v1)))
	  (if (not (feql v2 (list 1.0 0.2 3.0 0.6))) (snd-print (format #f ";window-envelope: ~A?" v2))))
...
(if (or full-test (= snd-test 10))
	...
	(vector-set! vc 2 (mix-vct v0 2000 1 ind0))
	(vector-set! vc 3 (mix-vct v0 3000 1 ind0))
	(vector-set! vc 4 (mix-vct v0 4000 1 ind0))
	(vector-set! vc 5 (mix-vct v0 0 1 ind1))
	(vector-set! vc 6 (mix-vct v0 1000 1 ind1))
	(vector-set! vc 7 (mix-vct v0 2000 1 ind1))
	(vector-set! vc 8 (mix-vct v0 3000 1 ind1))
	(vector-set! vc 9 (mix-vct v0 4000 1 ind1))
	(let ((t0 (make-track 1 (list (vector-ref vc 0) (vector-ref vc 3) (vector-ref vc 5))))
	  ...
	  (let ((t0e (track-end t0)))
	    ...
	    (if (not (= (track-position t0) 1000)) (snd-print (format #f ";track-position: ~A?" (track-position t0))))
	    (if (not (= (track-end t0) (+ t0e 1000))) (snd-print (format #f ";track-end: ~A ~A?" t0e (track-end t0)))))
	  (if (not (= (track-length t0) 3100)) (snd-print (format #f ";track-length: ~A?" (track-length t0))))
	  (set-track-tempo t0 2.0)
	  (let ((col (color->list (track-color t1))))
	    (if (or (fneq (car col) 0.0) (fneq (cadr col) 0.0) (fneq (caddr col) 1.0))
		(snd-print (format #f ";track-color: ~A?" col))))
	  (if (not (= (track-length t0) 1600)) (snd-print (format #f ";track-tempo -> length: ~A?" (track-length t0)))))
	(close-sound ind0)
	(close-sound ind1)
	...
	(let ((v0 (make-vector 10)))
	  ...
	  (insert-samples 0 10 v0 ind0) (w)
	  (env-sound '(0 0 1 1) 0 10 1.0 ind0) (w)
	  (do ((i 0 (1+ i))) ((= i 10)) (if (fneq (sample i) (* i .1)) (snd-print (format #f ";env-sound[~D]: ~A?" i (sample i)))))
	  (undo) (w)
	  (do ((i 1 (1+ i))) ((= i 10)) (set-sample i 0.0))
	  (filter-sound '(0 1 1 0) 4) (w)
	  (if (or (fneq (sample 1) 0.3678) (fneq (sample 2) .3678) (fneq (sample 3) .132) (fneq (sample 4) 0.0))
	      (snd-print (format #f ";filter-sound env: ~A?" (samples 0 8))))
	  (undo)
	  (let ((vc0 (make-vct 4)))
	    (vct-set! vc0 0 .125) (vct-set! vc0 1 .25) (vct-set! vc0 2 .25) (vct-set! vc0 3 .125) 
	    (filter-sound vc0 4) (w)
	    (if (or (fneq (sample 0) 0.125) (fneq (sample 1) .25) (fneq (sample 2) .25) (fneq (sample 5) 0.0))
		(snd-print (format #f ";filter-sound direct: ~A?" (samples 0 8))))
	    (revert-sound)))
	(close-sound ind0)
	...
	(let ((v0 (make-vct 10))
	  ...
	  (do ((i 0 (1+ i))) ((= i 10)) 
	    (if (fneq (sample (+ i 10) ind0) (* i .02)) (snd-print (format #f ";env-sound [~D]: ~A?" (+ i 10) (sample (+ i 10) ind0)))))
	  (if (fneq (sample 5 ind0) old5) (snd-print (format #f ";env-sound 5: ~A ~A?" old5 (sample 5 ind0))))
	  (undo)
	  (env-sound '(0 0 1 2) 10 10 4.0 ind0)
	  (set! v0 (samples->vct 10 10))
	  (if (or (fneq (vct-ref v0 3) 0.034) (fneq (vct-ref v0 8) .135)) (snd-print (format #f ";env-sound 4: ~A" v0)))
	  (undo)
	  (env-sound '(0 0 1 2) 10 10 .05 ind0)
	  (set! v0 (samples->vct 10 10))
	  (if (or (fneq (vct-ref v0 3) 0.125) (fneq (vct-ref v0 8) .191)) (snd-print (format #f ";env-sound 05: ~A" v0))))
	...
	(let ((v0 (make-vector 10)))
	  (do ((i 0 (1+ i))) ((= i 10)) (vector-set! v0 i 1.0))
	  (insert-samples 0 10 v0 ind0 0) (w)
	  (do ((i 0 (1+ i))) ((= i 10)) (vector-set! v0 i 0.1))
	  (insert-samples 0 10 v0 ind0 1) (w)
	  (do ((i 0 (1+ i))) ((= i 10)) (vector-set! v0 i 0.01))
	  (insert-samples 0 10 v0 ind1 0) (w)
	  (let ((val (scan-chan (data-max1) 0 9 ind0 0)))
	    (if (fneq val 1.0) (snd-print (format #f ";scan-chan[0,0]: ~A?" val))))
	  (let ((val (scan-chan (data-max1) 0 9 ind0 1)))
	    (if (fneq val 0.1) (snd-print (format #f ";scan-chan[0,1]: ~A?" val))))
	  (let ((val (scan-chan (data-max1) 0 9 ind1 0)))
	    (if (fneq val 0.01) (snd-print (format #f ";scan-chan[1,0]: ~A?" val))))
	  (let ((val (scan-chans (data-max1) 0 9)))
	    (if (fneq val 0.01) (snd-print (format #f ";scan-chans: ~A?" val))))
	  (let ((val (scan-all-chans (data-max1) 0 9)))
	    (if (fneq val 1.0) (snd-print (format #f ";scan-all-chans: ~A?" val))))
	  (let ((val (scan-across-sound-chans (data-max) 0 9 ind0)))
	    (if (fneq val 1.0) (snd-print (format #f ";scan-across-sound-chans: ~A?" val)))))
	(close-sound ind0)
	(close-sound ind1)
	...
	(let ((fd (open-sound "oboe.snd"))
	  ...
	  (let ((m2 (add-mark 12345 fd 0)))
	    ...
	    (if (not (= (mark-sync m2) sync-val)) (snd-print (format #f ";set-mark-sync (~A): ~A?" sync-val (mark-sync m2))))
	    (let* ((syncs (syncd-marks sync-val))
		   (chans (marks fd 0))
		   (samps (map mark-sample chans)))
	      (if (not (equal? syncs (list m2))) (snd-print (format #f ";syncd-marks: ~A?" syncs)))
	      (if (not (equal? chans (list m1 m2))) (snd-print (format #f ";marks: ~A?" chans)))
	      (if (not (equal? samps (list (mark-sample m1) (mark-sample m2)))) (snd-print (format #f ";map samps: ~A?" samps)))
	      (delete-samples 200 100 fd 0)
	      (set! chans (marks fd))
	      (set! samps (map mark-sample (car chans)))
	      (if (not (equal? samps (list (mark-sample m1 0) (- (mark-sample m2 0) 100)))) (snd-print (format #f ";map samps: ~A?" samps)))
	      (let ((descr (describe-mark m2)))
		(if (not (equal? descr (list (list 'mark m2 'sound fd "oboe.snd" 'channel 0) 12345 12245)))
		    (snd-print (format #f ";describe-mark: ~A?" descr))))
	      (set-mark-sync m1 (mark-sync m2))
	      (move-syncd-marks sync-val 100)
	      (set! chans (marks fd))
	      (set! samps (map mark-sample (car chans)))
	      (if (not (equal? samps (list (+ (mark-sample m1 0) 100) (mark-sample m2 0)))) (snd-print (format #f ";syncd move samps: ~A?" samps)))
	      (set-cursor 500)
	      (backward-mark)
	      (if (not (= (cursor) (mark-sample m1))) (snd-print (format #f ";backward-mark: ~A?" (cursor))))
	      (forward-mark 1)
	      (if (not (= (cursor) (mark-sample m2))) (snd-print (format #f ";forward-mark: ~A?" (cursor))))
	      (delete-mark m1)
	      (set! chans (marks fd 0))
	      (if (not (equal? chans (list m2))) (snd-print (format #f ";delete-mark? ~A" chans)))
	      (undo)
	      (set! chans (marks fd 0))
	      (if (not (equal? chans (list m1 m2))) (snd-print (format #f ";delete-mark then undo? ~A" chans)))
	      (redo)
	      (if (not (string=? (mark-name m2) "")) (snd-print (format #f ";init mark-name: ~A?" (mark-name m2))))
	      (set-mark-name m2 "hiho!")
	      (if (not (string=? (mark-name m2) "hiho!")) (snd-print (format #f ";set-mark-name: ~A?" (mark-name m2))))
	      (undo)
	      (if (not (string=? (mark-name m2) "")) (snd-print (format #f ";undo mark-name: ~A?" (mark-name m2))))
	      (redo)
		...
		(if (not (equal? samps '(1000 2000 3500 4500))) (snd-print (format #f ";insert ripple: ~A?" samps)))
		(set-mark-sample m3 300)
		(set-cursor 500)
		(backward-mark)
		(if (not (= (cursor) 300)) (snd-print (format #f "sort marks: ~A?" (cursor))))
		(if (not (equal? (mark->sound m2) (list fd 0))) (snd-print (format #f ";mark->sound: ~A?" (mark->sound m2))))
		(let ((sd (open-sound "4.aiff")))
		  (set! m3 (add-mark 1000 sd 2))
		  (set! m4 (add-mark 1000 sd 3))
		  (if (not (equal? (mark->sound m3) (list sd 2))) (snd-print (format #f ";marks->sound 4: ~A?" (mark->sound m3))))
		  (close-sound sd))
	      (close-sound fd))))))))
...
(if (or full-test (= snd-test 11))
    (begin
	 ...
	 (begin
	   ...
	   (if (not (= (recorder-in-format) mus-mulaw)) (snd-print (format #f ";set-recorder-in-format: ~A?" (recorder-in-format))))
	   (set-recorder-out-format mus-mulaw)
	   (if (not (= (recorder-out-format) mus-mulaw)) (snd-print (format #f ";set-recorder-out-format: ~A?" (recorder-out-format))))
	   (set-recorder-srate 44100)
	   (if (not (= (recorder-srate) 44100)) (snd-print (format #f ";set-recorder-srate: ~A?" (recorder-srate))))
	   (set-recorder-gain 0 0.5)
	   (if (fneq (recorder-gain 0) 0.5) (snd-print (format #f ";set-recorder-gain: ~A?" (recorder-gain 0))))
	   (set-recorder-out-amp 0 0.5)
	   (if (> (abs (- (recorder-out-amp 0) 0.5)) .01) (snd-print (format #f ";set-recorder-out-amp: ~A?" (recorder-out-amp 0))))
...
(if (or full-test (= snd-test 12))
	...
	(let ((open-files '())
	  ...
	  (if (not (= (length (sounds)) 0)) (snd-print (format #f ";active-sounds: ~A?" (sounds))))
	  (let* ((name (string-append sf-dir (vector-ref sf-dir-files 0)))
		 (index (view-sound name)))
	    (if (not (string=? name (file-name index))) (snd-print (format #f ";file-name: ~A?" (file-name index))))
	    (close-sound index))
...
(if (or full-test (= snd-test 13))
	...
	(if (not (equal? (edits) (list (+ (car eds) 1) (cadr eds)))) (snd-print (format #f ";copyfile-1 (select eds): ~A ~A?" eds (edits)))))
	  ...
	  (let ((v0 (transform-samples->vct fd))
	    ...
	    (if (and v0 vc)
		(begin
		  (if (fneq val (vector-ref vc 50)) (snd-print (format #f ";transform-sample: ~A ~A?" val (vector-ref vc 50))))
		  (do ((i 0 (1+ i))) ((= i 100)) 
		    (if (fneq (vector-ref vc i) (vct-ref v0 i)) 
			(snd-print (format #f ";transform-samples[~D]: ~A ~A?" i (vector-ref vc i) (vct-ref v0 i))))))
...
(if (or full-test (= snd-test 14))
	...
	(let* ((len (length open-files))
	  ...
	  (let ((choose-fd (lambda () (w) (list-ref open-files (my-random open-ctr)))))

	    (if (> tests 1) (snd-print (format #f ";main test ~D " test-ctr)))

	    (let* ((frame-list (map frames open-files))
		   (curloc (min 1200 (1- (list-ref frame-list 0))))
		   (curfd (choose-fd))
		   (old-marks (length (marks curfd 0))))
	      (set-x-bounds 0.0 (min (duration curfd) 1.0) curfd)
	      (let ((xb (x-bounds curfd)))
		(if (or (fneq (car xb) 0.0) (fneq (cadr xb) (min (duration curfd) 1.0))) (snd-print (format #f ";x-bounds: ~A?" xb))))
	      (set-y-bounds -0.5 0.5 curfd)
	      (let ((yb (y-bounds curfd)))
		(if (or (fneq (car yb) -0.5) (fneq (cadr yb) 0.5)) (snd-print (format #f ";y-bounds: ~A?" yb))))
	      (set-cursor curloc curfd)
	      (let ((cl (cursor curfd)))
		(if (not (= cl curloc)) (snd-print (format #f ";cursor ~A /= ~A?" cl curloc))))
	      (let* ((id (add-mark curloc curfd)))
		(if (and (number? id) (not (= id -1)))
		    (let* ((cl (mark-sample id))
			   (new-marks (length (marks curfd 0))))
		      (if (not (= cl curloc)) (snd-print (format #f ";mark ~A /= ~A?" cl curloc)))
		      (if (not (= new-marks (1+ old-marks))) (snd-print (format #f ";marks ~A ~A?" new-marks old-marks)))
		      (let ((new-id (find-mark curloc curfd)))
			(if (not (= id new-id)) (snd-print (format #f ";find-mark (by sample): ~A ~A?" id new-id))))
		      (set-mark-name id "hiho")
		      (let ((new-id (find-mark "hiho" curfd)))
			(if (not (= id new-id)) (snd-print (format #f ";find-mark (by name): ~A ~A?" id new-id))))
		      (if (not (string=? (mark-name id) "hiho")) (snd-print (format #f ";mark name: ~A?" (mark-name id))))
		      (set-mark-sample id (max 0 (- curloc 100)))
		      (set! cl (mark-sample id))
		      (if (not (= cl (max 0 (- curloc 100)))) (snd-print (format #f ";set mark ~A /= ~A?" cl curloc)))
		      (delete-mark id)))
		(if (> (duration curfd) 1.2) (set-x-bounds 1.0 1.1 curfd))
		(add-mark 10 curfd)
		(add-mark 20 curfd)
		(key (char->integer #\m) 0 curfd)
		(set-cursor 0 curfd)
		(forward-mark 1 curfd)
		(if (not (= (cursor curfd) 10)) (snd-print (format #f ";forward-mark (10): ~A?" (cursor curfd))))
		(forward-mark 1 curfd)
		(if (not (= (cursor curfd) 20)) (snd-print (format #f ";forward-mark (20): ~A?" (cursor curfd))))
		(set-cursor 25 curfd)
		(backward-mark 2 curfd)
		(if (not (= (cursor curfd) 10)) (snd-print (format #f ";backward-mark (10): ~A?" (cursor curfd))))
		(forward-sample 5 curfd)
		(if (not (= (cursor curfd) 15)) (snd-print (format #f ";forward-sample (5): ~A?" (cursor curfd))))
		(backward-sample 1 curfd)
		(if (not (= (cursor curfd) 14)) (snd-print (format #f ";backward-sample (1): ~A?" (cursor curfd))))
		(let ((new-marks (length (marks curfd 0))))
		  (delete-marks curfd)
		  (set-x-bounds 0.0 (min (duration curfd) 0.1) curfd)
		  (set-y-bounds -1.0 1.0 curfd)
		  (if (or (> (length (marks curfd 0)) 0)
			  (not (= new-marks (+ old-marks 2))))
		      (snd-print (format #f ";delete marks: ~A ~A?" new-marks old-marks))))
		))
	    ...
	    (key (char->integer #\q) 4 (choose-fd))
	    (set-cursor 8000 (choose-fd))
	    ...
	    (let ((cfd (choose-fd)))
		  ...
		  (let ((uval (my-random 3)))
		    (set-uniting uval cfd)
		    (if (not (= uval (uniting cfd))) (snd-print (format #f ";uniting: ~A ~A?" uval (uniting cfd))))))
		  ...
		  (begin
		   ...
		   (if (fneq (sample 1200 cfd) .1) (snd-print (format #f ";insert-sample(looped): ~A?" (sample 1200 cfd))))))
	      (revert-sound cfd))
		    ...
		    (lambda (lst)
			  ...
			  (let ((name (list-ref (car lst) 0))
				...
				(if (equal? minval #f)
				  ...
				  (if (exact? minval)
					  ...
					  (setfnc (inexact->exact
						   (floor (expt 2 (ceiling (/ (log (+ minval (floor (* (- maxval minval) (my-random 1.0))))) (log 2)))))) index)
				...
				(if (exact? minval)
				    (if (equal? name #t)
					(setfnc (inexact->exact
						 (floor (expt 2 (ceiling (/ (log (+ minval (floor (* (- maxval minval) (my-random 1.0))))) (log 2)))))))
...
(if (or full-test (= snd-test 15))
	...
	(if (and (not (equal? (all-chans) (list (list obi s2i s2i) (list 0 0 1))))
		 ...
		 (snd-print (format #f ";all-chans(2): ~A?" (all-chans))))
	(if (not (string=? (finfo "oboe.snd") "oboe.snd: chans: 1, srate: 22050, Sun, big endian short (16 bits), len: 2.305"))
	    (snd-print (format #f ";finfo: ~A?" (finfo "oboe.snd"))))
	(close-sound s2i)
	(close-sound obi)
	(if (not (equal? (all-chans) '(() ()))) (snd-print (format #f ";all-chans(0): ~A?" (all-chans))))
	(set! obi (open-sound "oboe.snd"))
	(set-cursor 1000 obi)
	(if (not (= (locate-zero .001) 1050)) (snd-print (format #f ";locate-zero: ~A?" (locate-zero .001))))
	...
	(let ((m1 (add-mark 100 obi 0)))
	  ...
	  (if (> (abs (- (left-sample obi 0) 100)) 1) (snd-print (format #f ";mark-in-window: ~A ~A?" (left-sample obi 0) (mark-sample m1))))
	  (delete-mark m1))
	(close-sound s2i)
	(make-region 1000 2000 obi 0)
	(delete-selection-and-smooth)
	(if (not (equal? (edit-fragment 0 obi 0) '(#f "" 0 50828))) (snd-print (format #f ";edit-fragment(0): ~A?" (edit-fragment 0 obi 0))))
	(if (not (equal? (edit-fragment 1 obi 0) '("delete-samples" "delete" 1000 1001))) (snd-print (format #f ";edit-fragment(0): ~A?" (edit-fragment 0 obi 0))))
	(if (not (equal? (edit-fragment 2 obi 0) '("smooth" "set" 984 32))) (snd-print (format #f ";edit-fragment(0): ~A?" (edit-fragment 0 obi 0))))
	...
	(let ((samp100 (sample 1100 obi 0)))
	  ...
	  (let ((nsamp100 (sample 1100 obi 0)))
	    ...
	    (let ((m2 (add-mark 1000 obi 0))
	      ...
	      (if (not (equal? (marks obi 0) (list m2 m3))) (snd-print (format #f ";add-mark: ~A ~A?" (marks obi 0) (list m2 m3))))
	      (eval-between-marks (lambda (val) (* 2.0 val)) obi)
	      (let ((msamp100 (sample 1100 obi 0)))
		(if (fneq (* 2.0 nsamp100) msamp100) (snd-print (format #f ";eval-between-marks: ~A ~A?" nsamp100 msamp100)))
		(revert-sound obi)))))
	(let ((maxa (maxamp obi)))
	  (normalized-mix "pistol.snd" 1000 0 obi 0)
	  (let ((nmaxa (maxamp obi)))
	    (if (fneq maxa nmaxa) (snd-print (format #f ";normalized-mix: ~A ~A?" maxa nmaxa))))
	  (revert-sound obi))
	...
	(let ((o1 (sample 1000 obi 0))
	  ...
	  (let ((o11 (sample 1000 obi 0))
	    ...
	    (if (or (fneq (* 2.0 o1) o11)
		    (fneq (* 2.0 s1) s11)
		    (fneq (* 2.0 s2) s21))
		(snd-print (format #f ";do-all-chans: ~A?" (list o1 s1 s2 o11 s11 s21))))))
	(update-graphs)
	(let ((m1 (maxamp obi 0))
	      (m2 (maxamp s2i 0))
	      (m3 (maxamp s2i 1))
	      (mc (apply map maxamp (all-chans))))
	  (if (or (and (> s2i obi)
		       (or (fneq m1 (car mc))
			   (fneq m2 (cadr mc))
			   (fneq m3 (caddr mc))))
	  ...
	  (let ((mc1 (apply map maxamp (all-chans))))
	    (if (or (and (> s2i obi)
			 (or (fneq (* 2.0 m1) (car mc1))
			     (fneq (* 2.0 m2) (cadr mc1))
			     (fneq (* 2.0 m3) (caddr mc1))))
		    (and (< s2i obi)
			 (or (fneq (* 2.0 m1) (caddr mc1))
			     (fneq (* 2.0 m2) (car mc1))
			     (fneq (* 2.0 m3) (cadr mc1)))))
		(snd-print (format #f ";do-chans: ~A ~A?" mc mc1)))
	    (set-syncing 0 obi)
	    (set-syncing 0 s2i)
	    (select-sound s2i)
	    (do-sound-chans (lambda (val) (if val (* 0.5 val) #f)) "/2")
	    (let ((mc2 (apply map maxamp (all-chans))))
	      (if (or (and (> s2i obi)
			   (or (fneq (* 2.0 m1) (car mc2))
			       (fneq m2 (cadr mc2))
			       (fneq m3 (caddr mc2))))
		      (and (< s2i obi)
			   (or (fneq (* 2.0 m1) (caddr mc2))
			       (fneq m2 (car mc2))
			       (fneq m3 (cadr mc2)))))
		  (snd-print (format #f ";do-sound-chans: ~A ~A ~A?" mc mc1 mc2))))
	    (if (every-sample? (lambda (val) (> val .5))) (snd-print (format #f ";every-sample(0)?"))) 
	    (if (not (every-sample? (lambda (val) (< val 5.0)))) (snd-print (format #f ";every-sample(1)?"))) 
	    (select-sound obi)
	    (let ((bins (scan-chan (sort-samples 32))))
	      (if (not (= (vector-ref bins 1) 4504)) (snd-print (format #f ";sort-samples: ~A?" bins))))
	    ))
	(revert-sound s2i)
	(revert-sound obi)
	(set-syncing 3 obi)
	...
	(if (or (fneq ((compand) 0.0) 0.0)
		...
		(fneq ((compand) .99) .997)
		(fneq ((compand) .95) .984))
	    (snd-print (format #f ";compand: ~A?" (list ((compand) 0.0) ((compand) 1.0) ((compand) .1) ((compand) .99) ((compand) .95)))))
	...
	(let ((s1 (sample 1000 s2i 0))
	  ...
	  (if (or (fneq s1 (sample 1000 s2i 1))
	      ...
	      (snd-print (format #f ";swap-selection-channels: ~A?" (list s1 s2 (sample 1000 s2i 0) (sample 1000 s2i 1))))))
	(revert-sound s2i)
	(close-sound s2i)
	...
	(if (not (= (prefix-arg) 1000)) (snd-print (format #f ";prefix-arg: ~A?" (prefix-arg))))
	(key (char->integer #\b) 4 id)
	(let ((left (left-sample id)))
	  (if (and (not (= left 1000)) (not (= left 1001))) (snd-print (format #f ";u1000: ~A" left))))
	(prefix-it 0 id)
	(key (char->integer #\x) 4 id)
	(key (char->integer #\b) 4 id)
	(let ((left (left-sample id)))
	  (if (not (= left 0)) (snd-print (format #f ";u0: ~A" left))))
	(set-cursor 1234 id)
	(prefix-it 0 id)
	(key (char->integer #\f) 4 id)
	(let ((cr (cursor id)))
	  (if (not (= cr 1234)) (snd-print (format #f ";0f: ~A" cr))))
	(prefix-it 100 id)
	(key (char->integer #\f) 4 id)
	(let ((cr (cursor id)))
	  (if (not (= cr 1334)) (snd-print (format #f ";100f: ~A" cr))))
	(prefix-it -100 id)
	(key (char->integer #\f) 4 id)
	(let ((cr (cursor id)))
	  (if (not (= cr 1234)) (snd-print (format #f ";-100f: ~A" cr))))
	(prefix-it 1 id)
	(key (char->integer #\f) 4 id)
	(let ((cr (cursor id)))
	  (if (not (= cr 1235)) (snd-print (format #f ";1f: ~A" cr))))
	(key (char->integer #\+) 4 id)
	(key (char->integer #\f) 4 id)
	(let ((cr (cursor id)))
	  (if (not (= cr 1236)) (snd-print (format #f ";+f: ~A" cr))))
	(key (char->integer #\-) 4 id)
	(key (char->integer #\f) 4 id)
	(let ((cr (cursor id)))
	  (if (not (= cr 1235)) (snd-print (format #f ";-f: ~A" cr))))
	(prefix-it 1000 id)
	(key (char->integer #\x) 4 id)
	(key (char->integer #\p) 4 id)
	(let ((left (left-sample id))
	      (right (right-sample id)))
	  (if (> (abs (- right left 1000)) 2) (snd-print (format #f ";1000xp: ~A:~A" left right))))
	(prefix-it 1 id)
	...
	(let ((left (left-sample id))
	      (right (right-sample id)))
	  (if (> (abs (- right left (* 22050 1.2))) 2) (snd-print (format #f ";1.2xp: ~A:~A" left right))))
	
	(prefix-uit 1000 id)
	(key (char->integer #\x) 4 id)
	(key (char->integer #\b) 4 id)
	(let ((left (left-sample id)))
	  (if (and (not (= left 1000)) (not (= left 1001))) (snd-print (format #f ";uu1000: ~A" left))))
	(prefix-uit 0 id)
	(key (char->integer #\x) 4 id)
	(key (char->integer #\b) 4 id)
	(let ((left (left-sample id)))
	  (if (not (= left 0)) (snd-print (format #f ";uu0: ~A" left))))
	(set-cursor 1234 id)
	(prefix-uit 0 id)
	(key (char->integer #\f) 4 id)
	(let ((cr (cursor id)))
	  (if (not (= cr 1234)) (snd-print (format #f ";u0f: ~A" cr))))
	(prefix-uit 100 id)
	(key (char->integer #\f) 4 id)
	(let ((cr (cursor id)))
	  (if (not (= cr 1334)) (snd-print (format #f ";u100f: ~A" cr))))
	(prefix-uit -100 id)
	(key (char->integer #\f) 4 id)
	(let ((cr (cursor id)))
	  (if (not (= cr 1234)) (snd-print (format #f ";u-100f: ~A" cr))))
	(prefix-uit 1 id)
	(key (char->integer #\f) 4 id)
	(let ((cr (cursor id)))
	  (if (not (= cr 1235)) (snd-print (format #f ";u1f: ~A" cr))))
	(key (char->integer #\+) 4 id)
	(key (char->integer #\f) 4 id)
	(let ((cr (cursor id)))
	  (if (not (= cr 1236)) (snd-print (format #f ";u+f: ~A" cr))))
	(key (char->integer #\-) 4 id)
	(key (char->integer #\f) 4 id)
	(let ((cr (cursor id)))
	  (if (not (= cr 1235)) (snd-print (format #f ";u-f: ~A" cr))))
	(prefix-uit 1000 id)
	(key (char->integer #\x) 4 id)
	(key (char->integer #\p) 4 id)
	(let ((left (left-sample id))
	      (right (right-sample id)))
	  (if (> (abs (- right left 1000)) 2) (snd-print (format #f ";u1000xp: ~A:~A" left right))))
	(prefix-uit 1 id)
	(key (char->integer #\.) 0 id)
	(key (char->integer #\2) 0 id)
	(key (char->integer #\x) 4 id)
