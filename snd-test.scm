		  ...
		  (if (not (null? lst))
			...
			(let ((nowval (getfnc)))
			  ...
			  (let ((nowval (getfnc)))
				...
				(if (and (number? newval) (inexact? newval))
				    (if (> (abs (- newval nowval)) .01)
					(snd-print (format #f ";set! ~A /= ~A (~A)~%" name newval nowval)))
				    (snd-print (format #f ";set! ~A /= ~A (~A)~%" name newval nowval))))
			    (setfnc initval))
			  (test-vars (cdr lst))))))))
	(test-vars 
...
(if (or full-test (= snd-test 5))
	...
	(let ((newamp (maxamp obind)))
	  (if (> (abs (- (* 2.0 vol) newamp)) .05) (snd-print (format #f ";apply amp: ~A -> ~A?" vol newamp)))
	  (set! (speed obind) 0.5)
	  (call-apply obind)
...
(if (or full-test (= snd-test 8))
	...
	(set! (mus-feedback gen) 0.5) (if (fneq (mus-feedback gen) .5) (snd-print (format #f ";all-pass set-feedback: ~F?" (mus-feedback gen))))
	(set! (mus-feedforward gen) 0.5) (if (fneq (mus-feedforward gen) .5) (snd-print (format #f ";all-pass set-feedforward: ~F?" (mus-feedforward gen)))))
	...
	(set! (mus-scaler gen) .5) (if (fneq (mus-scaler gen) 0.5) (snd-print (format #f ";sum-of-cosines set-scaler: ~F?" (mus-scaler gen)))))
	...
	(set! (mus-a0 gen) .5) (if (fneq (mus-a0 gen) 0.5) (snd-print (format #f ";formant set-a0: ~F?" (mus-a0 gen))))
	(set! (mus-a1 gen) .5) (if (fneq (mus-a1 gen) 0.5) (snd-print (format #f ";formant set-a1: ~F?" (mus-a1 gen))))
	(set! (mus-a2 gen) .5) (if (fneq (mus-a2 gen) 0.5) (snd-print (format #f ";formant set-a2: ~F?" (mus-a2 gen))))
	(set! (mus-b1 gen) .5) (if (fneq (mus-b1 gen) 0.5) (snd-print (format #f ";formant set-b1: ~F?" (mus-b1 gen))))
	(set! (mus-b2 gen) .5) (if (fneq (mus-b2 gen) 0.5) (snd-print (format #f ";formant set-b2: ~F?" (mus-b2 gen))))
	(set! (mus-formant-radius gen) .01) (if (fneq (mus-formant-radius gen) 0.01) (snd-print (format #f ";formant set-radius: ~F?" (mus-formant-radius gen)))))
	...
	(set! (mus-hop gen) 1000) (if (not (= (mus-hop gen) 1000)) (snd-print (format #f ";granulate set-hop: ~A?" (mus-hop gen))))
	(set! (mus-ramp gen) 1000) (if (not (= (mus-ramp gen) 1000)) (snd-print (format #f ";granulate set-ramp: ~A?" (mus-ramp gen))))
	(set! (mus-length gen) 3000) (if (not (= (mus-length gen) 3000)) (snd-print (format #f ";granulate set-length: ~A?" (mus-length gen))))
...
(if (or full-test (= snd-test 13))
    (let ((fd (view-sound "oboe.snd"))
      ...
      (if (menu-sensitive mb "not here") (snd-print ";menu-sensitive?"))
