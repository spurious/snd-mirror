	...
	(do ((i 1 (1+ i)))
	  ...
	  (let ((hdat (* (sqrt (+ (* (vct-ref xdat i) (vct-ref xdat i))
		...
		(fdat (* (sqrt (+ (* (vct-ref rdat i) (vct-ref rdat i))
			 ...
			 (/ 2 size))))
	    (if (fneq fdat hdat)
		(snd-print (format #f ";~D: fft ~A, fht ~A " i fdat hdat)))))
	(fft rdat idat -1)
	(fht xdat)
	(if (fneq (vct-ref rdat 3) size)
	    (snd-print (format #f ";ifft: ~A" (vct-ref rdat 3))))
	(if (fneq (vct-ref xdat 3) size)
	    (snd-print (format #f ";ifht: ~A" (vct-ref xdat 3))))
	...
	(let ((fd (open-sound "oboe.snd"))
	  ...
	  (let ((m2 (without-errors (add-mark 12345 fd 0))))
		...
		(let ((fd (open-sound "oboe.snd")))
		  (load "oboe.marks")
		  (let ((mlst (marks fd 0)))
		    (if (not (= (length mlst) 2)) (snd-print (format #f ";restore-marks: ~A?" mlst)))
		    (if (or (not (= (mark-sample (car mlst)) 123))
			    (not (= (mark-sample (cadr mlst)) 12345)))
			(snd-print (format #f ";restored-marks: ~D ~D?" (mark-sample (car mlst)) (mark-sample (cadr mlst))))))
		  (close-sound fd)))))))))
