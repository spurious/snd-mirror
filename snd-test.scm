...
(if (or full-test (= snd-test 5))
	...
	(let ((newmaxa (maxamp index)))
	  ...
	  (revert-sound index)
	  (select-all index) (w)
	  (if (not (= (regions) 2)) (snd-print (format #f ";regions(2): ~A?" (regions))))
...
(if (or full-test (= snd-test 12))
	...
	(let ((open-files '())
	  ...
	  (if (not (= (active-sounds) 0)) (snd-print (format #f ";active-sounds: ~A?" (active-sounds))))
	...
	(let* ((len (length open-files))
	  ...
	  (let ((choose-fd (lambda () (w) (list-ref open-files (my-random open-ctr)))))
	    ...
	    (let ((cfd (choose-fd)))
	      ...
	      (reverse-selection)
	      (undo 2 cfd))
	    (if (> (regions) 2) (delete-region 2))
	    (if (> (regions) 2) (select-region 2))
