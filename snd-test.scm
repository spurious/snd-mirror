...
(if (or full-test (= snd-test 5))
	...
	(set-amp 0.1 obind)
	(select-channel 2)
	(if (eq? (without-errors (call-apply obind 1)) 'no-such-sound) (snd-print "call-apply can't find 4.aiff?"))
	(let ((newamps (maxamp obind #t)))
	  (if (or (fneq (car amps) (car newamps))
	      ...
	      (snd-print (format #f ";apply amps:~%  ~A ->~%  ~A?" amps newamps)))
	  (undo 1 obind 2)
	  (set-amp 0.1 obind)
...
(if (or full-test (= snd-test 8))
	...
	(pvoc :pitch 0.5 :time 1.0 :snd nind) (w)
	(play-and-wait 0 nind)
	(revert-sound nind)
