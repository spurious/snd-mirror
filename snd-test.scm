...
(if (or full-test (= snd-test 22) (and keep-going (<= snd-test 22)))
    (begin
	  ...
	  (let ((snd-return-key #xFF0D)
	    ...
	    ;; TODO: everything! 
	    ;;       rest of UI event/button-click tests
	    ;;       rest of key codes
	    ;; x-synchronize bool
	    ;; force-event
	    ;; widget-window wid
	    ...
	    ;; COLOR_DIALOG, ORIENTATION_DIALOG, ENVED_DIALOG, ERROR_DIALOG, YES_OR_NO_DIALOG, TRANSFORM_DIALOG,
		...
		(let* ((pos (selection-position))
		  ...
		  (let ((ls (left-sample))
		    ...
		    (key-event cwin snd-right-key 0)
		    (force-event)
		  ...
		  (key-event cwin snd-space-key 0)
		  (force-event)
		  (if (selection?)
		      (snd-display "C-space did not cancel selection?"))
		  (set! (cursor) 100)
		  (key-event cwin (char->integer #\d) 4)
		  (force-event)
		  ...
		  (key-event cwin (char->integer #\h) 4)
		  (force-event)
		  ...
		  (set! (cursor) 5000)
		  (let ((fr (frames)))
		    (key-event cwin (char->integer #\o) 4)
		    (force-event)
		  ...
		  (key-event cwin (char->integer #\m) 4)
		  (force-event)
		  (if (null? (marks ind 0))
	    ...
	    (let* ((ind (open-sound "oboe.snd"))
	      ...
	      (force-event)
	      (key-event (widget-window name-button) (char->integer #\g) 4)
	      (force-event)
	      (key-event (widget-window name-button) (char->integer #\x) 4)
	      (force-event)
	      (key-event (widget-window name-button) (char->integer #\f) 4)
	      (force-event)
	      (widget-string minibuffer "pistol.snd")
	      (key-event (widget-window minibuffer) snd-return-key 0)
	      (force-event)
	      ...
	      (key-event (widget-window name-button) (char->integer #\x) 4)
	      (force-event)
	      (key-event (widget-window name-button) (char->integer #\k) 0)
	      (force-event)
		...
		(key-event (widget-window name-button) (char->integer #\x) 4)
		(force-event)
		(key-event (widget-window name-button) (char->integer #\a) 4)
		(force-event)
		...
		(let ((grf (car (channel-widgets))))
		  (focus-widget grf)
		  (key-event (widget-window grf) (char->integer #\_) 5)
		  (force-event)
		...
		(force-event)
		(click-event (widget-window filter-grf) 1 0 100 100)
		(force-event)
	    ...
	    (let* ((enved-widgets (enved-dialog-widgets))
	      ...
	      (force-event)
	      (click-event ewin 1 0 (inexact->exact (* .5 (+ axis-x0 axis-x1))) axis-y1)
	      (force-event)
