...
'snd-nogui provided? [if]
  ...
  before-repl-hook lambda: <{ -- }>
    "" #f clm-message
    "Starting session on %s." '( "%a %b %d %r %Z %Y" current-time strftime ) clm-message
    "" #f clm-message
  ; add-hook!
  ...
  before-prompt-hook lambda: <{ prompt pos -- new-prompt }>
    ...
    "[%s %s] (%d)> " #( *program-name* tm pos ) string-format
  ; add-hook!
[then]
...
'snd-nogui provided? [unless]
  ...
  *init-lisp-graph-extra-hooks* [if]
  ...
  graph-hook lambda: <{ snd chn y0 y1 -- #f }>
    "freq: %.3f" #( snd chn left-sample  snd chn spot-freq ) string-format snd-print drop
    #f
  ; add-hook!
  
  mouse-click-hook lambda: <{ snd chn button state x y axis -- a }>
    axis time-graph = if
      "freq: %.3f" #( snd chn #f cursor  snd chn spot-freq ) string-format snd-print
    else
      #f
    then
  ; add-hook!
