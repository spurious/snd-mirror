...
snd_info *make_snd_info(snd_info *sip, snd_state *state, char *filename, file_info *hdr, int snd_slot)
{
  ...
  sp->local_explen = DEFAULT_EXPAND_LENGTH;
  sp->local_exprmp = DEFAULT_EXPAND_RAMP;
  sp->local_exphop = DEFAULT_EXPAND_HOP;
  sp->local_revfb = DEFAULT_REVERB_FEEDBACK;
  sp->local_revlp = DEFAULT_REVERB_LOWPASS;
