   ...
   (lambda (size)
     ...
     (let ((a (let ((lst (make-list size #f)))
	   ...
	   (f (let ((ht (let ((ht1 (make-hash-table size)))
		...
		(list (find-if-a (make-iterator ht))
		      (find-if-b (make-iterator ht))
		      (find-if-c (make-iterator ht))
		      (find-if-d (make-iterator ht)))))
	   (g (let ((lt (apply inlet (make-list (* 2 size) 'abc))))
		(list (find-if-a (make-iterator lt))
		      (find-if-b (make-iterator lt))
		      (find-if-c (make-iterator lt))
		      (find-if-d (make-iterator lt)))))
	   (h (if with-blocks
