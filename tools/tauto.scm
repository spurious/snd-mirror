	 ...
	 (catch #t 
	   ...
	   (lambda (type info)
	     (if (and (positive? args-now)
		      (memq type '(wrong-type-arg wrong-number-of-args out-of-range syntax-error division-by-zero format-error missing-method error invalid-escape-function)))
		 (quit)))))
	 ...
	 (if (= args-left 1)
		...
		(catch #t
		  (lambda ()
		    (apply func c-args))
		  (lambda (type info)
		    (if (or (memq type '(wrong-number-of-args out-of-range syntax-error division-by-zero format-error error missing-method invalid-escape-function))
