...
(define (string-search seq1 seq2)
	 ...
	 (lambda (return)
	   (do ((i 0 (+ i 1)))
		 ...
		 (do ((j 1 (+ j 1))
		      (k (+ i 1) (+ k 1)))
		     ((or (= j len1)
			  (not (char=? (seq1 j) (seq2 k))))
		      (if (= j len1)
			  (return i)))))))))))
	

(define (string-search-from seq1 seq2 start)
  (let* ((len1 (length seq1))
	 (len2 (length seq2))
	 (c0 (seq1 0)))
    (if (or (= len2 0)
...
(define (string-ci-search seq1 seq2)
  (let* ((len1 (length seq1))
	 (len2 (length seq2))
	 (c0 (seq1 0)))
    (if (or (= len2 0)
	 ...
	 (lambda (return)
	   (do ((i 0 (+ i 1)))
		 ...
		 (do ((j 1 (+ j 1))
		      (k (+ i 1) (+ k 1)))
		     ((or (= j len1)
			  (and (not (char=? (seq1 j) (seq2 k)))
			       (not (char=? (char-upcase (seq1 j)) (char-upcase (seq2 k))))))
		      (if (= j len1)
			  (return i)))))))))))
...
(define (clean-and-downcase-first-char str caps topic file)
  (if (char=? (str 0) #\|)
      ...
      (let* ((colonpos (or (string-search ":" str) 
	...
	(make-ind :name line 
      ...
      (begin 
       (let ((def-pos (string-search " class=def" str)))
	 (when def-pos
       ...
       (let* ((line (string-append "<a href=\"" 
				   ...
				   "#" 
				   (substring str 9)))
	      (ipos (string-search "<em" line)))
	 (when ipos
	   (let ((ispos (string-search "</em>" line)))
	 ...
	 (let ((hpos (or (string-search "<h2>" line) 
			 (string-search "<h1>" line) 
			 (string-search "<h3>" line) 
			 (string-search "<h4>" line))))
	   (when hpos
	     (let ((hspos (or (string-search "</h2>" line) 
			      (string-search "</h1>" line) 
			      (string-search "</h3>" line) 
			      (string-search "</h4>" line))))
...
(define (clean-up-xref xref file)
  (let* ((len (length xref))
    ...
    (let ((loc 0))
      (do ()
	  ((>= loc len))
	(let* ((leof (or (string-search-from (string #\newline) xref loc)
			 len))
	       (href-normal-start (or (string-search-from "<a href=" xref loc)
				      (string-search-from "<A HREF=" xref loc)))
	       (href-quiet-start (string-search-from "<a class=quiet href=" xref loc))
	       (href-def-start (string-search-from "<a class=def href=" xref loc))
	       (href-start (or href-normal-start href-quiet-start href-def-start))
	       (href-len (if href-normal-start 8 20))
	       (href-end (and href-start
			      (< href-start leof)
			      (string-search-from ">" xref (+ 1 href-start))))
	  ...
	  (let line-loop ((line (read-line f)))
		...
		(let ((len (length line)))
		  (when (and line (positive? len))
		    (let* ((dline line)
			   (compos (string-search "<!-- INDEX" dline))
			   (indpos (string-search "<!-- main-index" dline))
			   (xpos (string-search "<TABLE " dline))
			   (unxpos (string-search "</TABLE>" dline))
			   (pos-simple (and (not compos) 
					    (not indpos)
					    (or (string-search "<a name=" dline)
						(and with-clm-locals 
						     (string-search "<a Name=" dline)))))
			   (pos-def (and (not compos) 
					 (not indpos)
					 (or (string-search "<a class=def name=" dline)
					     (and with-clm-locals 
						  (string-search "<a class=def Name=" dline)))))
			   (pos (or pos-simple pos-def))
			   (tpos (and (not pos) 
				      (string-search "<!-- TOPIC " line))))
		      (if unxpos 
			  (set! xrefing #f))
		      (if tpos
			  (let ((epos (string-search " -->" dline)))
			    (if (not epos) 
			  ...
			  (if compos
				...
				(if (not epos) 
				  ...
				  (if xpos
					...
					(let ((epos (or (string-search "</a>" dline) 
							(string-search "</A>" dline))))
					  (if (not epos) 
						...
						(set! pos-simple (or (string-search "<a name=" dline)
								     (and with-clm-locals 
									  (string-search "<a Name=" dline))))
						(set! pos-def (or (string-search "<a class=def name=" dline)
								  (string-search "<A class=def name=" dline)
								  (and with-clm-locals 
								       (string-search "<a class=def Name=" dline))))
		      ...
		      (if (and xrefing
			       (or (not (char=? (dline 0) #\<))
				   (string-search "<a href" dline)
				   (string-search "<a class=quiet href" dline)
				   (string-search "<a class=def href" dline)))
			  (set! (xrefs current-general) (string-append (xrefs current-general) dline (format #f "~%"))))
		      (when topic
			(let ((hpos (string-search "<hr>" dline)))
			  (when hpos 
			    (set! topic #f))))))
	   ...
	   (let ((linectr -1)
	     ...
	     (let line-loop ((line (read-line f)))
		   ...
		   (begin
			 ...
			 ;; search for name
			 (let* ((dline line)
				(pos-simple (string-ci-search "<a name=" dline))
				(pos-def (string-ci-search "<a class=def name=" dline))
				(pos (or pos-simple pos-def))
			   ...
			   (do ()
			     ...
			     (let ((epos (or (string-search "</a>" dline) 
				   ...
				   (let ((min-epos (string-search " " dline)))
				     (set! epos (string-search ">" dline))
					 ...
					 ((= i name))
					   ...
					   (format #t "ambiguous name: ~A (~A[~D])~%" (names i) file linectr)))
				     (incf name)
				     (set! dline (checked-substring dline epos))
				     (set! pos-simple (string-ci-search "<a name=" dline))
				     (set! pos-def (string-ci-search "<a class=def name=" dline))
				     (set! pos (or pos-simple pos-def))
				     (set! pos-len (if pos-simple 9 19)))))))
					  
			 ;; search for href
			 (let* ((dline line)
				(pos-norm (string-search "<a href=" dline)) ; ignore A HREF
				(pos-quiet (string-search "<a class=quiet href=" dline))
				(pos-def (string-search "<a class=def href=" dline))
			   ...
			   (do ()
			     ...
			     (let ((epos (or (string-search "</a>" dline) 
				   ...
				   (begin
				     ...
				     (incf href)
				     (set! dline (checked-substring dline epos))
				     (set! pos-norm (string-search "<a href=" dline))
				     (set! pos-quiet (string-search "<a class=quiet href=" dline))
				     (set! pos-def (string-search "<a class=def href=" dline))
