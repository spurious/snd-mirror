...
(let ((names (make-hash-table)))
  ...
  ;;  copy-sample-reader free-sample-reader make-mix-sample-reader make-region-sample-reader make-sample-reader mix-sample-reader? region-sample-reader? sample-reader-at-end? sample-reader-home sample-reader? sample-reader-position spectro-cutoff spectro-end clear-selection cursor-follows-play nssb? nxysin? nxycos? nxy1cos? nxy1sin? noddsin? noddcos? noddssb? ncos2? npcos? nrsin? nrcos? nrssb? nkssb? nsincos? n1cos? rcos? rssb? rxysin? rxycos? safe-rxycos? ercos? erssb? eoddcos? rkcos? rksin? rkssb? rk!cos? rk!ssb? r2k!cos? k2sin? k2cos? k2ssb? dblsum? rkoddssb? krksin? abcos? absin? r2k2cos? asyfm? bess? jjcos? j0evencos? j2cos? jpcos? jncos? j0j1cos? jycos? blackman blackman? fmssb? k3sin? izcos? adjustable-square-wave? adjustable-triangle-wave? adjustable-sawtooth-wave? adjustable-oscil? round-interp? nchoosekcos? sinc-train? pink-noise? brown-noise? green-noise? green-noise-interp? moving-sum? moving-variance? moving-rms? moving-length? weighted-moving-average? exponentially-weighted-moving-average? polyoid polyoid-tn polyoid-un noid waveshape? waveshape tanhsin? moving-fft? moving-spectrum? moving-scentroid? moving-autocorrelation? moving-pitch? flocsig? penv? make-penv big-ring-modulate big-oscil? make-big-oscil big-ncos? make-big-ncos big-nsin? make-big-nsin big-table-lookup? make-big-table-lookup big-one-zero? make-big-one-zero big-one-pole? make-big-one-pole hilbert-transform highpass lowpass bandpass bandstop differentiator butter channel-lp-inf savitzky-golay-filter stop-dac dlocsig vector-copy array-ref moog? make-moog sound-comment vector-for-each string-for-each list-copy r7rs-string-fill! char-foldcase exact-integer? inexact exact truncate-quotient truncate-remainder floor-remainder open-binary-input-file open-binary-output-file bytevector-length write-bytevector open-input-bytevector open-output-bytevector read-u8 write-u8 u8-ready? peek-u8 write-simple raise raise-continuable interaction-environment jiffies-per-second current-jiffy current-second get-environment-variable get-environment-variables r7rs-file-exists? os-type cpu-architecture machine-name os-version implementation-name implementation-version box-type? make-box-type
