...
(define (make-env-1 size) (make-env '(0 0 1 1) :length 10))

(define pulsed-env-1 pulsed-env)
(define (make-pulsed-env-1 size) (make-pulsed-env '(0 0 1 1) .01 1000.0))
...
(define (make-filter-1 size) (make-filter 3 (float-vector .1 -.2 .3) (float-vector -.1 .02 -.3)))

(define* (fir-filter-1 g (f 0.1)) (fir-filter g f))
(define (make-fir-filter-1 size) (make-fir-filter 3 (float-vector .1 -.2 .3)))

(define* (iir-filter-1 g (f 0.1)) (iir-filter g f))
(define (make-iir-filter-1 size) (make-iir-filter 3 (float-vector .1 -.2 .3)))
...
(define (make-granulate-1 size) (make-granulate (lambda (dir) .1) 2.0))
...
(define (test-gen gen)
  ...
  (eval-string
   (call-with-output-string
	...
	(format p "  (let ((o (make-~A 1000))~%" gen)
	(format p "        (o1 (make-~A 1000))~%" gen)
	(format p "        (o2 (make-~A 1000))~%" gen)
	(format p "        (o3 (make-~A 1000))~%" gen)
	(format p "        (o4 (make-~A 1000))~%" gen)
	(format p "        (oscs (vector (make-~A 400) (make-~A 500) (make-~A 600)))~%" gen gen gen)
  ...
  (define (out-args)
    ...
    (for-each 
     (lambda (a) 
       (try (format #f "~A" a))
       (try (format #f "(~A o ~A)" gen a))
       (try (format #f "(abs (~A o ~A))" gen a))
       )
     args1)
	...
	(lambda (b) 
	  ...
	  (try (format #f "(~A o (+ ~A ~A))" gen a b))
	  (try (format #f "(~A o (* ~A ~A))" gen a b))
	  (try (format #f "(+ ~A (~A o ~A))" a gen b))
	  (try (format #f "(* ~A (~A o ~A))" a gen b))
	  (try (format #f "(+ (~A o ~A) ~A)" gen a b))
	  (try (format #f "(* (~A o ~A) ~A)" gen a b))
	  ;(try (format #f "(~A o ~A ~A)" gen a b))
	  ;(try (format #f "(abs (~A o ~A ~A))" gen a b))
	  (try (format #f "(* (abs (~A o ~A)) ~A)" gen a b))
	  )
	args2))
     args1)
	...
	(lambda (b)
	  (for-each
	   (lambda (a)
	     ...
	     (try (format #f "(* ~A (+ ~A ~A))" a b c))
	     (try (format #f "(* (+ ~A ~A) ~A)" a b c))
	     (try (format #f "(~A o (+ ~A ~A ~A))" gen a b c))
	     (try (format #f "(~A o (* ~A ~A ~A))" gen a b c))
	     (try (format #f "(~A o (* ~A (+ ~A ~A)))" gen a b c))
	     (try (format #f "(~A o (+ ~A (* ~A ~A)))" gen a b c))
	     (try (format #f "(~A o (* (+ ~A ~A) ~A))" gen a b c))
	     (try (format #f "(~A o (+ (* ~A ~A) ~A))" gen a b c))
	     (try (format #f "(+ ~A (~A o (+ ~A ~A)))" a gen b c))
	     (try (format #f "(+ ~A (~A o (* ~A ~A)))" a gen b c))
	     (try (format #f "(* ~A (~A o (+ ~A ~A)))" a gen b c))
	     (try (format #f "(* ~A (~A o (* ~A ~A)))" a gen b c))
	     
	     (try (format #f "(+ ~A ~A (~A o ~A))" a b gen c))
	     (try (format #f "(* ~A ~A (~A o ~A))" a b gen c))
	     (try (format #f "(+ (* ~A ~A) (~A o ~A))" a b gen c))
	     (try (format #f "(* (+ ~A ~A) (~A o ~A))" a b gen c))
	     (try (format #f "(+ ~A (* ~A (~A o ~A)))" a b gen c))
	     ...
	     ;(try (format #f "(* (cos ~A) (~A o ~A ~A))" a gen b c))
	     ;(try (format #f "(+ (~A o ~A ~A) ~A)" gen a b c))
	     ;(try (format #f "(+ (abs (~A o ~A ~A)) ~A)" gen a b c))
	     ;(try (format #f "(+ (cos (~A o ~A ~A)) ~A)" gen a b c))
	     ;(try (format #f "(+ (sin (~A o ~A ~A)) ~A)" gen a b c))
	     )
	   args3))
	args2))
     args1)
	...
	(lambda (c)
	  (for-each
	   (lambda (b)
		...
		(try (format #f "(+ (~A o ~A) ~A ~A ~A)" gen a b c d))
		(try (format #f "(* (~A o ~A) ~A ~A ~A)" gen a b c d))
		(try (format #f "(+ (* (~A o ~A) ~A) (* ~A ~A))" gen a b c d))
		(try (format #f "(* (+ (~A o ~A) ~A) (+ ~A ~A))" gen a b c d))
		(try (format #f "(+ (~A o ~A) (* ~A ~A ~A))" gen a b c d))
		...
		(try (format #f "(+ ~A (~A o ~A) ~A ~A)" a gen b c d))
		(try (format #f "(* ~A (~A o ~A) ~A ~A)" a gen b c d))
		(try (format #f "(+ (* ~A (~A o ~A)) (* ~A ~A))" a gen b c d))
		(try (format #f "(* (+ ~A (~A o ~A)) (+ ~A ~A))" a gen b c d))
		(try (format #f "(+ ~A (* (~A o ~A) ~A ~A))" a gen b c d))
		...
		(try (format #f "(+ ~A ~A (~A o ~A) ~A)" a b gen c d))
		(try (format #f "(* ~A ~A (~A o ~A) ~A)" a b gen c d))
		(try (format #f "(+ (* ~A ~A) (* (~A o ~A) ~A))" a b gen c d))
		(try (format #f "(* (+ ~A ~A) (+ (~A o ~A) ~A))" a b gen c d))
		(try (format #f "(+ ~A (* ~A (~A o ~A) ~A))" a b gen c d))
		...
		(try (format #f "(+ ~A ~A ~A (~A o ~A))" a b c gen d))
		(try (format #f "(* ~A ~A ~A (~A o ~A))" a b c gen d))
		(try (format #f "(+ (* ~A ~A) (* ~A (~A o ~A)))" a b c gen d))
		(try (format #f "(* (+ ~A ~A) (+ ~A (~A o ~A)))" a b c gen d))
		(try (format #f "(+ ~A (* ~A ~A (~A o ~A)))" a b c gen d))
		...
		(try (format #f "(+ ~A ~A ~A ~A (~A o))" a b c d gen))
		(try (format #f "(* ~A ~A ~A ~A (~A o))" a b c d gen))
		)
	      args4))
	   args3))
  ...
  ) ; end of out-args function
  (out-args)) ; end test gen
