				...
				gpointer fd)
{
  ...
  set_user_data(G_OBJECT(new_dialog), (gpointer)wrap_filer_callbacks(ok, cancel, help, extract, fd));
  ...
  if (!edit_header_dialog)
    {
      ...
      SG_SIGNAL_CONNECT(edit_header_save_button, "clicked", edit_header_ok_callback, NULL);
