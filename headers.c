 ...
 * int mus_header_write (const char *name, int type, int in_srate, int in_chans, int loc, int size, int format, const char *comment, int len): writes file's header
 * int mus_header_update (const char *name, int type, int size, int srate, int format, int chans, int loc): update file's header (unset fields should be 0)
 * int mus_header_initialize (void): initialize (allocate) various buffers -- should be called before using mus_header_read and others
 *
 ...
 *      8SVX (IFF), EBICSF, INRS, ESPS, SPPACK, ADC (OGI), AVR, VOC, CSL, snack "SMP",
 ...
 * in the sound editor (snd-4.tar.gz).
 *
...
static int write_aif_header (int chan, int srate, int chans, int siz, int format, const char *comment, int len, int aifc_header)
{
  ...
  int i,j,lenhdr,lenloop,curend,extra,err=0; /* set aifc to 0 to get old-style AIFF header */
  char *str;
  lenhdr=0;
  extra=0;
  curend=0;
  ...
  if (aifc_header) mus_bint_to_char((unsigned char *)(hdrbuf+16),18+10); else mus_bint_to_char((unsigned char *)(hdrbuf+16),18);
  ...
  if (aifc_header)
    {
      switch (format)
	{
	case MUS_BSHORT: case MUS_B24INT: case MUS_BINT: case MUS_BYTE: str = (char *)I_NONE; break;
	case MUS_LSHORT: case MUS_L24INT: case MUS_LINT: str = (char *)I_sowt; break;
	case MUS_BFLOAT: str = (char *)I_fl32; break;
	case MUS_BDOUBLE: str = (char *)I_fl64; break;
	case MUS_UBYTE: case MUS_UBSHORT: str = (char *)I_raw_; break;
	case MUS_MULAW: str = (char *)I_ulaw; break;
	case MUS_ALAW: str = (char *)I_alaw; break;
...
static void update_aiff_header (int chan, int siz)
{
  ...
  mus_bint_to_char((unsigned char *)hdrbuf,siz+update_form_size-mus_samples_to_bytes(data_format,data_size));
  /* cancel old data_size from previous possible write */
  write(chan,hdrbuf,4);
  lseek(chan,update_frames_location,SEEK_SET);
  mus_bint_to_char((unsigned char *)hdrbuf,siz/(chans*mus_data_format_to_bytes_per_sample(data_format)));
  write(chan,hdrbuf,4);
...
static int write_riff_header (int chan, int srate, int chans, int siz, int format, const char *comment, int len)
{
  ...
  mus_lshort_to_char((unsigned char *)(hdrbuf+22),(short)chans);
  mus_lint_to_char((unsigned char *)(hdrbuf+24),srate);
  mus_lint_to_char((unsigned char *)(hdrbuf+28),srate * chans * mus_data_format_to_bytes_per_sample(format)); /* added chans 10-Mar-99 */
  mus_lshort_to_char((unsigned char *)(hdrbuf+32),(short)(chans * mus_data_format_to_bytes_per_sample(format)));
	      ...
	      while (!srate)
		{
		  ...
		  if (match_four_chars((unsigned char *)hdrbuf,I_shdr))
		      ...
		      while (i < cksize)
			{
			  ...
			  if ((type != 1) || (mus_char_to_lint((unsigned char *)(hdrbuf+24)) == 0)) break;
			  if (srate == 0) srate = mus_char_to_lint((unsigned char *)(hdrbuf+36));
	  ...
	  if (match_four_chars((unsigned char *)hdrbuf,I_VHDR))
	    {
	      ...
	      if (original_data_format != 0) data_format = MUS_UNSUPPORTED;
	    }
	  else
	    {
	  ...
	  if (voc_extended == 0) 
	    {
	      ...
	      if (hdrbuf[5] == 0) data_format = MUS_UBYTE; else data_format = MUS_UNSUPPORTED;
	    }
	  happy = 0;
	}
      else
...
static int read_spl_header(int chan)
{
  ...
  true_file_length=lseek(chan,0L,SEEK_END);
  if (hdrbuf[60]&1) data_format = MUS_BSHORT; else data_format = MUS_BYTE; /* unsigned? */
...
static int read_csl_header (int chan)
{
  ...
  while (happy)
	...
	{
	  mus_error(MUS_HEADER_READ_FAILED,"CSL header chunk bogus at %d",offset);
	  return(-1);
	}
      chunksize = mus_char_to_lint((unsigned char *)(hdrbuf+4));
...
static int read_file_samp_header (int chan)
{
  ...
  while (i < 1024)
	...
	sscanf((const char *)(&locbuf[i+6]),"%d",&srate); /* "sftot"??? who invented this silly thing? */
...
int mus_header_write_with_fd (int chan, int type, int in_srate, int in_chans, int loc, int size, int format, const char *comment, int len)
{
  ...
  switch (type)
    ...
    default:
      {
	mus_error(MUS_UNSUPPORTED_HEADER_TYPE,"can't write %s header",mus_header_type_name(type));
	return(-1);
	break;
      }
    }
...
int mus_header_update_with_fd(int chan, int type, int size)
{
  ...
  switch (type)
    ...
    default:
      {
	mus_error(MUS_UNSUPPORTED_HEADER_TYPE,"can't udpate %s header",mus_header_type_name(type));
	return(-1);
	break;
      }
    }
...
int mus_header_update_comment (const char *name, int loc, const char *comment, int len, int typ)
{
  ...
  switch (typ)
    ...
    default:
      {
	mus_error(MUS_UNSUPPORTED_HEADER_TYPE,"can't udpate %s header comment",mus_header_type_name(typ));
	return(-1);
	break;
      }
    }
