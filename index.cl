...
<!-- created ~A -->~%"
	...
	(format ofil "~%</table>~%</body></html>~%"))
	  ...
	  (progn
		...
		(format rfil "# Snd documentation index (generated by index.cl)~%~%def Snd_names_and_urls~%  [~%")
		(dotimes (i n)
		  (let* ((line (subseq (ind-name (aref tnames i)) 8))
			 (dpos (search ">" line))
		...
		(let* ((dline line)
		  ...
		  (loop while pos do
			...
			(progn
			  ...
			  (setf pos (or (search "<a name=" dline)
					(search "<A NAME=" dline))))))))
		;; search for href
		(let* ((dline line)
