  ...
  (define (display-current-window-location snd chn)
	...
	(let* ((axinf (axis-info snd chn))
	  ...
	  (if (and (> width 10)
		...
		(let ((rx (round (* width (/ (right-sample snd chn) (frames snd chn)))))
		      (lx (round (* width (/ (left-sample snd chn) (frames snd chn))))))
		  ...
		  (if (and old-env
			...
			(set! data1 (list-ref old-env 4)))
		      (let* ((data (make-graph-data snd chn current-edit-position 0 (frames snd chn)))
			     ...
			     (new-len (* width 2))
			     (data-len (if (vct? data) (length data) (length (car data))))
			     (step (/ data-len width)))
			...
			(if (< data-scaler 0.00000001)
				  ...
				  (let ((j 0)
					...
					((or (= j new-len) (= i data-len)))
					  ...
					  (begin
					    ...
					    (vector-set! data0 (+ j 1) (round (- y-offset (* max-y data-scaler))))
					    (set! max-y (- data-max))
					    (if data1
						(begin
						  (vector-set! data1 j x-offset) 
						  (vector-set! data1 (+ j 1) (round (- y-offset (* min-y data-scaler))))
						  (set! min-y data-max)))
				...
				(let ((xstep (/ width data-len)))
				  ...
				  (do ((i 0 (+ 1 i))
				      ...
				      ((= i data-len))
				    (vector-set! data0 j (round xj))
				    (if (not data1)
					(vector-set! data0 (+ j 1) (round (- y-offset (* (vct-ref data i) data-scaler))))
					(begin
					  (vector-set! data0 (+ j 1) (round (- y-offset (* (vct-ref (cadr data) i) data-scaler))))
					  (vector-set! data1 j (floor xj))
					  (vector-set! data1 (+ j 1) (round (- y-offset (* (vct-ref (car data) i) data-scaler))))))))))
  ...
  (define (click-current-window-location snd chn button state x y axis)
	...
	(let* ((axinf (axis-info snd chn))
	  ...
	  (if (and (> width 0)
	      ...
	      (let* ((samp (round (* (frames snd chn) (/ (- x x-offset) width))))
