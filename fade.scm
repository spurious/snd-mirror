...
(define (cross-fade beg dur amp file1 file2 ramp-beg ramp-dur ramp-type bank-dur fs fwidth)
  ...
  (let* ((fil1 (make-sample-reader 0 file1))
	 ...
	 (start (inexact->exact (floor (* beg *srate*))))
	 (end (+ start (inexact->exact (floor (* dur *srate*)))))
	 (ramp 0.0)
	 (bank1 0.0)
	 (bank2 1.0)
	 (half-fs (/ fs 2))
	 (ramp-samps (inexact->exact (floor (* *srate* ramp-dur))))
	 (bank-samps (inexact->exact (floor (* *srate* bank-dur))))
	 (ramp-incr (/ 1.0 ramp-samps))
	 (ramp-start (+ start (inexact->exact (floor (* *srate* ramp-beg)))))
...
(define (dissolve-fade beg dur amp file1 file2 fsize r lo hi)
  (let* ((fil1 (make-sample-reader 0 file1))
	 ...
	 (bin (inexact->exact (/ *srate* fsize)))
	 (radius (- 1.0 (/ r fsize)))
	 (spectrum (make-vct freq-inc 1.0))
	 (ramp-inc (/ 1.0 1024.0))
	 (trigger (inexact->exact (/ (* dur *srate*) freq-inc)))
